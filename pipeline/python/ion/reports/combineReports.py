    ...
    while len(jobIds) > max_running_jobs:
        ...
        for jobid in jobIds:
            ...
            if jobstatus == "done" or jobstatus == "failed" or jobstatus == "DRMAA BUG":
                printtime(
                    "DEBUG: Job %s has ended with status %s" % (str(jobid), jobstatus)
                )
                jobIds.remove(jobid)
...
def get_parent_barcode_files(parent_folder, datasets_path, barcodeSet):
    ...
    try:
        ...
        for dataset in datasets_json.get("datasets", []):
            ...
            elif "legacy_prefix" in list(dataset.keys()):
                old_bamfile = os.path.join(
                    parent_folder, dataset["legacy_prefix"] + ".bam"
                )
                    ...
                    barcode_json.update(
                        {
                            ...
                            key
                            + "_mean_read_length": stats[key].get("mean_read_length"),
...
def generate_datasets_pipeline_json(
    barcodeSet, found_barcode_names, barcodeSet_Info, sample, barcodeSamples, runID
):
    ...
    datasets["read_groups"][no_barcode_group] = {"index": 0, "sample": sample}
    ...
    if barcodeSet:
        ...
        try:
            for bcname in sorted(found_barcode_names):
                ...
                bcsample = [
                    k
                    for k, v in list(barcodeSamples.items())
                    if bcname in v.get("barcodes", [])
                ]
...
def generate_datasets_basecaller_json(datasets, parentBAMs, barcodeSet):
    ...
    for bamfile in parentBAMs:
        ...
        try:
            ...
            for parent_read_group in list(parent_datasets_json["read_groups"].values()):
                if barcodeSet:
                    prefix = "%s_rawlib" % parent_read_group.get(
                        "barcode_name", "nomatch"
                    )
                else:
                ...
                by_file_prefix[prefix]["total_bases"] += parent_read_group[
                    "total_bases"
                ]

                if parent_read_group.get("reference"):
                    by_file_prefix[prefix]["reference"] = parent_read_group["reference"]
                by_file_prefix[prefix]["filtered"].append(
                    parent_read_group.get("filtered", False)
                )
        except Exception:
            printtime(
                "DEBUG: unable to update datasets_basecaller.json from %s"
                % datasetsFile
            )
            traceback.print_exc()
            break
...
def find_barcodes_to_process(parentBAMs, barcodeSet):
    ...
    for bamfile in parentBAMs:
        ...
        if os.path.exists(os.path.join(parent_folder, barcodelist_path)):
            ...
            bcSetName_new = (
                open(bcList_file, "r").readline().split("file_id")[1].strip()
            )
            if barcodeSet != bcSetName_new:
                printtime(
                    "Warning: different barcode sets: %s and %s"
                    % (barcodeSet, bcSetName_new)
                )
            ...
            barcode_bams = get_parent_barcode_files(
                parent_folder, datasets_path, barcodeSet
            )
            ...
            for bc_path in barcode_bams:
                try:
                    bcname = [
                        name
                        for name in sorted(list(barcodeSet_Info.keys()), reverse=True)
                        if os.path.basename(bc_path).startswith(name)
                    ][0]
                except Exception:
                    bcname = "unknown"

                if bcname not in barcode_files:
                    barcode_files[bcname] = {"count": 0, "bcfiles_to_merge": []}
...
if __name__ == "__main__":
    ...
    if barcodeSet:
        ...
        for bcname, barcode_file_dict in barcode_files.items():
            ...
            if barcode_file_dict["count"] > 1:
                ...
                if NUM_BARCODE_JOBS and (len(bc_jobs) > NUM_BARCODE_JOBS - 1):
                    wait_on_jobs(
                        bc_jobs, "barcode", "Processing barcodes", NUM_BARCODE_JOBS - 1
                    )
    ...
    if override_samples:
        if barcodeSet:
            bam_files_to_merge = [
                barcode_file_dict["filename"]
                for barcode_file_dict in list(barcode_files.values())
            ]
        else:
    ...
    for bam in bam_files_to_merge:
        if not os.path.exists(bam):
            ...
            bam = "%s_%s.bam" % (
                parent_env.get("expName", ""),
                parent_env.get("resultsName", ""),
            )
    ...
    if barcodeSet:
        barcode_report_stats(list(barcode_files.keys()))

    # Generate files needed by Plugins
    try:
        datasets = generate_datasets_pipeline_json(
            barcodeSet,
            list(barcode_files.keys()),
            barcodeSet_Info,
            sample,
            barcodeSamples,
            runID,
    ...
    try:
        ...
        os.symlink(
            filename + ".bai", os.path.join(download_links, newname + ".bam.bai")
        )
        # barcodes:
        if barcodeSet:
            # os.symlink(os.path.join(mycwd, zipname), os.path.join(download_links, newname+'.barcode.bam.zip'))
            for bcname in list(barcode_files.keys()):
                filename = os.path.join(mycwd, bcname + "_rawlib.bam")
                os.symlink(
                    filename,
                    os.path.join(
                        download_links, newname + "." + bcname + "_rawlib.bam"
                    ),
                )
                os.symlink(
                    filename + ".bai",
                    os.path.join(
                        download_links, newname + "." + bcname + "_rawlib.bam.bai"
                    ),
                )
    except Exception:
        traceback.print_exc()
