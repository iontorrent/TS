...
def initTLReport():
    ...
    if not os.path.isdir(plugin_folder):
        oldmask = os.umask(0o0000)  # grant write permission to plugin user
        os.mkdir(plugin_folder)
        os.umask(oldmask)
    os.chmod(plugin_folder, 0o777)
...
def initBlockReport(
    ...
    if blockObj["id_str"] == "wholechip":
        ...
        try:
            os.symlink(
                os.path.join("..", _SIGPROC_RESULTS),
                os.path.join(resultDir, SIGPROC_RESULTS),
            )
            os.symlink(
                os.path.join("..", _BASECALLER_RESULTS),
                os.path.join(resultDir, BASECALLER_RESULTS),
            )
        ...
        # update path to data
        TMP_PARAMS["pathToData"] = os.path.join(
            TMP_PARAMS["pathToData"], blockObj["datasubdir"]
        )
        ...
        try:
            os.symlink(
                os.path.join(cwd, "Default_Report.php"),
                os.path.join(resultDir, "Default_Report.php"),
            )
        except Exception:
            printtime("couldn't create symbolic link")
...
def get_plugins_to_run(plugins, report_type):
    ...
    for name in list(plugins.keys()):
        ...
        if selected:
            plugin["runlevels"] = (
                plugin.get("runlevels")
                if plugin.get("runlevels")
                else [RunLevel.DEFAULT]
            )
            printtime(
                "Plugin %s is enabled, runlevels=%s"
            ...
            if (
                report_type == RunType.COMPOSITE
                and RunLevel.BLOCK in plugin["runlevels"]
            ):
                blocklevel = True
        else:
            printtime(
                "Plugin %s (runtypes=%s) is not enabled for %s report"
                % (plugin["name"], ",".join(plugin.get("runtypes", "")), report_type)
            )
...
def GetBlocksToAnalyze(env):
    ...
    if is_thumbnail or is_single:
        ...
        if is_thumbnail:
            ...
            blocks.append(
                {"id_str": "wholechip", "datasubdir": "", "jobid": None, "status": None}
            )
...
def get_mem_usage():
    ...
    return "Memory [MB]  Total: {0:6d}   Used: {1:6d}   Free: {2:6d}   Buffers: {3:6d}   Cached: {4:6d}   TotalFree: {5:6d}".format(
        mem_total, mem_used, mem_free, mem_buffers, mem_cached, mem_total_free
    )
...
def write_jobid_list(block_job_dict, merge_job_dict=None):
    ...
    job_list = {}
    for block, jobid in list(block_job_dict.items()):
...
if __name__ == "__main__":
    ...
    for barcode_name, barcode_info in sorted(env["barcodeInfo"].items()):
        ...
        if env["previousReport"]:
            ...
            if env["oninstranalysis"]:
                if is_composite:
                    ...
                    sigproc_target = os.path.join(
                        env["pathToRaw"], "onboard_results", env["SIGPROC_RESULTS"]
                    )
                elif is_thumbnail:
                    root_target = os.path.join(env["pathToRaw"], "..")
                    sigproc_target = os.path.join(
    ...
    try:
        jobserver = xmlrpclib.ServerProxy(
            "http://%s:%d" % (JOBSERVER_HOST, JOBSERVER_PORT),
            verbose=False,
            allow_none=True,
        )
    ...
    def set_result_status(status):
        try:
            ...
            printtime(
                "TLStatus %s\tpid %d\tpk file %s started in %s"
    ...
    if doblocks:
        ...
        result_dirs = {}
        for block in blocks:
            result_dirs[block["id_str"]] = initBlockReport(
                block,
                env["SIGPROC_RESULTS"],
                env["BASECALLER_RESULTS"],
                env["ALIGNMENT_RESULTS"],
                from_sigproc_analysis=(env["blockArgs"] == "fromRaw"),
            )
        ...
        while len(blocks_to_process) > 0 and timeout > 0:
            ...
            printtime(
                "waiting for %s block(s) to schedule    %s"
                % (str(len(blocks_to_process)), get_mem_usage())
            )
            sys.stdout.flush()
            sys.stderr.flush()
            ...
            if doSigproc:
                for block in blocks_to_process:
                    if is_single:
                        ...
                        data_file = os.path.join(
                            env["pathToRaw"], "..", block["id_str"], "acq_0000.dat"
                        )  # TODO
                    else:  # is_composite
                        data_file = os.path.join(
                            env["pathToRaw"], block["id_str"], "acq_0000.dat"
                ...
                for block in blocks_to_process:
                    if is_composite:
                        ...
                        data_file = os.path.join(
                            env["SIGPROC_RESULTS"], "analysis_return_code.txt"
                        )
            ...
            printtime(
                "try to schedule %s new block(s)" % str(len(blocks_to_process_ready))
            )
            ...
            for block in blocks_to_process_ready:
                ...
                try:
                    ...
                    block["jobid"] = spawn_cluster_job(
                        result_dirs[block["id_str"]],
                        "BlockTLScript.py",
                        block_tlscript_options,
                        wait_list,
                    )
                    block_job_dict[block["id_str"]] = str(block["jobid"])
                    printtime(
                        "Submitted block (%s) job with job ID (%s)"
                        % (block["id_str"], str(block["jobid"]))
                    )
                except Exception:
        ...
        if is_composite:
            merge_job_dict["merge"] = spawn_cluster_job(
                ".",
                "MergeTLScript.py",
                ["--do-sigproc", "--do-basecalling", "--do-zipping"],
                list(block_job_dict.values()),
            )
            printtime(
                "Submitted composite merge job with job ID (%s)"
                % (str(merge_job_dict["merge"]))
            )
        else:
            merge_job_dict["merge"] = spawn_cluster_job(
                ".", "MergeTLScript.py", ["--do-zipping"], list(block_job_dict.values())
            )
            printtime(
                "Submitted zipping job with job ID (%s)"
                % (str(merge_job_dict["merge"]))
            )
        ...
        pl_started = False
        block_job_list = list(block_job_dict.values())
        while len(block_job_list) > 0:
            for job in block_job_list:
                ...
                if (
                    block["status"] == "done"
                    or block["status"] == "failed"
                    or block["status"] == "DRMAA BUG"
                ):
                    printtime(
                        "Job %s has ended with status %s"
                        % (str(block["jobid"]), block["status"])
                    )
            ...
            if (
                os.path.exists(
                    os.path.join(env["SIGPROC_RESULTS"], "separator.mask.bin")
                )
                and not pl_started
            ):
            ...
            printtime(
                "waiting for %d blocks to be finished    %s"
                % (len(block_job_list), get_mem_usage())
            )
            time.sleep(10)

        merge_job_list = list(merge_job_dict.keys())
        while len(merge_job_list) > 0:
            for key in merge_job_list:
                ...
                if (
                    merge_status == "done"
                    or merge_status == "failed"
                    or merge_status == "DRMAA BUG"
                ):
                    printtime(
                        "Job %s, %s has ended with status %s" % (key, jid, merge_status)
                    )
                    merge_job_list.remove(key)
                    break

            printtime(
                "waiting for %d merge jobs to be finished    %s"
                % (len(merge_job_list), get_mem_usage())
            )
            time.sleep(10)
    ...
    try:
        ...
        tfmapperstats_outputfile = os.path.join(
            env["BASECALLER_RESULTS"], "TFStats.json"
        )
        ionstats_basecaller_json_path = os.path.join(
            env["BASECALLER_RESULTS"], "ionstats_basecaller.json"
        )
        ...
        ionstats_alignment_json_path = os.path.join(
            env["ALIGNMENT_RESULTS"], "ionstats_alignment.json"
        )
        reportLink = True
        ...
        if is_thumbnail or is_single:
            ...
            try:
                raw_return_code_file = os.path.join(
                    env["BASECALLER_RESULTS"], "composite_return_code.txt"
                )
        ...
        if not os.path.exists(os.path.join(env["pathToRaw"], "checksum_status.txt")):
                    ...
                    if is_composite:
                        ...
                        if reference_selected:
                            if os.path.exists(ionstats_alignment_json_path):
                                ...
                                printtime(
                                    "ERROR: keep_raw_data check failed: ionstats_alignment.json is missing"
                                )
                        else:
                            printtime(
                                "INFO: reference library not set, skip AQ17 check"
                            )
            ...
            if (is_single or is_composite) and os.path.isfile(raw_return_code_file):
                if keep_raw_data:
                    ...
                    shutil.copyfile(
                        raw_return_code_file + ".keep",
                        os.path.join(env["pathToRaw"], "checksum_status.txt"),
                    )
                else:
                    shutil.copyfile(
                        raw_return_code_file,
                        os.path.join(env["pathToRaw"], "checksum_status.txt"),
                    )
