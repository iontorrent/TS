...
def merge_bam_files(
    bamfilelist, composite_bam_filepath, mark_duplicates, new_sample_name=""
):
...
def generate_ionstats(
    bam_filename, ionstats_filename, ionstats_h5_filename, histogram_length
):
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-z", "--zip", dest="zip", action="store", default="", help="zip input files"
    )
    parser.add_argument(
        "-s",
        "--new-sample-name",
        ...
        action="store",
        default="",
        help="overwrite BAM header sample name",
    )
    ...
    if args.merge_out and len(args.files) > 1:
        ...
        printtime(
            "Merging bam files to %s, mark duplicates is %s"
    ...
    if args.align_stats and len(args.files) > 0:
        ...
        for bamfile in args.files:
            if bamfile == "rawlib.bam":
                ...
                error_summary_file = (
                    bamfile.split(".bam")[0] + ".ionstats_error_summary.h5"
                )
    ...
    if args.merge_plots:
        ...
        try:
            ...
            ionstats_plots.alignment_rate_plot2(
                ionstats_file, "alignment_rate_plot.png", int(graph_max_x)
            )
            ionstats_plots.base_error_plot(
                ionstats_file, "base_error_plot.png", int(graph_max_x)
            )
        except Exception:
            traceback.print_exc()
