...
def addTfMetrics(tfMetrics, keyPeak, BaseCallerMetrics, res):
    ...
    for tf, metrics in tfMetrics.items():
        ...
        kwargs = {
            ...
            "keypass": metrics.get(
                "Num", 0.0
            ),  # Deprecated, populating by next best thing
            ...
            "HPAccuracy": ", ".join(
                "%d : %d/%d" % (x, y[0], y[1])
                for x, y in enumerate(zip(hpAccNum, hpAccDen))
            ),
        }

        tfm, created = models.TFMetrics.objects.get_or_create(
            report=res, name=tf, defaults=kwargs
        )
        if not created:
...
def addAnalysisMetrics(beadMetrics, BaseCallerMetrics, res):
    ...
    if BaseCallerMetrics:
        try:
            kwargs["libFinal"] = BaseCallerMetrics["Filtering"]["ReadDetails"]["lib"][
                "valid"
            ]
            kwargs["tfFinal"] = BaseCallerMetrics["Filtering"]["ReadDetails"]["tf"][
                "valid"
            ]
        except Exception as err:
            print(
                "During AnalysisMetrics creation, reading from BaseCaller.json: %s", err
            )
    ...
    if beadMetrics:
        ...
        if kwargs["adjusted_addressable"]:
            kwargs["loading"] = (
                100 * float(kwargs["bead"]) / kwargs["adjusted_addressable"]
            )
...
def addLibMetrics(
    ...
    if ionstats_alignment != None:
        ...
        for q in quallist:
            kwargs["q%s_longest_alignment" % q] = ionstats_alignment["AQ" + q][
                "max_read_length"
            ]
            kwargs["q%s_mean_alignment_length" % q] = ionstats_alignment["AQ" + q][
                "mean_read_length"
            ]
...
def get_some_quality_metrics(quality, ionstats, qv_histogram):
    ...
    for size in [50, 100, 150]:
        if len(read_length_histogram) > size:
            kwargs["q%d_%dbp_reads" % (quality, size)] = sum(
                read_length_histogram[size:]
            )
        else:
...
def addQualityMetrics(ionstats_basecaller, res):
    ...
    for quality, key in [(0, "full"), (17, "Q17"), (20, "Q20")]:
        try:
            metrics = get_some_quality_metrics(
                quality,
                ionstats_basecaller["full"],
...
def writeDbFromFiles(
    ...
    try:
        if extra[u"total_reads"]:
            extra[u"fraction_with_adaptor"] = extra[u"reads_with_adaptor"] / float(
                extra[u"total_reads"]
            )
...
def writeDbFromDict(
    ...
    try:
        ...
        if ionstats_basecaller:
            ...
            e.write(
                "Failed to add QualityMetrics, missing object ionstats_basecaller\n"
            )
    except IntegrityError:
...
if __name__ == "__main__":
    ...
    ionstats_alignment_json_path = os.path.join(
        folderPath, ALIGNMENT_RESULTS, "ionstats_alignment.json"
    )
