...
def makeContourMap(arr, HEIGHT, WIDTH, outputId, maskId, outputdir, barcodeId=-1, vmaxVal=100):    
    score,scores,average = calculateScores(arr, HEIGHT, WIDTH)
    makeContourPlot(score, scores, average, HEIGHT, WIDTH, outputId, maskId, outputdir, barcodeId, vmaxVal)
...
def genHeatmapBarcode(barcodeMaskFilePath, outputdir, HEIGHT=False, WIDTH=False, regMaskFilename=''):
    ...
    for (score,scores,average,barcodeId) in scoreArr:
        ...
        if(not numpy.isnan(average)):
            makeContourPlot(score, scores, average, HEIGHT, WIDTH, outputFilenamePrefix, "", outputdir, barcodeId,
                            autoGetVmaxFromAverage(maxAverage))
    return barcodeCountArr

def genHeatmap(filePath, outputdir, barcodeMaskFilePath=""):
    #
    #Called from TLScript.py
    #
    pylab.clf()
    beadarr, HEIGHT, WIDTH, outputId, maskId = extractMaskInfo(filePath)
    makeContourMap(beadarr, HEIGHT, WIDTH, outputId, maskId, outputdir)
    
    # If this has barcode information, then split it up.
    if barcodeMaskFilePath:
        return genHeatmapBarcode(barcodeMaskFilePath,outputdir,HEIGHT,WIDTH,filePath)
