...
def makeContourMap(
    ...
    scores = scipy.ndimage.correlate(
        arr, numpy.ones((10, 10), dtype=numpy.int16), mode="reflect"
    )
    del arr
...
def getTicksForMaxVal(maxVal):
    ...
    if maxVal <= 10:
        ticksVal = map(lambda x: x / 10.0, ticksVal)
    elif maxVal <= 20:
        ticksVal = map(lambda x: x / 5.0, ticksVal)
    elif maxVal <= 50:
        ticksVal = map(lambda x: x / 2.0, ticksVal)
    return ticksVal
...
def write_full_bleed_map(normal_scores, dimensions, path):
    ...
    indexed_im = im.convert("RGB", palette=Image.ADAPTIVE, dither=None).convert(
        "P", palette=Image.ADAPTIVE, dither=None
    )
...
def makeContourPlot(
    ...
    if barcodeId != -1:
        if barcodeId == 0:
            maskId = "No Barcode Match,"
        else:
            maskId = "Barcode Id %d," % barcodeId
    if plt_title != "":
        maskId = "%s\n%s" % (plt_title, maskId)
...
def extractBarcodeMaskInfo(filePath):
    ...
    unique_barcodeIds = list(dict.fromkeys(bcbead_bcIds).keys())
...
def genHeatmap(filePath, bfmaskstatspath, outputdir, plot_title):
    ...
    maskId = ""
    makeContourMap(
        beadarr, HEIGHT, WIDTH, outputId, maskId, plot_title, average, outputdir
    )
