...
def merge(folder, infile, out_list, verbose, offset_str):
    ...
    if verbose:
        print(
            "MaskMerge: Append block with offsets x: "
            + str(offsetx)
            + " y: "
            + str(offsety)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", dest="inputfile", default="MaskBead.mask", help="mask to be merged"
    )
    parser.add_argument(
        "-o", dest="outputfile", default="MaskBead.mask", help="mask to be merged"
    )
    parser.add_argument(
        "-s",
        "--offset_str",
    ...
    main_merge(
        args.inputfile, args.blockfolder, args.outputfile, args.verbose, args.offset_str
    )
