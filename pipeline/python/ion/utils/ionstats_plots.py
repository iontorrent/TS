...
def read_length_sparkline(
    ionstats_basecaller_filename, output_png_filename, max_length
):
    ...
    try:
        ...
        for read_length, frequency in enumerate(
            ionstats_basecaller["full"]["read_length_histogram"]
        ):
        ...
        ax.bar(
            histogram_x, histogram_y, width=6.5, color="#2D4782", linewidth=0, zorder=2
        )
...
def read_length_histogram(
    ...
    try:
        ...
        ax = fig.add_subplot(
            111, frame_on=False, yticks=[], position=[0, 0.15, 1, 0.88]
        )
        ax.bar(
            histogram_x, histogram_y, width=2.5, color="#2D4782", linewidth=0, zorder=2
        )
...
def alignment_rate_plot(
    ...
    try:
        for read_length_bin, frequency in enumerate(
            ionstats_basecaller["full"]["read_length_histogram"]
        ):
    ...
    if not y_ticks:
        ax = fig.add_subplot(
            111, frame_on=False, yticks=[], position=[0.1, 0.15, 0.8, 0.89]
        )
    else:
        ...
        ax = fig.add_subplot(
            111, frame_on=False, yticks=yticks, position=[0.14, 0.15, 0.8, 0.89]
        )
    ...
    if sum(nread) > 0:
        map_percent = int(
            round(
                100.0
                * float(alignStats_json["total_mapped_target_bases"])
                / float(sum(nread))
            )
        )
    ...
    if not y_ticks:
        alignment_rate_plot(
            alignStats,
            ionstats_basecaller_filename,
            "large_" + output_png_filename,
            graph_max_x,
            y_ticks=True,
        )


def alignment_rate_plot2(
    ionstats_alignment_filename, output_png_filename, graph_max_x, y_ticks=None
):
    """ Generate cumulative read length plot of called vs aligned read length """
    ...
    try:
        ...
        for read_length_bin, frequency in enumerate(
            ionstats_alignment["full"]["read_length_histogram"]
        ):
            current_bin = min(read_length_bin, graph_max_x - 1)
            histogram_full[current_bin] += frequency

        for read_length_bin, frequency in enumerate(
            ionstats_alignment["aligned"]["read_length_histogram"]
        ):
            current_bin = min(read_length_bin, graph_max_x - 1)
        ...
        max_x = min(
            graph_max_x, len(ionstats_alignment["full"]["read_length_histogram"])
        )
        ...
        if (
            sum(histogram_full) > 0
            and float(ionstats_alignment["full"]["num_bases"]) > 0
        ):
            map_percent = int(
                round(
                    100.0
    ...
    try:
        ...
        for i in range(graph_max_x):
            if histogram_aligned[i] > 1000:
                accuracy.append(
                    100
                    * (1 - float(n_err_at_position[i]) / float(histogram_aligned[i]))
                )
                reads.append(i + 1)
        ...
        max_x = min(
            graph_max_x, len(ionstats_alignment["aligned"]["read_length_histogram"])
        )
...
def _get_per_base_accuracy(error_values, depth_values):
    ...
    for i in range(0, max_count):
        if depth_values[i] != 0:
            per_base_accuracy[i] = 100 * (
                1 - float(error_values[i]) / float(depth_values[i])
            )
            ...
            printtime(
                "DEBUG: BAD should not happen!!! _get_per_base_accuracy() depth_values[i=%d] is zero!!"
                % (i)
            )
...
def base_error_plot(ionstats_alignment_filename, output_png_filename, graph_max_x):
    ...
    try:
        printtime(
            "DEBUG: Generating NEW base error (AKA per-base accuracy plot %s"
            % output_png_filename
        )
        ...
        my_max_base = max(
            _which_depth_indices(
                cumulative_depth, total_depth, depth_throughput_fraction
            )
        )
        ...
        printtime(
            "DEBUG: >>>>> depth_throughput_fraction=%0.4f; total_depth=%d; my_max_base=%d; max_base=%d"
            % (depth_throughput_fraction, total_depth, my_max_base, max_base)
        )

        # get per-base error and depth
        per_base_err = _get_per_base_err(
            by_base_data["ins"], by_base_data["del"], by_base_data["sub"], max_base
        )
        ...
        overall_accuracy = 100 * (
            1 - float(sum(per_base_err)) / float(sum(per_base_depth))
        )
        ...
        print(
            "DEBUG: >>>>>  graph_max_x=%d; len(by_base_data[depth]=%d; x_axis_max=%d"
            % (graph_max_x, len(by_base_data["depth"]), x_axis_max)
        )
...
def old_aq_length_histogram(
    ionstats_alignment_filename, output_png_filename, aq_string, color
):
...
def quality_histogram(ionstats_basecaller_filename, output_png_filename):
    ...
    try:
        ...
        fig = plt.figure(figsize=(4, 4), dpi=100)
        ax = fig.add_subplot(
            111, frame_on=False, xticks=[], yticks=[], position=[0.1, 0.1, 1, 0.9]
        )
...
def tf_length_histograms(ionstats_tf_filename, output_dir):
    ...
    try:
        ...
        for tf_name, tf_data in ionstats_tf["results_by_tf"].items():
            ...
            ax = fig.add_subplot(
                111, frame_on=False, xticks=[], yticks=[], position=[0, 0.3, 1, 0.7]
            )
...
if __name__ == "__main__":
    ...
    a = alignment_rate_plot2(
        "ionstats_alignment.json", "alignment_rate_plot.png", int(graph_max_x)
    )
