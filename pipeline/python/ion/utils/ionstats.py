...
def merge_ionstats_total(
    ...
    if len(ionstats_alignment_file_list) > 0:
        reduce_stats(ionstats_alignment_file_list, os.path.join(ALIGNMENT_RESULTS, 'ionstats_alignment.json'))
    ...
    if len(ionstats_basecaller_file_list) > 0:
        reduce_stats(ionstats_basecaller_file_list, os.path.join(BASECALLER_RESULTS, 'ionstats_tmp_basecaller.json'))
    else:  # barcode classification filtered all barcodes or no reads available
        # TODO: ionstats needs to produce initial json file
        try:
            # cmd = "echo $'@HD\tVN:1.5\tSO:coordinate\n@SQ\tSN:ref\tLN:4\n@RG\tID:filename\tSM:filename' | samtools view -F4 -S -b - > empty_dummy.bam"
    ...
    if len(ionstatslist) > 0:
        reduce_stats(ionstatslist, os.path.join(BASECALLER_RESULTS, 'ionstats_basecaller_with_aligninfos.json'))
        reduce_stats(reversed(ionstatslist), os.path.join(BASECALLER_RESULTS, 'ionstats_basecaller.json'))
    ...
    try:
        ...
        for dataset in basecaller_datasets["datasets"]:
            ...
            if len(block_filename_list) > 0:
                composite_filename = os.path.join(ionstats_folder, dataset['file_prefix']+'.'+ionstats_file)
    ...
    try:
        ...
        for tf_name, tf_data in ionstats_tf['results_by_tf'].iteritems():
            ...
            tfstats_json[tf_name] = {
                ...
                'Percent 50Q17': 100.0 * sum(tf_data['AQ17']['read_length_histogram'][50:]) / tf_data['full']['num_reads'],
                '100Q17': sum(tf_data['AQ17']['read_length_histogram'][100:]),
                'Percent 100Q17': 100.0 * sum(tf_data['AQ17']['read_length_histogram'][100:]) / tf_data['full']['num_reads'],
            }
