...
from ion.reports import beadDensityPlot, plotKey
from ion.utils.blockprocessing import printtime, isbadblock


def beadfind_cmd(
    beadfindArgs, libKey, tfKey, pathtorawblock, SIGPROC_RESULTS, block_offset_xy
):
    if beadfindArgs:
...
def generate_raw_data_traces(
    libKey, tfKey, floworder, SIGPROC_RESULTS, plot_output_dir=os.getcwd()
):
    #
...
def mergeSigProcResults(
    dirs, SIGPROC_RESULTS, plot_title, exclusionMask="", initExclude=False
):
...
def mergeAvgNukeTraces(dirs, SIGPROC_RESULTS, key, beads, from_rawdata=False):
    ...
    try:
        ...
        for subdir in dirs:
            ...
            try:
                input_trace_file = os.path.join(
                    input_basedir, "avgNukeTrace_%s.txt" % key
                )
                if os.path.exists(input_trace_file):
                    ...
                    labels = numpy.genfromtxt(
                        input_trace_file, delimiter=" ", usecols=[0], dtype=str
                    )
                    currentAvgNukeTraceData = numpy.genfromtxt(
                        input_trace_file, delimiter=" "
                    )[:, 1:]
                else:
                    continue
            except Exception:
