<?php
	...
	//replace the title
	$header = str_replace("<title>","<title>". $resultsName . ", ", $header );
...
print '<div id="LibrarySummary" class="report_block">';
	...
	print '<div><!-- start wrapper -->';
        ...
        if (file_exists($alignment_results . "/alignTable.txt")) {
		$alignTable = tabs_parse_to_keys($alignment_results . "/alignTable.txt");
		if ($alignTable){ 
	  ...
	  <div class="half_width">
			<div class='beadTable'>
				...
				?>
					...
					<?php
                        $Bead_density_contour_file = $basecaller_results . '/Bead_density_contour.png';
                ...
                ?>
                    ...
                    <col width="520px" />
                        <tr>
                            ...
                            <td><strong><?php print($version); ?></strong></td>
                        </tr>
                        <?php foreach($versions as $name => $version): ?>
                            <tr>
                                <th><?php print($name); ?></th>
...
<!--  end plugin dialog -->
 ...
 $(document).ready(function(){
	...
	$("#pluginDialogButton").button({
        ...
        plugin_ids = plugins.map(function(d){return d.id}).join(";");
        //get plugin metadata
        $.ajax({
