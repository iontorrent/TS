...
class IonBedReader:
    ...
    def __init__(self, file=None, filename=None):
        ...
        if nCols < 3:
            print(
                "IonBedReader ERROR bedfile %s needs at least chrom,chromStart,chromEnd fields."
                % self.filename
            )
            return
        elif nCols > 12:
    ...
    def load(self):
        ...
        for line in self.bed:
            ...
            if len(bsplit) != len(self.columns):
                print(
                    "IonBedReader ERROR unexpected number of columns in line %d of file %s"
                    % (lnum, self.filename)
                )
                return {}
    ...
    def bedLine2Dict(self, bline):
        ...
        if len(bsplit) != len(self.columns):
            print(
                "IonBedReader ERROR bedfile %s needs chrom,chromStart,chromEnd fields"
                % self.filename
            )
            return {}
...
class IonBedWriter:
    ...
    def write(self, fdict):
        ...
        for line in range(0, len(fdict["chrom"])):
            myline = (
                str(fdict["chrom"][line])
                + "\t"
                + str(fdict["chromStart"][line])
                + "\t"
                + str(fdict["chromEnd"][line])
            )
    ...
    def writeline(self, ldict):
        ...
        if not type(ldict["chrom"]) == str:
            print(
                "IonBedWriter ERROR expected type(chrom)==str for writeline function."
            )
            return
