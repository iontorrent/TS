...
COLUMNS = {
    ...
    "nucleotide_type": {
        ...
        "width": "80px",
        "options": [
            {"display": "DNA", "value": "DNA"},
            {"display": "RNA", "value": "RNA"},
        ],
...
CUSTOM_COLUMNS = {
    ...
    "custom_dropdown": {
        ...
        "editable": True,
        "width": "150px",
        "options": [{"display": "My option", "value": "myoption"}],
...
def custom_types():
    return list(CUSTOM_COLUMNS.keys())
    ...
    for column in list(COLUMNS.values()):
        ret.append(
            dict(
                (k, column[k])
                for k in column
                if (k in column) and (k in ["field", "description", "editable"])
            )
        )

    ret.sort(key=lambda v: v["field"])
    ret.extend(list(CUSTOM_COLUMNS.values()))
    return ret
