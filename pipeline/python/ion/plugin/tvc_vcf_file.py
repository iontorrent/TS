...
class TvcVcfFile:
    ...
    def flush(self):
        ...
        for contig_idx, contig_id in enumerate(self.__contig_id_list):
            ...
            for pos_key in sorted_pos:
                data_to_be_written = (
                    set(data_in_contig[pos_key])
                    if self.__uniq_flag
                    else data_in_contig[pos_key]
                )
                if (not self.__allow_same_position) and len(data_to_be_written) > 1:
                    raise ValueError(
                        "Writing multiple vcf lines at the same position %s:%d is not allowed. Use self.allow_same_position(True) to allow."
                        % (contig_id, pos_key)
                    )
    ...
    def is_valid_position(self, allele_dict):
        ...
        if (
            my_pos < 1
            or my_pos + len(my_ref) > self.contig_list[my_contig_idx]["length"] + 1
        ):
            return False
        return True
    ...
    def compare_positions(self, allele_dict_0, allele_dict_1):
        ...
        if contig_idx_0 is None:
            contig_idx_0 = self.__contig_id_list.index(
                allele_dict_0.get("contig", allele_dict_0.get("CHROM", None))
            )
        contig_idx_1 = allele_dict_1.get("contig_idx", None)
        if contig_idx_1 is None:
            contig_idx_1 = self.__contig_id_list.index(
                allele_dict_1.get("contig", allele_dict_1.get("CHROM", None))
            )
    ...
    def __vcf_record_basic_sanity_check(self, my_vcf_dict, my_offset=None):
        ...
        try:
            ...
            raise ValueError(
                "The CHROM of the vcf line %s:%d is not in the contigs specified in the header."
                % (my_vcf_dict["CHROM"], my_vcf_dict["POS"])
            )
            return False
        ...
        if not self.is_valid_position(
            {"contig_idx": my_contig_idx, "pos": my_pos, "ref": my_vcf_dict["REF"]}
        ):
            raise ValueError(
                "The reference allele %s at %s:%d is beyond the contig of length %d."
                % (
                    my_vcf_dict["REF"],
                    my_vcf_dict["CHROM"],
                    my_vcf_dict["POS"],
                    self.contig_list[my_contig_idx]["length"],
                )
            )
            return False
        ...
        if (
            current["where"] > self.__previous["where"]
        ):  # The most common case  (i.e., iterate over the vcf file) comes first
        ...
        if compare_positions > 0:
            ...
            raise ValueError(
                "The vcf file is not sorted: The line %s:%d comes before the line %s:%d"
                % (
                    self.contig_list[small_offset["contig_idx"]]["ID"],
                    small_offset["pos"],
                    self.contig_list[large_offset["contig_idx"]]["ID"],
                    large_offset["pos"],
                )
            )
        else:
            if self.__allow_same_position:
                ...
                raise ValueError(
                    "Two vcf lines are at the same position %s:%d. Use self.allow_same_position(True) to bypass this check."
                    % (
                        self.contig_list[small_offset["contig_idx"]]["ID"],
                        small_offset["pos"],
                    )
    ...
    def is_missing_header(self):
        ...
        missing_header = (
            ...
            or self.format_tag_dict == {}
            or self.contig_list == []
            or self.__contig_id_list == []
            or self.column_to_index == {}
            or self.columns == []
            or self.vcf_double_hash_header == []
        )
    ...
    def set_header(self, template_tvc_vcf):
        ...
        if self.__data_to_write != {}:
            print(
                "WARNING: set_header: All the previously written vcf records will be lost."
            )
            self.__data_to_write = {}
        ...
        self.vcf_double_hash_header = copy.deepcopy(
            template_tvc_vcf.vcf_double_hash_header
        )
        self.set_samples(template_tvc_vcf.samples)
        # sanity check
        for column, index in list(self.column_to_index.items()):
            assert self.columns[index] == column
        self.__contig_id_list = [contig["ID"] for contig in self.contig_list]
        self.__data_to_write = dict(
            [(contig_id, {}) for contig_id in self.__contig_id_list]
        )
        return
    ...
    def set_samples(self, sample_list):
        ...
        if self.__data_to_write != {}:
            print(
                "WARNING: set_samples: All the previously written vcf records will be lost."
            )
            self.__data_to_write = dict(
                [(contig_id, {}) for contig_id in self.__contig_id_list]
            )
        ...
        self.sample_to_index = dict(
            [(key, index) for index, key in enumerate(self.samples)]
        )
        return
    ...
    def __read_column_header(self, line):
        ...
        self.column_to_index = dict(
            [(key, index) for index, key in enumerate(splitted_line[: format_idx + 1])]
        )
    ...
    def __read_info_and_format_from_header(self, line):
        ...
        my_dict["TypeConvMethod"] = {"Float": float, "Integer": int, "String": str}.get(
            my_dict["Type"], lambda x: None
        )
        try:
            ...
            if my_dict["Number"] not in ["A", "R", "G", "."]:
                raise ValueError(
                    "Unsupported Number=%s in %s" % (my_dict["Number"], line)
                )
    ...
    def __read_contig_from_header(self, line):
        ...
        my_dict = {
            "ID": line[find_indices[0] + len(find_text_list[0]) : find_indices[1]],
            "length": int(
                line[find_indices[1] + len(find_text_list[1]) : find_indices[2]]
            ),
    ...
    def __convert_to_type(self, text, my_dict):
        ...
        if value is None:
            if my_dict["Type"] == "Flag":
                ...
                raise ValueError(
                    "Unsupported type for data conversion: %s" % my_dict["Type"]
                )
    ...
    def __encode_values(self, my_value, my_dict, num_alt):
        ...
        if my_dict["IsMultipleEntries"]:
            return ",".join(
                [
                    self.__encode_one_value(self.__convert_to_type(v, my_dict))
                    for v in my_value
                ]
            )
        else:
    ...
    def __get_key_dict(self, key, is_info_field):
        ...
        try:
            return (
                self.info_field_dict[key]
                if is_info_field
                else self.format_tag_dict[key]
            )
        except KeyError:
            raise KeyError(
                'Unknow key "%s" in %s: not specified in the header.'
                % (key, "INFO" if is_info_field else "FORMAT")
            )
            return None
    ...
    def __check_length(self, value, format_dict, num_alt):
        if (
            value in [".", ["."]]
            or format_dict["Number"] in [".", "G"]
            or format_dict["ID"] in self.__bypass_size_check_tags
        ):
            # TS-14886, IR-29193
        ...
        if expected_len != my_len:
            if format_dict["Number"] in ["A", "R"]:
                raise IndexError(
                    "The length of %s conflicts len(ALT) = %d"
                    % (str({format_dict["ID"]: value}), num_alt)
                )
            else:
                raise TypeError(
                    '%s conflicts the "Number" specified in the header.'
                    % str({format_dict["ID"]: value})
                )
            return False
        return True
    ...
    def __decode_format_tag(self, format_keys, samples_tags, num_alt):
        ...
        for sample_idx in range(len(self.samples)):
            ...
            if len(splitted_format_tag_key) != len(splitted_format_tag_value):
                raise ValueError(
                    'FORMAT "%s"and TAGS "%s"mismatches:'
                    % (format_keys, samples_tags[sample_idx])
                )
                return {}
            format_tag_list.append(
                dict(
                    [
                        self.__decode_format_tag_one_entry(
                            tag_key, splitted_format_tag_value[tag_idx], num_alt
                        )
                        for tag_idx, tag_key in enumerate(splitted_format_tag_key)
                    ]
                )
            )
    ...
    def __decode_info_field(self, info_text, num_alt):
        zipped_info_fields = [
            self.__decode_info_field_one_entry(info_text_one_entry, num_alt)
            for info_text_one_entry in info_text.split(";")
        ]
    ...
    def __decode_info_field_one_entry(self, info_text_one_entry, num_alt):
        if "=" not in info_text_one_entry:
            ...
            if len(splitted_info_text) > 2:
                raise ValueError(
                    'The INFO FIELD "%s" contains more that one "="'
                    % info_text_one_entry
                )
                return None, None
        ...
        if info_value is None:
            if my_type != "Flag":
                raise TypeError(
                    'The INFO FIELD "%s" conflicts %s'
                    % (info_text_one_entry, str(my_info_dict))
                )
    ...
    def read_vcf_record(self, line):
        ...
        try:
            vcf_dict = {
                "CHROM": splitted_line[self.column_to_index["CHROM"]],
                ...
                "INFO": self.__decode_info_field(
                    splitted_line[self.column_to_index["INFO"]], num_alt
                ),
                "FORMAT": self.__decode_format_tag(
                    splitted_line[self.column_to_index["FORMAT"]],
                    splitted_line[self.column_to_index["FORMAT"] + 1 :],
                    num_alt,
                ),
    ...
    def __encode_info_field(self, info_dict, num_alt):
        ...
        for key in info_keys:
            ...
            if my_info_dict["Type"] == "Flag":
                ...
                info_list.append(
                    "%s=%s"
                    % (key, self.__encode_values(info_dict[key], my_info_dict, num_alt))
                )
    ...
    def __encode_format_tag(self, format_tag_dict, num_alt, format_order=None):
        ...
        for key in format_order:
            ...
            format_tag_list.append(
                self.__encode_values(format_tag_dict[key], my_format_dict, num_alt)
            )
    ...
    def write(self, vcf_dict, is_write_raw_line=False):
        ...
        if self.__previous is not None:
            my_comp = self.compare_positions(
                self.__previous, {"contig": my_chrom, "pos": my_pos}
            )
            if my_comp > 0:
                pass
                ...
                raise ValueError(
                    "Writing the position %s:%d that comes before the position %s:%d that has been flushed is not allowed."
                    % (
                        my_chrom,
                        my_pos,
                        self.__contig_id_list[self.__previous["contig_idx"]],
                        self.__previous["pos"],
                    )
                )
                return
    ...
    def vcf_dict_to_text(self, vcf_dict):
        ...
        for format_tag_dict in vcf_dict["FORMAT"]:
            my_format_order_text, my_format_order, format_tag = self.__encode_format_tag(
                format_tag_dict, num_alt, format_order
            )
            if format_order is None:
        ...
        for key, index in list(self.column_to_index.items()):
...
class TestTvcVcfFileReadMode:
    ...
    def write_vcf_headers(f):
        ...
        f.write(
            b"#CHROM    POS    ID    REF    ALT    QUAL    FILTER    INFO    FORMAT    5pctMM-rep1\n"
        )
        f.seek(0)
    ...
    def runTest(cls):
        ...
        test_prefix = "test_"
        test_methods = [
            m for m in dir(t) if callable(getattr(t, m)) and m.startswith(test_prefix)
        ]
