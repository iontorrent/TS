...
def sort_vcf(vcf_in, vcf_out, contig_list):
    """
    Sort VCF file.
    vcf_in is the path to the input VCF file to be sorted.
    vcf_out is the path to the output VCF file that is sorted.
    contig_list is the (ordered) list of contigs, e.g. ['chr1', 'chr2', ..., 'chr22', 'chrX', 'chrY', 'chrM'] for hg19
    """    
    header_lines = []
    vcf_line_dict = dict([(chrom, []) for chrom in contig_list])
    # Read VCF files
    with open(vcf_in, 'r') as f_vcf:
        for line in f_vcf:
            ...
            vcf_line_dict[chrom].append((int(pos), line))
    # Sort and Write VCF file
    with open(vcf_out, 'w') as f_vcf:
        map(f_vcf.write, header_lines)
        for chrom in contig_list:
            vcf_line_dict[chrom].sort()
            map(lambda my_pair : f_vcf.write(my_pair[1]), vcf_line_dict[chrom])
...
class TvcVcfFile:
    ...
    def get_last_position_in_write_buffer(self):
        ...
        for contig_id in self.__contig_id_list[::-1]:
            keys = self.__data_to_write[contig_id].keys()
            if keys:
    ...
    def close(self):
        ...
        if self.__mode == 'w':
            try:
                self.flush()
                self.__f_vcf.close()
                if self.__do_final_sort:
                    sort_vcf(self.__vcf_path, self.__vcf_path, self.__contig_id_list)
            except:
                self.__f_vcf.close()
                return 1
        return 0
    ...
    def write(self, vcf_dict, is_write_raw_line=False):
        ...
        if self.__previous is not None:
            ...
            if my_comp > 0:
                ...
                raise ValueError('Writing multiple vcf lines at the same position %s:%d is not allowed. Use self.allow_same_position(True) to allow.' %(my_chrom, my_pos))
                return
            elif my_comp < 0:
                self.__do_final_sort = True
