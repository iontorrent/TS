...
class Validatetable(Utility):
        ...
        def expandtable(self, section):
            ...
            if "title" not in self.section:
                logging.error("title field for section type %s not provided. Using a default title." % self.section["type"])
                self.section["title"] = "Table Section"

            if "filename" not in self.section:
                logging.error("Comma Seperated (csv) file for section type %s not provided. Refer to a standard Json structure." % self.section["type"])
                ...
                try:
                    with open(self.section["filename"],'r') as f:
                        ...
                        if "columns" not in self.section:
                            logging.warning("columns fields not provided. Generating table for the first six columns of csv file.")
                            allColumns = [i.keys() for i in out][1]
                        ...
                        self.temp_dict = {'data': table_block, 'headers': table_block[0].keys()}
                        print self.temp_dict

                except IOError, e:
                    logging.error(e.errno)
                    logging.error("Can't open csv file %s or partial file or truncated file" % self.section["filename"])
                self.section["content"] = self.generateHTML("ion_table_template.html",self.temp_dict)
