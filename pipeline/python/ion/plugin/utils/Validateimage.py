...
class Validateimage(Utility):
    ...
    def expandimage(self, section):
        ...
        if "title" not in self.section:
            logging.error(
                "title not provided for field section type %s. Using the default title. "
                % self.section["type"]
            )
            self.section["title"] = "Image Section"
        if "resultsDir" in self.section:
            logging.debug(
                "Looking for images in resultsDir %s" % self.section["resultsDir"]
            )
            imagesinDir = glob(
                os.path.join(self.results_dir, self.section["resultsDir"], "*.png")
            )
            # imagesinDir.append(glob.glob("*.jpg"))
            imageList = [
                os.path.join(self.section["resultsDir"], os.path.basename(img))
                for img in imagesinDir
            ]
            self.section["content"] = imageList
            logging.debug(
                "Found images %s in results directory" % self.section["content"]
            )
        ...
        if "content" in self.section and self.section["content"]:
                    ...
                    if os.path.isfile(os.path.join(self.results_dir, image["source"])):
                        ...
                        if "caption" in image and image["caption"]:
                            try:
                                ...
                                imageJson["caption"] = ".".join(
                                    os.path.basename(image["source"]).split(".")[0:-1]
                                )  # Default caption for images
                                logging.debug("Using default filename as caption")
                        else:
                            logging.debug(
                                "No image caption found. Using the default filename as caption"
                            )
                            imageJson["caption"] = ".".join(
                                os.path.basename(image["source"]).split(".")[0:-1]
                            )  # Default caption for imag
                        ...
                        if "description" in image and image["description"]:
                            try:
                                ...
                                imageJson[
                                    "description"
                                ] = ""  # blank is the default description
                        else:
                            logging.debug(
                                "No description found. Using the default description as blank"
                            )
                            imageJson[
                                "description"
                            ] = ""  # blank if the default description

                        self.imageArray.append(imageJson)
                    else:
                        logging.error(
                            "Image %s does not exist"
                            % os.path.join(self.results_dir, image["source"])
                        )
            ...
            logging.debug(
                "No content section found. Checking for images in the results directory"
            )
