...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-z", "--do-zipping", dest="do_zipping", action="store_true", help="zipping"
    )
    ...
    def set_result_status(status):
        try:
            if os.path.exists(primary_key_file):
                ...
                printtime(
                    "MergeTLStatus %s\tpid %d\tpk file %s started"
    ...
    if args.do_sigproc:
        ...
        sigproc.mergeSigProcResults(
            dirs, env["SIGPROC_RESULTS"], env["shortRunName"], exclusionMaskFile
        )
    ...
    if args.do_basecalling:
                    ...
                    with open(blockstatus_return_code_file, "r") as f:
                        ...
                        if "Basecaller=0" in text:
                            ...
                            with open(
                                ...
                                if (
                                    return_code_text == "3"
                                    and subdir in corner_P0_blocks + corner_P1_blocks
                                ):
                                    printtime(
        ...
        try:
            sigproc.mergeAvgNukeTraces(
                dirs, env["SIGPROC_RESULTS"], env["libraryKey"], "Library Beads"
            )
        except Exception:
            printtime(
        ...
        try:
            sigproc.mergeAvgNukeTraces(
                dirs, env["SIGPROC_RESULTS"], env["tfKey"], "Test Fragment Beads"
            )
        except Exception:
            printtime(
        ...
        try:
            ...
            basecaller.merge_datasets_basecaller_json(dirs, env["BASECALLER_RESULTS"])
            basecaller.merge_basecaller_json(dirs, env["BASECALLER_RESULTS"])
            basecaller.merge_EndBarcodeReadCounts(dirs, env["BASECALLER_RESULTS"])
        except Exception:
            traceback.print_exc()
        ...
        try:
            ...
            block_bam_list = [
                os.path.join(adir, env["BASECALLER_RESULTS"], "rawtf.basecaller.bam")
                for adir in dirs
            ]
            block_bam_list = [
                block_bam_filename
        ...
        if do_unfiltered_processing:
            ...
            try:
                ...
                basecaller.merge_datasets_basecaller_json(
                    dirs, os.path.join(env["BASECALLER_RESULTS"], "unfiltered.trimmed")
                )
    ...
    if args.do_zipping:
        ...
        if not os.path.exists(
            os.path.join(env["BASECALLER_RESULTS"], "datasets_basecaller.json")
        ):
    ...
    ### OLD per block processing
        ...
        try:
            ...
            for dataset in basecaller_datasets["datasets"]:
                ...
                ionstats_plots.read_length_sparkline(
                    ...
                    os.path.join(
                        env["BASECALLER_RESULTS"],
                        dataset["file_prefix"] + ".sparkline.png",
                    ),
                    graph_max_x,
                )
        ...
        if reference_selected:
            try:
                ...
                ionstats_plots.base_error_plot(
                    "ionstats_alignment.json", "base_error_plot.png", graph_max_x
                )
        ...
        try:
            wells_beadogram.generate_wells_beadogram(
                env["BASECALLER_RESULTS"], env["SIGPROC_RESULTS"]
            )
        except Exception:
        ...
        try:
            ...
            if os.path.exists(
                ...
                tf_reference_filename = os.path.join(
                    "/results/referenceLibrary/TestFragment",
                    env["tfKey"],
                    "DefaultTFs.fasta",
                )
                ...
                ionstats_tf_filename = os.path.join(
                    env["BASECALLER_RESULTS"], "ionstats_tf.json"
                )
                tfstatsjson_path = os.path.join(
                    env["BASECALLER_RESULTS"], "TFStats.json"
                )

                printtime("TF: Mapping '%s'" % tf_basecaller_bam_filename)
                alignment.alignTFs(
                    tf_basecaller_bam_filename, tfbam_filename, tf_reference_filename
                )

                ionstats.generate_ionstats_tf(
                    tfbam_filename, tf_reference_filename, ionstats_tf_filename
                )

                ionstats_plots.tf_length_histograms(ionstats_tf_filename, ".")

                ionstats.generate_legacy_tf_files(
                    ionstats_tf_filename, tfstatsjson_path
                )
        ...
        prefix_list = [
            dataset["file_prefix"]
            for dataset in basecaller_datasets.get("datasets", [])
        ]
            ...
            for prefix in prefix_list:
                try:
                    ...
                    if os.path.exists(src):
                        os.symlink(
                            os.path.relpath(src, os.path.dirname(filename)), filename
                        )
                except Exception:
        ...
        dst = os.path.join(
            download_links, "%s_%s.rawtf.bam" % (env["expName"], env["resultsName"])
        )
