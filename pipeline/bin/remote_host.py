...
def check_connection(connection_string, timeout=10):
    try:
        ...
        print('The connection string "%s" is invalid: %s' % (connection_string, err))
        return False
    try:
...
if __name__ == "__main__":
    parser = optparse.OptionParser(
        usage="%prog [options] [HOST:PORT]...", description=__doc__
    )
    parser.add_option(
        "-t",
        "--timeout",
    ...
    for remote in remotes:
        t = threading.Thread(target=queued_check, args=(queue, remote, opts.timeout))
        threads.append(t)
        t.start()
