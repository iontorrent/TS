...
if __name__ == '__main__':
    ...
    parser.add_option('-o', '--out-base-name',
                      help='Base name for the output file', dest='out_base_name')
    ...
    parser.add_option('-b', '--threads',            help='Number of tmap threads',
                      dest='threads', type="int", default=multiprocessing.cpu_count())
    ...
    parser.add_option('--mark-duplicates',    help='??',
                      dest='mark_duplicates', action="store_true", default=False)
    parser.add_option('--bidirectional',      help='??',
                      dest='bidirectional', action="store_true", default=False)
    parser.add_option('--indexing',           help='??', dest='indexing', action="store_true", default=False)
    parser.add_option('--skip-sorting',       help='??',
                      dest='skip_sorting', action="store_true", default=False)
    parser.add_option('-c', '--align-all-reads',    help='??',
                      dest='align_all_reads', action="store_true", default=False)
    parser.add_option('-a', '--genome-path',        help='??', dest='genome_path',
                      default="/referenceLibrary:/results/referenceLibrary:/opt/ion/referenceLibrary")
    parser.add_option('-p', '--sam-parsed',         help='??',
                      dest='sam_parsed', action="store_true", default=False)
    parser.add_option('--realign',            help='Enable realignment',
                      dest='realign', action="store_true", default=False)
    parser.add_option('--aligner-format-version',
                      help='aligner format version e.g. tmap-f3', dest='aligner_format_version')
    ...
    if options.aligner == 'tmap':
        ...
        command = "java -Xmx8g -jar /opt/picard/picard-tools-current/picard.jar SamToFastq I=%s F=%s" % (
            options.readFile, fastqpipe)
        command += " | /results/plugins/bowtielauncher/bowtie2 -p%d -x %s -U %s" % (
            options.threads, refFasta, fastqpipe)
    ...
    if options.mark_duplicates:
        ...
        json_name = ('BamDuplicates.%s.json') % (os.path.normpath(options.out_base_name)) if os.path.normpath(
            options.out_base_name) != 'rawlib' else 'BamDuplicates.json'
