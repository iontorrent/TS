...
if __name__ == "__main__":
    ...
    def set_result_status(status):
        try:
            if not isBlock:
                ...
                printtime(
                    "BlockTLStatus %s\tpid %d\tpk file %s started"
    ...
    def upload_analysismetrics(bfmaskstatspath):
        ...
        try:
            if not isBlock:
                ...
                result = jobserver.uploadanalysismetrics(
                    full_bfmaskstatspath, primary_key_file
                )
    ...
    def update_bfmask_artifacts(bfmaskPath, bfmaskstatspath, outputdir, plot_title):
        ...
        try:
            beadDensityPlot.genHeatmap(
                bfmaskPath, bfmaskstatspath, outputdir, plot_title
            )
    ...
    if args.do_sigproc:
        ...
        if status != 0:
            ...
            try:
                f = open(
                    os.path.join(env["SIGPROC_RESULTS"], "analysis_return_code.txt"),
                    "w",
                )
                f.write(str(status))
                f.close()
        ...
        try:
            ...
            update_bfmask_artifacts(
                bfmaskPath, bfmaskstatspath, "./", plot_title=env["shortRunName"]
            )
        except Exception:
            traceback.print_exc()
        ...
        try:
            f = open(
                os.path.join(env["SIGPROC_RESULTS"], "analysis_return_code.txt"), "w"
            )
            f.write(str(status))
            f.close()
            os.chmod(
                os.path.join(env["SIGPROC_RESULTS"], "analysis_return_code.txt"), 0o0775
            )
        except Exception:
            traceback.print_exc()
    ...
    if args.do_basecalling:
        ...
        sigproc.generate_raw_data_traces(
            env["libraryKey"], env["tfKey"], env["flowOrder"], env["SIGPROC_RESULTS"]
        )
        ...
        if not os.path.exists(os.path.join(env["SIGPROC_RESULTS"], "1.wells")):
            printtime(
                "ERROR: missing %s" % os.path.join(env["SIGPROC_RESULTS"], "1.wells")
            )
        ...
        if calibration_mode == "no_recal":
            ...
            printtime(
                "DEBUG: Flow Space Recalibration is enabled, Mode: %s"
                % calibration_mode
            )
            ...
            try:
                ...
                if calibration_mode == "panel_recal":
                    ...
                    if not "--calibration-training=" in prebasecallerArgs:
                        prebasecallerArgs = (
                            prebasecallerArgs + " --calibration-training=100000"
                        )
                ...
                additional_basecallerArgs += " --phase-estimation-file " + os.path.join(
                    env["BASECALLER_RESULTS"], "recalibration", "BaseCaller.json"
                )
                ...
                for dataset in basecaller_recalibration_datasets["datasets"]:
                    ...
                    # Otherwise we align the BAM files and provide the aligned BAMs to Calibration
                        ...
                        referenceName = basecaller_recalibration_datasets[
                            "read_groups"
                        ...
                        calibration_bam_base = os.path.join(
                ...
                if len(calibration_input_bams) > 0:
                    ...
                    additional_basecallerArgs += " --calibration-json " + os.path.join(
                        env["BASECALLER_RESULTS"], "Calibration.json"
                    )
