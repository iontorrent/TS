...
def parse_vcf_line(line,info_a_list,format_a_list):
    ...
    for item in info_items:
        ...
        if len(subitem) == 1:
            subitem.append('')
        if len(subitem) != 2:
            print '[unfiy_variants] Problem parsing INFO field in %s:%d' % (vcf_record['chr'],vcf_record['pos'])
...
def merge_annotation_into_vcf(vcf_record, vcf_annotation):
    ...
    ref_extension = vcf_record['ref'][len(vcf_annotation['ref']):]
...
def main():
    ...
    parser.add_option('-s', '--hotspot-tvc-vcf',        help='VCF file with hotspot tvc variants', dest='hotspot_tvc_vcf')
