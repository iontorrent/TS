...
def main():
    ...
    parser.add_option('-s', '--hotspot-vcf',    help='VCF.gz (+.tbi) file specifying exact hotspot positions', dest='hotspot_vcf') 
    parser.add_option('-i', '--input-bam',      help='BAM file containing aligned reads', dest='bamfile') 
    parser.add_option('-r', '--reference-fasta',help='FASTA file containing reference genome', dest='reference') 
    parser.add_option('-o', '--output-dir',     help='Output directory', dest='outdir', default='.')
    ...
    if options.bedfile and num_variants_before_bed > 0:
        ...
        bedfilter_command  =    '%s/vcftools' % options.rundir
