...
class ConfigureOptionsManager:
    ...
    def load_parameters(self, configuration):
        ...
        # Dump a temp parameter JSON file for determining the bin version
        mini_param_dict = {'meta': dict([(key, self.__options['parameters'].get('meta', {}).get(key, '')) for key in ('tvcargs', 'unifyargs', 'tmapargs') if key in self.__options['parameters'].get('meta', {})])}
        _, tmp_json_path = tempfile.mkstemp(suffix='.json')
        with open(tmp_json_path, 'w') as f_json:
            json.dump(mini_param_dict, f_json)
        
        # Get the versions of tvc and tvcutils that will be actually used.
        # bin_version_lines should be of the format: "tvc <MAJOR>.<MINOR>-<BUILD> (<GITHASH>)\ntvcutils <MAJOR>.<MINOR>-<BUILD> (<GITHASH>)\n<MAJOR>.<MINOR>.<BUILD> (<GITHASH>) (<TIMESTAMP>)"
        bin_version_lines = execute_output('python %s/bin/variant_caller_pipeline.py -v -p %s' %(DIRNAME, tmp_json_path))
        self.__options['tvc_version'] = 'tvc unknown version'
        self.__options['tvcutils_version'] = 'tvcutils unknown version'
        for line in bin_version_lines.split('\n'):
            if line.startswith('tvc '): 
                self.__options['tvc_version'] = line
            elif line.startswith('tvcutils '):
                self.__options['tvcutils_version'] = line
    ...
    def get_options(self, configuration):
        ...
        # The default reference/targets/hotspots/sse files 
        ...
        self.__options['sse_bed']                  = json_in['pluginconfig']['meta'].get('sse_bed', '').strip(' \t\n') 
    ...
    def __load_barcode_info(self, configuration):
        ...
        for bam_dict in configuration['bams']:
            ...
            my_barcode_option['sse_bed']                  = self.__options['sse_bed'] 
            ...
            if self.is_auto_start() or my_barcode_option['targets_bed_unmerged'] in ['.BARCODESJSON', '\\BARCODESJSON']:
                ...
                # Use the default if custom sse_bed is not used
                if not my_barcode_option['sse_bed']:
...
def prepare_sse_mask(options, queued_bams):
    ...
    if not os.path.exists(sse_vcf):         
        ...
        if os.path.exists(validated_sse_bed):
            ...
            prepare_sse_vcf_command += ' --left-alignment off '
            prepare_sse_vcf_command += ' --allow-block-substitutions on '
...
def print_options(vc_options, bam):
    ...
    printtime('                               %s' %vc_options.serve_option('tvcutils_version'))    
...
def load_render_context(render_context, bam, options):
    ...
    if options.serve_option('has_umt', bam['name']):
        with open(os.path.join(bam['results_directory'], 'consensus_metrics.txt'), 'r') as f_cm:
            consensus_metrics_dict = dict([line.split(':') for line in f_cm])    
        render_context['summary']['median_depth'] = consensus_metrics_dict.get('Median read coverage', 'N/A').strip()
        render_context['summary']['median_num_fam3'] = consensus_metrics_dict.get('Median molecular coverage', 'N/A').strip()     
        render_context['summary']['fm3_pass80'] = '%s - %s' %(consensus_metrics_dict.get('Median LOD percent', 'N/A').strip(), consensus_metrics_dict.get('80th percentile LOD percent', 'N/A').strip())
        for cons_key in ('median_depth', 'median_num_fam3'):
            try:
                render_context['summary'][cons_key] = int(float(render_context['summary'][cons_key]))
            except ValueError:
                pass
    else:
        for cons_key in ('median_depth', 'median_num_fam3', 'fm3_pass80'):
            render_context['summary'][cons_key] = 'N/A' 
