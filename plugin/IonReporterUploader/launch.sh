...
echo TSP_ANALYSIS_CYCLES=$TSP_ANALYSIS_CYCLES >>${metaDataFile}
...
echo TSP_FILEPATH_FASTQ=$TSP_FILEPATH_FASTQ >>${metaDataFile}
echo TSP_URLPATH_FASTQ=$TSP_URLPATH_FASTQ >>${metaDataFile}
echo TSP_FILEPATH_SFF=$TSP_FILEPATH_SFF >>${metaDataFile}
echo TSP_URLPATH_SFF=$TSP_URLPATH_SFF >>${metaDataFile}
echo TSP_FILEPATH_TF_SFF=$TSP_FILEPATH_TF_SFF >>${metaDataFile}
echo TSP_URLPATH_TF_SFF=$TSP_URLPATH_TF_SFF >>${metaDataFile}
echo TSP_FILEPATH_BAM=$TSP_FILEPATH_BAM >>${metaDataFile}
echo TSP_URLPATH_BAM=$TSP_URLPATH_BAM >>${metaDataFile}
echo TSP_FILEPATH_SAM=$TSP_FILEPATH_SAM >>${metaDataFile}
echo TSP_URLPATH_SAM=$TSP_URLPATH_SAM >>${metaDataFile}
...
echo "${RUNINFO__API_URL}/v1/plugin/?format=json&name=IonReporterUploader&active=true" 
wget  -q -O ${RESULTS_DIR}/objects.json "${RUNINFO__API_URL}/v1/plugin/?format=json&name=IonReporterUploader&active=true" || true
wgetexitcode=$?
sleep 2
...
if [ ! -s  ${RESULTS_DIR}/objects.json ] ; then
   ...
   echo "http://127.0.0.1/rundb/api/v1/plugin/?format=json&name=IonReporterUploader&active=true"
   wget -q -O ${RESULTS_DIR}/objects.json "http://127.0.0.1/rundb/api/v1/plugin/?format=json&name=IonReporterUploader&active=true"  || true
   wgetexitcode=$?
   sleep 2
   if [ ! -s  ${RESULTS_DIR}/objects.json ] ; then
