...
if __name__ == "__main__":
    ...
    parser.add_option('-V', '--vcf-file2', help='the VCF file name (no path)', dest='vcf_file2')
    ...
    if options.vcf_file != None:
        vcf_url1 = "{plugin_url}" + '/' + options.vcf_file
        fxml.write('\t\t<Resource name="'+ options.vcf_file + '" path="' + vcf_url1 + '"/>\n')
    if options.vcf_file2 != None:
        vcf_url2 = "{plugin_url}" + '/' + options.vcf_file2
        fxml.write('\t\t<Resource name="'+ options.vcf_file2 + '" path="' + vcf_url2 + '"/>\n')
    fxml.write('\t\t<Resource name="'+ options.bam_file + '" path="' + bam_url + '"/>\n')
    fxml.write('\t</Resources>\n')
    fxml.write('\t<Panel name="DataPanel" height="100">\n')
    if options.vcf_file != None:
        fxml.write('\t\t<Track displayMode="EXPANDED" id="' + vcf_url1 + '" name="SNP Variants" visible="true"/>\n')
    if options.vcf_file2 != None:
        fxml.write('\t\t<Track displayMode="EXPANDED" id="' + vcf_url2 + '" name="INDEL Variants" visible="true"/>\n')
    fxml.write('\t</Panel>\n')
