  ...
  <div id="TC-titlebar" class="grid-header" style="min-height:24px;border:0">\
    ...
    <span id="TC-PlotOptions">\
      ...
      <select class="TC-selectParam TC-shy txtSelect" id="barAxis">\
       <option value=0 selected="selected">Total Mol Cov</option>\
       //<option value=2 style="display:none">Read Depths</option>\
       <option value=1>Strand Mol Cov</option>\
      </select>\
    </span>\
    ...
    <select class="TC-selectParam TC-shy txtSelect" id="overPlot">\
     ...
     <option value=1>Convert Rate %</option>\
     <option value=2>Strict Tag %</option>\
     //<option value=3 style="display:none">Strand Bias</option>\
     //<option value=4 style="display:none">GC/AT Bias</option>\
    </select>\
...
$(function () {
  ...
  var DataField = {
    ...
    covDepth_T2 : 13, // covDepth_T2
    covDepth_T3 : 14 // covDepth_T3
  };
  ...
  var LegendLabels = {
    ...
    allReads_e2e : "Func Mol Coverage",
    fwdReads_e2e : "Forward Only Mol Coverage",
    revReads_e2e  : "Reverse Only Mol Coverage",
  ...
  function customizeChart() {
    ...
    LegendLabels.rcovType = baseCoverage ? (plotParams.aveBase ? "Base Read Depth" : "Total Base Reads") : "Mol Counts";
    ...
    $("#TC-overlayLabel").attr( "title",
      ...
      "For example, Convert Rate would be reads to functional mol conversion rate in the combined "+trg+" sequences. " +
  ...
  function customizePlotOptions() {
    ...
    $("#TC-plotLabel").attr( "title",
      "Select the how the data is plotted.\n'Total Mol Cov' shows bar plots of"+rtp+" to both DNA strands.\n"+
      "'Strand Mol Cov' shows bar plots of forward and reverse DNA strand only "+rtp+" separately, above and below the 0 reads line." );
  ...
  //    msg += leadStr+" GC content: "+pcGC+"%"+br;
    ...
    if( barData ) {
      if( baseCoverage ) {
        ...
        msg += leadStr+" "+strand_tag+" mol coverage : "+commify(famc)+br;
      }
      msg += leadStr+" all mol coverage: "+commify(totalReads)+br;
    ...
    if( id === LegendLabels.fwdReads || id === LegendLabels.fwdBaseReads ) {
      ...
      msg += "Percent Functional Mol Ratio : "+sigfig(pcFunc)+'%'+br;
      if( baseCoverage ) {
        msg += "Uncovered target 5' length: "+sigfig(pcU5p)+"%"+br;
        msg += "Uncovered target 3' length: "+sigfig(pcU3p)+"%"+br;
      } else {
    ...
    if( enableReadDepthPlot && barData ) {
      if( baseCoverage ) {
       ...
       if( binBar(bin) && dataTable[bin][DataField.bin_size] > 1 ){
        ...
        msg += "Reads to Function Mol Conversion Rate : "+sigfig(100*dataTable[bin][DataField.r2f_conv_rate_all])+'%'+br ;
      }
  ...
  function updatePlot(captureScale) {
    ...
    for( var i = 0; i < plotStats.numPoints; ++i ) {
      ...
      var ucov3 = dataTable[i][revReads];
      var ucov5 = dataTable[i][fwdReads];
      ...
      if( plotParams.barAxis == 0 ) {
        if( baseCoverage ) {
          ...
          ucov5 = numReads == 0 ? 0 : ncov * (dataTable[i][funcReads]/numReads);
	...
	dmax = 0;
        for( var i = 0; i < plotStats.numPoints; ++i ) {
         ...
         for( var i = 0; i < plotStats.numPoints; ++i ) {
           ...
           var bin_size = binBar(i) ? dataTable[i][binSiz] : 1;
	   var Conver_rate = dataTable[i][DataField.r2f_conv_rate_all]/bin_size*100;
