...
<div id="DOC-chart" class="unselectable" style="border:2px solid #666;page-break-inside:avoid;display:none">\
  <div id="DOC-titlebar" class="grid-header" style="min-height:24px;border:0">\
    ...
    <select class="DOC-shy txtSelect" id="DOC-graphType">\
      <option value="Full">Maximum Molecular Depth</option>\
      <option value="View" selected="selected">99.9% of Molecules</option>\
...
This chart shows the distribution of amplicon molecular coverage.<br/><br/>\
The plot area may be re-sized by dragging the borders of the chart or hidden using<br/>\
the Collapse View button in the upper left corner.<br/><br/>\
...
$(function () {
  ...
  var LegendLabels = {
    readDepth : "Molecular Depth",
    covCount : "Amplicons",
    cumCover : "Cumulative Amplicons"
  };
  ...
  function customizeChart() {
    ...
    $("#DOC-plotLabel").attr( "title",
      "Select how the depth of molecular coverage data is plotted.\n"+
      "'Maximum Molecular Depth' specifies to plot molecular coverage out to the maximum depth. This is the default "+
      "setting if the there is a significant coverage out at the largest molecular depths.\n"+
      "'99.9% of Molecules' specifies to plot up to 99.9% of the total molecular depths from 0x, i.e. up to where the "+
      "Cumulative Amplicons curve cuts the right y-axis at 0.01%. This view is useful to avoid outlier molecular "+
      "depths that add would otherwise make the majority of distribution squeezed up against the y-axis.\n"+
      "This is the default plot if 99.9% of the molecules fall in the first half of the full range of molecular depths.\n"+
      "'Normalized Coverage' uses a depth axis that is normalized (divided) by the average molecular depth "+
      "and plotted to twice the average coverage molecular depth. This plot is useful for generalized analysis of the "+
      "coverage when comparing different experiments or reading off molecular coverage at some fraction of the mean value." );
    $("#DOC-unzoomToggle").attr( "title",
      "Click this button to 'zoom out' the view to show coverage over the whole molecular depth range in the view "+
      "specified by the Plot selector. (Has no effect if the view was not previously zoomed-in.)" );
    $("#DOC-autoZoomLabel").attr( "title",
      "Select how the y-axis zoom works when selecting a set of molecular depths (x-axis data) to zoom in to view. " +
      "When checked the y-axis range will be automatically set to the largest molecular count "+
      "of the molecular depths currently in view. This mode is particularly useful to magnify the view for molecular depths "+
      "with relatively low representation. When unchecked the range is set to the largest molecular count for any "+
      "molecular depth, regardless of the current data in view.\n(The right-hand y-axis scale is fixed to 0% - 100%.)" );
    $("#DOC-showLegendLabel").attr( "title", "Select whether the legend is displayed over the plot area." );
    $("#DOC-binSizeLabel").attr( "title",
      "Sets the size (width) of the data bins for counting molecular depth. Using larger bins typically allows the "+
      "distribution of molecular counts for ranges of molecular depths to be more easily visualized or interactively "+
      "assessing finer or broader distributions of target coverage. An appropriate default value for Bin Size "+
      "is determined based on the maximum molecular depth and this value is reset whenever the value for 'Plot' is "+
    ...
    $("#DOC-include0xLabel").attr( "title",
      "Uncheck this value to have the 0x coverage ignorred in the plotted data. This will cause the 0x molecular depth data, e.g. "+
      "the number of target bases that had no molecular counts, to be ignorred in the display. The molecular depth will be binned starting "+
      "1x rather than 0x so that at Bin Size 1 no data point will be plotted or at Bin Size 10 the first bin will "+
      "represent 1x-10x molecular depths rather than 0x-9x molecular depths. This is useful where you are just interested in coverage "+
      "over amplicons that were covered, especially if the number of missed targets was high and the 0x data pushes the "+
  ...
  function tooltipMessage(item,bin) {
    ...
    msg += "Number of amplicons : "+commify(y.toFixed(0))+br;
    msg += "Percentage of amplicons : "+sigfig(ypc)+"%"+br;
