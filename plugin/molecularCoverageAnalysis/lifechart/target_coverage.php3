...
if( $allout )
{
  ...
  system("../bin/target_coverage.pl $options \"$dataFile\" \"$chrom\" \"$gene\" $covmin $covmax $maxrows $clipleft $clipright $numrec $redirect");
}
else
{
  system("../bin/target_coverage.pl $options \"$dataFile\" \"$chrom\" \"$gene\" $covmin $covmax $maxrows $clipleft $clipright $numrec");
}
?>
