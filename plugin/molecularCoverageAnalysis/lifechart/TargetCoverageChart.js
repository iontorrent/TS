...
document.write('\
	...
	<option value=0 selected="selected">Total Molecules</option>\
	<option value=1>Strand Molecules</option>\
	<option value=2>Reads per Molecule </option>\
	</select>\
	</span>\
	...
	<option value=1>Perc Functional Reads</option>\
	<option value=2>Strict Tag</option>\
	</select>\
	...
	<td><input id="TC-export" type="button" value="Export" style="display:none" ></td>\
	</tr></table>\
	<table><tr>\
	...
	<input type="button" id="TC-OpenInIGV" value="View in IGV" style="display:none" >\
	<input type="button" id="TC-OpenInRCC" value="View in Reference Coverage Chart" style="display:none" >\
	</div>\
	</div>\
...
');
	...
	$(function () {
		...
		var DataField = {
			...
			target_id : 3,
			gene_id : 4,
			target_gc : 5, // func_fam_cov
			func_fam_cov : 5,
			cov_length : 6, // lod
			LOD : 6, 
			uncov_5p : 7 , // func_strict_rate
			func_strict_rate : 7,
			uncov_3p : 8, // loss_from_strand
			loss_from_strand : 8 ,
			reads_tot : 9, // fwd_only_fam
			fwd_only_fam : 9,
			reads_fwd : 10, // rev_only_fam
			rev_only_fam : 10,
			reads_rev : 11, // both_fam_cov 
			both_fam_cov : 11,
			bin_size : 1, // bin_size
			bin_length : 2, // bin_length
			sum_gcbias : 4, // sum_gcbias
			covDepth_T2 : 12, // r2f_conv_rate_all
			r2f_conv_rate_all : 12,
			average_famsize : 13,
			covDepth_T2 : 14, // per_small_family fam_size =1~2
			covDepth_T3 : 15, //  per_median_family fam_size = 3~10
			per_small_family : 14, 
			per_median_family : 15,
			per_large_family : 16 
		};
		...
		var LegendLabels = {
			...
			rcovType : "Molecules",
			rcovOrig : "Reads",
			allReads : "Total Molecules",
			fwdReads : "w/ FWD Molecules",
			revReads : "w/ REV Molecules",
			...
			covDepth_1x : "% Reads contributed to molecules with reads < 3",
			covDepth_T1 : "% Reads contributed to molecules with 3 &#8804 reads < 30",
			covDepth_T2 : "% Reads contributed to molecules with reads &#8805 30",
			...
			allReads_e2e : " Func Molecules",
			fwdReads_e2e : "w/ FWD Func Molecules",
			revReads_e2e  : "w/ REV Func Molecules",
			percentGC : "GC Content",
			percentage : "Percentage",
			ConvertRate : "Perc Functional Reads",
			targLen : "Target Length",
			strictUMT: "% w/ strict Mol Tags",
		...
		var ColorSet = {
			...
			bothReads : "rgb(200,100,0)",
			covDepth_1x : "rgb(240,120,100)",
			covDepth_T1 : "rgb(100,240,120)",
			covDepth_T2 : "rgb(120,100,240)",
			covDepth_T3 : "rgb(129,105,0)",
			allReads_pss : "% Passing Reads",
			percentGC : "rgb(200,62,128)",
			strictUMT : "rgb(200,62,128)",
			targLen : "rgb(64,126,200)",
			ConvertRate : "rgb(196,145,2)",
		...
		function customizeChart() {
			...
			LegendLabels.rcovType = baseCoverage ? (plotParams.aveBase ? "Base Read Depth" : "Total Base Reads") : "Molecules";
			LegendLabels.rcovOrig = baseCoverage ? "base reads" : "assigned molecules";
			...
			var barShading = ampliconReads ? "number of amplicons covered " + (reportPassingCov ? "at 70% of their length" : "functional mol counts")
			...
			$("#TC-overlayLabel").attr( "title",
				...
				"For example, Perc Functional reads would be percentage of reads contributed to functional molecules in the combined "+trg+" sequences. " +
			...
			$("#TC-filterGeneSymLabel").attr( "title",
				...
				"Alternatively you can filter on any set of attributes specified in the "+trg+"s file by giving the name and value, "+
				"e.g. 'Pool=1' or 'GENE_ID=TBP', or in combination, e.g. 'GENE_ID=TBP;POOL=1'. Attribute name and value have to "+
				"be wholey matched but the matching is case-insensitive. Press the enter or tab key to perform the filter." );
		...
		function customizePlotOptions() {
			...
			$("#TC-plotLabel").attr( "title",
				"Select the how the data is plotted.\n'Total Molecules' shows bar plots of"+rtp+" to both DNA strands.\n"+
				"'Strand Molecules' shows bar plots of forward(FWD) and reverse(REV) DNA strand only "+rtp+" separately, above and below the 0 reads line." );
		...
		function tooltipMessage(id,bin) {
			...
			var funcfam = dataTable[bin][DataField.func_fam_cov];  // or functional families 
			var fwdReads = dataTable[bin][DataField.fwd_only_fam]; // families only assigned to fwd strand
			var revReads = dataTable[bin][DataField.rev_only_fam]; // families only assigned to rev strand
			var bothReads = dataTable[bin][DataField.both_fam_cov]; // families assigned to both strand
			...
			// derived data
			var totalReads = fwdReads + revReads + bothReads;
			var pcFwd = totalReads > 0 ? 100 * fwdReads / totalReads : 50;
			var pcRev = totalReads > 0 ? 100 * revReads / totalReads : 50;
			var pcFunc = totalReads > 0 ? 100 *  funcfam/ totalReads : 100;
			...
			if( lengthNormal ) {
				...
				meanReads = binBar(bin) ? showReads/dataTable[bin][DataField.bin_size] : totalReads;
			}
			// compose message
			var br = "<br/>";
			var msg = "(Bin#"+(bin+1)+")"+br;
			var contigType = amplicons == 3 ? "Contig: " : (transcriptBed ? "Transcript: " : "Chromosome: ");
			...
			if( barData ) {
				if( baseCoverage ) {
					...
					msg += leadStr+" "+strand_tag+" assigned molecules: "+commify(famc)+br;
				}
				msg += leadStr+" assigned functional molecules: "+commify(funcfam)+br;
			...
			if( id === LegendLabels.fwdReads || id === LegendLabels.fwdBaseReads ) {
				...
				msg += "Percent "+readStr+sigfig(pcRev)+'%'+br;
				if( baseCoverage ) {
					msg += "Uncovered target 5' length: "+sigfig(pcU5p)+"%"+br;
					msg += "Uncovered target 3' length: "+sigfig(pcU3p)+"%"+br;
				} else {
				...
				msg += "Percentage of functional molecules: "+sigfig(pcFunc)+'%'+br;
				if( baseCoverage ) {
					msg += "Uncovered target 5' length: "+sigfig(pcU5p)+"%"+br;
					msg += "Uncovered target 3' length: "+sigfig(pcU3p)+"%"+br;
				} else {
			...
			if( enableReadDepthPlot && barData ) {
				...
				if( binBar(bin) && dataTable[bin][DataField.bin_size] > 1 ){
					...
					msg += "LOD: "+sigfig(100*dataTable[bin][DataField.LOD])+'%'+br;
					msg += "Reads to functional molecules conversion rate: "+sigfig(100*dataTable[bin][DataField.r2f_conv_rate_all])+'%'+br ;
					msg += " Percentage of reads contributed to molecules with reads < 3: "+sigfig(dataTable[bin][DataField.per_small_family])+'%'+br;
				 	msg += " Percentage of reads contributed to molecules with 3 &#8804 reads < 30: "+sigfig(dataTable[bin][DataField.per_median_family])+'%'+br;
				 	msg += " Percentage of reads contributed to molecules with &#8805 30 reads: "+sigfig(dataTable[bin][DataField.per_large_family])+'%'+br;
				}
		...
		function updatePlot(captureScale) {
			...
			options = {
				...
				series: {stack:null, axis:1, bars:{show:true,align:"left"}, line:{show:false}},
			...
			var fwdReads = DataField.fwd_only_fam; // forward only families
			var revReads = DataField.rev_only_fam; // reverse only families
			var bothReads = DataField.both_fam_cov; // both families
			var funcReads = DataField.func_fam_cov;
			var lossPer = DataField.loss_from_strand;
			var PerSmallFam  = DataField.per_small_family;
			var PerMedianFam  = DataField.per_median_family;
			var PerLargeFam  = DataField.per_large_family;
			var logAxis = plotParams.logAxis;
			var barScale;
			for( var i = 0; i < plotStats.numPoints; ++i ) {
				var numReads = dataTable[i][fwdReads]+dataTable[i][revReads]+dataTable[i][bothReads];
				if(plotParams.barAxis == 2){
					numReads = 110;
					logAxis = 0; 
				}
				var targLen = binBar(i) ? dataTable[i][binLen] : dataTable[i][posEnd]-dataTable[i][posSrt]+1;
				var ucov3 = dataTable[i][funcReads];
				var ucov5 = dataTable[i][funcReads];
				if (numReads == dataTable[i][funcReads]){
					ucov3 = dataTable[i][fwdReads]+dataTable[i][bothReads];
					ucov5 = dataTable[i][revReads]+dataTable[i][bothReads];
				}
				var lossP = dataTable[i][lossPer];
				if( lengthNormal ) {
				...
				var frds = (dataTable[i][fwdReads]+dataTable[i][bothReads])* axisScale;
				var rrds = (dataTable[i][revReads]+dataTable[i][bothReads])* axisScale;
				var trds = numReads * axisScale;
				var lrds = (dataTable[i][funcReads])* axisScale;
				...
				var lcov = logAxis ? log10(lrds) : lrds;
				if( plotParams.barAxis == 0 ) {
					if( baseCoverage ) {
						...
						ucov5 = numReads == 0 ? 0 : lcov;
						//ucov5 = numReads == 0 ? 0 : ncov * 0.5 ;
					}
					if( baseCoverage ) {
						d1.push( [i,ucov3] );
						d2.push( [i,ncov] );
					...
					if( baseCoverage ) {
						...
						r_e2e = ucov3;
						if(logAxis){
						ucov3 = (dataTable[i][fwdReads]+dataTable[i][bothReads]) == 0 ? 0 : fcov * log10(f_e2e)/log10(dataTable[i][fwdReads]+dataTable[i][bothReads]);
						ucov5 = (dataTable[i][revReads]+dataTable[i][bothReads]) == 0 ? 0 : rcov * log10(r_e2e)/log10(dataTable[i][revReads]+dataTable[i][bothReads]);
						}
						else{
						ucov3 = (dataTable[i][fwdReads]+dataTable[i][bothReads]) == 0 ? 0 : fcov * f_e2e/(dataTable[i][fwdReads]+dataTable[i][bothReads]);
						ucov5 = (dataTable[i][revReads]+dataTable[i][bothReads]) == 0 ? 0 : rcov * r_e2e/(dataTable[i][revReads]+dataTable[i][bothReads]);
						}
					}
					d1.push( [i,fcov] );
					d2.push( [i,ucov3] );
					d3.push( [i,rcov] );
					d4.push( [i,ucov5] );
				} else if(plotParams.barAxis == 2){
					var Scale = 1/numReads;
					d1.push( [i,ncov*dataTable[i][PerSmallFam]*Scale] );
					d2.push( [i,ncov*dataTable[i][PerMedianFam]*Scale] );
					d3.push( [i,ncov*dataTable[i][PerLargeFam]*Scale] );
			...
			if(plotParams.barAxis == 2) {
				ytitle=LegendLabels.percentage;
			}
			ymin = roundAxis(ymin);
			ymax = roundAxis(ymax);
			var dplace = baseCoverage ? null : 0;
			if(plotParams.barAxis == 2){	
				options.yaxes.push( {position:"left", axisLabel:ytitle, min:ymin, max:ymax, tickDecimals:dplace, tickFormatter: percentFormat} ) ;
			}
			else{
			...
			if( plotParams.overPlot > 0 ) {
				...
				if( plotParams.overPlot == 2 ) {
					aLabel = "Percentage of Strict Tag";
					pLabel = LegendLabels.strictUMT;
					pColor = ColorSet.strictUMT;
					dmax = 0;
					for( var i = 0; i < plotStats.numPoints; ++i ) {
						//var targLen = binBar(i) ? dataTable[i][binLen] : dataTable[i][posEnd]-dataTable[i][posSrt]+1;
						var bin_size = binBar(i) ? dataTable[i][binSiz] : 1;
						var pcGC = dataTable[i][DataField.func_strict_rate]/bin_size*100;
						d5.push( [(i+0.5),pcGC] );
						if( pcGC > dmax ) dmax = pcGC;
						if( pcGC < dmin || dmin == 0 ) dmin = pcGC;

					}
				} else if( plotParams.overPlot == 1 ) {
					aLabel = "Perc Functional Reads";
					pLabel = LegendLabels.ConvertRate;
					pColor = ColorSet.ConvertRate;
					dmax = 0;
					//formatter = absFormat;
					for( var i = 0; i < plotStats.numPoints; ++i ) {
						//var targLen = binBar(i) ? dataTable[i][binLen] / dataTable[i][binSiz] : dataTable[i][posEnd]-dataTable[i][posSrt]+1;
						var bin_size = binBar(i) ? dataTable[i][binSiz] : 1;
						var Conver_rate = dataTable[i][DataField.r2f_conv_rate_all]/bin_size;
						d5.push( [(i+0.5),Conver_rate] );
						if( Conver_rate > dmax ) dmax = Conver_rate;
						if( Conver_rate < dmin || dmin == 0 ) dmin = Conver_rate;
					}
				}
				/*
	...
	for( var i = 0; i < plotStats.numPoints; ++i ) {
	  ...
	  if( gcbias > dmax ) dmax = gcbias;
				...
				options.yaxes.push( {position:"right", axisLabel:aLabel, min:dmin, max:dmax,tickDecimals:0, tickFormatter: formatter} );
