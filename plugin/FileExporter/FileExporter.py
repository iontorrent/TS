...
class FileExporter(IonPlugin):
    ...
    def bamRename(self):
        for barcode in self.barcodeNames:
            fileName = self.json_dat['runinfo']['report_root_dir'] + '/' + self.json_barcodes[barcode]['bam_file']
            if self.isBarcodedRun:
                # build our new filename and move this file
                finalName = self.renameString.replace('@BARINFO@', self.barcodeFileNames[barcode])
    ...
    def launch(self, data=None):
        ...
        # in this section of code we have to make sure that non of the barcode names are substrings of each other
        # due to downstream dependencies where we are required to distinquish barcodes based on substrings of file names
        barcodeNamesToBeModified = list()
        if self.isBarcodedRun:
            for barcodeNameSub in self.barcodeNames:
                for barcodeNameFull in self.barcodeNames:
                    if barcodeNameSub != barcodeNameFull and barcodeNameSub in barcodeNameFull:
                        barcodeNamesToBeModified.append(barcodeNameSub)

        # if any barcode names have been flagged as duplicated, then we need to rename them
        for barcodeName in self.barcodeNames:
            self.barcodeFileNames[barcodeName] = barcodeName
            if barcodeName in barcodeNamesToBeModified:
                self.barcodeFileNames[barcodeName] += "_" + str(self.barcodeNames.index(barcodeName)).zfill(4)
        ...
        # replace the report name selection with the actual valid content
