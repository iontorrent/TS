...
GetOptions(
    ...
    "t|alignment-dir=s"     => \$opt->{"alignment-dir"},
...
mkdir $opt->{"out-dir"} || die "$0: unable to make directory ".$opt->{"out-dir"}.": $!\n";
  ...
  my $rLogFile = $opt->{"out-dir"} . "/" . $opt->{"base-name"} . ".Rout";
...
&writeHtml($rLogFile,$opt,$analysisName, $retVal, $rTempFile, $analysisName, @flowsToPlot);
exit(0);



sub writeHtml {
    my($rLogFile,$opt,$n,$retVal,$rFile, $analysisName, @flowsToPlot) = @_;
