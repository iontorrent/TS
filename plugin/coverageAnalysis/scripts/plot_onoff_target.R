...
if( length(ccov$Mapped_Reads) == 0 || sum(ccov$Mapped_Reads) == 0 ) 
{
     write(sprintf("No data output to %s\n",nFileOut),stderr())
     q()
}

# check for on/off target presence in data
