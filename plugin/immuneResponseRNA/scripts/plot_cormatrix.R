...
nFileOut2 <- ifelse(is.na(args[3]),"corplot3.png",args[3])
nBarcode <- ifelse(is.na(args[3]),0,args[4])
title    <- ifelse(is.na(args[5]),"",args[5])
rValuOut <- ifelse(is.na(args[6]),"",args[6])
...
bcrmat <- read.table(nFileIn, header=TRUE, sep="\t", as.is=TRUE, check.names=F, comment.char="")
ncols = ncol(bcrmat)
if( ncols < 2 ) {
...
#########################################
##- PCA plot using top 50 genes with highest variance
rowVar <- apply(bcrrep, 1, var)
idx_rowVar_sorted <- sort(rowVar, decreasing=T, index.return=T)$ix

pc <- prcomp(t(bcrrep[idx_rowVar_sorted[1:50], ]))
p_colors <- c("#3F3F3F",brewer.pal(10,"Paired"), 'cyan', 'green', 'blue', 'brown', 'orange')
plot_colors <- p_colors[1:ncol(bcrrep)]
pch_shapes <- rep(21:25, 3)[1:ncol(bcrrep)]
png(file=nFileOut2, width=800, height=800)
#par(mar=c(4,4,4,12))
xyplot(PC2 ~ PC1, data = as.data.frame(pc$x), pch=pch_shapes, fill=plot_colors, cex=3, aspect='iso', col=plot_colors, groups = NULL,
       main='PCA plot for samples',
       key = list(title='Samples', space='right', type='p', lwd=12, points=F, pch=pch_shapes, cex=1.25, col=plot_colors,
              padding.text = 4, text=list(lab=colnames(bcrrep))))
