...
thresh   <- as.numeric(ifelse(is.na(args[5]),"-1",args[5]))
...
ncols = ncol(bcrmat)
if( ncols < 1 ) {
    write(sprintf("ERROR: Expected at least 1 r-value in matrix file %s\n",nFileIn),stderr())
    q(status=1)
}
nrows = nrow(bcrmat)
if( nrows < 1 ) {
    write(sprintf("ERROR: Expected at least 1 row of data plus header line in matrix file %s\n",nFileIn),stderr())
    q(status=1)
}

hmat <- data.matrix(bcrmat)
#hmat[ hmat < thresh ] <- thresh
thresh <- min(hmat)

ncolors = 100
pbreaks = ncolors+1
if( thresh >= 0 ) {
  pbreaks <- seq( thresh, 1, (1-thresh)/ncolors )
}

#colors=heat.colors(ncolors)
#colors=colorRampPalette(c("black","darkblue","blue","cyan","white","yellow","red"))(ncolors)
...
#heatmap_2( hmat, col=colors, main=title, symkey=FALSE,
#  lmat=rbind(c(0,3,0),c(2,1,0),c(0,0,0),c(0,4,0)), lwid=c(1,5,0.2), lhei=c(1.4,5,0.25,0.9),
#  density.info="none", trace="none", breaks=pbreaks, key.abs=TRUE,
#  key.xlab = "", key.title = keytitle, cexRow=1.5, cexCol=1.5, margins=c(8,8) )

heatmap_2( hmat, col=colors, main=title, symkey=FALSE, cexRow=1.5, cexCol=1.5,
  lmat=rbind(c(4,3,0), c(2,1,0)), lwid=c(0.75,2,1), lhei=c(0.75,2),
  density.info="none", trace="none", breaks=pbreaks, key.abs=TRUE,
  key.xlab = "", key.title = keytitle, keysize=1, margins=c(16,8) )
