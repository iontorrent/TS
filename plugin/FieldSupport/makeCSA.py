    ...
    for item in filelist:
        if os.path.exists(item):
            ...
            try:
                ...
                print("FieldSupport.makeCSA IO Error: %s" % str(e))
                continue
            except ValueError as e:
                print("FieldSupport.makeCSA Value Error: %s" % str(e))
                continue
            except Exception as e:
                print("FieldSupport.makeCSA Unknown Error: %s" % str(e))
                continue
...
def makeCSA(reportDir, rawDataDir, csa_file_name=None, chefLogPath=""):
    ...
    print("Report Dir: %s" % reportDir)
    print("Raw Data Dir: %s" % rawDataDir)
    ...
    def integrate_chef_log(csa_file_path, chef_log_path):
        ...
        print("Chef Log Path: %s" % chefLogPath)
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Standalone script to create the CSA zip bundle"
    )
    parser.add_argument(
        "-r",
        "--reportDir",
        dest="reportDir",
        required=True,
        help="report directory of the run report",
    )
    parser.add_argument(
        "-w",
        "--rawDataDir",
        dest="rawDataDir",
        required=True,
        help="Raw data directory of the run",
    )
    parser.add_argument(
        "-f",
        "--csaFileName",
    ...
    parser.add_argument(
        ...
        dest="chefLogPath",
        required=True,
        help="chefLogPath directory of the run",
    )
    args = parser.parse_args()

    csa_path = makeCSA(
        args.reportDir, args.rawDataDir, args.csaFileName, args.chefLogPath
    )
