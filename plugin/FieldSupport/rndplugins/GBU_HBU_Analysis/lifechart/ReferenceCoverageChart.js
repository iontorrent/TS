...
<div id="RC-chart" class="unselectable" style="border:2px solid #666;display:none">\
  <div id="RC-titlebar" class="grid-header" style="min-height:24px;border:0">\
    ...
    <select class="RC-selectParam txtSelect RC-shy" id="barAxis">\
     <option value=0>Total Base Reads</option>\
     <option value=1 selected="selected">Strand Base Reads</option>\
    </select>\
    <span class="RC-shy flyhelp" id="RC-overlayLabel" style="padding-left:10px">Overlay:</span>\
    <select class="RC-selectParam txtSelect RC-shy" id="RC-overPlot">\
...
<div id="RC-controlpanel" class="filter-panel" style="display:none">\
  <table><tr>\
    ...
    <td class="nwrap"><span class="flyhelp" id="RC-showLegendLabel">Show Legend</span>:\
      <input type="checkbox" id="RC-showLegend"></td>\
...
$(function () {
  ...
  var nooverlay = true; //$("#ReferenceCoverageChart").attr("nooverlay");
  ...
  function targetBarClick(e) {
    ...
    var trg = dataTable[bin][7];
    if( trg.indexOf(',') < 0 ) {
      // determine bin range over same target(s) for clicked bin
      var binSrt = bin, binEnd = bin;
      while( binSrt > 0 ) {
        if( dataTable[--binSrt][7] !== trg ) break;
      }
      while( binEnd < plotStats.numPoints ) {
        if( dataTable[++binEnd][7] !== trg ) break;
      }
      if( plotStats.binnedBases ) {
        zoomViewToBinRange(binSrt,binEnd);
      } else {
        centerViewOnBin( parseInt(0.5*(binSrt+binEnd)) );
      }
  ...
  // INSTANTLY collapse view after EVERYTHING has been drawn in open chart (to avoid flot issues)
  if( startCollapsed ) {
