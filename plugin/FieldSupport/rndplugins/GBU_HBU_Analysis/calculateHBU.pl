...
coordinates for the HBU grouping. Output for HBU are a set of base coverage stats in CSV (table) format to STDOUT.";
my $USAGE = "Usage:\n\t$CMD [options] <BBC file> <gene-grouped sorted BED file>";
my $OPTIONS = "Options:
  -p Output PBU stats to STDOUT and do not create HBU and WIG output to STDOUT. All other options are ignored.
  ...
  -M <num>  Mean read depth used for HBU/PBU calculations. Default: Use mean base read depth for region provided.
  ...
  -X <file> Create WIG files using this gene-grouped sorted BED file rather than the input (where HBU is calculated).
...
# disable any conflicting options with noHBUout option
if( $noHBUout ) {
  $statsout = "";
  $fullpanel = "";
}
...
# generate the PBU stats for the whole panel first - this gives the mean base read depth required for HBU
...
if( $statout ) {
  ...
  #print STATOUT "\nMean base read depth used for HBU: $mbrd\n";
  close(STATOUT);
}
# do nothing else with noHBUout option
exit 0 if( $noHBUout );
...
# coverage beyond panel/gene instersection (PGI) affects the numerator of HBU calculation.
...
# output the HBU for each set of target regions assmed to be grouped by gene (4th field of BED)
# Note: stats recorded no longer match original APD_plugin output
printf "Gene,MinCov,MaxCov,NMinCov,NMaxCov,PGIMBRD,PGIBU,PGISUB,Cov1x,Cov100x,Cov350x,Cov500x,PGILen%s\n",($haveGeneLen ? ",GLen,GMBRD,HBU,GSUB" : "");

my $tmphbubed = "gene_cds.tmp.bed";
my $lastGene, $lastChr;
...
while(<BEDIN>) {
  ...
  if( $gene ne $lastGene ) {
    ...
    $reglen = 0;
    open(TMPBED,">",$tmphbubed) || die "Could not write local file '$tmphbubed'\n";
  }
...
# create empty records to capture CDS length for genes with no HBU output (if available via -G)
if( $haveGeneLen ) {
  for my $gene (sort keys %CDSLen) {
    print "$gene,0,0,0,0,0,0,0,0$CDSLen{$gene},0,0,0\n" unless($outputHBU{$gene});
  }
}
exit(0);
...
sub calcHBU {
  ...
  my $pos;
  open(BBCVIEW,"$bbctools view -nstR '$tmphbubed' '$bbcfile' |") || die "Could not run BBCtools view command: $!\n";
  while(<BBCVIEW>) {
    ($pos,$cov) = split('\t',$_);
  ...
  $cov20 += $cov0;
  $gcov20 += $cov0;
  printf "$lastGene,$minCov,$maxCov,%.2f,%.2f,%.2f,%.4f,$cov20,$ncov,$cov100,$cov350,$cov500,$reglen", $minCov/$mbrd, $maxCov/$mbrd, $totCov/$reglen, ($reglen-$cov20)/$reglen;
