...
# This script creates a default heatmap from a given cufflinks isoforms.fpkm barcode output matrix
...
maxisofs <- as.numeric(ifelse(is.na(args[5]),"-1",args[5]))
thrsfpkm <- as.numeric(ifelse(is.na(args[6]),"0.3",args[6]))
minisos  <- as.numeric(ifelse(is.na(args[7]),"0",args[7]))
...
novmin <- sum(apply(data,1,max) >= minfpkm)
if( novmin < maxisofs ) {
  write(sprintf("Warning: Transcript iosoform heatmap plot reduced number of isoforms from %d to %d isoforms where at least one barcode had FPKM >= %g",maxisofs,novmin,minfpkm),stderr())
  maxisofs <- novmin
}
...
heatmap_2( data, col=colors, main=title, symkey=FALSE,
  lmat=rbind(c(0,3,0),c(2,1,0),c(0,0,0),c(0,4,0)), lwid=c(1,5,1.2), lhei=c(a,b,c,d),
