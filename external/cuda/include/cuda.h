 ...
 *
 * - ::CU_CTX_PRIMARY: Instruct CUDA to initialize the primary context on
 * the specified device.  If the primary context is already initialized
 * then this wil return ::CUDA_ERROR_PRIMARY_CONTEXT_ACTIVE.
 *
 ...
 * by the current context.  In this case, explicitly registering allocations 
 * using ::cuMemPeerRegister() is not necessary.  The device pointer value 
 * through which any peer memory may be accessed in the current context is the 
 * same pointer value through which that memory may be accessed in the peer 
 * context, so it is not necessary to call ::cuMemPeerGetDevicePointer() to get 
 * the device pointer for these allocations.
 *
 ...
 *
 * Enables registering memory from \p peerContext for direct access in the current 
 * context.  On success, allocations from \p peerContext may be registered for access 
 * in the current context using ::cuMemPeerRegister().  Registering peer memory will 
 * be possible until it is explicitly disabled using ::cuCtxDisablePeerAccess(), 
 * or either the current context or \p peerContext is destroyed.
 *
 ...
 * by the current context.  In this case, explicitly sharing allocations using 
 * ::cuMemPeerRegister() is not necessary.  See \ref CUDA_UNIFIED for additional details. 
 *
 ...
 *
 * Disables registering memory from \p peerContext for direct access in the 
 * current context.  If there are any allocations from \p peerContext which were 
 * registered in the current context using ::cuMemPeerRegister() then these
...
/**
 * \brief Registers an existing memory allocation in a peer context for 
 * direct access from the current context.
 *
 * Registers the memory associated with \p peerPointer in \p peerContext 
 * for direct access in the current context.  Memory that has been
 * registered will be read and written directly when accessed through
 * ::cuMemcpyPeer() and its variants (as opposed to being staged through
 * host memory).  Memory which has been registered with the flag 
 * ::CU_MEMPEERREGISTER_DEVICEMAP may be accessed directly by pointers
 * in kernels.
 *
 * This memory will remain accessible until explicitly unregistered via
 * ::cuMemPeerUnregister() or implicitly unregistered when peer access
 * to \p peerContext is disabled by ::cuCtxDisablePeerAccess() or
 * context destruction.
 *
 * Note that explicitly registering memory is not required for direct access 
 * between contexts on devices which support unified addressing (as may be queried 
 * using ::CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING).  In such cases all 
 * memory allocated in \p peerContext is automatically made accessible to the 
 * current context by ::cuCtxEnablePeerAccess().  See \ref CUDA_UNIFIED for 
 * additional details.
 *
 * The \p Flags parameter enables different options to be specified that
 * affect the registration, as follows.
 * - ::CU_MEMPEERREGISTER_DEVICEMAP: Maps the allocation into the CUDA 
 *   address space. The device pointer to the memory may be obtained by 
 *   calling ::cuMemPeerGetDevicePointer().
 *
 * Returns ::CUDA_ERROR_PEER_ACCESS_NOT_ENABLED if direct access has 
 * not yet been enabled from \p peerContext to the current context.
 *
 * Returns ::CUDA_ERROR_PEER_MEMORY_ALREADY_REGISTERED if \p peerPointer
 * from \p peerContext has already been registered with the current context.
 *
 * Returns ::CUDA_ERROR_INVALID_VALUE if \p peerPointer is not a pointer value
 * return by ::cuMemAlloc() or ::cuMemAllocPitch() in \p peerContext.
 *
 * Returns ::CUDA_ERROR_INVALID_CONTEXT if there is no current context, or if
 * \p peerContext is not a valid context.
 *
 * \param peerPointer - Pointer in peer context to register for direct access in the
 *                      current context
 * \param peerContext - Peer context in which \p peerPointer was allocated
 * \param Flags       - Flags affecting allocation registration.
 *
 * \return
 * ::CUDA_SUCCESS,
 * ::CUDA_ERROR_DEINITIALIZED,
 ...
 * ::cuMemPeerGetDevicePointer
 */
CUresult CUDAAPI cuMemPeerRegister(CUdeviceptr peerPointer, CUcontext peerContext, unsigned int Flags);

/**
 * \brief Unregisters a peer context's memory allocation, disabling direct access.
 *
 * Unregisters peer memory which was registered with the current context using
 * ::cuMemPeerRegister().
 *
 * Returns ::CUDA_ERROR_PEER_MEMORY_NOT_REGISTERED if \p peerPointer in 
 * \p peerContext has not been registered yet.
 *
 * Returns ::CUDA_ERROR_INVALID_VALUE if \p peerPointer is not a pointer value
 * return by ::cuMemAlloc() or ::cuMemAllocPitch() in \p peerContext.
 *
 ...
 * \p peerContext is not a valid context.
 *
 * \param peerPointer - Pointer in \p peerContext to unregister.
 * \param peerContext - Peer context in which \p peerPointer was allocated.
 *
 * \return
 * ::CUDA_SUCCESS,
 ...
 * ::cuMemPeerGetDevicePointer
 */
CUresult CUDAAPI cuMemPeerUnregister(CUdeviceptr peerPointer, CUcontext peerContext);

/**
 * \brief Retrieves a device pointer through which a peer context's registered 
 * memory may be directly accessed.
 *
 * Retrieves a device pointer through which memory registered using 
 * ::cuMemPeerRegister() may be directly accessed in the current context.  
 * On success, returns in \p *pdptr a device pointer through which the memory
 * at \p peerPointer in \p peerContext may be accessed through kernels and copies
 * in the current context.
 *
 * If both the current context and \p peerContext are on devices which support unified 
 * addressing (as may be queried using ::CU_DEVICE_ATTRIBUTE_UNIFIED_ADDRESSING), then
 * the pointer to access \p peerPointer from \p peerContext in the current context
 * will be \p peerPointer itself, and querying this pointer value is not necessary.
 * See \ref CUDA_UNIFIED for additional details. 
 *
 * Returns ::CUDA_ERROR_NOT_MAPPED_AS_POINTER if \p peerPointer in 
 * \p peerContext was not registered with the flag ::CU_MEMPEERREGISTER_DEVICEMAP.
 *
 * Returns ::CUDA_ERROR_PEER_MEMORY_NOT_REGISTERED if \p peerPointer in 
 * \p peerContext has not been registered yet.
 *
 * Returns ::CUDA_ERROR_INVALID_VALUE if \p peerPointer is not a pointer value
 * return by ::cuMemAlloc() or ::cuMemAllocPitch() in \p peerContext, or if
 * \p Flags is not 0.
 *
 * Returns ::CUDA_ERROR_INVALID_CONTEXT if there is no current context, or if
 * \p peerContext is not a valid context.
 *
 * \param pdptr       - Pointer in peer context to unregister.
 * \param peerPointer - Pointer in peer context to unregister.
 * \param peerContext - Peer context in which \p peerPointer was allocated.
 * \param Flags       - Reserved for future use and must be set to 0
 *
 * \return
 ...
 * ::cuMemPeerUnregister
 */
CUresult CUDAAPI cuMemPeerGetDevicePointer(CUdeviceptr *pdptr, CUdeviceptr peerPointer, CUcontext peerContext, unsigned int Flags);
