 ...
 * It is not necessary to call ::cuMemHostGetDevicePointer() to get the device 
 * pointer for these allocations.  
 *
 * Note that this is not the case for memory allocated using the flag
 * ::cudaHostAllocWriteCombined, as discussed below.
 *
 * \section CUDART_UNIFIED_autopeerregister Automatic Registration of Peer Memory
 *
 ...
 * by the current device.  In this case, explicitly registering allocations 
 * using ::cudaPeerRegister() is not necessary.  The device pointer value 
 * through which any peer's memory may be accessed in the current device is 
 * the same pointer value through which that memory may be accessed from the 
 * peer device, so it is not necessary to call ::cuMemPeerGetDevicePointer() to 
 * get the device pointer for these allocations.
 *
...
/**
 * \brief Registers an existing memory allocation on a peer device for 
 * direct access from the current device.
 *
 * Registers the memory associated with \p peerPointer on \p peerDevice 
 * for direct access from the current device.  Memory that has been
 * registered will be read and written directly when accessed through
 * ::cudaMemcpyPeer() and its variants (as opposed to being staged through
 * host memory).  Memory which has been registered with the flag 
 * ::cudaPeerRegisterMapped may also be accessed directly via pointer
 * dereferences in kernels.
 *
 * This memory will remain accessible until explicitly unregistered via
 * ::cudaPeerUnregister() or implicitly unregistered when peer access 
 * to \p peerDevice is disabled by ::cudaDeviceDisablePeerAccess() or
 * either the current device or \p peerDevice is reset by ::cudaDeviceReset().
 *
 * Note that explicitly registering memory is not required for direct access 
 * between devices which support unified addressing.  In such cases all 
 * memory allocated in \p peerDevice is automatically made accessible to the 
 * current device by ::cudaDeviceEnablePeerAccess().
 *
 * The \p flags parameter enables different options to be specified that
 * affect the registration, as follows.
 * - ::cudaPeerRegisterDefault: Allow direct access only when copying
 *   to and from the allocation using ::cudaMemcpyPeer() and its variants.
 * - ::cudaPeerRegisterMapped: Maps the allocation into the CUDA 
 *   address space. The device pointer to the memory may be obtained by 
 *   calling ::cudaPeerGetDevicePointer().
 *
 * Returns ::cudaErrorPeerAccessNotEnabled if direct access has 
 * not yet been enabled from \p peerDevice to the current device.
 *
 * Returns ::cudaErrorPeerMemoryAlreadyRegistered if \p peerPointer
 * from \p peerDevice has already been registered for direct access by 
 * the current device.
 *
 * Returns ::cudaErrorInvalidValue if \p peerPointer is not a pointer value
 * returned by ::cudaMalloc() or its variants on \p peerDevice.
 *
 * Returns ::cudaErrorInvalidDevice if \p peerDevice is not a valid device.
 *
 * \param peerDevicePointer - Pointer in peer device to register for direct access 
 *                            from the current device.
 * \param peerDevice        - Peer device on which \p peerPointer was allocated.
 ...
 * ::cudaPeerGetDevicePointer
 */
extern __host__ cudaError_t CUDARTAPI cudaPeerRegister(void *peerDevicePointer, int peerDevice, unsigned int flags);

/**
 * \brief Unregisters a peer device's memory allocation, disabling direct access.
 *
 * Unregisters peer memory which was registered with the current device using
 * ::cudaPeerRegister().
 *
 * Returns ::cudaErrorPeerMemoryNotRegistered if \p peerPointer on 
 * \p peerDevice has not been registered yet.
 *
 * Returns ::cudaErrorInvalidValue if \p peerPointer is not a pointer value
 * returned by ::cudaMalloc() or its variants on \p peerDevice.
 *
 * Returns ::cudaErrorInvalidDevice if \p peerDevice is not a valid device.
 *
 * \param peerDevicePointer - Pointer in \p peerDevice to unregister.
 * \param peerDevice        - Peer device on which \p peerPointer was allocated.
 *
 * \return
 * ::cudaSuccess,
 ...
 * ::cudaPeerGetDevicePointer
 */
extern __host__ cudaError_t CUDARTAPI cudaPeerUnregister(void *peerDevicePointer, int peerDevice);

/**
 * \brief Retrieves a device pointer through which a peer device's registered 
 * memory may be directly accessed.
 *
 * Retrieves a device pointer through which memory registered using 
 * ::cudaPeerRegister() may be directly accessed from the current device.  
 * On success, returns in \p *pDevice a device pointer through which the memory
 * at \p peerPointer on \p peerDevice may be accessed through kernels and copies
 * in the current device.
 *
 * If both the current device and \p peerDevice support unified addressing 
 * then the pointer to access \p peerPointer on \p peerDevice from the current device
 * will be \p peerPointer itself, and querying this pointer value is not necessary.
 *
 * Returns ::cudaErrorInvalidValue if \p peerPointer on \p peerDevice was not  
 * registered with the flag ::cudaPeerRegisterMapped.
 *
 * Returns ::cudaErrorPeerMemoryNotRegistered if \p peerPointer on 
 * \p peerDevice has not been registered yet.
 *
 * Returns ::cudaErrorInvalidValue if \p peerPointer is not a pointer value
 * returned by ::cudaMalloc() or its variance on \p peerDevice, or if
 * \p flags is not 0.
 *
 * Returns ::cudaErrorInvalidDevice if \p peerDevice is not a valid device.
 *
 * \param pDevice           - Returned device pointer on the current device.
 * \param peerDevicePointer - Pointer on \p peerDevice to query.
 * \param peerDevice        - Peer device on which \p peerPointer was allocated.
 * \param flags             - Reserved for future use and must be set to 0
 *
 * \return
 ...
 * ::cudaPeerUnregister
 */
extern __host__ cudaError_t CUDARTAPI cudaPeerGetDevicePointer(void **pDevice, void *peerDevicePointer, int peerDevice, unsigned int flags);
