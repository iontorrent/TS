...
our ( %OPT, 
     %CONF, $DIMS, 
     $IM, $IM_BRUSHES, $IM_TILES, $IM_TILES_COLORED, 
     $PNG_MAKE, $SVG_MAKE,
...
if($PNG_MAKE) {
    ...
    if($bgfill) {
	$IM->fill( 0, 0, fetch_color($CONF{image}{background}) );
    }
	...
	for my $i ( 1 .. @{ $link->{data} } - 1 ) {
	  ...
	  if ( seek_parameter( "ribbon", @i_link_param_path ) ) {
	    ...
	    ribbon(
		   ...
		   pattern =>
		   seek_parameter( "pattern", @i_link_param_path ),
	...
	if ($datum) {
		...
		if ( $datum->{data}{value} =~ /,/ ) {
			...
			if ( $options->{param} ) {
				...
				my @param_values = split(/,/,($datum->{param}{$param}
							      || 
							      $options->{param}{$param}
							 ));
...
sub validate_rgb {
    ...
    my @values = ref $definition eq "ARRAY" ? @$definition : str_to_list($definition);
    my $n = @values;
...
sub allocate_color {
    ...
    printdebug("allocate_color",int(keys %$colors),$name,@rgb);
}
...
sub create_transparent_colors {
	...
	for my $i ( 1 .. $CONF{image}{auto_alpha_steps} ) {
	    ...
	    printdebug("auto_alpha_color",$color_name_alpha,@rgb,$alpha);
	}
    }
}
...
sub allocate_colors {
  ...
  for my $color_name ( sort keys %{ $CONF{colors} } ) {
      if($color_name eq "transparent" || $color_name eq "clear") {
	  confess "You have defined a color named 'transparent' or 'clear'. These names are reserved for transparency, which is automatically defined for you. Please remove these definitions from your <color> block.";
      }
  ...
  for my $color_name ( sort keys %{ $CONF{colors} } ) {
	  ...
	  printdebug("colorlookup",$color_definition);
	  ...
	  $colors->{$color_name} = $color_definition;	  
	  printdebug("colorlookupassign",$color_name,$color_definition);
  ...
  for my $color_name ( sort keys %{ $CONF{colors} } ) {
	  ...
	  my @matches = sample_list($color_definition,[keys %$colors]);
	  if(! @matches) {
	      confess "The color list [$color_name] included a definition [$color_definition] that does not correspond to any previously defined color.";
	  }
	  push @match_set, @matches;
	  push @color_set, map { $colors->{$_} } @matches;
      }
      $colors->{$color_name} = \@match_set;
      printdebug("colorlist",$color_name,@match_set);
  }
  return $colors;
}
...
sub fetch_fill_pattern {
	...
	printdebug("created tile from file",$tile_file);
...
sub increment_counter {
    ...
    printdebug("incrementing counter",$counter,$value,"now",$CONF{counter}{$counter});
}
sub set_counter {
    my ($counter,$value) = @_;
    $CONF{counter}{$counter} = $value;
    printdebug("set counter",$counter,$value,"now",$CONF{counter}{$counter});
}
...
sub repopulateconfiguration {
  ...
  for my $key ( keys %$root ) {
	...
	for my $item (@$value) {
		...
		printdebug("zeroing counter",$key);
		$CONF{counter}{$key} = 0;
	    }
	    repopulateconfiguration($item,$key) if ref($item);
	    increment_counter($key, 
			      exists $root->{increment_counter} ? $root->{increment_counter} : 
			      exists $item->{increment_counter} ? $item->{increment_counter} : 1);
	}
    } else {
	if($key =~ /\s+/) {
	    confess "Error parsing configuration. Your parameter [$key] contains a white space. This is not allowed. Did you mean to use " . join("_",split(" ",$key))." instead?";
	}
	my $delim = "__";
	while ( $value =~ /$delim([^_].+?)$delim/g ) {
	    my $source = $delim . $1 . $delim;
	    my $target = eval $1;
	    printdebug("repopulate","key",$key,"value",$value,"var",$1,"target",$target);
	    $value =~ s/\Q$source\E/$target/g;
	    printdebug("repopulate",$key,$value,$target);
	}
	if($value =~ /\s*eval\s*\((.+)\)/ && $parent ne "rule") {
	    ...
	    printdebug("repopulateeval",$fn,$value);
	}
	$root->{$key} = $value;
    }
  }
...
sub loadconfiguration {
  my $arg = shift;
  printdebug("using configuration file from argument %s\n",$arg);
  my @possibilities = (
		       $arg,
  ...
  for my $f ( @possibilities ) { 
    printdebug("looking for configuration file %s\n",$f);
    if ( -e $f && -r _ ) {
      printdebug("found configuration file %s\n",$f);
      $file = $f;
      last;
    }
  }
