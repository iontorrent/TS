...
class None {
	  public:
	    ...
	    None(void) : p_kind(NONE) {std::cout << "None()" << std::endl;};
		  ...
		  //virtual bool operator()(class Graph<Label, Weight, Compare>::iterator<> const& node) {return false;}
  ...
  class Graph {
    public:
	    ...
	    Graph(bool s=SIMPLE, bool d=!DIRECTED) : simple(s), directed(d), m_color(BLACK), g_size(0), g_order(0), m_bottom(NULL) {};
       ...
       */
  	  ...
  	  class iterator : public std::iterator< std::input_iterator_tag, Node<Label, Weight> > {
  	    public:
	    	  ...
	    	  iterator& operator++(void) {
	    	    ...
	    	    i_path.push(iterator(Pruner(), i_color, node)); // TODO: Fix bug, changing Pruner() to prune breaks print capability. Probably because you need a deep copy of ostream* and ofstream* in the Pruner.
	    		  colored.pop();
  	  ...
  	  iterator<Pruner> begin(bool traverse=!TRAVERSE, const Pruner& prune=Pruner()) {
  	  	...
  	  	return iterator<Pruner>(prune, traverse ? ++m_color : m_color, m_bottom);
  	  }
  	  ...
  	  iterator<Pruner> end(const Pruner& prune=Pruner()) {
  	  	return iterator<Pruner>(prune, m_color);
  	  }
	    ...
	    void insert(iterator<Pruner> parent, iterator<Pruner> child, Weight const& weight=Weight()) {
  	  	if(parent != end<Pruner>() && child != end<Pruner>()) {
	    ...
	    iterator<Pruner> insert(iterator<Pruner> parent, Label const& label, Pruner const& prune=Pruner(), Weight const& weight=Weight()) {
	    	iterator<Pruner> child(prune, m_color, new Node<Label, Weight>(label));

	    	if(parent != end<Pruner>())
	    		insert(parent, child, weight);
	    	else
	    		if(!empty())
	    			insert(iterator<Pruner>(prune, m_color, m_bottom), child, weight);
  	  ...
  	  template<class Pruner>
  	  std::stack< iterator<Pruner> > traverse(iterator<Pruner> first, iterator<Pruner> last, Pruner prune=Pruner()) {
  	  	//std::cout << "first color = " << first.i_color << " last.color = " << last.i_color << std::endl;

  	  	iterator<Pruner> start(prune, ++m_color, &*first);
  	  ...
  	  iterator<Pruner> find(const Label& label, const Pruner &prune=Pruner()) {
  	  	...
  	  	return iterator<Pruner>(prune, m_color, &*found);
  	  }

  	  template<class Pruner>
  	  void erase(iterator<Pruner> first, iterator<Pruner> last) {
  	  	std::stack< Node<Label, Weight>* > doomed;
  	  	iterator<Pruner> start(first.prune, ++m_color, &*first);
    ...
    protected:
	    ...
	    int m_color, g_size, g_order;
	    Node<Label, Weight> *m_bottom;
	    Compare compare;
