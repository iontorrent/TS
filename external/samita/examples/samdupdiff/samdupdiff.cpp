...
struct AlignLess
{
    bool operator()(Align const & a1, Align const & a2) const
    {
        if (a1.getStart() == a2.getStart())
            if (a1.isPcrDuplicate() == a2.isPcrDuplicate())
                /// return one with earlier name...
                return a1.getName() > a2.getName();
            else
                return a1.isPcrDuplicate();
        else
...
//#endif
    ...
    log4cxx::LoggerPtr g_log = log4cxx::Logger::getLogger("lifetechnologies.samdupdiff");
    ...
    const AlignLess aless = AlignLess();
    ...
    while(ia != a.end())
    {
        ...
        if(ia->getName() == ib->getName() && ia->getStart() == ib->getStart() && ia->isPcrDuplicate() == ib->isPcrDuplicate()) {
        ...
        {
            if(wantA) output.write(*ia); // If mode disjoint or a-only
            if(ia->getName() == ib->getName()) ++ib;
            ++ia;
        }
        else
        {
            if(wantB) output.write(*ib); // If mode disjoint or b-only
            if(ia->getName() == ib->getName()) ++ia;
            ++ib;
        }
    }
    cerr << "Done A" << endl;
