...
vector<int> Sample::baseCount(void) {
...
bool sufficientAlternateObservations(Samples& samples, string _sampleName, int mincount, float minfraction) {
    ...
    for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
        ...
        int alternateCount = 0;
        int observationCount = 0;
        if (sampleName.compare(_sampleName) == 0) {
		for (Sample::iterator group = sample.begin(); group != sample.end(); ++group) {
		    //const string& base = group->first;
		    //cerr << base << endl;
		    vector<Allele*>& alleles = group->second;
    ...
    // this is meant to catch the case in which the reference is the rare allele
    // it will probably also catch cases in which we have very low coverage
    if ((totalReferenceCount > 0 || totalAlternateCount > 0) && totalReferenceCount < totalAlternateCount) {
        return true;
    }
...
map<string, vector<pair<int, int> > > getObservationCountBySample(Samples & samples, vector<Allele> altAlleles) {
	map<string, vector<pair<int, int> > > observationsBySampleByStrand;
	...
	for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
		...
		vector< pair<int,int> > altAlleleCountVector;
		for (vector<Allele>::iterator alt = altAlleles.begin(); alt != altAlleles.end(); ++alt) {
			base = (*alt).base();
			//reset counts
			forwardCount = 0;
			reverseCount = 0;
			forwardCount = sample.baseCount(base, STRAND_FORWARD);
			reverseCount = sample.baseCount(base, STRAND_REVERSE);
			altAlleleCountVector.push_back(make_pair(forwardCount, reverseCount));
		...
		observationsBySampleByStrand[sampleName] = altAlleleCountVector;
	}
	
	return observationsBySampleByStrand;
	
}

map<string, pair<int, int>  > getObservationCountBySample(Samples & samples, string base) {
	map<string, pair<int, int>  > observationsBySampleByStrand;
	...
	for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
		...
		observationsBySampleByStrand[sampleName] = make_pair(forwardCount, reverseCount);
	}
...
pair<int, int> getObservationCount(Samples & samples, string base) {
	int forwardCount = 0;
	int reverseCount = 0;
	for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
	...
	return make_pair(forwardCount, reverseCount);
	
}

vector<int> getObservationCountForSingleSample(Samples & samples, string _sampleName) {
	vector<int> counts;
	string sampleName;
	for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
...
map<string, int> countAllelesBySample(Samples& samples) {

    int count = 0;
    map<string, int> coverageBySample;
    string sampleName;
    for (Samples::iterator s = samples.begin(); s != samples.end(); ++s) {
        Sample& sample = s->second;
        sampleName = s->first;
        ...
        coverageBySample[sampleName] = count;
    }
