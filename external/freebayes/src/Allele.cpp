...
int Allele::subquality(const Allele &a) const {
    ...
    if (a.type == ALLELE_INSERTION) {
        ...
        if (L > (int)baseQualities.size()) {
            ...
            if (spanstart + L > (int)baseQualities.size()) {
...
string Allele::genotypeBase(void) const {
	string str = "";
	switch (this->type) {
        case ALLELE_REFERENCE:
            if (genotypeAllele)
    ...
    switch (this->type) {
        case ALLELE_GENOTYPE:
           masked = false;
            break;
        case ALLELE_REFERENCE:
            masked = indelMask.at(referenceOffset());
            break;
        case ALLELE_SNP:
            masked = indelMask.at(0);
            break;
