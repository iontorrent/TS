...
class LevMarFitterV2
{
  protected:
    LevMarFitterV2() : len (0), x (NULL), residual (0.0), dp (NULL), residualWeight (NULL),
        ...
        fval_cache (NULL), enable_fval_cache (false), lambda (1.0), lambda_threshold (1.0E+10)
    {
    }
...
//                e = (*err_vec)(i) = residualWeight[i] * (refVals[i] - testVals[i]);
    ...
    void GridSearch (int steps,float *y,float *params)
    {
      ...
      if (enable_fval_cache)
        memcpy(fval_cache,fval,sizeof(float[len]));
    ...
    int Fit (int max_iter, float *y, float *params, float *std_err = NULL)
    {
      ...
      while (!DoneTest (iter,max_iter,&delta,lambda,done_cnt,r_start,r_chg))
      {
        ...
        TryLevMarStep (y, fval, params, err_vect, bfjtj, bfrhs, done_cnt, r_start, r_chg);
  ...
  public:
    ...
    float GetResidual (float *y = NULL,bool use_fval_cache = false)
    {
      if (y == NULL)
        return residual;
      else
    ...
    void SetFvalCacheEnable(bool enable) { enable_fval_cache = enable; }
