...
class DumpMismatches {
  public:
    DumpMismatches(bool dump): _dump(dump) 
    {
      if (_dump) {
        dumpFile.open("mismatches.txt");
        if (!dumpFile.is_open())
          std::cout << "Couldn't open dump file for outputting mismatches" << std::endl; 
      }
    }

    DumpMismatches(bool dump, const char* filename) : _dump(dump)                   
    {
      if (_dump) {
    ...
    bool needDump() { return (_dump && dumpFile.is_open()); }

    void dumpMismatchWellData(WellData& gold, WellData& test, int flowNum)
    {
      dumpFile << gold.x << " " << gold.y << " " << flowNum << " ";
      dumpFile << gold.flowValues[flowNum] << " " << test.flowValues[flowNum];
      dumpFile << "\n";
    }
...
void printUsage()
{
  cout << "RawWellsEquivalent - Check to see how similar two wells files are to each other" << endl 
       ...
       << "   -o,--mismatch-wells output wells which mismatch between the two files." << endl
