...
//! @file     Realign.h
//! @brief    Perform local realignment of read. Derived from bamrealigner's Realigner.h: Adjusted for use from within TMAP
...
public:
  ...
  // DVK
  //! @brief Prepares Realigner object for aligning new sequence pair
  void Reset ();
  //! @brief Sets the soft clipping from left side
  void ClipStart (unsigned len);
  //! @brief Sets the soft clipping from right side
  void ClipEnd (unsigned len);
...
protected:
  ...
  bool ClipAnchors(bool& clipp_failed);
