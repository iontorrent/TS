...
def main(options):
        ...
        if temp_line1[0].strip() == temp_line2[0].strip():
            ...
            if (flag_sam1 & 4) == 0 and (flag_sam2 & 4) == 0:  # both reads mapped
                if not diff_field(rname_sam1, rname_sam2):  # if chr are not equal
                    ...
                    pos_sam1, pos_sam2, options.wiggle
                ):  # if pos are not same
                    reads_mapped_diff_aln_pos += 1
                else:
                    (cigar_sam1_MD, count1) = re.subn(
                        r"[MD]", "|", re.subn(r"\d+[SIHNP]", "", cigar_sam1)[0]
                    )
                    (cigar_sam2_MD, count2) = re.subn(
                        r"[MD]", "|", re.subn(r"\d+[SIHNP]", "", cigar_sam2)[0]
                    )
                ...
                if (
                    mapq_sam1 > options.min_mapq and mapq_sam2 > options.min_mapq
                    ...
                    if not diff_field(rname_sam1, rname_sam2):  # if chr are not equal
                        ...
                        (sam1_aln_len, sam2_aln_len) = aln_len(
                            cigar_sam1_MD, cigar_sam2_MD
                        )
                        if sam1_aln_len < sam2_aln_len:
    ...
    print(
        ...
        + " mapping only in version1 = "
        + str(reads_mapped_with_min_mapq_only_in_sam1)
    )
    print(
        "Reads above mapqv "
        + str(options.min_mapq)
        + " mapping only in version2 = "
        + str(reads_mapped_with_min_mapq_only_in_sam2)
    )
    print("")
    print("Reads mapping in both versions:")
    print("At different alignment positions = " + str(reads_mapped_diff_aln_pos))
    print(
        "With different alignment lengths = "
        + str(reads_mapped_le_aln_len + reads_mapped_ge_aln_len)
    )
    ...
    print(
        "At different alignment positions above mapq "
        + str(options.min_mapq)
        + " = "
        + str(reads_mapped_with_min_mapq_diff_aln_pos)
    )
    print(
        "With different alignment lengths above mapq "
        + str(options.min_mapq)
        + " = "
        + str(
            reads_mapped_with_min_mapq_le_aln_len
            + reads_mapped_with_min_mapq_ge_aln_len
        )
    )
    print(
        "With version1 smaller alignment lengths above mapq "
        + str(options.min_mapq)
        + " = "
        + str(reads_mapped_with_min_mapq_le_aln_len)
    )
    print(
        "With version2 smaller alignment lengths above mapq "
        + str(options.min_mapq)
        + " = "
        + str(reads_mapped_with_min_mapq_ge_aln_len)
    )
    print(
        "With different cigar strings above mapq "
        + str(options.min_mapq)
        + " = "
        + str(reads_mapped_with_min_mapq_diff_cigar_string)
    )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--wiggle",
        help="allow positions to be within this window to be considered the same",
        default=0,
        dest="wiggle",
    )
    ...
    parser.add_option(
        "--min-mapq",
        help="the minimum mapping quality for secondary analysis",
        type="int",
        dest="min_mapq",
        default=8,
    )
