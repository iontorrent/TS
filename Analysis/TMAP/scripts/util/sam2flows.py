...
def __main__():
    ...
    parser.add_option(
        "-f", "--flow-order", dest="flow_order", help="the flow order to simulate"
    )
    parser.add_option(
        "-k", "--key-sequence", dest="key_sequence", help="the key sequence to simulate"
    )
    parser.add_option("-s", "--sam-file", dest="sam_file", help="the input SAM file")
    parser.add_option(
        "-S", "--seed", dest="seed", help="the seed for the random number generator"
    )
    parser.add_option(
        "-d",
        "--sigma",
        ...
        default=0.25,
        type=float,
        help="the standard deviation for the flow signal",
    )
