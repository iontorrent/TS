...
def check_dir(dir_name):
    if not os.path.isdir(dir_name):
        ...
        try:
            ...
            print(
                "Directory creation failed.  You apparently don't have sufficient write permissions to: "
                + str(dir_name)
            )
            sys.exit(1)
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option(
        "-f", "--fn-fasta", help="the FASTA reference file name", dest="fn_fasta"
    )
    parser.add_option("-r", "--fn-reads", help="the reads file name", dest="fn_reads")
    parser.add_option(
        "-s",
        "--sam-directory",
        help="where the SAM file should be made",
        dest="sam_dir",
    )
    parser.add_option(
        "-n", "--num-threads", help="the number of threads", dest="num_threads"
    )
    parser.add_option(
        "--tmap-path", help="the path to the tmap binary", dest="tmap_path"
    )
    parser.add_option(
        "--dwgsim-eval-path",
        help="the path to the dwgsim_eval binary (if the reads were simulated with dwgsim_eval)",
        dest="dwgsim_eval_path",
        default="",
    )
    parser.add_option(
        "--alignStats-path",
        help="the path to the alignStast binary",
        dest="alignStats_path",
        default="",
    )
    parser.add_option(
        "--mapping-algorithm",
        help="the mapping command (ex. mapall, map1, map2 map3)",
        dest="mapping_algorithm",
    )
    parser.add_option(
        "--mapall-algorithms",
        help="the mapping algorithms and options for mapall (only used when --mapping-algorithm=mapall)",
        dest="mapall_algorithms",
    )
    parser.add_option(
        "--sam-sff-tags",
        help="include SFF specific SAM tags",
        action="store_true",
        default=False,
        dest="sam_sff_tags",
    )
    ...
    parser.add_option(
        "--redirect-stdout",
        help="redirects stdout of tmap to the specified file",
        dest="tmap_stdout",
        default="",
    )
    parser.add_option(
        "--profile-tmap",
        help="profiles tmap and requires google-perftools be in your LD_LIBRARY_PATH",
        default="",
        dest="profile_tmap",
    )
    parser.add_option(
        "--submit-script-dir",
        help="name for submission script, override default",
        dest="submit_script_dir",
    )
    parser.add_option(
        "--additional-pbs-settings",
        help="comman seperated list of additioanl PBS/environemnt settings.  formating is up to you, user.",
        dest="pbs_settings",
        default="",
    )
    ...
    if mapall_use:
        ...
        temp = temp_mapping_algorithm.split()[0] + (
            "_".join(temp_mapping_algorithm.split(" ")[1:])
        )
    fn_sam = (
        options.sam_dir + "/" + os.path.basename(options.fn_reads) + "." + temp + ".sam"
    )
    fn_script = (
        options.submit_script_dir
        + "/"
        + os.path.basename(options.fn_reads)
        + "."
        + temp
        + ".sh"
    )
    ...
    if "" != options.alignStats_path:
        alignStats_cmd = (
            "%s -a /dev/null -x -i %s -o %s -g %s -q 7,10,17,20,47 -n %s -B 100000"
            % (
...
"""
    ...
    returncode = call(
        [
            ...
            fn_script_stderr,
            "-l",
            "nodes=1:ppn=" + options.num_threads,
            fn_script,
        ]
    )
    if 0 != returncode:
