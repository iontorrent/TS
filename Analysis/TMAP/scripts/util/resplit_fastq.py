...
def main(options):
    ...
    while (not f_one.eof) or (not f_two.eof):
        ...
        if (not f_one.loaded) and (not f_two.loaded):
            ...
            case = 0
        elif (f_one.row < f_two.row) or (
            f_one.row == f_two.row and f_one.col < f_two.col
        ):
            case = 0
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option(
        "-1", "--fn-read-one", dest="fn_read_one", default=None, help="Read one"
    )
    parser.add_option(
        "-2", "--fn-read-two", dest="fn_read_two", default=None, help="Read two"
    )
    parser.add_option(
        "-p", "--fn-prefix", dest="fn_prefix", default=None, help="Output prefix"
    )
