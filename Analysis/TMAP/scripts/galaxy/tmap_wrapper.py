...
def __main__():
    ...
    parser.add_option(
        "--input", dest="input", help="The input file to use for the mapping"
    )
    parser.add_option("--inputtype", dest="inputtype", help="The input file type")
    parser.add_option(
        "--output", dest="output", help="The file to save the output (SAM format)"
    )
    parser.add_option(
        "--params", dest="params", help="Parameter setting to use (pre_set or full)"
    )
    parser.add_option(
        "--fileSource",
        dest="fileSource",
        help="Whether to use a previously indexed reference sequence or one from history (indexed or history)",
    )
    parser.add_option(
        "--algorithm",
        dest="algorithm",
        help="The algorithm (ex. mapall, map1, map2, map3, map4, ...)",
    )
    parser.add_option(
        "--globalOptions", dest="globalOptions", help="The global options "
    )
    parser.add_option(
        "--flowspaceOptions", dest="flowspaceOptions", help="The flowspace options"
    )
    parser.add_option(
        "--pairingOptions", dest="pairingOptions", help="The pairing options"
    )
    parser.add_option(
        "--algorithmOptions", dest="algorithmOptions", help="The algorithm options"
    )
    ...
    try:
        ...
        proc = subprocess.Popen(
            args="tmap --version 2>&1", shell=True, stdout=tmp_stdout
        )
        tmp_stdout.close()
    ...
    if options.fileSource == "history" and not options.do_not_build_index:
        ...
        try:
            ...
            proc = subprocess.Popen(
                args=cmd1, shell=True, cwd=tmp_index_dir, stderr=tmp_stderr.fileno()
            )
            returncode = proc.wait()
            tmp_stderr.close()
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                tmp_stderr = open(tmp, "wb")
                proc = subprocess.Popen(
                    args=cmd, shell=True, cwd=tmp_dir, stderr=tmp_stderr.fileno()
                )
                returncode = proc.wait()
                tmp_stderr.close()
                # get stderr, allowing for case where it's very large
            ...
            if options.suppressHeader == "true":
                ...
                try:
                    ...
                    raise Exception(
                        "Error moving output file before removing headers. " + str(e)
                    )
