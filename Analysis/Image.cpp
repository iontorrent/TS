...
void Image::CalcBeadfindMetric_1(Mask *mask, Region region, char *idStr, int frameStart, int frameEnd)
{
	...
	//fprintf (stdout, "\nPrint Count: %d\n\n", printCnt);
		...
		//debug
		// generate a histogram suitable for plotting
		FILE *fp = NULL;
		char file[] = "bfMetric.hst";
		char *fileName = NULL;
		int numBins = 200;
		int i;
		
		fileName = (char *) malloc (strlen(experimentName)+strlen(idStr)+strlen(file)+3);
		sprintf (fileName, "%s/%s_%s", experimentName, idStr,file);
		
		Histogram hMetric(numBins,-3000,3000);

		if (region.row == 125 && region.col == 624) {
			// This plots only the region defined
			for(y=region.row;y<region.h+region.row;y++) {
				for(x=region.col;x<region.w+region.col;x++) {
					if ((*mask)[x+y*raw->cols] & (MaskPinned | MaskIgnore | MaskWashout)) {
						continue;
					}
					else {
						hMetric.Add(results[x+y*raw->cols]);
					}
				}
			}
		}
		else {		
			// This plots whole chip
			for(y=0;y<raw->rows;y++) {
				for(x=0;x<raw->cols;x++) {
					if ((*mask)[x+y*raw->cols] & (MaskPinned | MaskIgnore | MaskWashout)) {
						continue;
		...
		if (fp) {
			for (i=0;i< numBins;i++) {
				fprintf (fp, "%0.1lf %d\n", hMetric.GetVal (i), hMetric.GetCount (i));
			}
			fclose (fp);
		}
		else {
