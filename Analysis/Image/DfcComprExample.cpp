...
void help_abort() {
  cout << "DfcComprDriver - Try compressing data using our modules." << endl;
  cout << "options: " << endl;
  cout << "  -d,--dat-file   dat file to try and compress." << endl;
  cout << "  -f,--freqs      number of frequency elements." << endl;
  cout << "     --min-bits   minimum number of magnitude bits per frequency element." << endl;
  cout << "     --max-bits   maximum number of magnitude bits per frequency element." << endl;
  cout << "     --win-length tapering window length." << endl;
  cout << "     --win-alpha  tapering window alpha coefficient." << endl;
...
int main(int argc, const char *argv[]) {
  ...
  opts.GetOption(numFreqs, "15", 'f', "freqs");
  opts.GetOption(minBits, "2", '-', "min-bits");
  opts.GetOption(maxBits, "7", '-', "max-bits");
  opts.GetOption(winLen, "5", '-', "win-length");
  opts.GetOption(winAlpha, "2.15", '-', "win-alpha");
...
//  Mask mask(mask_file.c_str());
  ...
  Mask mask(raw->cols, raw->rows);  // dummy mask to avoid crashes for now
  ...
  DfcCompr dfc;
  float * __restrict keyFrameI = new float[numFreqs];
  float * __restrict keyFrameQ = new float[numFreqs];
  unsigned char * __restrict bitsPerFreq = new unsigned char[numFreqs];
  float * __restrict scaleVectorI = new float[numFreqs];
  float * __restrict scaleVectorQ = new float[numFreqs];
  short * __restrict deltaI = new short[numFreqs*raw->rows*raw->cols];
  short * __restrict deltaQ = new short[numFreqs*raw->rows*raw->cols];
  
  dfc.n_maxBits = maxBits;
  dfc.n_minBits = minBits;
  dfc.n_basis   = numFreqs;
  dfc.n_wells   = raw->rows * raw->cols;
  dfc.SetNumFrames(raw->frames);
  dfc.SetWindowLength(winLen);
  dfc.SetWindowAlpha(static_cast<float>(winAlpha));
  // allocate output buffers
  dfc.keyFrameI = keyFrameI;
  dfc.keyFrameQ = keyFrameQ;
  dfc.bitsPerFreq = bitsPerFreq;
  dfc.scaleVectorI = scaleVectorI;
  dfc.scaleVectorQ = scaleVectorQ;
  dfc.deltaI = deltaI;
  dfc.deltaQ = deltaQ;

  // Compress image, results will populate into above buffers.  Normally the source
  // image should be partitioned into analysis region size groups of wells for best
  // results.
  dfc.LossyCompress(Y.data());
  ...
  // Compressed data is still loaded in vectors (we can pretend it's just been
  // loaded from file), so now we can try decompressing it into a fresh buffer
  float * __restrict newImage = new float[raw->rows*raw->cols*raw->frames];
  dfc.LossyUncompress(newImage);
