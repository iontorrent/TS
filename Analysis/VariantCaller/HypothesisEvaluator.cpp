...
int CalculateHypPredictions(
    ...
    my_read.measurementValue.resize(global_context.treePhaserFlowOrder.num_flows(), 0);
    master_read.SetData(my_read.measurementValue, global_context.treePhaserFlowOrder.num_flows());
    ...
    for (unsigned int i_hyp=0; i_hyp<hypothesesReads.size(); ++i_hyp) {
        ...
        if ((i_hyp>0) & (Hypotheses.at(i_hyp).compare(Hypotheses.at(0))==0)) {
            predictions.at(i_hyp) = predictions.at(0);
            predictions.at(i_hyp).resize(nFlows);
            normalizedMeasurements[i_hyp] = normalizedMeasurements[0];
            normalizedMeasurements[i_hyp].resize(nFlows);
        } else {
            hypothesesReads.at(i_hyp) = master_read;
            ...
            while (i_base<Hypotheses[i_hyp].length() and i_base<(2*(unsigned int)global_context.treePhaserFlowOrder.num_flows()-prefix_size)) {
              IncrementFlow(global_context.treePhaserFlowOrder, Hypotheses[i_hyp].at(i_base), i_flow);
              ...
              hypothesesReads.at(i_hyp).sequence.push_back(Hypotheses[i_hyp].at(i_base));
              i_base++;
            }
            ...
            if (global_context.use_SSE_basecaller)
                thread_objects.treephaser_sse.SolveRead(hypothesesReads[i_hyp], i_flow, nFlows);
            else
...
int GetStartOfMasterRead(PersistingThreadObjects  &thread_objects,
  ...
  while (base < master_read.sequence.size()) {
    while (flow < global_context.treePhaserFlowOrder.num_flows()
           and global_context.treePhaserFlowOrder.nuc_at(flow) != master_read.sequence.at(base)) {
      flow++;
    }
  ...
  for (unsigned int i_base = 0; i_base < master_read.sequence.size(); i_base++)
    IncrementFlow(global_context.treePhaserFlowOrder, master_read.sequence.at(i_base), prefix_flow);
  return prefix_flow;
}
...
void PredictionGenerationVerbose(const vector<string>         &Hypotheses,
  ...
  for (unsigned int iHyp=0; iHyp<Hypotheses.size(); ++iHyp) {
    for (unsigned int iBase=0; iBase<Hypotheses.at(iHyp).length(); ++iBase)
      printf("%c", Hypotheses.at(iHyp).at(iBase));
    printf("\n");
  }
  printf("Solved read prefix: ");
  for (int iBase=0; iBase<prefix_size; ++iBase)
    printf("%c", hypothesesReads.at(0).sequence.at(iBase));
  printf("\n");
  printf("Extended Hypotheses reads to:\n");
  for (unsigned int iHyp=0; iHyp<hypothesesReads.size(); ++iHyp) {
    for (unsigned int iBase=0; iBase<hypothesesReads.at(iHyp).sequence.size(); ++iBase)
      printf("%c", hypothesesReads.at(iHyp).sequence.at(iBase));
    printf("\n");
  }
  ...
  for (unsigned int i_flow=0; i_flow<global_context.flowOrder.length(); i_flow++) {
    cout << global_context.flowOrder.at(i_flow) << "    ";
  }
  ...
  for (unsigned int i_Hyp=0; i_Hyp<hypothesesReads.size(); ++i_Hyp) {
    ...
    for (unsigned int i_flow=0; i_flow<predictions[i_Hyp].size(); ++i_flow) {
      printf("%.2f", predictions.at(i_Hyp).at(i_flow));
      if (predictions.at(i_Hyp).at(i_flow) < 10)
        cout << " ";
    }
    cout << endl;
  }
