...
{
private:
    static const unsigned long long DEF_TEST_DURATION = 100000; // approx. time in micrseconds (usec) over which the statistics is to be collected == 0.1 sec
    const char* name_;
...
protected:
    ...
    long long timing (unsigned repetitions, bool (Benchmark::*proc)() = &Benchmark::loaded_iter); 
    ...
    unsigned calibrate (bool (Benchmark::*proc) () = &Benchmark::loaded_iter, unsigned long long duration = DEF_TEST_DURATION); // returns 0 on error, number of repetitions running in 'duration' ms on success
...
public:
    ...
    double run (unsigned long long duration = DEF_TEST_DURATION); 
    /// access to name
