        ...
        for (; itr != args_format_.end () && ok_; itr ++, argno ++)
        {
            ...
            if (arg.repeatable_)
            {
                if (repeatable_pos_ != -1)
                    ERROR ("Internal: More then one repeatable arguments"); // second repeatable
                else
            ...
            if (arg.optional_)
            {
                if (repeatable_pos_ != -1 && repeatable_pos_ != argno)
                    ERROR ("Internal: Optional argument follows repeatable"); // optional after repeatable
                if (last_optional_pos_ != -1)
                    ERROR ("Internal: Non-compact optional group"); // optional after the end of optional group
...
int CmdLine :: getFmtPos (int argno) const
{
    ...
    if (space < 0)
        ERROR ("Number of non-optional arguments is smaller then argument list size");
    for (unsigned fmtpos = 0; fmtpos < args_format_.size (); fmtpos ++)
    {
        advance = 0;
