...
void BandedScore <Src1SymbolType, Src2SymbolType, DestSymbolType, ValueType, AveType, GapType>
::dynamic ()
{
...
::dynamic_seq2 (unsigned pos1)
{
    ...
    unsigned boxbase = (pos1 / this->translator1_->unit ()) * this->translator2_->unit ();
    unsigned lb = (boxbase < this->dev_) ? 0 : boxbase - this->dev_;
    unsigned ub = std::min (len2_, boxbase + this->translator2_->unit () + this->dev_);
    for (unsigned pos2 = lb; pos2 < ub; pos2 ++)
    {
...
::dynamic_position (unsigned pos1, unsigned pos2)
{
    ...
    ValueType cur  = score_1 (pos1, pos2);
    cur = std::max (cur, score_2 (pos1, pos2));
    cur = std::max (cur, score_diag  (pos1, pos2));
    return cur;
}
...
template <typename T> T diff__ (T l1, T l2)
{
...
template <class Src1SymbolType, class Src2SymbolType, class DestSymbolType, class ValueType, class AveType, class GapType>
ValueType BandedAlign <Src1SymbolType, Src2SymbolType, DestSymbolType, ValueType, AveType, GapType>
::dynamic_position (unsigned pos1, unsigned pos2)
{
    // compute best scores for three paths that can lead to current cell (at pos1, pos2 of dynamic matrix)
