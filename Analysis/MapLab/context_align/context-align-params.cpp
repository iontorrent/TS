...
static const char* VALID_ALGORITHMS [] = {"plain", "poly", "phase", "polyphase", "templ"};
static std::string ALGO_HELP_STR = "Algorithm, one of: ";
static const char*  populate_valid_algorithms_help ()
{
    for (unsigned idx = 0, sent = sizeof (VALID_ALGORITHMS) / sizeof (*VALID_ALGORITHMS); idx != sent; ++idx)
    {
        if (idx)
            ALGO_HELP_STR.append (",");
        ALGO_HELP_STR.append (VALID_ALGORITHMS [idx]);
    }
    return ALGO_HELP_STR.c_str ();
}
static const char* ALGORITHM_HELP = populate_valid_algorithms_help ();
...
bool ContalignParams::prepareCmdlineFormat ()
{
    ...
    if (res)
    {
        keys_format_.push_back (KeyFormat ("a",              NULL,   "algo",   CALIGN_SECTNAME,  "ALGO", true, true, STRING_STR,  algostr_default (), algostr_help ()));
...
bool ContalignParams::prepareParameters ()
{
    ...
    if (toRet)
    {
        ...
        {
            {"ALGO",STRING_STR,   algostr_default (), algostr_help ()},
...
bool ContalignParams::interpreteParameters ()
{
    ...
    if (result)
    {
        algostr (parameters_->getParameter (CALIGN_SECTNAME, "ALGO"));
...
bool ContalignParams::interprete_algorithm ()
{
    bool found = false;
    for (unsigned idx = 0, sent = sizeof (VALID_ALGORITHMS) / sizeof (*VALID_ALGORITHMS); idx != sent && ! found; ++idx)
    {
        if (strcmp (algostr (), VALID_ALGORITHMS [idx]) == 0)
            found = true;
    }
    if (!found)
    {
        errlog << "Ignoring unknown algorithm: " << algostr () << ". Valid algorithms are: ";
        for (unsigned idx = 0, sent = sizeof (VALID_ALGORITHMS) / sizeof (*VALID_ALGORITHMS); idx != sent; ++idx)
        {
            if (idx)
                errlog << ", ";
            errlog << VALID_ALGORITHMS [idx];
        }
        errlog << std::endl;
        return false;
    }
    if (!strcmp (algostr (), "plain"))
        algo_ = PLAIN;
    else if (!strcmp (algostr (), "poly"))
        algo_ = POLY;
    else if (!strcmp (algostr (), "phase"))
        algo_ = PHASE;
    else if (!strcmp (algostr (), "polyphase"))
        algo_ = POLYPHASE;
    else if (!strcmp (algostr (), "templ"))
        algo_ = TEMPL;
    else
        ers << "Can not interprete seemingly valid algorithm: " << algostr () << Throw;
    return true;
}

const char* ContalignParams::algostr_default () const
{
...
const char* ContalignParams::algostr_help () const
{
    return ALGORITHM_HELP;
}
