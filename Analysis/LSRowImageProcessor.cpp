...
bool LSRowImageProcessor::GenerateGroupCorrection(int group_num,float *vect_output,int rows,int cols,uint16_t *img)
{
    ...
    // make lapackpp objects so I can easily call the solver routines
    LaSpdMatDouble lhs_matrix(nLen,nLen);
    LaVectorDouble rhs_vector(nLen);
    LaVectorDouble coeffs(nLen);
    LaSpdFactDouble lhs_matrix_fact(nLen,nLen);
    bool result_ok = true;
