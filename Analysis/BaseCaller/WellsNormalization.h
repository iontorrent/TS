...
class Mark01Data{
public:
	Mark01Data(const size_t rowStart, const size_t numRows, const size_t colStart, const size_t numCols, const size_t flowStart, const size_t numFlows, RawWells const *wells, Mask const *mask);
	...
	// initialize object
	void Initialize(const size_t rowStart, const size_t numRows, const size_t colStart, const size_t numCols, const size_t flowStart, const size_t numFlows, RawWells const *wells, Mask const *mask);
...
protected:
	  ...
	  Mask const            *mask_;                   //!< Beadfind and filtering outcomes for wells
...
public:
  ...
  WellsNormalization(ion::FlowOrder const  *flow_order, const string &norm_method);
  ...
  bool  SetWells(RawWells *wells, Mask const *mask);
...
protected:
  ...
  //! @brief Key normalizes the values in the current wells file chunk.  Use maskAll. Intended to be used in CorrectSignalBias.
  //! @param[in]   keys          Vector of key sequence objects
