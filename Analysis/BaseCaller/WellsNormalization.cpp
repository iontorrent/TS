...
//----------------------------------------------------------------------------------
Mark01Data::Mark01Data(const size_t rowStart, const size_t numRows, const size_t colStart, const size_t numCols, const size_t flowStart, const size_t numFlows, RawWells const *wells, Mask const *mask){
	Initialize(rowStart, numRows, colStart, numCols, flowStart, numFlows, wells, mask);
}

//----------------------------------------------------------------------------------
void Mark01Data::Initialize(const size_t rowStart, const size_t numRows, const size_t colStart, const size_t numCols, const size_t flowStart, const size_t numFlows, RawWells const *wells, Mask const *mask){
	rowStart_ = rowStart;
	numRows_ = numRows;
	colStart_ = colStart;
	numCols_ = numCols;
...
WellsNormalization::WellsNormalization(ion::FlowOrder const  *flow_order, const string &norm_method):
...
bool WellsNormalization::SetWells(RawWells *wells, Mask const *mask)
{
	is_disabled_ = true;
	if (wells == NULL) {
		return false;
...
{
	if (not mask_->Match(x, y, MaskLive))
		return true;
	if (not mask_->Match(x, y, MaskBead))
		return true;
...
void WellsNormalization::DoKeyNormalization(const vector<KeySequence> & keys)
{
	...
	for (unsigned int x=mChunk.colStart; x<mChunk.colStart+mChunk.colWidth; ++x) {
		for (unsigned int y=mChunk.rowStart; y<mChunk.rowStart+mChunk.rowHeight; ++y) {
			...
			if (mask_->Match(x, y, MaskLib))
				read_class = 0;
			else if (mask_->Match(x, y, MaskTF))
				read_class = 1;
			else
				continue;

//			if (x==100 && y == 450){
//				cout << "x\t" << x << "\ty\t" << y << "\tread_class\t" << read_class << endl;
//			}
...
//			if (x==100 && y == 450){
//				cout << "x\t" << x << "\ty\t" << y << "\tkeynorm\t" << key_normalizer << endl;
//			}
...
if (DEBUG_ && (mChunk.rowStart==50 && mChunk.colStart==100)){
	...
	delete maskGood_;
	DeleteMark01Data();
//	size_t t18 = timer.GetMicroSec();
//	if (DEBUG_ && (mChunk.rowStart==0 && mChunk.colStart==100)){
//		printf("t1 %lu t2 %lu t3 %lu t4 %lu t5 %lu t6 %lu t7 %lu t8 %lu t9 %lu t10 %lu\n", t1, t2-t1,t3-t2,t4-t3,t5-t4,t6-t5, t7-t6, t8-t7, t9-t8,  t10-t9);
//		printf("t11 %lu t12 %lu t13 %lu t14 %lu t15 %lu t16 %lu t17 %lu t18 %lu\n", t11-t10, t12-t11,t13-t12,t14-t13,t15-t14,t16-t15, t17-t16, t18-t17);
//	}
}
// ---------------------------------------------------------------------------------
...
unsigned int WellsNormalization::Update01(const unsigned int numMem, const unsigned int goodFlowStart, const unsigned int goodFlowEnd){
	...
	if (! m01data_){
		m01data_ = new Mark01Data(mChunk.rowStart, mChunk.rowHeight, mChunk.colStart, mChunk.colWidth, mChunk.flowStart, mChunk.flowDepth, wells_, mask_);
	}
	m01data_->Initialize(mChunk.rowStart, mChunk.rowHeight, mChunk.colStart, mChunk.colWidth, mChunk.flowStart, mChunk.flowDepth, wells_, mask_);
