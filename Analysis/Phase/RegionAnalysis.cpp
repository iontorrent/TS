    ...
    const string& _flowOrder, int _numFlows, int numWorkers)
{
  ...
  assert (wellsReader.OpenForRead(_wells, mask->W(), mask->H(), clo->cfiedrRegionsX, clo->cfiedrRegionsY));
...
void *RegionAnalysisWorker(void *arg)
{
  ...
  if (analysis->clo->libPhaseEstimator == "nel-mead-treephaser")
    analysis->worker_Treephaser();
  else if (analysis->clo->libPhaseEstimator == "nel-mead-adaptive-treephaser")
...
{
  float metric = 0;
  if (clo->libPhaseEstimator == "nel-mead-treephaser") {
  ...
  } else if (clo->libPhaseEstimator == "nel-mead-adaptive-treephaser") {
