...
int main ( int argc, char *argv[] )
{
  ...
  bool EXCLUDE = false;
  int c;
  while ( ( c = getopt ( argc, argv, "ehm:d:v" ) ) != -1 ) {
    switch ( c ) {
    case 'e':
    ...
    case 'm':   // type of mask to extract
      ...
      if ( strncmp ( optarg, "MaskBead", sizeof ( "MaskBead" ) ) == 0 ) {
        ...
        maskType = MaskDud;
      } else if ( strncmp ( optarg, "MaskReference", sizeof ( "MaskReference" ) ) == 0 ) {
        sprintf ( outputFileName, "MaskReference.mask" );
        fprintf ( stdout, "Generating Reference maskfile\n" );
        maskType = MaskReference;
  ...
  int ret_asprintf = 1;
  if ( outputDir ) {
    ret_asprintf = asprintf(&outputFilePath, "%s/%s", outputDir, outputFileName);
  } else {
    ret_asprintf = asprintf(&outputFilePath, "%s", outputFileName);
  }
  fprintf ( stdout, "BeadmaskParse: outputDir: %s \n", outputDir );
  fprintf ( stdout, "BeadmaskParse: outputFileName: %s \n", outputFileName );
  fprintf ( stdout, "BeadmaskParse: outputFilePath: %s \n", outputFilePath );

  if ( ret_asprintf  == -1) {
    fprintf ( stdout, "asprintf error \n");
    exit ( 1 );
  }

  FILE *fOut = NULL;
  fopen_s ( &fOut, outputFilePath, "wb" );
  if ( !fOut ) {
