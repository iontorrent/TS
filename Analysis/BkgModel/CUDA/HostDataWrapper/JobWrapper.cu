...
int * WorkSet::getFineNucStart(){   return _info->bkgObj->region_data->my_regions.cache_step.i_start_fine_step; }
int * WorkSet::getCoarseNucStart(){   return _info->bkgObj->region_data->my_regions.cache_step.i_start_coarse_step; }
...
void WorkSet::calculateFineNucRise(){   
...
float *WorkSet::getCoarseNucRise() {
...
int WorkSet::getFineNucRiseSize(bool padded)
{
  int size = sizeof(float) * ISIG_SUB_STEPS_SINGLE_FLOW * getNumFrames() * getFlowBlockSize();
  return (!padded)?(size):(padTo128Bytes(size));
}

int WorkSet::getCoarseNucRiseSize(bool padded)
{
...
bool WorkSet::useSlowKmultInit() const {
 return _info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.always_start_slow;
}

int WorkSet::getPostKeyFitAllWellsTrainingLevel() const {
  return _info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.post_key_train;
}

int WorkSet::getPostKeyFitAllWellsTrainingStep() const {
  return _info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.post_key_step;
}

void WorkSet::performPreFitStepsForMultiFitStream() {
  
  if (_info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.revert_regional_sampling)
    return;

  KeyNormalize();

  if (_info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.skipFirstFlowBlockRegFitting)
    return;

  _info->bkgObj->region_data->my_beads.SetCopyCountOnUnSampledBeads(getFlowBlockSize());
  _info->bkgObj->region_data->my_beads.SetBufferingRatioOnUnSampledBeads();
}

void WorkSet::prepareMultiFlowFitMatrixConfig() {
  _fd[0] = _multiFlowFitControl.GetMatrixConfig("FitWellAmplBuffering", getLevMarSparseMatrices());  
  
  if (_info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.fit_region_kmult)
    _fd[1] = _multiFlowFitControl.GetMatrixConfig("FitWellAll", getLevMarSparseMatrices());
  else 
    _fd[1] = _multiFlowFitControl.GetMatrixConfig("FitWellPostKey", getLevMarSparseMatrices());
}

const master_fit_type_table* WorkSet::getLevMarSparseMatrices() const {
  return _info->table;
}

bool WorkSet::fitTmidNucShift() const {
  return _info->bkgObj->getGlobalDefaultsForBkgModel().signal_process_control.per_flow_t_mid_nuc_tracking;
}
