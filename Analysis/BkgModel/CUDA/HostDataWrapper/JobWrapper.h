...
public:
  ...
  int * getFineNucStart();
  float * getFineNucRise();
  void calculateFineNucRise();
  float * getCoarseNucRise();
  void calculateCoarseNucRise();
  int * getCoarseNucStart();
  bound_params * getBeadParamsMax();
  bound_params * getBeadParamsMin();
  float* getClonalCallScale(); 
  bool useDynamicEmphasis();
  bool useSlowKmultInit() const;
  ...
  int getFineNucRiseSize( bool padded = false);
  int getCoarseNucRiseSize( bool padded = false);
...
// CPU side Work
  ...
  const master_fit_type_table *getLevMarSparseMatrices() const;
  int getPostKeyFitAllWellsTrainingLevel() const;
  int getPostKeyFitAllWellsTrainingStep() const; 
  void performPreFitStepsForMultiFitStream();
  void prepareMultiFlowFitMatrixConfig();
