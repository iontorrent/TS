...
public:
  ...
  LayoutCubeWithRegions(LayoutCubeWithRegions<T> & that, MemoryType mtype):
  ...
  size_t getRegW(size_t regId);
  size_t getRegH(size_t regId);
...
//returns actual region width and height
template<typename T>
size_t LayoutCubeWithRegions<T>::getRegW(size_t regId){ size_t b = this->getDimX()-regBaseX(regId); return ( b > _regWidth)?(_regWidth):(b); }
template<typename T>
size_t LayoutCubeWithRegions<T>::getRegH(size_t regId){size_t b = this->getDimY()-regBaseY(regId); return ( b > _regHeight)?(_regHeight):(b); }
...
template<typename T>
void LayoutCubeWithRegions<T>::print(){
