...
public:
  ...
  bool memTypeHost() const { return ( _type == HostMem || _type ==  HostPageLocked) ;}
  bool memTypeDevice() const { return ( _type == DeviceGlobal /*|| _type == DeviceTexture */ );}
  ...
  void* getVoidPtr() const {return (void*)_basePointer;}
...
public:
  ...
  T* getPtr() const {return (T*)getVoidPtr();}
...
public:
  ...
  TMemSegAlloc( const TMemSegAlloc<T> & that, MemoryType type):TMemSegment<T>(that),_resource(that.getSize(),type),_tracker(NULL){
  ...
  TMemSegAlloc( const TMemSegAlloc<T> & that):TMemSegment<T>(that),_resource(that.getSize(),that.getType()),_tracker(NULL){
  ...
  TMemSegAlloc( const MemSegment & that, MemoryType type):TMemSegment<T>(that),_resource(that.getSize(),type),_tracker(NULL){
    //  cout << "Allocating and copying Buffer of type: " << that.getType() << " to " << type << " and size: " << that.getSize() << endl;
    TMemSegment<T>::_basePointer = (char*)_resource.getSegment(that.getSize()); //if local resource was created, overwrite copied segment info
    TMemSegment<T>::_type = type;
    this->copy(that);
