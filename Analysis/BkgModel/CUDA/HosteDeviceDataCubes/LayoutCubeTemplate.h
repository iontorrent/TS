...
public:
  ...
  LayoutCube(const LayoutCube<T> & that, MemoryType mtype): TMemSegAlloc<T>(that, mtype), _dim(that._dim), _accessAt(0), _stride(that.getRWStrideZ()) {};
  ...
  size_t getRWStride() const {return _stride;}
  //sets predefined stride to advance through buffer cube in X,Y or Z direction
  void setRWStrideX(){_stride = 1;}
  size_t getRWStrideX() const {return 1;}
  //sets predefined stride to advance through buffer cube in X,Y or Z direction
  void setRWStrideY(){_stride = _dim.x;}
  size_t getRWStrideY() const {return _dim.x;}
  //sets predefined stride to advance through buffer cube in X,Y or Z direction
  void setRWStrideZ(){_stride = _dim.x*_dim.y;}
  size_t getRWStrideZ() const {return _dim.x*_dim.y;}
  size_t getStrideLimit() const {
  ...
  //const access functions , RW pointer will not be moved
  ...
  //for Device Buffers it is recommended to create a host side copy of the cube before calling getCSV since otherwise for every call a
  //temporary host buffer is created and the data is trasfered via PCIe
  template <typename P>
  string getCSV(size_t x=0, size_t y=0, size_t z=0, size_t limit = 0, size_t offsetBytes = 0, char delimiter = ',');
...
bool LayoutCube<T>::checkCoords(size_t x, size_t y, size_t z) const {
  ...
  cerr << "provided coordinates (" << x << "," << y << "," << z << ") are out of bounds (" << _dim.x << "," << _dim.y << "," << _dim.z << ") and will cause segfault if accessed" << endl;
...
template<typename T>
template <typename P>
string LayoutCube<T>::getCSV(size_t x, size_t y, size_t z, size_t limit, size_t offsetBytes, char delimiter)
{
  ostringstream csvstring;
  P value;
  LayoutCube<T> * me = this;
  if(this->getType() == DeviceGlobal)
    me = new LayoutCube<T>(*this,HostMem);
