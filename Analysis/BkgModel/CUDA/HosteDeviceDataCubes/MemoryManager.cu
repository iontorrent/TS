...
void * MemoryResource::getSegment(size_t size)
{
  ...
  switch(_type){
      case DeviceGlobal:
        cout << "CUDA: getSegment Device " << segSizePadded <<" used:"  << memoryUsed() <<  " free:"  << memoryAvailable() << endl;
        break;
      default:
        cout << "CUDA: getSegment Host   " << segSizePadded <<" used:"  << memoryUsed() <<  " free:"  << memoryAvailable()  << endl;
  }
#endif
