...
{
  __shared__ volatile double sum[block_size];
...
void dtrsv_ltn (int n, double* a, double* x)
{
  ...
  __shared__ volatile double temp[block_size];
  for (int j = n-1; j >= 0; j--)
  {
    temp[threadIdx.x] = 0;
...
__global__
void DfdgainStep_k (float* f, bead_params* p, float* EmphasisVectorByHomopolymer, int* WhichEmphasis, 
    int* active_bead_list, int num_fb, int num_pts, int num_steps, int step)
{
  ...
  for (int fnum = 0; fnum < num_fb; fnum++)
  {
    ...
    float* em = &EmphasisVectorByHomopolymer[WhichEmphasis[fnum] *num_pts];
...
__global__
void DfderrStep_k (float* f, bead_params* p, float* EmphasisVectorByHomopolymer, int* WhichEmphasis, 
    float* dark_matter_compensator, int* nucMap, int *active_bead_list, int num_fb, int num_pts, int num_steps, int step)
{
  int ibd = active_bead_list[blockIdx.x];
  int i = threadIdx.x;
...
__global__
void YerrStep_k (float* f, bead_params* eval_params, float* EmphasisVectorByHomopolymer, int* WhichEmphasis, 
    float* EmphasisScale, int* active_bead_list, FG_BUFFER_TYPE* fg_buffers, float* residual, int num_fb, 
    int num_pts, int num_steps, int num_beads, int step, int num_active_beads)
{
  // Bead list index
  ...
  float* fv = &f[ibd * (num_fb * num_pts * num_steps) ];
  float* ye = fv + (step * (num_pts * num_fb));
  fg_buffers += ibd * num_fb * num_pts + fnum * num_pts;
  WhichEmphasis += ibd*num_fb;

  int emndx = WhichEmphasis[fnum];
  ...
  {
    float eval = ( (float) (fg_buffers[i]) - fv[i])*em[i];
    rerr += eval*eval;
    ye[i] = eval;
  }

  res[threadIdx.x][threadIdx.y] = rerr;
...
template<int beads_per_block>
__global__
void CalcPartialDerivWithEmphasis_k (float* p, float* pe, int len, float dp, int pelen, int* WhichEmphasis,
    bead_params* eval_params, int* active_bead_list, int step, int num_fb, int num_pts, int num_steps, 
    int num_active_beads)
{
  ...
  for (int fnum = 0; fnum < num_fb; fnum++)
  {
    ...
    float* pev = &pe[WhichEmphasis[fnum]*num_pts];
                             ...
                             int num_active_beads)
{
  ...
  __shared__ float buffer[beads_per_block][block_size];
  ...
  for (int chunk = 0; chunk < length; chunk += block_size)
  {
    int i = chunk + tid;
    if (i < length)
      buffer[threadIdx.y][tid] += f1[i] * f2[i];
  }
  ...
  if (tid == 0)
    sums[ibd] += (double)buffer[threadIdx.y][0];
}
...
template<int beads_per_block>
__global__
void UnifiedWriteMatrixFromSums_k(int numInstr, AssyMatID* matId, int* row, 
  int* col, double* sum, int mat_dim, double* jtj, double* rhs, 
  int* active_bead_list, int num_active_beads, int num_beads)
{
    int bead_index = (blockIdx.x * beads_per_block) + threadIdx.x;
    ...
    for (int i=0; i<numInstr; ++i) {
      if (matId[i] == JTJ_MAT)
        jtj[ (col[i] * mat_dim + row[i]) ] = sum[num_beads*i + ibd];
      else
        rhs[row[i]] = sum[num_beads*i + ibd];
    }
}


__global__
void CalculateResidual_k (float* fval, bead_params* eval_params, float* ev, float* EmphasisScale, int* WhichEmphasis, 
    int* active_bead_list, FG_BUFFER_TYPE* fg_buffers, float* residual, int num_fb, int num_pts, int num_steps, 
    int num_beads)
{
  // Bead index
  int ibd = active_bead_list[blockIdx.x];
...
__global__
void CalculateRegionalResidual_k (float* fval, bead_params* eval_params, float* ev, float* EmphasisScale, int* WhichEmphasis, 
    int* active_bead_list, FG_BUFFER_TYPE* fg_buffers, float* residual, int num_fb, int num_pts, int num_steps, int num_beads)
{
  // Bead index
  int ibd = active_bead_list[blockIdx.x];
...
void MultiCycleNNModelFluxPulse_base_CUDA_k (float restrict_clonal, float* clonal_call_scale, float* fval, bead_params* p, reg_params* reg_p, float* ival, float* sbg, int* flow_ndx_map, float* delta_frame, float* dark_matter_compensator, int* buff_flow, int num_pts, int* active_bead_list, int step, int num_steps, int num_active_beads, int num_beads)
{
  ...
  // ASSUME TRansposed Data
  //extern __shared__ float smem_buffer[];
 
   
  //int bead_index = (blockIdx.x * beads_per_block +  threadIdx.x );

  int work_flow_index = blockIdx.x*flows_per_block + threadIdx.x;

  int flow_count = num_beads*num_fb;
  

  // Bounds check
  if (work_flow_index >=flow_count)
    return;


  // Adjust pointers
  int beadFrameProduct = num_beads * num_pts;
 
  // Bead index (assume all beads are active)
  int ibd = work_flow_index % num_beads;  ; //active_bead_list[bead_index];

  int fnum = work_flow_index/num_beads    ; //= threadIdx.x;
  ...
  // each thread iterates over flows !! embarresingly parallel can be done in parallel !!
  ...
  //{
    ...
    float Ampl = p->Ampl[fnum];
    // values for time shifting
    int intcall = ( (int) (Ampl + 0.5f));
    clamp (intcall, 0, 5);
    ...
    */
    ...
    //force dumb compiler to do vector 2 read!! 
    float2 pt = *(float2*)&p->gain; 
    float gain = pt.x;//  = p->gain;
    float pR = pt.y;  //   = p->R;    

    // calculate some constants used for this flow

    float R = pR * reg_p->NucModifyRatio[nnum] + reg_p->RatioDrift * buff_flow[fnum] * (1.0 - pR*reg_p->NucModifyRatio[nnum]) *0.001;
    ...
    float* ve = &SBG_CONST_CUDA[fnum * num_pts];
    //float* smem = &bead_smem[fnum * num_pts];
    float darkness = reg_p->darkness[0];
    float* et = &dark_matter_compensator[nnum*num_pts];

    float scaleTau = 2.0f * tau;
    float Roffset = R - 1.0f;
    float curSbgVal;
    float temp;

    for (int i = 0; i < num_pts; i++)
    {
      dt = DELTAFRAME_CONST_CUDA[i];
      aval = dt/scaleTau;
      // calculate new dv

      curSbgVal = ve[i];
      dvn = ( *ival + Roffset * curSbgVal - dv_rs / tau - dv * aval) / (1 + aval);
      dv_rs += (dv + dvn) * dt * 0.5f;
      dv = dvn;

      temp = (dv + curSbgVal) * gain + darkness * et[i];

      /*if (i < MAXCLONALMODIFYPOINTSERROR)
        temp += clonal_error_term * ( (float) (i & 1) - 0.5f) * 1600.0f;
      */
...
void AdjustParameters_k (int* active_bead_list, bead_params* eval_params, bead_params* params_nn, int num_outputs, float* lambda, double* jtj_lambda, double* delta,
                         delta_mat_output_line* output_list, bound_params* params_low, bound_params* params_high)
{
  // Get bead index
  int ibd = active_bead_list[blockIdx.x];
  ...
  if (threadIdx.x < NUMFB) {
    clamp (eval_params[ibd].Ampl[threadIdx.x], params_low[ibd].Ampl, params_high[ibd].Ampl);
    clamp (eval_params[ibd].kmult[threadIdx.x], params_low[ibd].kmult, params_high[ibd].kmult);
  }
...
__global__
void AdjustRegionalParameters_k (int* active_bead_list, reg_params* rp, bead_params* params_nn, bound_params* params_low, bound_params* params_high, int num_fb)
{
  // Get bead index
  int ibd = active_bead_list[ blockIdx.x ];
  ...
  for (i = 0; i < NUMFB; i++)
  {
    ...
    clamp (params_nn[ibd].Ampl[i], params_low[ibd].Ampl, params_high[ibd].Ampl);
    clamp (params_nn[ibd].kmult[i], params_low[ibd].kmult, params_high[ibd].kmult);
  }
                                        ...
                                        bead_params* params_nn, bead_params* eval_params, float* lambda, bool is_reg_fit, bool is_under_iter, int num_fb, int* fit_flag, int* req_flag)
{
  ...
  if ( (achg < 0.001f) && (lambda[ibd] >= 1e10))
  {
...
__global__
void MultiCycleNNModelFluxPulse_tshiftPartialDeriv_k (int* active_bead_list, int num_active_beads, int* WhichEmphasis, 
    float* fval, bead_params* p, reg_params* reg_p, float* ev, float* sbg, float* deltaFrame, int* buff_flow, 
    int* flow_ndx_map, int num_fb, int num_pts, int num_steps, int step)
{
  // Get bead index
  int ibd = active_bead_list[blockIdx.x];
  ...
  float* em = &ev[WhichEmphasis[fnum]*num_pts];
  ...
  {
    dt = deltaFrame[i];
    afact_denom = 1.0f + dt / (2.0f * tau);

    // calculate new dv
    dvn = ( (R - 1.0f) * (ve[i]) - dv_rs / tau - dt * dv / (2.0f * tau)) / afact_denom;
    dv_rs += (dv + dvn) * dt / 2.0f;
    dv = dvn;
...
InitializeArraysForBinarySearch_k (bool restart, bead_params* params_nn_cuda, float* ac, float* step,
    int* WhichEmphasis, int* active_bead_list, int num_beads, int num_fb, int num_active_beads)
{
...
__global__
void ErrorForBinarySearchNew_k (float* fval, bead_params* eval_params, float* ev, 
    int* WhichEmphasis,int* active_bead_list, FG_BUFFER_TYPE* fg_buffers, float* err, 
    int num_fb, int num_pts, int num_steps)
{
  ...
  for (i = 0; i < num_fb; i++)
  {
    ...
    emndx = WhichEmphasis[i]*num_pts + threadIdx.x;
    eval = ((float)(fg_buffers[temp]) - fval[temp]) *ev[emndx];
...
BinarySearchStepTwo_k (float* ap, float* ac, float* ep, float* ec, float* step,
                       bool* cont, int* done, float min_step, int* active_bead_list, int num_fb)
{
    extern __shared__ int sdone[];
    ...
    if (fnum < 10) {
        sdone[fnum] += sdone[fnum + 10];
    }
    __syncthreads();

    if (fnum < 5) {
        sdone[fnum] += sdone[fnum + 5];
    }
    __syncthreads();
                       ...
                       int num_fb, int num_pts, int numAmpParams, int numKrateParams, int num_active_beads)
{
  ...
  Rval = p->R * rp->NucModifyRatio[nnum] + rp->RatioDrift * buff_flow[fnum]
         * (1.0-p->R * rp->NucModifyRatio[nnum]) * 0.001;
                        ...
                        int* buff_flow, int* active_bead_list, int num_fb, int num_pts)
{
  ...
  float shiftRatio = (temp + rp->RatioDrift * buff_flow[fnum] * (1.0-temp) * 0.001) - 1.0;
  float tau = tauB[fnum];
                                   ...
                                   int num_active_beads)
{
  ...
  if (threadIdx.x < num_fb)
  {
    if (flowDone[threadIdx.x])
      return;
    temp = threadIdx.x*num_pts;
    double* ptr = &rvalShared[temp];
    for (i=0; i<num_pts; ++i)
    {
      rVal += ptr[i];
    }
    r[threadIdx.x] = rVal/ (double) wtScale[threadIdx.x];
  }
}
...
void RedHydrogenForXtalk_k (bead_params* p, reg_params* reg_p, float* vb_out,
                            ...
                            int num_pts, int* active_bead_list, int num_active_beads, int flowFrameProduct)
{
  extern __shared__ float smem_buffer[];
  ...
  p += ibd;
  i_start += ibd*NUMNUC;
  vb_out += ibd*flowFrameProduct;
  red_hydrogen += ibd*flowFrameProduct;
  ...
  // Coalesced input read from global memory
  for (int i = 0; i < flowFrameProduct; i += num_fb)
    bead_smem[i + threadIdx.x] = red_hydrogen[i + threadIdx.x];
  __syncthreads();
  ...
  for (; i < num_pts; i++)
  {
    ...
    // calculate new dv
    dvn = (smem[i] - dv_rs / tau - dv * aval) / (1.0 + aval);
    dv_rs += (dv + dvn) * dt * 0.5;
    smem[i] = dv = dvn;
  }
  __syncthreads();
  ...
  // Coalesced output write from global memory
  for (i = 0; i < flowFrameProduct; i += num_fb)
    vb_out[i + threadIdx.x] = bead_smem[i + threadIdx.x];
                                      ...
                                      int* numNeis, int* neiIdxMap, int* active_bead_list, int num_active_beads, int beadOffset)
{
  ...
  int nei_idx;
  float* trace;
  float* buffer = &smem[threadIdx.x*beadOffset];

  for (i=0; i<beads_per_block*beadOffset; i+=beads_per_block)
    smem[i + threadIdx.x] = 0;
  __syncthreads();
...
template <int beads_per_block>
__global__
void MultiFlowComputeCumulativeIncorporationSignalNew_CUDA_k (bead_params* p, reg_params* reg_p, float* ivalPtr, int* flow_ndx_map, float* deltaFrame, int* buff_flow, int num_pts, int num_fb, int* active_bead_list, int num_active_beads,float* nucRise, int* i_start)
{
  // Shared memory buffer
  extern __shared__ float smem[];
  ...
  float half_kr = (kr * 0.5f) / ISIG_SUB_STEPS_MULTI_FLOW;
  ...
  for (i = start; i < num_pts; i++)
  {
    if (totgen > 0.0)
    {
      ...
      for (st=1; (st <= ISIG_SUB_STEPS_MULTI_FLOW) && (totgen > 0.0);st++)
      {
        ...
        c_dntp_bot = c_dntp_top/ (1.0f + scaled_kr*pact*c_dntp_bot_plus_kmax);
                              ...
                              int ampParams, int krateParams, int num_active_beads)
{
  ...
  float A, kmult;
  float gain;
  float sign_and_sens = sens_cuda[fnum];
  ...
  float scaled_kr = kr*n_to_uM_conv/d;
  float half_kr = (kr*0.5f)/ISIG_SUB_STEPS_SINGLE_FLOW;
  float tmp;
  for (i=start;i < num_pts;i++)
  {
    frame = deltaFrame[i];
    ...
    // calculate new dv
    dvn = ( (totocc-totgen) *sign_and_sens - dv_rs/tau - dv*aval) / (1.0f+aval);
                                    ...
                                    int num_active_beads)
{
  ...
  float t, tnew;
  int i, st;
  for (i=0; (i<num_pts) && (scaled_dt < 1.0f); i++)
  {
    ...
    {
      tnew = tlast+ (t-tlast) * (float) st/ISIG_SUB_STEPS_MULTI_FLOW;
      scaled_dt = (tnew-t_mid_nuc) /sigma + 0.5f;

      if (scaled_dt > 0 && scaled_dt <= 1)
      {
        last_nuc_value = C*scaled_dt*scaled_dt* (3.0f-2.0f*scaled_dt);
...
template<int beads_per_block>
__global__ void CalculateEmphasisVectorIndexForFlow_k(bead_params* p, int num_fb, 
    int max_emphasis, int* whichEmphasis, int* active_bead_list, int num_active_beads)
{
  int idx = (blockIdx.x * beads_per_block) + threadIdx.y;
  int fnum = threadIdx.x;
  ...
  int nev = (int) (p->Ampl[fnum]);
  whichEmphasis[fnum] = nev > max_emphasis ? max_emphasis : nev;
   
}

template<int block_size, int beads_per_block>
__global__
void UnifiedDotProductMatrixSums_k(double* sums, float* fval, int* offset_1, 
    int* offset_2, int* length, int numInstr, int* subInstr, int* active_bead_list, 
    int num_fb, int num_pts, int num_steps, int num_active_beads, int totalbeads)
{
    // Shared memory buffer
    __shared__ volatile float buffer[beads_per_block][block_size];
    ...
    int count = 0;
    fval += ibd * num_steps * num_fb * num_pts;
    for (inst=0; inst < numInstr; ++inst) {
       
      sub = subInstr[inst];      
      buffer[threadIdx.y][tid] = 0;
      for (j=0; j < sub; ++j) {
        f1 = fval + offset_1[count];
        f2 = fval + offset_2[count];
        len = length[count];

        for (chunk = 0; chunk < len; chunk += block_size)
        {
          i = chunk + tid;
          if (i < len)
            buffer[threadIdx.y][tid] += f1[i] * f2[i];
      ...
      if (tid < 16) { 
          buffer[threadIdx.y][tid] += buffer[threadIdx.y][tid + 16];
          buffer[threadIdx.y][tid] += buffer[threadIdx.y][tid + 8];
          buffer[threadIdx.y][tid] += buffer[threadIdx.y][tid + 4];
          buffer[threadIdx.y][tid] += buffer[threadIdx.y][tid + 2];
          buffer[threadIdx.y][tid] += buffer[threadIdx.y][tid + 1];
      }

      if (tid == 0)
        sums[inst*totalbeads + ibd] += (double)buffer[threadIdx.y][0];
      __syncthreads();
    }
}
