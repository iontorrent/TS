...
{
    params_high = new bead_params[numLBeads];
    memset(params_high,0,sizeof(bead_params[numLBeads]));
...
{
    params_low = new bead_params[numLBeads];
    memset(params_low,0,sizeof(bead_params[numLBeads]));
    for (int i=0;i<numLBeads;i++)
    {
...
{
    ssq.resize(numLBeads);
    ppf.resize(numLBeads);
...
void BeadTracker::LimitBeadEvolution(bool first_block, float R_change_max, float copy_change_max)
{
    for (int ibd=0;ibd < numLBeads;ibd++)
    {
        ...
        struct bead_params *pl = &params_low[ibd];
        struct bead_params *ph = &params_high[ibd];
...
void BeadTracker::SelectKeyFlowValuesFromBeadIdentity(int *keyval, float *observed, int my_key_id, int keyLen)
{
  if (my_key_id<0)
    SelectKeyFlowValues(keyval,observed,keyLen);
      ...
      //@TODO keylen should be "usable sequence" and passed to upper level function somehow, probably via returnval
      for (int i=0; i<keyLen; i++)
...
float BeadTracker::KeyNormalizeOneBead(int ibd)
{
    ...
    if (oldkeynorm)
    {
      ...
      //@TODO: handle key flows for longer keys properly
...
void BeadTracker::LockAllBeadKeys()
{
  for (int ibd=0; ibd<numLBeads; ibd++)
    LockKeyFlows(ibd);
}

void BeadTracker::UnLockAllBeadKeys()
{
  for (int ibd=0; ibd<numLBeads; ibd++)
    UnLockKeyFlows(ibd);
}

void BeadTracker::LockKeyFlows(int ibd)
{
  // force key flows not to change from values
    struct bead_params *p = &params_nn[ibd];
    struct bead_params *pl = &params_low[ibd];
    struct bead_params *ph = &params_high[ibd];
    int key_val[NUMFB];
    ...
    SelectKeyFlowValuesFromBeadIdentity(key_val,p->Ampl, key_id[ibd], KEY_LEN);
    SetKeyFlows(p->Ampl,key_val,KEY_LEN);
    params_LockKey(pl,p->Ampl,KEY_LEN);
    params_LockKey(ph,p->Ampl,KEY_LEN); 
}

void BeadTracker::UnLockKeyFlows(int ibd)
{
    struct bead_params *pl = &params_low[ibd];
    struct bead_params *ph = &params_high[ibd];
    
    params_UnLockKey(pl,0.001,KEY_LEN);
    params_UnLockKey(ph,MAX_HPLEN-1.0,KEY_LEN); 
}
...
void BeadTracker::ResetFlowParams(int bufNum,int flow)
{
    ...
    for (int i=0;i<numLBeads;i++)
    {
        ...
        params_low[i].Ampl[bufNum] = 0.001;
        params_high[i].Ampl[bufNum] = MAX_HPLEN-1; // really?
    }
}
...
//for all bead
void BeadTracker::AssignEmphasisForAllBeads(int max_emphasis)
{
    for (int ibd=0;ibd <numLBeads;ibd++)
    {
        for (int i=0;i < NUMFB;i++)
        {
            ...
            int nev = (int)(params_nn[ibd].Ampl[i]);
            //if (nev > emphasis_data.numEv-1) nev = emphasis_data.numEv-1;
            if (nev> max_emphasis) nev=max_emphasis;
            params_nn[ibd].WhichEmphasis[i] = nev;
        }
    }
}


void BeadTracker::AllBeadAverageError()
{
    for (int ibd=0;ibd < numLBeads;ibd++)
    {
        float sum = 0.0;
        for (int fnum=0;fnum < NUMFB;fnum++) sum += params_nn[ibd].rerr[fnum];
        params_nn[ibd].avg_err = sum/NUMFB;
    }
}

int BeadTracker::UpdateClonalFilter()
{
...
int BeadTracker::FindClonalReads()
{
    // temp for debug:
    ofstream debug("debug.filter");
    assert(debug);
    ...
    for(int bead=0; bead<numLBeads; ++bead){
        ...
        // temp for debug:
        debug << setw( 6) << bead
              << setw(12) << setprecision(2) << scientific << ppf[bead]
              << setw(12) << setprecision(2) << scientific << ssq[bead]
              << endl;
    }

    // fit_noramls();
    // fill out params_nn[bead].clonal_read
...
void BeadTracker::MeanResidualByFlow(float *flow_res_mean)
{
    for (int fnum=0; fnum<NUMFB; fnum++)
      flow_res_mean[fnum] = 0;
    
       for (int ibd=0; ibd<numLBeads; ibd++)
        {
            if (params_nn[ibd].clonal_read)
            {
              for (int fnum=0; fnum<NUMFB; fnum++)
              {
               flow_res_mean[fnum] += params_nn[ibd].rerr[fnum];
              }
            }
        }
    ...
    for (int fnum=0; fnum<NUMFB; fnum++)
    {
        flow_res_mean[fnum] /= numLBeads;
    }
}

void BeadTracker::RescaleResidualByMeanPerFlow(float *flow_res_mean)
{
    for (int ibd = 0;ibd < numLBeads;ibd++)
    {
        for (int fnum=0;fnum < NUMFB;fnum++)
        {
            params_nn[ibd].rerr[fnum] /= flow_res_mean[fnum];
        }
    }    
}

void BeadTracker::DetectCorrupted()
{
   // if there are any wells that suddenly have more error in the fit, they are probably wash outs
    for (int ibd=0;ibd < numLBeads;ibd++)
    {
        if (params_nn[ibd].clonal_read or params_nn[ibd].random_samp)
        {
            int high_err_cnt = 0;

            for (int fnum = NUMFB-1; (fnum >= 0) && (params_nn[ibd].rerr[fnum] > params_nn[ibd].avg_err*1.5);fnum--) high_err_cnt++;

            if (high_err_cnt > TOOMANYERRORS)
            {
              params_nn[ibd].corrupt = true;
            }
        }
    }
}
