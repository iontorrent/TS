            ...
            while (!cont_proc)
            {
              ...
              if (well_fit->GetOutput (reinterpret_cast<float*> (&eval_params), bkg.lev_mar_fit->lm_state.lambda[ibd]) != LinearSolverException)
              {
                ...
                for (int fnum=0; fnum < num_fb; fnum++)
                {
                  ...
                  //eval_params.my_err.rerr[fnum] = sqrt (rerr/ (float) num_pts);
                  //mean_res += eval_params.my_err.rerr[fnum];
                  mean_res += sqrt (rerr/ (float) num_pts);
                }
                res = sqrt (res/scale);
                mean_res /= num_fb;

                //for (int fnum=0;fnum < num_fb;fnum++)
                 // eval_params.my_err.rerr[fnum] /= mean_res;
...
void BkgModelCuda::Fit()
{
  ...
  for (int i=0; i<num_beads; ++i)
  {
    ...
    for (int fnum=0; fnum<num_fb; ++fnum)
    {
      tot_res += sqrt(r1_host[i*num_fb + fnum]);
      flow_res_mean[fnum] += r1_host[i*num_fb+fnum];
  ...
  for (int ibd = 0;ibd < num_beads;ibd++)
  {
    ...
    for (int fnum=0;fnum < NUMFB;fnum++)
    {
      //bkg.my_beads.params_nn[ibd].my_err.rerr[fnum] /= flow_res_mean[fnum];
      //tot_res += bkg.my_beads.params_nn[ibd].my_err.rerr[fnum];
    }
    tot_res /= NUMFB;
  }
