...
void SearchAmplitude::BinarySearchAmplitude (BeadTracker &my_beads, float min_step,bool restart)
{
  ...
  my_scratch->FillShiftedBkg(*empty_trace, my_regions->rp.tshift, *time_c, true);
...
void ProjectAmplitude (bead_params *p, float *observed, float **model_trace, float *em_vector, int npts)
{
  for (int fnum=0; fnum<NUMFB; fnum++)
  {
    ...
    // bounds check
    if (p->Ampl[fnum]<0.001f)
      p->Ampl[fnum] = 0.001f; // projecting onto zero vector would be bad
...
void SearchAmplitude::ProjectionSearchOneBead (bead_params *p)
{
  ...
  my_regions->cache_step.CalculateNucRiseCoarseStep (reg_p, *time_c, *my_flow); // technically the same over the whole region
...
{
  float tr = float((int) (2*tx+0.5f) )/ 2.0f;
  if (tr<low) tr = low;
  if (tr>hi) tr = hi;
  return(tr);
}
