...
public:
  ...
  float sanity_nuc_step_lower;
   ...
   void  AdjustBackground(float *incorporation_traces,float *bkg_leakage_fraction, float *bkg_traces,BeadParams *p,reg_params *rp,FlowBufferInfo *my_flow,TimeCompression &time_c, int flow_block_size, int flow_block_start);
   ...
   void ResetMat();
   void SetupXtX(float *trc, int start_pt, int end_pt, std::vector<float> &ftimes, float tau);
   // fits the specified points to an exponential-decay + dc offset and returns the dc-offset term
   float fit_exp_tail_to_data(float *trc, int start_pt, int end_pt, std::vector<float> &ftimes, float tau,  bool debug = false);
