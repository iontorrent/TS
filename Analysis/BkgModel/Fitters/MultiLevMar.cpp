...
int MultiFlowLevMar::MultiFlowSpecializedLevMarFitParameters (int max_iter, int max_reg_iter, BkgFitMatrixPacker *well_fit, BkgFitMatrixPacker *reg_fit,float lambda_start,int clonal_restriction)
{
  ...
  bkg.my_beads.CorruptedBeadsAreLowQuality(); // make sure we're up to date with quality estimates
  ...
  while ( (iter < max_iter) && (lm_state.ActiveBeads > 0))
  {
    ...
    {
      // for safety
      memset(sbg, 0, sizeof(float[bkg.my_scratch.bead_flow_t]));

      bkg.emptytrace->GetShiftedBkg (bkg.my_regions->rp.tshift, bkg.time_c, sbg);
    ...
    if (reg_proc)
    {
      ...
      int reg_wells = LevMarAccumulateRegionDerivsForActiveBeadList (ival,sbg,eval_rp,
      ...
      IdentifyParameters (bkg.my_beads,*bkg.my_regions, lm_state.well_mask, lm_state.reg_mask,lm_state.skip_beads);
    ...
    else
    {
      ...
      // region-wide fit
      if (!well_only_fit && (failed_frac > 0.5f))
        iter = max_reg_iter; // which will be incremented later
    }
    iter++;
  }

  lm_state.FinalComputeAndSetAverageResidual(bkg.my_beads.high_quality);
...
void MultiFlowLevMar::DynamicEmphasis(bead_params &p)
{
  ...
  bkg.my_scratch.SetEmphasis (p.Ampl,bkg.my_beads.max_emphasis);
  bkg.my_scratch.CreateEmphasis (bkg.emphasis_data.EmphasisVectorByHomopolymer,bkg.emphasis_data.EmphasisScale);
}
  ...
  int iter, float *fvdbg, float *BackgroundDebugTapA, float *BackgroundDebugTapB)
{
  ...
  for (int nbd=0; (nbd < lm_state.ActiveBeads) && (defend_against_infinity<lm_state.numLBeads);defend_against_infinity++)
  {
    ...
    if (!lm_state.ValidBeadGroup (ibd) || !bkg.my_beads.high_quality[ibd])
    {
      nbd++;
      continue;
    }
    ...
    if ( (ibd == bkg.my_beads.DEBUG_BEAD) && (bkg.my_debug.trace_dbg_file != NULL))
    {
      ...
      bkg.emptytrace->GetShiftedBkg (bkg.my_regions->rp.tshift, bkg.time_c, BackgroundDebugTapA);
      //CalcXtalkFlux(ibd,BackgroundDebugTapB);
      memcpy (fvdbg,bkg.my_scratch.fval,sizeof (float) *bkg.my_scratch.bead_flow_t);
    }

//    params_CopyHits(&eval_params, &bkg.my_beads.params_nn[ibd]); // keep evaluations
                                       ...
                                       BkgFitMatrixPacker *reg_fit)
{
  ...
  while (!cont_proc && defend_against_infinity<EFFECTIVEINFINITY)
  {
    ...
    if (reg_fit->GetOutput ( (float *) (&eval_rp),lm_state.reg_lambda) != LinearSolverException)
    {
      ...
      if (bkg.global_defaults.generic_test_flag)
      ...
      bkg.emptytrace->GetShiftedBkg (new_rp.tshift, bkg.time_c, sbg);
...
float MultiFlowLevMar::CalculateCurrentResidualForTestBeads (float *sbg)
{
  float avg_error = 0.0;
  int cnt = 0;
  ...
  for (int nbd=0;nbd<lm_state.ActiveBeads;nbd++)
  {
    ...
    // if this iteration is a region-wide parameter fit, then only process beads
    // in the selection sub-group
    if (!lm_state.ValidBeadGroup (ibd) || (bkg.my_beads.high_quality[ibd]==false))
      continue;

    bkg.my_scratch.FillObserved (bkg.my_trace,bkg.my_beads.params_nn[ibd].trace_ndx); // set scratch space for this bead
...
float MultiFlowLevMar::TryNewRegionalParameters (reg_params *new_rp, float *sbg)
{
  ...
  for (int nbd=0;nbd<lm_state.ActiveBeads;nbd++)
  {
    ...
    bkg.my_scratch.FillObserved (bkg.my_trace, bkg.my_beads.params_nn[ibd].trace_ndx);
    FillScratchForEval (&eval_params, new_rp,sbg);
    // calculate error b/w current fit and actual data
//    params_CopyHits(&eval_params,&bkg.my_beads.params_nn[ibd]);
...
{
  int defend_against_infinity=0;
  float req_done = 0; // beads have improved?
  float executed_bead = 0.001; // did we actually check this bead, or was it skipped for some reason

  //@TODO nuc_rise step computed here, applied to all beads individually
  for (int nbd=0; (nbd < lm_state.ActiveBeads) && (defend_against_infinity<lm_state.ActiveBeads);defend_against_infinity++)
  {
    // get the index to the next bead to fit in the data matricies
    int ibd = lm_state.fit_indicies[nbd];

    if (!bkg.my_beads.high_quality[ibd] & lm_state.skip_beads)  // primarily concerned with regional fits for this iteration, catch up remaining wells later
    {
      nbd++; // must go to next bead if we are skipping one
      continue;
    }

    LevMarBuildMatrixForBead (ibd, ival, sbg,well_only_fit,eval_rp,well_fit,PartialDeriv_mask,iter, fvdbg, BackgroundDebugTapA, BackgroundDebugTapB);
                                       ...
                                       bool well_only_fit)
{
  ...
  while ( (!cont_proc) && (defend_against_infinity<EFFECTIVEINFINITY))
  {
    ...
    if (well_fit->GetOutput ( (float *) (&eval_params),lm_state.lambda[ibd]) != LinearSolverException)
    {
      ...
      float res = bkg.my_scratch.CalculateFitError (NULL, NUMFB);
      ...
      {
//        params_CopyHits(&eval_params,&bkg.my_beads.params_nn[ibd]); // store hits
    ...
    if ( (achg < 0.001f) && (lm_state.lambda[ibd] >= lm_state.lambda_max))
    {
      bead_not_improved = 1;
      if (well_only_fit)
      {
...
void MultiFlowLevMar::ComputePartialDerivatives (bead_params &eval_params, reg_params &eval_rp, unsigned int PartialDeriv_mask, float *ival, float *sbg, bool debug_flag)
{
  float *output;
  CpuStep_t *StepP;
...
//    params_IncrementHits(&eval_params);
    ComputeOnePartialDerivative (output, StepP,  eval_params, eval_rp, ival, sbg);

    if (debug_flag && (bkg.my_debug.trace_dbg_file != NULL))
    {
    ...
    bead_params &eval_params, reg_params &eval_rp, float *ival, float *sbg)
{
  ...
  else if (StepP->PartialDerivMask == DFDTSH)
  {
    ...
    bkg.emptytrace->GetShiftedSlope (eval_rp.tshift, bkg.time_c, neg_sbg_slope);
  ...
  {
    // for safety
    memset(output, 0, sizeof(float[bkg.my_scratch.bead_flow_t]));
    memset(ival, 0, sizeof(float[bkg.my_scratch.bead_flow_t]));
...
// partial w.r.t. gain is the function value divided by the current gain
  ...
  for (int fnum=0;fnum < NUMFB;fnum++)
  {
    ...
    em[fnum] = &bkg.my_scratch.custom_emphasis[bkg.time_c.npts*fnum];
  }
  // execute in parallel
...
void MultiFlowLevMar::Dfderr_Step (float *output, bead_params *eval_p)
{
  ...
  for (int fnum=0;fnum < NUMFB;fnum++)
  {
    dst[fnum] = &output[bkg.time_c.npts*fnum];
    em[fnum] = &bkg.my_scratch.custom_emphasis[bkg.time_c.npts*fnum];
    et[fnum] = bkg.my_regions->missing_mass.dark_nuc_comp[bkg.my_flow.flow_ndx_map[fnum]];
  }
  //execute
#ifdef __INTEL_COMPILER
  {
...
bool BeadIncluded(bool *my_quality, bool skip_beads, int ibd)
{
    return(my_quality[ibd] || !skip_beads);
}
