...
void debug_collection::DebugBeadIteration (bead_params &eval_params, reg_params &eval_rp, int iter, float residual,RegionTracker *pointer_regions)
{
...
void debug_collection::DebugIterations(BeadTracker &my_beads, RegionTracker *pointer_regions)
{
  DumpRegionParamsCSV (iter_dbg_file,& (pointer_regions->rp));
  my_beads.DumpAllBeadsCSV (iter_dbg_file);
}


// debugging functions down here in the darkness
// so I don't have to read them every time I wander through the code
void debug_collection::MultiFlowComputeTotalSignalTrace (SignalProcessingMasterFitter &bkg, float *fval,struct bead_params *p,struct reg_params *reg_p,float *sbg)
{
  ...
  MultiFlowComputeTraceGivenIncorporationAndBackground (fval,p,reg_p,bkg.region_data->my_scratch.ival,sbg,
                                               bkg.region_data->my_regions,bkg.region_data->my_scratch.cur_buffer_block,bkg.region_data->time_c,
                                               bkg.region_data->my_flow,bkg.global_defaults.signal_process_control.use_vectorization, bkg.region_data->my_scratch.bead_flow_t);
}
...
void debug_collection::DumpRegionTrace (SignalProcessingMasterFitter &bkg)
{
  ...
  if (my_fp && only_trace)
  {
    ...
    if (bkg.region_data->my_flow.buff_flow[0] == 0)
    {
...
// because the routines to compute them are "hidden" in the code
    ...
    DUMP_N_VALUES ("flows:","\t%d",bkg.region_data->my_flow.buff_flow,NUMFB);
    ...
    DUMP_N_VALUES ("nnum:","\t%d",bkg.region_data->my_flow.flow_ndx_map,NUMFB);
    fprintf (my_fp,"end_section:\n");
...
//    float my_xtflux[my_scratch.bead_flow_t];
    ...
    bkg.region_data->emptytrace->GetShiftedBkg (bkg.region_data->my_regions.rp.tshift, bkg.region_data->time_c, sbg);
    float skip_num = 1.0;
    ...
    for (int ibd=0;ibd < bkg.region_data->my_beads.numLBeads;ibd++)
    {
      ...
      struct bead_params *p = &bkg.region_data->my_beads.params_nn[ibd];
      fprintf (my_fp,"bead_row:%d\n",p->y);
      fprintf (my_fp,"bead_col:%d\n",p->x);
      float R_tmp[NUMFB],tau_tmp[NUMFB];
      for (int i=0;i < NUMFB;i++)
        R_tmp[i] = AdjustEmptyToBeadRatioForFlow (p->R,& (bkg.region_data->my_regions.rp),bkg.region_data->my_flow.flow_ndx_map[i],bkg.region_data->my_flow.buff_flow[i]);
      DUMP_N_VALUES ("R:","\t%f",R_tmp,NUMFB);
      for (int i=0;i < NUMFB;i++)
        tau_tmp[i] = ComputeTauBfromEmptyUsingRegionLinearModel (& (bkg.region_data->my_regions.rp),R_tmp[i]);
...
//        fprintf(my_fp,"in_cnt:%d\n",in_cnt[my_beads.params_nn[ibd].y*region->w+my_beads.params_nn[ibd].x]);
      ...
      // run the model
      MultiFlowComputeTotalSignalTrace (bkg,bkg.region_data->my_scratch.fval,p,& (bkg.region_data->my_regions.rp),sbg);

      struct bead_params eval_params = bkg.region_data->my_beads.params_nn[ibd];
      ...
      MultiFlowComputeTotalSignalTrace (bkg,tmp,&eval_params,&eval_rp,sbg);
      ...
      bkg.region_data->my_trace.MultiFlowFillSignalForBead (tmp_signal,ibd);
      ...
      ONLYTRACE_DUMP_N_VALUES ("flows:","\t%d",bkg.region_data->my_flow.buff_flow,NUMFB);
