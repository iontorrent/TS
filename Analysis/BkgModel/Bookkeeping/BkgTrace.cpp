...
// this should be compatible with i_start from the nuc rise - which may change if we change the shape???
    ...
    if (pt1 > 0) {
        ...
        float overhang = (time_cp->frameNumber[pt1] - t_start)/(time_cp->frameNumber[pt1]-time_cp->frameNumber[pt1-1]);
    ...
    if (pt2 < time_cp->npts) {
        ...
        // normalize to a fraction in the spirit of "this somehow makes it worse"
        float overhang = (t_end - time_cp->frameNumber[pt2-1])/(time_cp->frameNumber[pt2]-time_cp->frameNumber[pt2-1]);
...
void TraceHelper::BuildT0Map (Region *region, std::vector<float> *sep_t0_est, float reg_t0_avg, int img_cols, float *output)
{
