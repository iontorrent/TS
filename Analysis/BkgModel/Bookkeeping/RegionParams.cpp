...
void DumpRegionParamsLine(FILE *my_fp,int my_row, int my_col, reg_params &rp)
{
  ...
  fprintf(my_fp,"%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t",rp.tau_R_m,rp.tau_R_o,rp.RatioDrift,rp.CopyDrift,rp.tshift);
  for (int i=0; i<NUMFB; i++)
...
float GetModifiedMidNucTime(nuc_rise_params *cur, int NucID, int fnum)
{
  ...
  retval_time +=  cur->t_mid_nuc_delay[NucID]* (cur->t_mid_nuc[0]-cur->valve_open) /cur->magic_divisor_for_timing;
