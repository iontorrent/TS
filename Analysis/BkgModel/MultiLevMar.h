...
public:
  ...
  float   CalculateAverageResidual(float *sbg); // within loop
    ...
    void    ComputePartialDerivatives(bead_params &eval_params, reg_params &eval_rp, int ibd, unsigned int PartialDeriv_mask, float *ival, float *sbg);
    ...
    void    Dfyerr_Step(float *y_minus_f_emphasized,  struct bead_params *actual_p);
    float   TryNewRegionalParameters(reg_params *new_rp,  float *sbg);
    float   ComputeLevMarResidual(float *y_minus_f_emphasized);
       float   EvaluateNewBeadParameters(bead_params *eval_p);
