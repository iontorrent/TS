        ...
        float kr, float kmax, float d, float molecules_to_micromolar_conversion, PoissonCDFApproxMemo *math_poiss, bool do_simple) // default value for external calls
{
  ...
  if (math_poiss==NULL)
  {
    ...
    if (do_simple)
    {
        SimplifyComputeCumulativeIncorporationHydrogens(ival_offset,npts,deltaFrameSeconds,nuc_rise_ptr,SUB_STEPS,my_start,C,A,SP,kr,kmax,d, molecules_to_micromolar_conversion, math_poiss);
        ...
        ComplexComputeCumulativeIncorporationHydrogens(ival_offset,npts,deltaFrameSeconds,nuc_rise_ptr,SUB_STEPS,my_start,C,A,SP,kr,kmax,d,molecules_to_micromolar_conversion, math_poiss);
        ...
        float *kr, float *kmax, float *d, float *molecules_to_micromolar_conversion, PoissonCDFApproxMemo *math_poiss)
{
    ...
    for (i=common_start;i < npts;i++)
    {
        ...
        for (int q=0; q<FLOW_STEP; q++)
          ival_offset[q][i] = sign[q]*(totocc[q]-totgen[q]);
...
//    mixed_poisson_struct mix_ctrl;
    ...
    // if this becomes a permanent feature...should probably put it inside the mix memo class
    if (A < 0.0f)
    {
        A = -A;
        sign = -1.0f;
    ...
    c_dntp_top = 0.0f; // concentration at top
    ...
    for (i=my_start;i < npts;i++)
    {
        if (totgen > 0.0f)
        {
            ...
            ival_offset[i] = sign*(totocc-totgen);
        } else {
           ival_offset[i] = sign*totocc;
        }
