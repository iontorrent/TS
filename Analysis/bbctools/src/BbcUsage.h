...
static void Usage( string subcmd, bool verbose = false ) {
	if( subcmd.empty() ) {
		...
		"  forward and reverse base coverage at each position along a (genome) reference and, optionally,\n"
		"  whether each base was on target to any region, as specified by an input BED file. It also provides\n"
		"  tools to view regions and analyze coverage statistics with various options. Performance of viewing\n"
		"  operations are highly optimized, especially when using BBC indexing files that may also be created.\n"
		"  Other options produce and query aligned read summary (text) files, which are well suited to enriched\n"
		"  (targeted) sequencing techniques, such as PCR and ThermoFisher AmpliSeq and TargetSeq applications.\n\n"
		"Sub-commands:\n"
		...
		"  view:     View base coverage from a BBC file with output, range and summation options.\n\n"
		...
		if( verbose ) {
			cout << "Description:\n"
			"  Creates one or more files from a BBC file or multiple BAM files as input, or prints text output\n"
			"  depending on the options selected. Some options may require BAM files as input.\n\n"
			...
			"  -R,--regions <file>       Input BED file for defining 'on-target' regions flagged in the BBC, CBC\n"
			"      and/or coverage statistics file created. A value of '-' may be used to explicitly indicate no\n"
			"      target regions and may be used with an input BBC file to remove on-target information.\n"
			...
			"    'covdepth' (default) => output region loci and coverage depths - see --covDepths (-D) option.\n"
			...
			"      Requires input BAM files, since BBC files do not record base read origins.\n"
			...
			"  -i,--index                Create a BBC index (BCI) file, either at the same time or from an input\n"
			...
			"  -c,--coarse               Create a coarse base coverage (CBC) file, either at the same time or\n"
			...
			"      calculated values. <list> is a 0-based index for target annotation fields to report and may be\n"
			"      negative, indicating an offset from the last field, or a comma-separated list of such fields.\n"
			"      Output field (column) names may also be provided by <list> in the form '3:idX,4:idY'.\n"
			...
			"      reads respectively. A value of '' may be provided to remove any coverage-at-depth statistics.\n"
			"  -E,--e2eGap <int>         Specifies the minimum distance the ends of an aligned read may fall short\n"
			"      of either end of the target region to still be considered an end-to-end reads.\n"
			"      Only used with the --covStats (-C) for --readType (-T) 'trgreads', 'amplicon' or 'AmpliSeq'\n"
			"      options, and may be overridden by the --minPcCov (-M) option (see below). Default value is 0.\n"
			...
			"      the --regions (-R) file. This option is only available if for BAM file input.\n"
			...
			"  -i,--index                Create a BBC index (BCI) file. (May use BBC file input.)\n"
		...
		if( verbose ) {
			...
			"      If <window> is also provided only regions, or partial regions, within this range are viewed.\n"
			"      NOTE: The regions file may alternatively be used to annotation only - refer to -A option below.\n"
			...
			"      assessed for that base position. (See -B,--strandBiasThreshold options.) Default 10.\n"
		...
		if( verbose ) {
			...
			"  Options may be provided to change this format and, in particular, the output when summing over\n"
			"  sub-regions (bins) uses a start and end position and appends separate forward and reverse on-target\n"
			"  read counts, while omitting the on-target flag used with single base coverage output.\n"
			"  <window> is an optional reference locus, e.g. 'chr1', 'chr1:1000001' or 'chr1:1000001-1100001, or\n"
			"  trans-contig, e.g. 'chr1..chr2' or 'NM10016:2001..NM10085:1000'. Default locus is the whole reference.\n\n"
			...
			"      will be reset to the number of contigs if less, so if neither --numbins or --binsize are\n"
			...
			"      that would be output given the other binning options (-B, -N or -i). The valpercentCoverageue may be\n"
			...
			"      negative, in which case it is an offset is from what would be the last binned region output.\n"
			"      The default value (0) indicates to output tRegion coalescence (binning) oo the last region bin.\n\n"
			...
			"      separated or <first>,...(X)...,<last> for more than 3 values (X>1). <list> is a 0-based index\n"
			"      for target annotation fields to report, e.g. '3' for the 4th (target name) field. It may also\n"
			"      be negative, indicating an offset from the last field, or a comma-separated list of such fields.\n"
			"      NOTE: The regions file is used for annotation only and no longer defines coverage target regions.\n"
			...
			"  -c,--compactCoverage      Output contig/chromosome names on a separate line rather than for every\n"
			"      base position. (Contig line starts with '>'.) This option is only available for the (default)\n"
			"      base coverage view (i.e. where no -B, -N or -i options are also provided).\n"
			"  -l,--lociOnly             Output only the (binned) region loci, i.e. no coverage data.\n"
