...
public:
  ...
  double DumpStep(int r, int c, int w, int h, double t, char *fileName, Mask *mask, int flowNumber);
  void Cleanup();
  ...
  static  void    CalibrateChannelXTCorrection(const char *exp_dir,const char *filename);
...
protected:
  ...
  char   *experimentName;
  int   flowOffset;  //time between start of image and nuke flow
  int         retry_interval;
  int         total_timeout;
  void    cleanupRaw();
  int  numAcqFiles;
