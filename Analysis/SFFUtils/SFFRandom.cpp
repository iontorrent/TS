...
int main(int argc, char *argv[])
{
  ...
  int padBytes = (8 - ((31 + h.number_of_flows_per_read + h.key_length) & 7))  & 7;
  char padData[8];
  if (padBytes > 0) {
  ...
  for (int i=0;i<numReads;i++) {
	...
	name[r.name_length] = '\0';
      ...
      int readPadLength = ((8 - ((16 + r.name_length) & 7)))  & 7;
      if (readPadLength > 0) {
