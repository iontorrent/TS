...
function alignQC_noref()
{
    ...
    if(file_exists($fileIn))
    {
        ...
        while(($line=fgets($file)) !== false)
        {	
            if (strncmp($line, "ERROR: /usr/local/bin/alignmentQC.pl: unable to find reference genome tmap-f2/", strlen("ERROR: /usr/local/bin/alignmentQC.pl: unable to find reference genome tmap-f2/")) == 0){
            	$break_line = explode("tmap-f2/", $line);
            	return $break_line[1];
            }
        }
