...
from os import path
import subprocess
#uncomment this line to allow runnning fromt he command line
#from ion.reports import id2xy, blast_to_ionogram, tfParse, plotRaw, parseFilterMetrics, parseBeadfind, parseProcessParams, parseCafie

def getCurrentAnalysis(tfMetrics, cafieMetrics, procMetrics, beadMetrics, filterMetrics, res):
...
def addTfMetrics(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, keyPeak, res):
    ...
    if tfMetrics != None:                 
        for tf, metrics in tfMetrics.iteritems(): 
           if tf != 'LIB':
                ...
                if tf in cafieMetrics:
                    temp = cafieMetrics[tf]
                    for dbname, key in cafie_metrics_map.iteritems():
                        kwargs[dbname]=set_type(temp.get(key,'0'))
                    kwargs['rawIonogram'] = temp.get('Avg Ionogram','0')
                    kwargs['corrIonogram'] = temp.get('Corrected Avg Ionogram','0')
                else:
                    for dbname,key in cafie_metrics_map.iteritems():
                        kwargs[dbname]=0
...
def addAnalysisMetrics(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, res):
    ...
    cafie_metrics_map = {'sysCF':'Estimated System CF',
                         'sysIE':'Estimated System IE',
                         'sysDR':'Estimated System Signal Droop'}
    ...
    if 'System' in cafieMetrics: 
        for dbname, key in cafie_metrics_map.iteritems():
            kwargs[dbname]=set_type(cafieMetrics['System'].get(key, '0'))
...
def addLibMetrics(libMetrics, tfMetrics, keyPeak, lib_cafie, res):
    #print 'addlibmetrics'
    ...
    tf_metrics_map = {'sysSNR':'LIB'}
    lib_cafie_map = {'cf':'Region_CF','ie':'Region_IE','dr':'Region_DR'}
    ...
    for dbname, key in tf_metrics_map.iteritems():
        temp = tfMetrics.get(key, None)
        if temp != None:
            kwargs[dbname] = temp.get('System SNR','0')
        else:
            kwargs[dbname] = 0.0
    ...
    for dbname, key in lib_cafie_map.iteritems():
        kwargs[dbname] = set_type(lib_cafie.get(key,'0'))
...
def updateStatus(status, reportLink = False):
    """
...
def populateDb(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, libPath, status, keyPath, libcafiePath, QualityPath):
    print "populateDB"
    f = open('primary.key', 'r')
    pk = f.readlines()
    pkDict = {}
    ...
    if path.exists(libcafiePath):
        lib_cafie = parse_metrics(libcafiePath)
    else:
        metric = ['cf','ie','dr']
        lib_cafie = {}
        for m in metric:
    ...
    try:
        ...
        getCurrentAnalysis(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, res)
    except:
    ...
    try:
        ...
        addTfMetrics(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, keyPeak, res)
    except:
    ...
    try:
        ...
        addAnalysisMetrics(tfMetrics, cafieMetrics, procParams, beadMetrics, filterMetrics, res)
    except:
    ...
    try:
        ...
        addLibMetrics(libMetrics, tfMetrics, keyPeak, lib_cafie, res)
    except:
...
if __name__=='__main__':
    ...
    print folderPath
    pk = os.path.join(folderPath, 'primary.key')
    f = open(pk,'r')
    pk = f.readlines()
    f.close()
    pkDict = {}
    for line in pk:
        ...
        value = parsline[1].strip()
        pkDict[key]=value
    tfmap = os.path.join(folderPath, 'TFMapper.stats')
    mF = open(tfmap, 'r')
    metricsData = mF.readlines()
    mF.close()
    try:
        tfMetrics = tfParse.generateMetrics(metricsData)
    except Exception:
        print "Metrics Gen Failed"

    cafiePath = os.path.join(folderPath,'cafieMetrics.txt')
    beadPath = os.path.join(folderPath, 'bfmask.stats')
    procPath = os.path.join(folderPath, 'processParameters.txt')
    filterPath = os.path.join(folderPath, 'filterMetrics.txt')
    libPath = os.path.join(folderPath, 'alignment.summary')

    filterMetrics = parseFilterMetrics.generateParams(filterPath)
    try:
        cafieMetrics = parseCafie.generateMetrics(cafiePath)
    except:
        print traceback.format_exc()
        print "Probably need to specify the floworder"
    procMetrics = parseProcessParams.generateMetrics(procPath)
    ...
    res.timeStamp = datetime.datetime.now()
    res.save()
    libMetrics = parseLib(libPath)
    
    getCurrentAnalysis(tfMetrics, cafieMetrics, procMetrics, beadMetrics, filterMetrics, res)
    addTfMetrics(tfMetrics, cafieMetrics, procMetrics, beadMetrics, filterMetrics, res)
    addAnalysisMetrics(tfMetrics, cafieMetrics, procMetrics, beadMetrics, filterMetrics, res)
    #addBfMetrics(tfMetrics, cafieMetrics, procMetrics, beadMetrics, filterMetrics, res)
    addLibMetrics(libMetrics, tfMetrics,keyPeak, keyres)
