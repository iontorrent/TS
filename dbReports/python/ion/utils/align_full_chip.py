...
def align_full_chip_core(libsff, libKey, tfKey, floworder, fastqName, align_full, graph_max_x, do_barcode, make_align_graphs, sam_parsed, DIR_BC_FILES, env):
    ...
    if env['aligner_opts_extra']:
        print '  found extra alignment otpions: "%s"' % env['aligner_opts_extra']
    ...
    if (align_full):
        ...
        com = "alignmentQC.pl --input %s --genome %s --max-plot-read-len %s --align-all-reads %s %s >> ReportLog.html 2>&1" % (libsff, env["libraryName"], graph_max_x, aligner_opts_rg, aligner_opts_extra)
    else:
        # Add -p 1 to enable default.sam file generation
        com = "alignmentQC.pl --input %s --genome %s --max-plot-read-len %s %s %s >> ReportLog.html 2>&1" % (libsff, env["libraryName"], graph_max_x, aligner_opts_rg, aligner_opts_extra)
    try:
	...
	if make_align_graphs:
        ...
        for bcid in (x['id_str'] for x in barcodeList):
            ...
            if not path.exists(sffName):
                printtime("No barcode SFF file found for '%s'" % bcid)
                continue
            if (align_full):
                printtime("Align All Reads")
                #If a full align is forced add a '--align-all-reads' flag
                com = "alignmentQC.pl --input %s --genome %s --max-plot-read-len %s --align-all-reads %s %s >> ReportLog.html 2>&1" % (sffName, env["libraryName"], graph_max_x, aligner_opts_rg, aligner_opts_extra)
            else:
                printtime("Align Subset of Reads")
                # Add -p 1 to enable default.sam file generation
                com = "alignmentQC.pl --input %s --genome %s --max-plot-read-len %s %s %s >> ReportLog.html 2>&1" % (sffName, env["libraryName"], graph_max_x, aligner_opts_rg, aligner_opts_extra)
            try:
                printtime("Alignment QC command line:\n%s" % com)
                retcode = subprocess.call(com, shell=True)
                if retcode != 0:
            ...
            if path.exists('alignment.summary'):
                try:
        ...
        for fname in files:
            if path.exists(fname + '.comprehensive'):
                os.rename(fname + '.comprehensive', fname)

        aggregate_alignment (DIR_BC_FILES,'barcodeList.txt')

def align_full_chip(libsff, libKey, tfKey, floworder, fastqName, align_full, DIR_BC_FILES, env, sam_parsed):
    ...
    if path.exists('alignment.summary'):
        try:
            ...
            printtime('error renaming')
            traceback.print_exc()
    align_full_chip_core(libsff, libKey, tfKey, floworder, fastqName, align_full, 1, True, True, sam_parsed, DIR_BC_FILES, env)
