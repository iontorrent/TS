...
def _ensure_virtualenv():
    if "VIRTUAL_ENV" not in os.environ:
        sys.stderr.write(
            "$VIRTUAL_ENV not found. Make sure to activate virtualenv first\n\n"
        )
        sys.exit(-1)
...
def _pylint():
    ...
    local(
        "pylint --rcfile=.pylintrc --ignore=migrations,tests.py iondb > reports/pylint.out"
    )
...
def check(warn_only=True, extended=False):
    ...
    env.warn_only = (
        warn_only
    )  # setting env.warn_only to True doesn't cause the fab task to abort on shell failures returning non-zero
    _pep8()
    _pylint()
    _jshint()
    if extended:
...
def _verify_coverage(
    packages=[".", "iondb.rundb"],
    line_rate_threshold=85,
    branch_rate_threshold=85,
    fail=True,
):
    ...
    for package in _packages:
        if package.attrib["name"] in packages:
            ...
            if line_rate < line_rate_threshold:
                errors.append(
                    "Line rate of %f%% is below %d%%" % (line_rate, line_rate_threshold)
                )
            branch_rate = float(package.attrib["branch-rate"]) * 100
            if branch_rate < branch_rate_threshold:
                errors.append(
                    "Branch rate of %f%% is below %d%%"
                    % (branch_rate, branch_rate_threshold)
                )
    if errors and fail:
        abort("\n\t" + "\n\t".join(errors))
...
def _verify_local_settings():
    errors = []
    if not "django_nose" in base_settings.INSTALLED_APPS or base_settings.INSTALLED_APPS.index(
        "south"
    ) > base_settings.INSTALLED_APPS.index(
        "django_nose"
    ):
        errors.append(
            "!!! Please add 'django_nose' to the INSTALLED APPS after the entry for 'south' in your local_settings.py"
        )
        errors.append(
            "!!! .TODO: can we simplify this? auto-gen local_settings; auto-update existing local_settings?? "
        )

    if not "iondb.test_runner.IonTestSuiteRunner" == base_settings.TEST_RUNNER:
        errors.append(
            "!!! Please set TEST_RUNNER='iondb.test_runner.IonTestSuiteRunner' in your settings.py or local_settings.py"
        )
...
def test(reusedb=0, verbosity=2, coverage=1, viewcoverage=1, fail=1):
    ...
    cmd = (
        "export REUSE_DB=%s; export PYTHONPATH=`pwd`; \
