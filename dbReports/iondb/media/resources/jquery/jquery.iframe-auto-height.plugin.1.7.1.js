...
(function ($) {
  ...
  $.fn.iframeAutoWidth = function (spec) {
    ...
    function showDiagnostics(iframe, calledFrom) {
      debug("Diagnostics from '" + calledFrom + "'");
      try {
        debug("  " + $(iframe, window.top.document).contents().find('body')[0].scrollWidth + " for ...find('body')[0].scrollWidth");
    ...
    return this.each(function () {
      ...
      function resizeHeight(iframe) {
        if (options.diagnostics) {
          showDiagnostics(iframe, "resizeWidth");
        }
      ...
      if ($.browser.safari || $.browser.opera) {
        ...
        $(this).load(function () {
          ...
          if (loadCounter === 0) {
            ...
            // Reset iframe width to 0 to force new frame size to fit window properly
            // this is only an issue when going from large to small iframe, not executed on page load
  ...
  }; // $.fn.iframeAutoWidth = function (options) {
}(jQuery)); // (function ($) {
