...
def get_project2(db, project_name, daterange, timeStart, timeEnd):
    ...
    results = results.exclude(project__icontains='test').exclude(project__icontains='library').exclude(project__icontains='enrichment').exclude(project__icontains='emulsionstability')
...
def data_find(project_name, big_list):
        ...
        for exp in place[1]:
            ...
            try:
                ...
                if True:
                    ...
                    csv.write ('"' + r.resultsName + '",' + runid + ',' + chipType + ',"' + result_dir + innerpath + '","' + place[0] + '","' + str(r.timeStamp) + '"\n')
    ...
    for top in reversed(top5):
        ...
        if not r.libmetrics_set.all()[0].align_sample:
            ...
            top5_100AQ17Reads = top5_100AQ17Reads + r.libmetrics_set.order_by('i100Q17_reads')[0].i100Q17_reads
        else:
            top5_AQ17Bases = top5_AQ17Bases + r.libmetrics_set.all()[0].extrapolated_mapped_bases_in_q17_alignments
