...
from iondb.rundb.models import Results, Experiment, ExperimentAnalysisSettings, DMFileSet, DMFileStat
...
def write_serialized_json(result, skip_if_exists=True):
    ...
    if not os.path.exists(destination):
        print "Error: no report directory for %s (%d)." % (result.resultsName, result.pk)
        return
    ...
    try:
        with open(sfile,'wt') as f:
            ...
            f.write(obj_json)
        print "Result: %s (%d), saved serialized objs file %s" % (result.resultsName, result.pk, sfile)
        ...
        # also want the serialized json in raw data folder so Sigproc and proton onboard Basecalling Input files can be imported
        ...
        print "Error: Unable to save serialized.json for %s (%d)." % (result.resultsName, result.pk)
        print traceback.format_exc()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--reportName', dest='name', action='store', default="", help="created serialized json for a report")
    parser.add_argument('--expName', dest='exp_name', action='store', default="", help="created serialized json for all reports of an experiment")
    parser.add_argument('--skip-existing', dest='skip_if_exists', action='store', default = "on", help='skip json creation if file already exists [default:on]')
    ...
    if args.name:
        ...
        if not results:
            print "Error: no results found for experiment name %s, exiting." % args.exp_name
    else:
        #dmfilestats = DMFileStat.objects.exclude(dmfileset__type=dmactions_types.INTR).exclude(action_state__in=['AG','DG','AD','DD'])
        dmfilestats = DMFileStat.objects.filter(dmfileset__type=dmactions_types.BASE).exclude(action_state__in=['AG','DG','AD','DD'])
