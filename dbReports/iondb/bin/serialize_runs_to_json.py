...
if __name__ == '__main__':
    ...
    parser.add_argument('--reportName', dest='name', action='store',
                        default="", help="created serialized json for a report")
    parser.add_argument('--expName', dest='exp_name', action='store',
                        default="", help="created serialized json for all reports of an experiment")
    parser.add_argument('--skip-existing', dest='skip_if_exists', action='store',
                        default="on", help='skip json creation if file already exists [default:on]')
    ...
    if args.name:
        ...
        dmfilestats = DMFileStat.objects.filter(dmfileset__type=dmactions_types.BASE).exclude(
            action_state__in=['AG', 'DG', 'AD', 'DD'])
