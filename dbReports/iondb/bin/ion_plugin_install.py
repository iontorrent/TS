...
for pluginPackageName in pluginPackageNames:
    ...
    if pluginPackageVersion and pluginPackageVersion != 'remove':
        ...
        if not installVersion:
            sys.exit('The version ' + pluginPackageVersion + \
                     ' cannot be found for the package ' + pluginPackageName + '.')
    ...
    if pluginPackageVersion == 'remove' and package.is_installed:
        ...
        if len(illegalPackages) > 0:
            sys.exit(
                "This package cannot be upgraded because the following packages would be upgraded: " + str(illegalPackages))
        ...
        sys.exit('The package ' + pluginPackageName + ' does not have any updates available.')
