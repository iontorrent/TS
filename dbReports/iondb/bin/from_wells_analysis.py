...
def get_name_from_json(exp, key, thumbnail_analysis):
    ...
    if not name or name == "None":
        ...
        return "Reanalysis_%s_%s_%s%s" % (
            exp.pretty_print().replace(" ", "_"),
            exp.pk,
            uniq,
            twig,
        )
    ...
    params = urllib.urlencode(
        {
            ...
            "previousThumbReport": os.path.join(
                data_path
            ),  # defined in case its a thumby, innocuous otherwise
...
def newExperiment(_explog_path, _plan_json=""):
    ...
    if _newExp is None:
        ...
        explog[
            ...
            ""
        )  # don't allow getting plan by shortId - other plans may exist with that id
        try:
...
def update_chefSummary(_newExp, chefSummary):
    if chefSummary:
        for k, v in list(chefSummary.items()):
            if not v:
                continue
            setattr(_newExp, k, v)
...
def update_plan_info(_plan_json, planObj, easObj):
    ...
    try:
        for key in list(_plan_json.keys()):
        ...
        planObj.applicationGroup = models.ApplicationGroup.objects.get(
            id=_plan_json["applicationGroup_id"]
        )
        planObj.metaData["runTransferFromSource"] = _plan_json.get(
            "runTransferFromSource", ""
        )
        easObj.save()
        planObj.save()
    except Exception:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "is_thumbnail",
        metavar="is_thumbnail",
        default=False,
        help="Flag indicating thumbnail analysis only",
    )
    parser.add_argument(
        "directory", metavar="directory", help="Path to data to analyze"
    )
    ...
    if os.path.exists(os.path.join(src_dir, "onboard_results", "sigproc_results")):
        ...
        os.symlink(
            os.path.join(src_dir, "onboard_results", "sigproc_results"), symlink_path
        )
        is_fullchip = True
    else:
        is_fullchip = False
    ...
    if not os.path.isfile(testpath):
        print("Does not exist: %s" % testpath)
        print(
            "Cannot create TF key signal graph without %s file"
    ...
    if not os.path.isfile(testpath):
        print("Does not exist: %s" % testpath)
        print(
            "Cannot create Library key signal graph without %s file"
            % "avgNukeTrace_TACG.txt"
        )
    ...
    report_name = generate_http_post(
        newExp, src_dir, thumbnail_analysis=args.is_thumbnail == "True"
    )
