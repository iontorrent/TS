...
chipTypes = (
    models.Experiment.objects.all()
    .values_list("chipType", flat=True)
    .distinct("chipType")
)
...
for chipType in filter(lambda x: "\\" in x, chipTypes):
    ...
    print(
        "FIX-1: Going to fix %d experiments by replacing chipType %s with %s"
...
for chipType in filter(lambda x: '"' in x, chipTypes):
    ...
    exps = models.Experiment.objects.all().filter(chipType=chipType)
    print(
        "FIX-2: Going to fix %d experiments by replacing chipType %s with %s"
        % (exps.count(), badChars, clean_chipType)
    )
