...
def validate_report(reportname, print_files=False, write_file=False):
...
def validate_result(result, filter_plugins=True, print_files=False, write_file=False):
    ...
    if write_file:
        current_fs_filelist = get_walk_filelist(
            dirs, list_dir=os.getcwd(), save_list=True
        )
    else:
    ...
    if filter_plugins:
        current_fs_filelist = [
            filename
            for filename in current_fs_filelist
            if not "/plugin_out" in filename
        ]
    ...
    current_fs_filelist = [
        filename
        for filename in current_fs_filelist
        if not "cached.filelist" in filename
    ]
    # Ignore the status.txt file
    current_fs_filelist = [
        filename for filename in current_fs_filelist if not "status.txt" in filename
    ]
    # Ignore the serialized_*.json file
    current_fs_filelist = [
        filename for filename in current_fs_filelist if not "serialized_" in filename
    ]
    ...
    if print_files:
        for i, item in enumerate(leftovers):
            if not i:
                print("FILES MISSING FROM CACHED.FILELIST:")
            print(item)
    else:
        if len(leftovers) > 0:
...
"""
    ...
    parser.add_argument("--report", default=None, help="Name of a Report to validate")
    parser.add_argument(
        "--all",
        dest="all_reports",
        action="store_true",
    ...
    parser.add_argument("--max_items", default=0, help="Limit number of items to check")
    parser.add_argument(
        "--list", action="store_true", default=False, help="Print name of missing files"
    )
    parser.add_argument(
        "--output",
        action="store_true",
    ...
    if args.report:
        ...
        sys.exit(
            validate_report(args.report, print_files=args.list, write_file=args.output)
        )
    else:
