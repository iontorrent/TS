...
def make_22_legacy_report_template():
    ...
    html = tmpl.render(template.Context({"base_site_name": name}))
    with open(generate, "w") as outfile:
        outfile.write(html.encode("utf8"))
    print(
        "Wrote base template for the PHP default report, versions 2.2 and earlier: "
        + generate
    )
...
def make_30_report_template():
    ...
    html = tmpl.render(
        template.Context(
            {"tab": "reports", "base_site_name": name, "global_messages": "[]"}
        )
    )
    with open(generate, "w") as outfile:
        outfile.write(html.encode("utf8"))
    print(
        "Wrote base template for the PHP default report, versions 3.0 and later: "
        + generate
    )
