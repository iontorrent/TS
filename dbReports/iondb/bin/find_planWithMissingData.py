...
if plans:
    ...
    print(
        "*** Error: ",
        plans.count(),
        " plan(s) have no experiment associated with them: ",
    )
for plan in plans:
    print(
        "Plan name=%s: database_id=%d; planStatus=%s; planExecuted=%s; isTemplate=%s"
        % (
            plan.planName,
            plan.id,
            plan.planStatus,
            str(plan.planExecuted),
            str(plan.isReusable),
        )
    )
...
if exps:
    ...
    print(
        "*** Error: ",
        exps.count(),
        " experiment(s) have no experimentAnalysisSettings associated with them: ",
    )
for exp in exps:
    print(
        "Experiment name=%s: database_id=%d; status=%s"
        % (exp.expName, exp.id, str(exp.status))
    )
