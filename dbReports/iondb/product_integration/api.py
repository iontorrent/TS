...
class DeepLaserResponseResource(ModelResource):
    ...
    def prepend_urls(self):
        urls = [
            url(
                r"^(?P<resource_name>%s)/send_iot_request%s$"
                % (self._meta.resource_name, trailing_slash()),
                self.wrap_view("send_iot_request"),
            ),
            url(
                r"^(?P<resource_name>%s)/receive_iot_response%s$"
                % (self._meta.resource_name, trailing_slash()),
                self.wrap_view("receive_iot_response"),
            ),
            url(
                r"^(?P<resource_name>%s)/receive_device_request%s$"
                % (self._meta.resource_name, trailing_slash()),
    ...
    def receive_iot_response(self, request, **kwargs):
        ...
        logger.debug(
            "Received async iot response from DL request_id=%s\n%s\n",
            request_id,
            json.dumps(request_json_object, sort_keys=True, indent=2),
        )

        for response in DeepLaserResponse.objects.order_by("-date_received")[
            self.MAX_RESPONSES_STORED :
        ]:
            response.delete()

        response_model, created = DeepLaserResponse.objects.get_or_create(
    ...
    def receive_device_request(self, request, **kwargs):
        ...
        logger.debug(
            "Received async device request from DL \n%s\n",
            json.dumps(request_json_object, sort_keys=True, indent=2),
        )

        device_request_succeeded = True
        try:
            handle_deep_laser_device_request(
                request_key, request_parameters, request_principle_id
            )
        ...
        if request_type == "DEVICE":
            if device_request_succeeded:
                send_deep_laser_device_response({"status": "SUCCESS", "result": {}})
            else:
                send_deep_laser_device_response({"status": "ERROR", "result": {}})
