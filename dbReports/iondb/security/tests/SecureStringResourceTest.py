...
class SecureStringResourceTest(ResourceTestCase):
    def setUp(self):
        ...
        settings.TEMPLATE_CONTEXT_PROCESSORS = [
            x
            for x in settings.TEMPLATE_CONTEXT_PROCESSORS
            if x != "iondb.rundb.context_processors.base_context_processor"
        ]
        settings.DEBUG = True
        settings.LOGGING = {
            "version": 1,
            ...
            "loggers": {
                "django_test": {"handlers": ["file", "console"], "level": "DEBUG"},
                "iondb.security": {"handlers": ["file", "console"], "level": "DEBUG"},
            },
        }
    ...
    def test_get_list_unauthenticated(self):
        self.assertHttpUnauthorized(
            self.api_client.get("http://%s/%s/" % (HOST, API_PATH), format="json")
        )

    def test_get_list_json(self):
        # resp = requests.get('https://%s/%s/' % (HOST, API_PATH), verify=False, auth=AUTH)
        resp = self.api_client.get(
            "https://%s/%s/" % (HOST, API_PATH),
            format="json",
    ...
    def test_post_item(self):
        SECURED_STRING = {
            "unencrypted": "this is my secret",
            "name": "secret for testing",
        }

        resp = self.api_client.post(
            "http://%s/%s/" % (HOST, API_PATH),
            format="json",
            data=SECURED_STRING,
            authentication=self.get_credentials(),
