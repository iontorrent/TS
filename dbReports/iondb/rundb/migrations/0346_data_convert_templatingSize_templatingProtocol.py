...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        updated_rec_count = 0
        for plan_or_template in orm.plannedexperiment.objects.filter(
                templatingKitName__in=['Ion Chef S530 V2', 'Ion PGM Hi-Q Chef Kit', 'Ion PGM Hi-Q View Chef Kit']):
            print "*** 0346 > converting templating size to samplePrepProtocol template.id=%d; name=%s; runType=%s;" % (
            plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType)
            templatingSize = plan_or_template.templatingSize
            samplePrepProtocol = plan_or_template.samplePrepProtocol
            if not samplePrepProtocol:
                if templatingSize == "200":
                    plan_or_template.samplePrepProtocol = 'pcr200bp'
                if templatingSize == "400":
                    plan_or_template.samplePrepProtocol = 'pcr400bp'

            plan_or_template.save()
            updated_rec_count += 1
    ...
    def backwards(self, orm):
        ...
        for plan_or_template in orm.plannedexperiment.objects.filter(templatingKitName__in=['Ion Chef S530 V2', 'Ion PGM Hi-Q Chef Kit', 'Ion PGM Hi-Q View Chef Kit']):
            print "*** 0346 > Updating samplePrepProtocol to templatingSize template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType)
            samplePrepProtocol = plan_or_template.samplePrepProtocol
            if samplePrepProtocol == "pcr200bp":
                plan_or_template.templatingSize = '200'

            if samplePrepProtocol == "pcr400bp":
                plan_or_template.templatingSize = '400'
