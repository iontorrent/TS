...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "For each chiptype with quotes, remove those quotes."
        Experiment = orm['rundb.Experiment']
        chip_types = Experiment.objects.all().values_list('chipType', flat=True).distinct('chipType')
        for chip_type in filter(lambda x: '"' in x, chip_types):
            clean_chip_type = chip_type.replace('"', '')
            print(" - Replacing chipType %s with %s" % 
                (chip_type, clean_chip_type))
            Experiment.objects.filter(chipType=chip_type).update(chipType=clean_chip_type)


    def backwards(self, orm):
        """You can't really reverse this migration, but the state is equally
        usable before and after.
        """
        pass
