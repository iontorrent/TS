...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # new format for selectedPlugins json:
        # { "myplugin":{"userInput":"","version":"1.0","id":"32","name":"myplugin","features":[]}, "myuploader":{"userInput":"","version":"1.0","id":"73","name":"myuploader","features":["export"] } }
        plans = orm.plannedexperiment.objects.all()
        for plan in plans:

            if plan.selectedPlugins and plan.selectedPlugins != '{}':
                selectedPluginsOld = json.loads(plan.selectedPlugins)
                ##print "*** 0123_ plan.pk=%s; selectedPluginsOld=%s;" %(str(plan.pk), selectedPluginsOld)                         
                
                selectedPluginsNew = {}
                for plugin in selectedPluginsOld.get('planplugins',[]):
                    ##print "... 0123 plan.pk=%s; plugin=%s; planName=%s" %(str(plan.pk), aPlugin, plan.planName)
                    
                    name = plugin['name']
                    selectedPluginsNew[name] = plugin
                    selectedPluginsNew[name]['features'] = []

                    #when variantCaller is selected, if librarytype or variationtype has no values yet, populate them with values from the plan                                               
                    if name == "variantCaller":
                        userInput = plugin.get("userInput", {})
                        if userInput:
                            hasChanged = False
                            plugin_variantFrequency = userInput.get("variationtype", "")
                            if not plugin_variantFrequency:
                                ##print "*** 0123_ has userInput - going to set variantFrequency with value %s in plan.pk=%s; " %(plan.variantfrequency, str(plan.pk))                         
                                userInput["variationtype"] = plan.variantfrequency
                                hasChanged = True
                        
                            plugin_libraryType = userInput.get("librarytype", "")
                            if not plugin_libraryType:
                                libType = "fullgenome"
                        
                                if plan.runType == "AMPS":
                                    libType = "ampliseq"
                                elif plan.runType == "TARS":
                                    libType = "targetseq"
                        
                                userInput["librarytype"] = libType
                                hasChanged = True
                        else:
                            userInput = {}
                            ##print "*** 0123_ no userInput - going to set variantFrequency with value %s in plan.pk=%s; " %(plan.variantfrequency, str(plan.pk))                             
                            userInput["variationtype"] = plan.variantfrequency
                            libType = "fullgenome"
                        ...
                        if hasChanged:
                            plugin["userInput"] = userInput
                            selectedPluginsNew["variantCaller"] = plugin
                            
                            ##print "*** 0123_ new selectePlugins=%s in plan.pk=%s;" %(selectedPluginsNew, str(plan.pk)) 
                    

                for plugin in selectedPluginsOld.get('planuploaders',[]):
                    name = plugin['name']
                    selectedPluginsNew[name] = plugin
                    selectedPluginsNew[name]['features'] = ['export']

                plan.selectedPlugins = json.dumps(selectedPluginsNew)
                plan.save()

    def backwards(self, orm):
        # old format for selectedPlugins json:
        # {"planplugins":[{"userInput":"","version":"1.0","id":"32","name":"myplugin"}], "planuploaders":[{"userInput":"","version":"1.0","id":"73","name":"myuploader"}]}
        plans = orm.plannedexperiment.objects.all()
        for plan in plans:
            if plan.selectedPlugins and plan.selectedPlugins != '{}':
                selectedPluginsOld = {'planplugins':[], 'planuploaders':[]}
                selectedPluginsNew = json.loads(plan.selectedPlugins)                
                for name, plugin in selectedPluginsNew.items():
                    if plugin.get('features',[]) == ['export']:
                        selectedPluginsOld['planuploaders'].append(plugin)
                    else:
                        selectedPluginsOld['planplugins'].append(plugin)

                plan.selectedPlugins = json.dumps(selectedPluginsOld)
                plan.save()
