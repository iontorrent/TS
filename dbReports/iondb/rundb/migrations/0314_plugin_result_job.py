...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # before deleting the columns we are going to create new plugin result jobs
        # based on the model that there is one and only one job per result
        for pluginResult in orm.PluginResult.objects.all():
            pluginResultJob = orm.PluginResultJob(
                endtime=pluginResult.endtime,
                grid_engine_jobid=pluginResult.jobid,
                state=pluginResult.state,
                starttime=pluginResult.starttime,
                config=pluginResult.config,
                plugin_result=pluginResult
            )
            pluginResultJob.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        # when going backwards we are going to have to loose information by throwing out all but the latest jobs in the list
        for pluginResult in orm.PluginResult.objects.all():
            try:
                pluginResultJob = pluginResult.plugin_result_jobs.latest('endtime')
                pluginResult.endtime = pluginResultJob.endtime
                pluginResult.jobid = pluginResultJob.grid_engine_jobid
                pluginResult.state = pluginResultJob.state
                pluginResult.starttime = pluginResultJob.starttime
                pluginResult.config = pluginResultJob.config
                pluginResult.save()
            except orm.PluginResultJob.DoesNotExist:
                continue
    ...
    models = {
        ...
        u'rundb.pluginresult': {
            ...
            'jobid': ('django.db.models.fields.IntegerField', [], {'blank': 'True'}),
