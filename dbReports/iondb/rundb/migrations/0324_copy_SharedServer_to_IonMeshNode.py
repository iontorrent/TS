...
class Migration(DataMigration):
    ...
    # Note we cannot use security app frozen ORM as this migration needs access to class method SecureString.create;
    #   using depends_on instead to make sure security app is active
    depends_on = (
        ("security", "0001_initial"),
    )

    def forwards(self, orm):
        # This migration attempts to convert objects from the old SharedServer model to IonMeshNode:
        # 1) copy all server names and addresses from SharedServer model to IonMeshNode
        # 2) set plan share to true if shared server was active, set data and monitoring share to False
        # 3) save SharedServer username/password as SecureString so we can later attempt to authenticate the node
        # Additionally: fill in IonMeshNode.name = IonMeshNode.hostname for all existing nodes

        from iondb.security.models import SecureString
        
        shared_servers = orm.sharedserver.objects.all()
        for server in shared_servers:
            # skip exising nodes
            if orm.IonMeshNode.objects.filter(hostname=server.address):
                continue
        
            secure_name = 'sharedserver_' + server.name
            secure_string =  json.dumps({'username':server.username, 'password':server.password})
            ss = SecureString.create(secure_string, secure_name)
            ss.save()

            node = orm.IonMeshNode(
                name=server.name, 
                hostname=server.address,
                system_id='sharedserver_%d' % ss.pk # temporary, will use it later to retrieve username/password
            )
            node.share_plans = node.share_data = server.active
            node.save()


        for node in orm.IonMeshNode.objects.all():
            if not node.name:
