...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        delete obsolete runType and applicationGroup associations
        """
        
        dna_rna_groups = orm.ApplicationGroup.objects.filter(name = "DNA + RNA")
        if dna_rna_groups:
            dna_rna_group = dna_rna_groups[0]
     
            ##runTypes = orm.RunType.objects.filter(runType__in = ["AMPS", "AMPS_RNA"])
            runTypes = orm.RunType.objects.filter(runType__in = ["AMPS"])            
            for runType in runTypes:
                for entry in orm.RunType.applicationGroups.through.objects.filter(runtype__id = runType.id, applicationgroup__id = dna_rna_group.id):
                    print ">>> 0244 Going to delete entry id=%d for runType=%s; applicationGroup=%s" %(entry.pk, runType.runType, dna_rna_group.name)
                    entry.delete()
