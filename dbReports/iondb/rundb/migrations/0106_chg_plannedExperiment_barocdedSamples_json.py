...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # new format for barcodedSamples json:
        # {"bcsample1":{"barcodes":["IonXpress_003"]},"bcsample2":{"barcodes":["IonXpress_004","IonXpress_001"]}}
        plans = orm.plannedexperiment.objects.all()
        for plan in plans:
            barcodedSamplesDict = {}
            if plan.barcodedSamples:                                
                for key, value in json.loads(plan.barcodedSamples).items():
                    try:
                        if 'sample' in value.keys():                                
                            id_str = orm.dnaBarcode.objects.get(pk=key).id_str
                            sample = value['sample']
                            barcodedSamplesDict.setdefault(sample, {}).setdefault('barcodes',[]).append(id_str)
                        else:
                            id_strs = value.values()
                            sample = key
                            barcodedSamplesDict.setdefault(sample, {}).setdefault('barcodes',[]).extend(id_strs)
                    except:
                        pass                        
                plan.barcodedSamples = json.dumps(barcodedSamplesDict)
            
            plan.save()

    def backwards(self, orm):
        # old format for barcodedSamples json:
        # {"12984": {"sample": "bcsample1"}, "12985": {"sample": "bcsample2"}}
        plans = orm.plannedexperiment.objects.all()
        for plan in plans:
            barcodedSamplesDict = {}
            if plan.barcodedSamples and plan.barcodeId:                                
                for key, value in json.loads(plan.barcodedSamples).items():
                    for barcode in value.get('barcodes'):
                        try:
                            barcode_id = orm.dnaBarcode.objects.filter(id_str=barcode)[0].pk                            
                            entry = {barcode_id: {'sample': key}}
                            barcodedSamplesDict.update(entry)                
                        except:
                            pass                        
                plan.barcodedSamples = json.dumps(barcodedSamplesDict)
