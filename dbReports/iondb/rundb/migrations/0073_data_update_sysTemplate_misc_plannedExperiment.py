...
#20120831 change log:
# 1) added MuSeek library kit
# 2) renamed control seq kit from Ion AmpliSeq Sample Id panel to Ion AmpliSeq Sample ID Panel
# 3.1) changed MuSeek Library template for its name, chip type and library kit name
# 3.2) removed "Template" from all customer-facing system template names
# 3.3) blanked out control seq selection from all AmpliSeq system templates
# 3.4) blanked out bed file pre-selection from all AmpliSeq system templates
# 3.5) renamed "Ion AmpliSeq Cancer Hotspot Panel 2_0 Template" to "Ion AmpliSeq Cancer Hotspot Panel v2"
# 4) updated all plannedExperiments' control seq kit names from Ion AmpliSeq Sample Id panel to Ion AmpliSeq Sample ID Panel
# 5.1) renamed library kit name from Ion AmpliSeq Cancer Primer Pool to Ion AmpliSeq Kit
# 5.2) changed experiment library kit name for Ion AmpliSeq Kit
# 5.3) changed plannedExperiment library kit name for Ion AmpliSeq Kit

class Migration(SchemaMigration):

    def forwards(self, orm):
                   
        # 1) add MuSeek lib kit
        libKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "LibraryKit", name = "MuSeek(tm) Library Preparation Kit", 
            description = "MuSeek(tm) Library Preparation Kit", flowCount = 0)

        print "*** AFTER get_or_create MuSeek(tm) Library Preparation Kit isCreated=%s " % (str(isCreated)) 

        libKit.nucleotideType = 'dna'
        libKit.save()
        
                  
        # 2) update control seq kit name
        controlSeqKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "ControlSequenceKit", name = "Ion AmpliSeq Sample Id panel", 
            description = "Ion AmpliSeq Sample Id panel", flowCount = 0)

        print "*** AFTER get_or_create Ion AmpliSeq Sample Id panel isCreated=%s " % (str(isCreated)) 

        controlSeqKit.name = "Ion AmpliSeq Sample ID Panel"
        controlSeqKit.save()
        
        # 3) update system template data
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = True, isReusable = True, isSystemDefault = False):   
            # 3.1) update system template data in MuSeek Library template        
            if plannedExp.planDisplayedName == "MuSeek Library Template":
                plannedExp.planDisplayedName = "MuSeek Library"
                plannedExp.planName = "MuSeek_Library"
                plannedExp.chipType = "318"
                plannedExp.librarykitname = "MuSeek(tm) Library Preparation Kit"
                
                print "*** going to update system template plannedExperiment=%s " % (plannedExp.planName) 
                plannedExp.save() 

            # 3.2) remove "Template" from all customer-facing system template names            
            # 3.3) blank out control seq selection from all AmpliSeq system templates
            # 3.4) blank out bed file pre-selection from all AmpliSeq system templates
            # 3.5) rename "Ion AmpliSeq Cancer Hotspot Panel 2_0 Template" to "Ion AmpliSeq Cancer Hotspot Panel v2"
                    
            if plannedExp.planDisplayedName == "Ion AmpliSeq Cancer Hotspot Panel 2_0 Template":
                plannedExp.planDisplayedName = "Ion AmpliSeq Cancer Hotspot Panel v2"
                plannedExp.planName = "Ion_AmpliSeq_Cancer_Hotspot_Panel_v2";
                plannedExp.controlSequencekitname = None
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
                
                print "*** going to update system template plannedExperiment=%s " % (plannedExp.planName) 
                plannedExp.save() 

            # 3.2), 3.3), 3.4)               
            if plannedExp.planDisplayedName == "Ion AmpliSeq Cancer Panel Template":
                plannedExp.planDisplayedName = "Ion AmpliSeq Cancer Panel"
                plannedExp.planName = "Ion_AmpliSeq_Cancer_Panel"
                plannedExp.controlSequencekitname = None
                ##plannedExp.bedfile = "HSMv12.1_reqions_NO_JAK2_NODUP.bed"
                ##plannedExp.regionfile = "HSMv12.1_hotspots_NO_JAK2.bed"
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
            ...
            # 3.2), 3.3), 3.4)
            if plannedExp.planDisplayedName == "Ion AmpliSeq Cancer Panel Template 1_0 Lib Chem":
                plannedExp.planDisplayedName = "Ion AmpliSeq Cancer Panel 1_0 Lib Chem"
                plannedExp.planName = "Ion_AmpliSeq_Cancer_Panel_1_0_Lib_Chem";
                plannedExp.controlSequencekitname = None
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
            ...
            # 3.2), 3.3), 3.4)    
            if plannedExp.planDisplayedName == "Ion AmpliSeq Cancer Panel Template Beta Lib Chem":
                plannedExp.planDisplayedName = "Ion AmpliSeq Cancer Panel Beta Lib Chem"
                plannedExp.planName = "Ion_AmpliSeq_Cancer_Panel_Beta_Lib_Chem";
                plannedExp.controlSequencekitname = None             
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
            ...
            if plannedExp.planDisplayedName == "Ion AmpliSeq Comprehensive Cancer Panel":
                plannedExp.controlSequencekitname = None
                ##plannedExp.bedfile = "4477685_CCP_bedfile_20120517.bed"
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
            ...
            if plannedExp.planDisplayedName == "Ion AmpliSeq Inherited Disease Panel":
                plannedExp.controlSequencekitname = None
                ##plannedExp.bedfile = "4477686_IDP_bedfile_20120613.bed"
                plannedExp.bedfile = ""
                plannedExp.regionfile = ""
            ...
            # 3.2) 3.3)
            if plannedExp.planDisplayedName == "Ion PGM E_coli DH10B Control 300 Template":
                plannedExp.planDisplayedName = "Ion PGM E_coli DH10B Control 300"
                plannedExp.planName = "Ion_PGM_E_coli_DH10B_Control_300";
                
                print "*** going to update system template plannedExperiment=%s " % (plannedExp.planName) 
                plannedExp.save() 
                
         
        # 4) update all plannedExperiments' control seq kit names        
        for plannedExp in orm.plannedexperiment.objects.filter(controlSequencekitname = "Ion AmpliSeq Sample Id panel"):  
            plannedExp.controlSequencekitname = "Ion AmpliSeq Sample ID Panel"
                            
            print "*** going to update controlSeqKitName in plannedExperiment=%s " % (plannedExp.planName) 
            plannedExp.save()                            

        # 5.1) rename library kit name from Ion AmpliSeq Cancer Primer Pool to Ion AmpliSeq Kit                        
        try:
            libKit = orm.kitinfo.objects.filter(kitType = "LibraryKit", name = "Ion AmpliSeq Cancer Primer Pool")[0]
            libKit.name = "Ion AmpliSeq Kit"
            libKit.description = "Ion AmpliSeq Kit" 
            libKit.save()
        except IndexError:
            print "*** Ion AmpliSeq Cancer Primer Pool is not found. Going to create libKit with name Ion AmpliSeq Kit. "
            
            libKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "LibraryKit", name = "Ion AmpliSeq Kit", 
                    description = "Ion AmpliSeq Kit")
            
            print "*** AFTER get_or_create Ion AmpliSeq Kit isCreated=%s " % (str(isCreated)) 

            libKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = libKit, barcode = "4471262")
            print "*** AFTER get_or_create libKit Part 4471262 isCreated=%s " % (str(isCreated)) 
        ...
        # 5.2) change experiment library kit name for Ion AmpliSeq Kit
        for exp in orm.experiment.objects.filter(librarykitname = "Ion AmpliSeq Cancer Primer Pool"):
            print "*** going to change libKitName in experiment=%s " % (exp.expName) 

            exp.librarykitname = "Ion AmpliSeq Kit"
            exp.save()            

        
        # 5.3) change plannedExperiment library kit name for Ion AmpliSeq Kit
        for plannedExp in orm.plannedexperiment.objects.filter(librarykitname = "Ion AmpliSeq Cancer Primer Pool"):
            print "*** going to change libKitName in plannedExperiment=%s " % (plannedExp.planName) 
            
            plannedExp.librarykitname = "Ion AmpliSeq Kit"
            plannedExp.save()   
    ...
    def backwards(self, orm): 
        # 1) revert addition of MuSeek lib kit
        #try:
         #   libKits = orm.kitinfo.objects.filter(kitType = "LibraryKit", name="MuSeek(tm) Library Preparation Kit")
          #  for libKit in libKits:
           #     libKit.delete()
        #except:            
         #   print "*** Cannot delete MuSeek(tm) Library Preparation Kit library kit " 

        # 1) no-op for #1
        
        # 2) revert control seq kit renaming
        try:
            controlSeqKit = orm.kitinfo.objects.get(kitType = "ControlSequenceKit", name = "Ion AmpliSeq Sample ID Panel")
            controlSeqKit.name = "Ion AmpliSeq Sample Id panel"
            controlSeqKit.save()
        except:
            print "*** Cannot rename Ion AmpliSeq Sample ID Panel "        
        ...
        for plannedExp in orm.plannedexperiment.objects.filter(controlSequencekitname = "Ion AmpliSeq Sample ID Panel"):  
            ...
            print "*** going to revert controlSeqKitName in plannedExperiment=%s " % (plannedExp.planName) 
            plannedExp.save()   
            
        # 5.1) revert library kit name from Ion AmpliSeq Kit back to Ion AmpliSeq Cancer Primer Pool                         
        try:
            libKit = orm.kitinfo.objects.filter(kitType = "LibraryKit", name = "Ion AmpliSeq Kit")[0]
            libKit.name = "Ion AmpliSeq Cancer Primer Pool"
            libKit.description = "Ion AmpliSeq Cancer Primer Pool" 
            libKit.save()
        except IndexError:
            print "***  Ion AmpliSeq Cancer Primer Pool is not found. Going to create libKit with name Ion AmpliSeq Cancer Primer Pool. "
            
            libKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "LibraryKit", name = "Ion AmpliSeq Cancer Primer Pool", 
                    description = "Ion AmpliSeq Cancer Primer Pool")
            
            print "*** AFTER get_or_create Ion AmpliSeq Cancer Primer Pool isCreated=%s " % (str(isCreated)) 
        ...
        # 5.2) revert experiment's library kit names
        for exp in orm.experiment.objects.filter(librarykitname = "Ion AmpliSeq Kit"):
            #print "*** going to revert libKitName in experiment=%s " % (exp.expName) 

            exp.librarykitname = "Ion AmpliSeq Cancer Primer Pool"
            exp.save()            

        
        # 5.3) revert plan's library kit names
        for plannedExp in orm.plannedexperiment.objects.filter(librarykitname = "Ion AmpliSeq Kit"):
            #print "*** going to revert libKitName in plannedExperiment=%s " % (plannedExp.planName) 
            
            plannedExp.librarykitname = "Ion AmpliSeq Cancer Primer Pool"
            plannedExp.save()   
