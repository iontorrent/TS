...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Required because previous migration creates Project table, and we need that ContentType
        db.send_pending_create_signals()

        ionuser = None

        timerange = datetime.datetime.now() - datetime.timedelta(days=180)
        for e in orm.Experiment.objects.filter(date__gt=timerange):
          if e.project:
            ...
            if created: 
                ''' 
                See http://stackoverflow.com/questions/6976879/in-django-south-howto-create-an-instance-of-a-model-from-a-different-app-during
                '''
                ct = orm['contenttypes.contenttype'].objects.get(app_label="rundb", model="project")
                ev = orm.EventLog(object_pk=p.id, content_type=ct, text='Created during migration from Experiment project label.')
                ev.save()
            for result in e.results_set.all():
                result.projects.add(p)
                result.save()

    def backwards(self, orm):
        # NOTE: DATA is LOST here
        # - experiment gets assigned most recent project name, other project tags are lost
        for project in orm.Project.objects.all().order_by('id'):
