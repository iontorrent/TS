...
#20121123 change log:
# for plans that have somehow associated with more than 1 experiment, fixed the problem by
# cloning the plan  (this should not happen but, as we're preparing to change db
# to have zero-to-one relationship between plan and experiment, we need ensure the implicit relationship
# is 1-1 already
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        exps = orm.experiment.objects.exclude(plan_id__isnull = True).order_by("plan", "date")

        prevPlanId = None
        for exp in exps:
            isOneToMany = False

            if prevPlanId is None:
                prevPlanId = exp.plan.id              
            elif (prevPlanId == exp.plan.id):
                isOneToMany = True
            else:
                prevPlanId = exp.plan.id
            ...
            if isOneToMany:
                ...
                planObj = copy.copy(plan)
                planObj.pk = None
                        
                planGUID = str(uuid.uuid4())
                planObj.planGUID = planGUID
                ...
                planObj.planShortID = planShortID

                planObj.isReusable = False
                planObj.isSystem = False
                planObj.isSystemDefault = False
                
                planObj.planName = "CopyOf_" + plan.planName
                planObj.planDisplayedName = "CopyOf_" + plan.planDisplayedName
                planObj.expName = exp.expName

                planObj.planExecuted = True                   
                planObj.save()

                #clone the qc thresholds as well
                qcValues = plan.plannedexperimentqc_set.all()
                ...
                print "0110 OneToMany... exp.id=%d; name=%s; old plan.id=%d; new plan.id=%d" % (exp.id, exp.expName, prevPlanId, exp.plan.id)
