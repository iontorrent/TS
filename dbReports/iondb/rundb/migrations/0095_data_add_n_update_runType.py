...
#20121028 change log:
# 1) updated AmpliSeq description
# 2) added AmpliSeq RNA as a new application
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        appl, isCreated =orm.runtype.objects.get_or_create(runType = "AMPS",
            defaults = {"description" : "AmpliSeq DNA",
            "nucleotideType" : "dna"})
        
        if not isCreated:
            appl.description = "AmpliSeq DNA"
            appl.save()
            print "*** AFTER AMPS runType description is updated " 
            
        appl, isCreated =orm.runtype.objects.get_or_create(runType = "AMPS_RNA",
            defaults = {"description" : "AmpliSeq RNA",
            "nucleotideType" : "rna"})
                    
        print "*** AFTER get_or_create runType for AmpliSeq RNA isCreated=%s " % (str(isCreated)) 


    def backwards(self, orm):

        for appl in orm.runtype.objects.filter(runType__in = ["AMPS", "AMPS_RNA"]):
            if appl.runType == "AMPS":
                appl.description = "AmpliSeq"
                print "*** AFTER reverting description change of runType AMPS"
            else:
                appl.delete()
                print "*** AFTER reverting the addition of runType AMPS_RNA"
