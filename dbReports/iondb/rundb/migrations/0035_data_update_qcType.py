...
#20120715 change log:
# 1) modified default threshold value

class Migration(SchemaMigration):

    def forwards(self, orm):
            
        # modify qc type default threshold
        try:
            usableSeq = orm.qctype.objects.filter(qcName='Usable Sequence (%)')[0] 
            usableSeq.defaultThreshold ='30'
            usableSeq.save()   
            print "*** AFTER updating Usable Sequence (%) default threshold " 
        except IndexError:
            usableSeq, isCreated = orm.qctype.objects.get_or_create(qcName='Usable Sequence (%)', minThreshold='0', maxThreshold='100', defaultThreshold='30');    

            print "*** AFTER get_or_create Usable Sequence (%) isCreated=%s " % (str(isCreated))             
