...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        #update any sampleGroup values in planned experiments to be either a valid int or a None.  Any entries, such as 
        #Self, need to be lookeup in SampleGrouping_CV, and if not found, changed to None.
        planned_experiments = orm.plannedexperiment.objects.all()
        for pe in planned_experiments:
            try:
                if pe.sampleGrouping:
                    sample_grouping = int(pe.sampleGrouping)
                    try:
                        sample_group_type_cv_obj = orm.samplegrouptype_cv.objects.get(pk=sample_grouping)
                    except orm.samplegrouptype_cv.DoesNotExist:
                        pe.sampleGrouping = None
            except:
                #this is not a valid int, so we look it up in SampleGroupType_CV
                sample_grouping = pe.sampleGrouping
                try:
                    sample_group_type_cv_obj = orm.samplegrouptype_cv.objects.get(displayedName__iexact=sample_grouping)
                    pe.sampleGrouping = sample_group_type_cv_obj.pk
                except orm.samplegrouptype_cv.DoesNotExist:
                    pe.sampleGrouping = None
