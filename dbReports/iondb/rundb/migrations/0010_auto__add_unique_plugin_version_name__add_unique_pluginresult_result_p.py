...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Purge any duplicate records, which may violate the unique constraint
        plugin_purge = "DELETE FROM rundb_plugin WHERE id IN " + \
            "(SELECT a.id FROM rundb_plugin a, rundb_plugin b " + \
            "WHERE a.id != b.id AND a.name = b.name AND a.version = b.version)"
        db.execute(plugin_purge)

        pluginresult_purge = "DELETE FROM rundb_pluginresult WHERE id IN "+\
            "(SELECT a.id FROM rundb_pluginresult a, rundb_pluginresult b "+\
            "WHERE a.id != b.id AND a.plugin_id = b.plugin_id AND a.result_id = b.result_id)"
        db.execute(pluginresult_purge)

        # Commit above DELETES and restart transaction
        db.commit_transaction()
        db.start_transaction()

        # Adding index on 'Experiment', fields ['date']
        db.create_index('rundb_experiment', ['date'])

        # Adding index on 'Experiment', fields ['expName']
        db.create_index('rundb_experiment', ['expName'])

        # Adding index on 'Plugin', fields ['name']
        db.create_index('rundb_plugin', ['name'])

        # Adding unique constraint on 'Plugin', fields ['version', 'name']
        db.create_unique('rundb_plugin', ['version', 'name'])

        # Adding unique constraint on 'PluginResult', fields ['result', 'plugin']
        db.create_unique('rundb_pluginresult', ['result_id', 'plugin_id'])

        # Adding index on 'PlannedExperiment', fields ['planShortID']
        db.create_index('rundb_plannedexperiment', ['planShortID'])

    def backwards(self, orm):
        
        # Removing index on 'PlannedExperiment', fields ['planShortID']
        db.delete_index('rundb_plannedexperiment', ['planShortID'])

        # Removing unique constraint on 'PluginResult', fields ['result', 'plugin']
        db.delete_unique('rundb_pluginresult', ['result_id', 'plugin_id'])

        # Removing unique constraint on 'Plugin', fields ['version', 'name']
        db.delete_unique('rundb_plugin', ['version', 'name'])

        # Removing index on 'Plugin', fields ['name']
        db.delete_index('rundb_plugin', ['name'])

        # Removing index on 'Experiment', fields ['expName']
        db.delete_index('rundb_experiment', ['expName'])

        # Removing index on 'Experiment', fields ['date']
        db.delete_index('rundb_experiment', ['date'])
