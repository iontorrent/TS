...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """De-duplicate the many to one metrics tables on a given report, and
        then update the one to one keys on Results to point at the survivors.
        """
        AnalysisMetrics = orm['rundb.AnalysisMetrics']
        LibMetrics = orm['rundb.LibMetrics']
        QualityMetrics = orm['rundb.QualityMetrics']

        def deduplicate(table):
            # order them by the report id with the oldest rows in 'table' first
            delete_ids = []
            last_report_id = None
            rows = table.objects.values_list('report__id', 'id').order_by('report__id', '-id')

            # Read through the records and find instances of multiple records pointing at the 
            # same report_id this way of doing it necessarily skips deleting the first record 
            # for a given report_id and since they're sorted by descending record.id, the 
            # newest such record will be kept.
            for report_id, row_id in rows:
                if report_id == last_report_id:
                    delete_ids.append(row_id)
                else:
                    last_report_id = report_id

            # Bulk delete all the metric table ids with duplicate report_ids that we found.
            if delete_ids:
                table.objects.filter(id__in=delete_ids).delete()

        deduplicate(AnalysisMetrics)
        deduplicate(LibMetrics)
        deduplicate(QualityMetrics)

        def awesome_update(table_name):
            # Raw SQL voodoo to efficiently update the results table through 
            # the existing relations on the metrics tables. Do not try this at ORM.
            sql = "UPDATE rundb_results AS r SET %(table)s_id = f.id FROM rundb_%(table)s AS f WHERE r.id = f.report_id;"
            db.execute(sql % {"table": table_name})

        awesome_update("analysismetrics")
        awesome_update("libmetrics")
        awesome_update("qualitymetrics")

    def backwards(self, orm):
        "Let there be nulls!"
        Results = orm['rundb.Results']
        Results.objects.all().update(analysismetrics=None, libmetrics=None, qualitymetrics=None)
    ...
    models = {
        ...
        'rundb.results': {
            ...
            'analysismetrics': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['rundb.AnalysisMetrics']", 'unique': 'True', 'null': 'True'}),
            ...
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'libmetrics': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['rundb.LibMetrics']", 'unique': 'True', 'null': 'True'}),
            ...
            'qualitymetrics': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['rundb.QualityMetrics']", 'unique': 'True', 'null': 'True'}),
