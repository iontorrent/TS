...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Compute the representative report for each experiment and assign it to the new repResult foreign key."
        db.execute("""UPDATE rundb_experiment AS ex 
            SET "repResult_id" = er.rep_result_id,
                "pinnedRepResult" = er.representative
            FROM (
                SELECT DISTINCT ON (e.id) e.id, r.id AS rep_result_id, r.representative
                FROM rundb_experiment AS e 
                JOIN rundb_results AS r 
                ON (r.experiment_id = e.id) 
                ORDER BY e.id, r.representative DESC NULLS LAST, r."timeStamp" DESC
            ) AS er 
            WHERE ex.id = er.id;
        """)
    ...
    def backwards(self, orm):
        ...
        db.execute("""UPDATE rundb_results AS er 
            SET representative = (ex."pinnedRepResult" AND er.id = ex."repResult_id")
            FROM  rundb_experiment as ex
    ...
    models = {
        ...
        u'rundb.experiment': {
            ...
            'repResult': ('django.db.models.fields.related.OneToOneField', [], {'blank': 'True', 'related_name': "'+'", 'unique': 'True', 'null': 'True', 'to': u"orm['rundb.Results']"}),
