...
from iondb.rundb.models import PlannedExperiment
import uuid
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for pe in planned_experiments:
            #print ">>0207 LOOP... going to process plan.id=%d; name=%s" %(pe.id, pe.planDisplayedName)

            try:
                experiment_check = pe.experiment
            except:
                print ">>0207 WARNING!! SKIPPING plan that has no experiment record. plan.id=%d" %(pe.id)
                continue

            experiment = pe.experiment

            eas_es = self.get_EAS(experiment)

            if (not eas_es):
                print ">>0207 WARNING!! SKIPPING plan that has no EAS record. plan.id=%d" %(pe.id)
                continue
            ...
            for eas in eas_es:
                ...
                sampleSetids.append(pe.sampleSet_uid)

                selected_plugins = eas.selectedPlugins
            
                if selected_plugins:
                    selected_plugins = simplejson.loads(str(selected_plugins))    
                    #print ">>0207 going to process IonReporterUploader at selectedPlugins for plan.id=%d; eas.id=%d" %(pe.id, eas.id)
                
                    iru = selected_plugins.get('IonReporterUploader', "")
                
                    if iru and 'userInput' in iru:
                        if 'userInputInfo' in iru['userInput']:
                            ...
                            if type(uii) == types.DictType:
                                ...
                                if setid_value:
                                    ...
                                    print ">>0207 NO SET ID found in userInputInfo DICT!!! SKIPPING!!!" 

                            elif type(uii) == types.ListType:
                            
                                for ui in uii:
                                    if ui:
                                        if not 'setid' in ui:
                                            print ">>0207 The KEY setid was not found in selectedPlugins for plan.id=%d; eas.id=%d" %(pe.id, eas.id)
                                            continue
                                        setid = str(ui['setid'])
