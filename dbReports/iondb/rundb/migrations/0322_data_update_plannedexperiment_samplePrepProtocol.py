...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # update samplePrepProtocol from myeloid to denature30_cycles45_20
        OLD_VALUE = "myeloid"
        NEW_VALUE = "denature30_cycles45_20"

        updated_rec_count = 0

        for plan_or_template in orm.plannedexperiment.objects.filter(samplePrepProtocol = OLD_VALUE, isReusable = True):
            print "*** 0322 > Updating samplePrepProtocol template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
            plan_or_template.samplePrepProtocol = NEW_VALUE
            plan_or_template.save()
            updated_rec_count += 1                     

        for plan_or_template in orm.plannedexperiment.objects.filter(samplePrepProtocol = OLD_VALUE, isReusable = False, planStatus = "pending"):
            print "*** 0322 > Updating samplePrepProtocol chef plan.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
            plan_or_template.samplePrepProtocol = NEW_VALUE
            plan_or_template.save()
            updated_rec_count += 1                     

        print ">> 0322 > Total %d plan or template records updated." %(updated_rec_count)


    def backwards(self, orm):
        OLD_VALUE = "denature30_cycles45_20" 
        NEW_VALUE = "myeloid"

        updated_rec_count = 0

        for plan_or_template in orm.plannedexperiment.objects.filter(samplePrepProtocol = OLD_VALUE, isReusable = True):
            print "*** 0322 > Reverting samplePrepProtocol template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
            plan_or_template.samplePrepProtocol = NEW_VALUE
            plan_or_template.save()
            updated_rec_count += 1                     

        for plan_or_template in orm.plannedexperiment.objects.filter(samplePrepProtocol = OLD_VALUE, isReusable = False, planStatus = "pending"):
            print "*** 0322 > Reverting samplePrepProtocol chef plan.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
            plan_or_template.samplePrepProtocol = NEW_VALUE
            plan_or_template.save()
            updated_rec_count += 1                     

        print ">> 0322 > Total %d plan or template records reverted." %(updated_rec_count)
