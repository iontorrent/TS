...
#20120726 change log:
# 1) somehow, all non-rna applProduct entries have disappeared from db locally. Fix that if needed.
# 2) for targetSeq, set default samplePrepKit info
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        defaultPELibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name__iexact = "Ion Xpress Plus Fragment Library Kit")        
        defaultAmpsPELibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name__iexact = "Ion AmpliSeq Kit")        
        defaultRnaPELibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name__iexact = "Ion Total RNA Seq Kit v2")
        ...
        defaultTarsSamplePrepKit = orm.kitinfo.objects.get(kitType = "SamplePrepKit", name__iexact = "Ion TargetSeq custom enrichment kits")
                
        # update applProduct data
        for appl in orm.runtype.objects.all():
            if appl.runType == 'AMPS':
                code = '%s_%s' % (appl.runType, '0')
                name = '%s_%s' % (appl.runType, 'default')
                
                applProduct, isCreated = orm.applproduct.objects.get_or_create(productCode=code, productName=name, 
                    applType = appl, isActive='True', isVisible='False', isDefault='True') 
                    
                print "*** AFTER get_or_create applProduct for %s isCreated=%s " % (appl.runType, str(isCreated)) 
                                    
                applProduct.isDefaultPairedEnd = False
                applProduct.defaultVariantFrequency ='Germ Line'
                
                applProduct.defaultLibraryKit = defaultAmpsLibKit
                applProduct.defaultPairedEndAdapterKit = defaultAmpsPELibAdapterKit
                applProduct.defaultTemplateKit = defaultAmpsTemplateKit
                applProduct.defaultSequencingKit = defaultAmpsSeqKit
                applProduct.defaultPairedEndSequencingKit = defaultAmpsPESeqKit
                applProduct.defaultPairedEndLibraryKit = defaultAmpsPELibKit
                
                applProduct.defaultFlowCount = 260
                
                applProduct.defaultGenomeRefName = 'hg19'
                applProduct.defaultTargetRegionBedFileName = ''
                applProduct.defaultHotSpotRegionBedFileName = ''
                
                applProduct.save()
                
                print "*** AFTER updating applProduct=%s " % (applProduct.applType.runType) 
                                
            else:
                if appl.nucleotideType == 'dna':
                    code = '%s_%s' % (appl.runType, '0')                    
                    name = '%s_%s' % (appl.runType, 'default')
                    ...
                    applProduct.defaultLibraryKit = defaultLibKit
                    applProduct.defaultPairedEndAdapterKit = defaultPELibAdapterKit
                    applProduct.defaultTemplateKit = defaultTemplateKit
                    applProduct.defaultSequencingKit = defaultSeqKit
                    applProduct.defaultPairedEndSequencingKit = defaultPESeqKit
                    applProduct.defaultPairedEndLibraryKit = defaultPELibKit
                
                    applProduct.defaultFlowCount = 520
                
                    applProduct.defaultGenomeRefName = ''
                    applProduct.defaultTargetRegionBedFileName = ''
                    applProduct.defaultHotSpotRegionBedFileName = ''
                    
                    if appl.runType == 'TARS':
                        applProduct.defaultSamplePrepKit = defaultTarsSamplePrepKit
                        print "*** setting TARS applProduct defaultSamplePrepKit to %s " % (defaultTarsSamplePrepKit.name) 
                                            
                    applProduct.save()
                
                    print "*** AFTER updating applProduct=%s " % (applProduct.applType.runType)  
                else:
                    if appl.nucleotideType == "rna":  
                        defaultLibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name__exact = "Ion Total RNA Seq Kit v2")
                        defaultRnaPELibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name__iexact = "Ion Total RNA Seq Kit v2")
                        ...
                        applProduct.isPairedEndSupported = False
                        applProduct.isDefaultPairedEnd = False
                        applProduct.defaultVariantFrequency ='Germ Line'
                
                        applProduct.defaultLibraryKit = defaultLibKit
                        applProduct.defaultPairedEndLibraryKit = defaultRnaPELibKit
                        applProduct.defaultTemplateKit = defaultTemplateKit
                        applProduct.defaultSequencingKit = defaultSeqKit
    ...
    def backwards(self, orm):      
        ...
        for applProduct in orm.applproduct.objects.all():
            ##print "runType is %s " % str(applProduct.applType_id)
            
            try:
                for appl in orm.runtype.objects.filter(runType__in=['GENS', 'AMPS', 'TARS', 'WGNM']):
                    if applProduct.isDefault == True and applProduct.applType_id == appl.id:
                                        
                        print "*** GOING to delete default applProduct=%s " % (applProduct.applType.runType) 
                        
                        applProduct.delete()

            except:
                print "applType not found "
