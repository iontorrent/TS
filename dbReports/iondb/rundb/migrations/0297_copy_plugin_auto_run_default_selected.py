...
def update_df_selected(pluginData, autoRunJson):
    for pluginName,autorun in autoRunJson.iteritems():
        if pluginData.name == pluginName:
            pluginData.defaultSelected = autorun
            pluginData.save()
            return
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        fileName = "/tmp/autorun_data.json"
        if os.path.isfile(fileName) and os.access(fileName, os.R_OK):
            with open(fileName, 'r') as data:
                autoRunJson = json.load(data)
            for pluginData in orm.Plugin.objects.filter(selected=True, active=True):
                update_df_selected(pluginData, autoRunJson)
            os.remove(fileName)
        else:
            print ("/tmp/autorun_data.json does not exists or permission denied or already been applied")
