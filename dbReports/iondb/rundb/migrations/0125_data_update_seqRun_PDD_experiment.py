...
#20121029 change log:
# 1) Plan data decentralization (PDD): for each non-planned run (aka experiment),   
#    update plan and Experiment, and create Sample and EAS records

#20130301 change log:
# 2) if user does not specify a library kit on the instrument, use the value in the plan
# 3) fixed attribute names in rollback


class Migration(DataMigration):

    def forwards(self, orm):
        exps = orm.experiment.objects.all().exclude(status = "planned")
        
        if exps:
            print ">>> migration script 0125 going to create Sample and EAS records for %d experiments. " %(exps.count())
        else:
            print ">>> migration script 0125 finds NO experiments meeting the criteria to create Sample or EAS record."
        ...
        for exp in exps:
            ...
            #get associated plan if there is any
            plan = exp.plan
                            
            sampleName = "" if not exp.sample else exp.sample
            ...
            if plan:
                #update plan status to "run"
                plan.planStatus = status
                plan.save()
                
                # Note-1: For pre-existing non-barcoded experiments, the sample name from the experiment 
                # takes precedence of the plan since user could have overridden the sample name on the instrument.
                # Note-2: Moreover, if user does not select a plan on the instrument, the system default plan 
                # template will be used for the experiment but it has no sample pre-specified.
                if not sampleName:
                    sampleName = "" if not plan.sample else plan.sample
                    sampleDisplayedName = "" if not plan.sampleDisplayedName else plan.sampleDisplayedName
                ...
                barcodedSamples = plan.barcodedSamples
                
            currentDate = datetime.datetime.now()
                            
            #don't bother to sample object if the experiment has no sample name
            if sampleName:              
                #create sample     
                sample_kwargs = {
                                 'name' : sampleName,
                ...
                sample = orm.sample.objects.get_or_create(name=sampleName, defaults=sample_kwargs)[0]
                sample.experiments.add(exp)
                sample.save()
                ##print "*** after creating sample oid=%s for exp=%s;" %(str(sample.pk), exp.expName)                   
                
            # add multiple barcoded samples
            if barcodedSamples:
                for displayedName, value in json.loads(barcodedSamples).items():                
                    ...
                    sample = orm.sample.objects.get_or_create(name=name, defaults=sample_kwargs)[0]
                    sample.experiments.add(exp)
                    sample.save()
                    
                    ##print "*** after creating barcoded sample oid=%s for exp=%s;" %(str(sample.pk), exp.expName)     
        
            #create ExperimentAnalysisSettings object as best as we could
            eas_3primeAdapter = "" if not exp.forward3primeadapter else exp.forward3primeadapter
            eas_libraryKey = "" if not exp.libraryKey else exp.libraryKey

            if exp.isReverseRun:
                eas_3primeAdapter = "" if not exp.reverse3primeadapter else exp.reverse3primeadapter
                eas_libraryKey = "" if not exp.reverselibrarykey else exp.reverselibrarykey
                ##print "Hello, I AM PAIRED-END REVERSE RUN!!!"
            ...
            #if user does not specify a library kit on the instrument, use the value in the plan
            libraryKitName = "" if not exp.librarykitname else exp.librarykitname
            if not libraryKitName:
                if plan and plan.librarykitname:
                    #print ">>> 0125 exp.id=%s; name=%s; will use plan libraryKitName=%s" %(str(exp.id), exp.expName, plan.librarykitname)   
                    libraryKitName = plan.librarykitname
            #else:
             #   print ">>> 0125 exp.id=%s; name=%s; will use experiment libraryKitName=%s" %(str(exp.id), exp.expName, libraryKitName)   

            eas_kwargs = {
                          'barcodedSamples' : barcodedSamples,
                          'barcodeKitName' : "" if not exp.barcodeId else exp.barcodeId,
                          'date' : currentDate,
                          'experiment' : exp,
                          ...
                          'reference' : "" if not exp.library else exp.library,
            ...
            # attach EAS to Results, reference is the only saved setting that could be overriden
            for result in exp.results_set.all():                
                if result.reference == eas.reference:
                    ##print "adding EAS to result=%s" % result.resultsName
                    result.eas = eas
                else:
                    ##print "creating non-reusable EAS for result=%s" % result.resultsName
                    override_eas = orm.experimentAnalysisSettings(**eas_kwargs)
                    override_eas.isOneTimeOverride  = True
                    override_eas.reference = result.reference
                    override_eas.save()
                    result.eas = override_eas
                result.save()
            
            
            ##print "*** after creating eas oid=%s for experiment=%s;" %(str(eas.pk), exp.expName)      
            
            ##print "*** after updating experiment=%s; oid=%s" %(exp.expName, str(exp.pk))


    def backwards(self, orm):
        exps = orm.experiment.objects.all().filter(status = "run")
        for exp in exps:
            status = ""
            plan = exp.plan

            eas_set = exp.eas_set.all().order_by("-date").filter(isOneTimeOverride=False)
            samples = exp.samples.all()
            ...
            # repopulate Experiment fields
            if eas_set:
                eas = eas_set[0]
                exp.barcodeId = eas.barcodeKitName
                exp.library = eas.reference
                exp.libraryKey = eas.libraryKey
                exp.librarykitname = eas.libraryKitName
                exp.librarykitbarcode = eas.libraryKitBarcode
                exp.forward3primeadapter = eas.threePrimeAdapter

            if samples:
                exp.sample = samples[0].name
            ...
            for sample in exp.samples.all():
                ...
                if not (sample.status == "loaded"): 
                    ##print "*** about to rollback PDD addition of sample=%s in experiment=%s" %(sample.name, exp.expName)                         
                    sample.delete()
                    
            eas_set = exp.eas_set.all()
            for eas in eas_set:
                
                # repopulate Results field
                for result in eas.results_set.all():
                    result.reference = eas.reference
            
                eas.results_set.clear()
                eas.delete()                                                   
 
            ##print "*** after rollback PDD data model change for experiment=%s; oid=%s" %(exp.expName, str(exp.pk))      
