...
#20130218 change log:
# added unique constraint to sample
# add default value to plannedExperiment.runType
#
#20130222 change log:
# default plannedExperiment.runType to be GENS is currently null or missing


class Migration(SchemaMigration):

    def forwards(self, orm):        
        plans = orm.plannedexperiment.objects.filter(Q(runType__isnull=True) | Q(runType__exact=''))
        for plan in plans:
            plan.runType = 'GENS'
            plan.save()
            print "*** 0136_ AFTER updating plan with missing runType to default to GENS. plan.id=%s; name=%s; status=%s; isTemplate=%s; " %(str(plan.id), plan.planDisplayedName, str(plan.planStatus), str(plan.isReusable))            

        # Commit the above and restart new transaction
        db.commit_transaction()
        db.start_transaction()
        
        # Adding unique constraint on 'Sample', fields ['externalId', 'name']
        db.create_unique('rundb_sample', ['externalId', 'name'])
        
        # Changing field 'PlannedExperiment.runType'
        db.alter_column('rundb_plannedexperiment', 'runType', self.gf('django.db.models.fields.CharField')(max_length=512))

    def backwards(self, orm):
        # Removing unique constraint on 'Sample', fields ['externalId', 'name']
        db.delete_unique('rundb_sample', ['externalId', 'name'])


        # Changing field 'PlannedExperiment.runType'
        db.alter_column('rundb_plannedexperiment', 'runType', self.gf('django.db.models.fields.CharField')(max_length=512, null=True))
