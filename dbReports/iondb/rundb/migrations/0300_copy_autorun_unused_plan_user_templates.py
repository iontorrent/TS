...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        #Get the Un-used plans and user Templates
        plans = orm.plannedexperiment.objects.filter(isSystem = False, planExecuted= False)
        #Get the default Selected Plugins
        defaultSelectedPlugins = orm.plugin.objects.filter(active=True, selected=True, defaultSelected=True).order_by("name")
        defaultSelectedPluginsToMigrate = []

        for plan in plans:
            # Make sure the plan is not sequenced already
            if plan.planStatus.lower() != "run":
                latest_eas_set = orm.plannedexperiment.objects.get(pk=plan.id).latestEAS
                eas_set = orm.experimentanalysissettings.objects.get(pk=latest_eas_set.id)
                if eas_set and eas_set.isEditable:
                    selectedPlugins = {}
                    if eas_set.selectedPlugins:
                        selectedPlugins = simplejson.loads(str(eas_set.selectedPlugins))

                    #Igonore if the default Plugin already exists in the EAS selected Plugins blob
                    for default_plugin in defaultSelectedPlugins:
                        if default_plugin.name not in selectedPlugins:
                            defaultSelectedPluginsToMigrate.append(default_plugin)

                    #Create the SelectedPlugins json blob for defaultSelected
                    pluginUserInput = {}
                    for plugin in defaultSelectedPluginsToMigrate:
                        pluginDict = {
                            "id": plugin.id,
                            "name": plugin.name,
                        ...
                        selectedPlugins[plugin.name] = pluginDict

                    #Finally save SelectedPlugins blob to EAS
                    eas_set.selectedPlugins = simplejson.dumps(selectedPlugins)
                    eas_set.save()
