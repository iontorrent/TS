...
#20120913 change log:
#1) fixed the pre-selected library kit for RNA's  2 system pre-loaded templates 
#2) changed Proton chip's display name
#3) deactivated obsolete sample prep kit named "Ion TargetSeq custom enrichment kits"

class Migration(SchemaMigration):           

    def forwards(self, orm):
        #1) update pre-selected library kit for RNA's system templates
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = True, isReusable = True, isSystemDefault = False, runType = "RNA"):   
            plannedExp.librarykitname = "Ion Total RNA Seq Kit v2"
                
            print "*** going to update RNA system template plannedExperiment=%s " % (plannedExp.planName) 
            plannedExp.save() 
        
        #2) update chip description
        for chip in orm.chip.objects.filter(name="900"):
            chip.description = "PI"                 
                
            print "*** going to update chip.description for chip=%s " % (chip.name)            
            chip.save()                            

        #3) deactivate old sample prep kit
        for samplePrepKit in orm.kitinfo.objects.filter(kitType = "SamplePrepKit",  name="Ion TargetSeq custom enrichment kits"):
            samplePrepKit.isActive = False
            samplePrepKit.save();
            
            print "*** AFTER marking samplePrepKit %s as inactive " % (samplePrepKit.name)                                   

    
    
    def backwards(self, orm):
        #1) no-op for RNA system template library kit fix
        
        #2) revert chip description to original
        for chip in orm.chip.objects.filter(name="900"):
            chip.description = "Proton I"                 
                
            print "*** going to revert chip.description change for chip=%s " % (chip.name)            
            chip.save()                      

        #3) revert deactivation of old sample prep kit
        for samplePrepKit in orm.kitinfo.objects.filter(kitType = "SamplePrepKit",  name="Ion TargetSeq custom enrichment kits"):
            samplePrepKit.isActive = True
            
            print "*** going to revert samplePrepKit %s back to be active " % (samplePrepKit.name)                                               
            samplePrepKit.save();
