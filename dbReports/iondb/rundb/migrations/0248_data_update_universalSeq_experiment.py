...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        convert all existing experiment's with IonPGMUniversal as the sequencing kit to IonPGMHiQ
        """
        
        rec_count = 0
        for exp in orm.experiment.objects.filter(sequencekitname = "IonPGMUniversal"): 
            rec_count += 1
            
            exp.sequencekitname = "IonPGMHiQ"
            print ">> 0248 experiement.id=%d; name=%s updated with new sequencekitname value" %(exp.id, exp.expName)            
            exp.save()
            
        print ">> 0248 Total %d experiment records updated." %(rec_count)


    def backwards(self, orm):        
        rec_count = 0
        for exp in orm.experiment.objects.filter(sequencekitname = "IonPGMHiQ"): 
            rec_count += 1
            
            exp.sequencekitname = "IonPGMUniversal"
            print ">> 0248 experiement.id=%d; name=%s REVERTED with original sequencekitname value" %(exp.id, exp.expName)            
            exp.save()
            
        print ">> 0248 Total %d experiment records reverted." %(rec_count)
