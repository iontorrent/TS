...
class Migration(SchemaMigration):
    """
        We first have to copy all the existing barcode entries for all sample set items, which we will
        retrieve via sampleset so that they are grouped together.  Clone them, then delete the column,
        then create the FK column, and do a lookup on the barcode to find the barcoding kit, and save the PK,
        which will be found by matching the barcoding kit and barcode id_str together, since the combonation of 
        the two make a unique entry.

        When looking up a barcoding kit for a group of barcode id_str's we have a slight problem:

        IonXpress and IonXpress16 both have IonXpress_01 through IonXpress_016.  If we encounter an IonXpress_017
        or above, we will take IonXpress as the barcoding kit, if not, we will take IonXpress16, which is actually OK 
        since they both have the same DNA sequence...
    """
    def forwards(self, orm):

        #first we add the column
        # Adding field 'SampleSetItem.dnabarcode'
        db.start_transaction()
        db.add_column(u'rundb_samplesetitem', 'dnabarcode',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['rundb.dnaBarcode'], null=True, blank=True),
                      keep_default=False)        
        db.commit_transaction()


        db.start_transaction()
        samples = orm.Sample.objects.all()
        list_of_id_strings = dict()
        for sample in samples:
            sample_set_items = sample.sampleSets.all()
            
            processed_sample_ids = []
            #first we need to group the sample set items' id_str's
            for sample_set_item in sample_set_items:
                sample_set = sample_set_item.sampleSet
                if not sample.pk in processed_sample_ids:
                    query = 'select barcode from rundb_samplesetitem where "sample_id" = %d and "sampleSet_id" = %d' % (sample.pk, sample_set.pk,)
                    single_samples_set_item = list(sample_set.samples.filter(sample=sample).extra(select={'barcode' : query}))[0]
                    if not sample_set.pk in list_of_id_strings:
                        list_of_id_strings[sample_set.pk] = [single_samples_set_item.barcode]
                    else:
                        list_of_id_strings[sample_set.pk].append(single_samples_set_item.barcode)
                else:
                    processed_sample_ids.append(sample.pk)

        for sample in samples:
            sample_set_items = sample.sampleSets.all()
            
            processed_sample_ids = []
            #then we process each sample set item individually
            for sample_set_item in sample_set_items:
                sample_set = sample_set_item.sampleSet
                if not sample.pk in processed_sample_ids:
                    query = 'select barcode from rundb_samplesetitem where "sample_id" = %d and "sampleSet_id" = %d' % (sample.pk, sample_set.pk,)
                    single_samples_set_item = list(sample_set.samples.filter(sample=sample).extra(select={'barcode' : query}))[0]
                    __list_of_id_strings = list_of_id_strings[sample_set.pk]
                    if single_samples_set_item.barcode:
                        dnabarcodes = orm['rundb.dnaBarcode'].objects.filter(id_str__in=__list_of_id_strings)
                        if dnabarcodes.count() > 0:
                            single_samples_set_item.dnabarcode = orm['rundb.dnaBarcode'].objects.get(name__iexact=dnabarcodes[0].name, id_str__iexact=single_samples_set_item.barcode)
                            single_samples_set_item.save()
                else:
                    processed_sample_ids.append(sample.pk)
        ...
        db.start_transaction()
        # Deleting field 'SampleSetItem.barcode'
        db.delete_column(u'rundb_samplesetitem', 'barcode')        
        db.commit_transaction()

    def backwards(self, orm):
        # Adding field 'SampleSetItem.barcode'
        db.start_transaction()
        db.add_column(u'rundb_samplesetitem', 'barcode',
                      self.gf('django.db.models.fields.CharField')(max_length=128, null=True, blank=True),
                      keep_default=False)
        db.commit_transaction()

        # db.start_transaction()
        # #now put back the barcode entries
        # sample_set_items = orm.SampleSetItem.objects.all()
        # for sample_set_item in sample_set_items:
        #     _pk = sample_set_item.dnabarcode.pk
        #     dnabarcode = orm['rundb.dnaBarcode'].objects.get(pk=_pk)
        #     sample_set_item.barcode 

        # Deleting field 'SampleSetItem.dnabarcode'
        db.delete_column(u'rundb_samplesetitem', 'dnabarcode_id')
