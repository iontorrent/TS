...
#20120724
#1) added additional SAP part number for the 300 kit
#Sequencing Kit:         Ion PGM Sequencing 300 Kit     - 4480446, 4480447, 4480448
#
#2) added new sequencing kit
#Sequencing Kit:         Ion PGM Install Kit     - 4480217, 4480282, 4480284
#
#3) updated 300 Kit to have flow count 780
#4) added SAP part number for 2x200 sequencing kit
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        #this seqKit should exist already from 0034 migration
        ...
        #3) update 300 kit flow count to be 780
        seqKit.flowCount = 780
        seqKit.save();
        
        #this part number should exist already from 0034 migration
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480445")

        print "*** AFTER get_or_create seqKit Part 4480445 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
       
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480446")

        print "*** AFTER get_or_create seqKit Part 4480446 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
                   
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480447")

        print "*** AFTER get_or_create seqKit Part 4480447 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
                   
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480448")

        print "*** AFTER get_or_create seqKit Part 4480448 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
        ...
        #Sequencing Kit:         Ion PGM Install Kit     - 4480217
        #this seqKit should be new
        seqKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SequencingKit", name = "Ion PGM Install Kit", 
            description = "Ion PGM Install Kit", flowCount = 100, nucleotideType="")

        print "*** AFTER get_or_create Ion PGM Install Kit isCreated=%s " % (str(isCreated)) 
       
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480217")

        print "*** AFTER get_or_create seqKit Part 4480217 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
                   
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480282")

        print "*** AFTER get_or_create seqKit Part 4480282 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
                   
        #this part number should be new
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4480284")

        print "*** AFTER get_or_create seqKit Part 4480284 isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            seqKit.kitpart_set.add(seqKitPart);
                        
        seqKit.save()

        #4) Paired-end seq kit:     Ion PGM 2x200 paired end sequencing kit 
        ...
        peSeqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = peSeqKit, barcode = "4476341")

        print "*** AFTER get_or_create peSeqKit Part 4476341 - isCreated=%s " % (str(isCreated)) 
        if (isCreated):
            peSeqKit.kitpart_set.add(peSeqKitPart);                
            peSeqKit.save();
    ...
    def backwards(self, orm):      
        ...
        try:
            ...
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480446")
            seqKitPart.delete()
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480447")
            seqKitPart.delete()   
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480448")
            seqKitPart.delete()      
                     
            print "after removing component kit parts from Ion PGM Sequencing 300 Kit "        
        except:
            print "cannot rollback sequencing kit: Ion PGM Sequencing 300 Kit "  
                        
        #Sequencing Kit:         Ion PGM Install Kit     - 4480217
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "Ion PGM Install Kit")
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480217")
            seqKitPart.delete()
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480282")
            seqKitPart.delete()   
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4480284")
            seqKitPart.delete()      

            seqKit.delete()
            print "after removing sequencing kit Ion PGM Install Kit "        
        except:
            print "cannot rollback sequencing kit: Ion PGM Install Kit "  
                        
        
        #Sequencing Kit:         Ion PGM 2x200 paired end sequencing kit     - 4476341
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "Ion PGM 2x200 paired end sequencing kit")
            seqKitPart = orm.kitpart.objects.get(kit = seqKit, barcode = "4476341")
            seqKitPart.delete()        

            seqKit.delete()
            print "after removing sequencing kit Ion PGM 2x200 paired end sequencing kit"        
        except:
            print "cannot rollback sequencing kit: Ion PGM 2x200 paired end sequencing kit "  
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 19, 12, 35, 29, 178034)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 19, 12, 35, 29, 177903)'}),
        ...
        'rundb.plugin': {
            ...
            'date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 19, 12, 35, 29, 101165)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
