...
#2012006 change log:
# 1) changed seqKit name per David Kupec's requirement (which script to use is driven by the seqKit name and embedded spaces can cause problems)
# 2) marked 2 old seqKits to be inactive
# 3) changed experiment's seqKit names to new names if old names were used
# 4) changed plannedExperiment's seqKit names to new names if old names were used
# 5) nullified plan library default value in system default template

class Migration(SchemaMigration):

    def forwards(self, orm):               
        #1) mark old kits as inactive
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name__in=["IonSeq200Kit", "IonPGM200Kit-v2"]):
            seqKit.isActive = False
            seqKit.save();
            
            print "*** AFTER marking seqKit %s as inactive " % (seqKit.name)   
            
        #2.1) change seq kit names
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit",  description = "Ion PGM Sequencing 300 Kit")
            seqKit.name = "IonPGM300Kit"
            seqKit.save()
        
            print "Name for sequencing kit Ion PGM Sequencing 300 Kit is updated"            
        except:
            print "Error: Name for sequencing kit Ion PGM Sequencing 300 Kit is not updated" 
            
        #2.2)
        try:
            peSeqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", description = "Ion PGM 2x200 paired end sequencing kit")
            peSeqKit.name = "IonPGM2x200PEKit"
            peSeqKit.save()
            print "Name for sequencing kit Ion PGM 2x200 paired end sequencing kit is updated"  
        except:
            print "Error: Name for sequencing kit Ion PGM 2x200 paired end sequencing kit is not updated"  

        #2.3)
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", description = "Ion PGM Install Kit")
            seqKit.name = "IonPGMInstallKit"
            seqKit.save()
            print "Name for sequencing kit Ion PGM Install Kit is updated"  
        except:
            print "Error: Name for sequencing kit Ion PGM Install Kit is not updated"  

        
        #3) change experiment's seqKit names
        for exp in orm.experiment.objects.filter(sequencekitname__in = ["Ion PGM Sequencing 300 Kit", "Ion PGM 2x200 paired end sequencing kit", "Ion PGM Install Kit"]):
            ##print "*** going to change seqKitName in experiment=%s " % (exp.expName) 
            
            if (exp.sequencekitname == "Ion PGM Sequencing 300 Kit"):
                exp.sequencekitname = "IonPGM300Kit"
                exp.save()
            if (exp.sequencekitname == "Ion PGM 2x200 paired end sequencing kit"):
                exp.sequencekitname = "IonPGM2x200PEKit"
                exp.save()
            if (exp.sequencekitname == "Ion PGM Install Kit"):
                exp.sequencekitname = "IonPGMInstallKit"
                exp.save()                

        
        #4) change plan's seqKit names
        for plannedExp in orm.plannedexperiment.objects.filter(sequencekitname__in = ["Ion PGM Sequencing 300 Kit", "Ion PGM 2x200 paired end sequencing kit", "Ion PGM Install Kit"]):
            ##print "*** going to change seqKitName in plannedExperiment=%s " % (plannedExp.planName) 
            
            if (plannedExp.sequencekitname == "Ion PGM Sequencing 300 Kit"):
                plannedExp.sequencekitname = "IonPGM300Kit"
                plannedExp.save()
            if (plannedExp.sequencekitname == "Ion PGM 2x200 paired end sequencing kit"):
                plannedExp.sequencekitname = "IonPGM2x200PEKit"
                plannedExp.save()
            if (plannedExp.sequencekitname == "Ion PGM Install Kit"):
                plannedExp.sequencekitname = "IonPGMInstallKit"
                plannedExp.save()                            

        # 5) changed system default                    
        for sysTemplate in orm.plannedexperiment.objects.filter(isReusable=True, isSystem=True, isSystemDefault=True):
            sysTemplate.library = ""
            print "*** reduced system default values in plannedExperiment system template %s " % (sysTemplate.planDisplayedName)
    ...
    def backwards(self, orm):      
        #revert changes
        #1) revert old kits as active
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name__in=["IonSeq200Kit", "IonPGM200Kit-v2"]):
            seqKit.isActive = True
            seqKit.save();
            
            print "*** AFTER marking seqKit %s as inactive " % (seqKit.name)   
            
        #2.1) revert seq kit name changes
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit",  description = "Ion PGM Sequencing 300 Kit")
            seqKit.name = "Ion PGM Sequencing 300 Kit"
            seqKit.save()
        
            print "Name for sequencing kit Ion PGM Sequencing 300 Kit is reverted"            
        except:
            print "Error: Name for sequencing kit Ion PGM Sequencing 300 Kit is not reverted" 
            
        #2.2)
        try:
            peSeqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", description = "Ion PGM 2x200 paired end sequencing kit")
            peSeqKit.name = "Ion PGM 2x200 paired end sequencing kit"
            peSeqKit.save()
            print "Name for sequencing kit Ion PGM 2x200 paired end sequencing kit is reverted"  
        except:
            print "Error: Name for sequencing kit Ion PGM 2x200 paired end sequencing kit is not reverted"  

        #2.3)
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", description = "Ion PGM Install Kit")
            seqKit.name = "Ion PGM Install Kit"
            seqKit.save()
            print "Name for sequencing kit Ion PGM Install Kit is reverted"  
        except:
            print "Error: Name for sequencing kit Ion PGM Install Kit is not reverted"  

        
        #3) change experiment's seqKit names
        for exp in orm.experiment.objects.filter(sequencekitname__in = ["IonPGM300Kit", "IonPGM2x200PEKit", "IonPGMInstallKit"]):
            ##print "*** going to revert seqKitName in experiment=%s " % (exp.expName) 
            
            if (exp.sequencekitname == "IonPGM300Kit"):
                exp.sequencekitname = "Ion PGM Sequencing 300 Kit"
                exp.save()
            if (exp.sequencekitname == "IonPGM2x200PEKit"):
                exp.sequencekitname = "Ion PGM 2x200 paired end sequencing kit"
                exp.save()
            if (exp.sequencekitname == "IonPGMInstallKit"):
                exp.sequencekitname = "Ion PGM Install Kit"
                exp.save()                

        
        #4) change plan's seqKit names
        for plannedExp in orm.plannedexperiment.objects.filter(sequencekitname__in = ["IonPGM300Kit", "IonPGM2x200PEKit", "IonPGMInstallKit"]):
            ##print "*** going to revert seqKitName in plannedExperiment=%s " % (plannedExp.planName) 
            
            if (plannedExp.sequencekitname == "IonPGM300Kit"):
                plannedExp.sequencekitname = "Ion PGM Sequencing 300 Kit"
                plannedExp.save()
            if (plannedExp.sequencekitname == "IonPGM2x200PEKit"):
                plannedExp.sequencekitname = "Ion PGM 2x200 paired end sequencing kit"
                plannedExp.save()
            if (plannedExp.sequencekitname == "IonPGMInstallKit"):
                plannedExp.sequencekitname = "Ion PGM Install Kit"
                plannedExp.save()                            

        #5) changed system default                    
        for sysTemplate in orm.plannedexperiment.objects.filter(isReusable=True, isSystem=True, isSystemDefault=True):
            sysTemplate.library = "hg19"
            print "*** system default values in plannedExperiment is reverted system template %s " % (sysTemplate.planDisplayedName)
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 6, 8, 49, 30, 443583)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 6, 8, 49, 30, 443477)'}),
        ...
        'rundb.plugin': {
            ...
            'date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 6, 8, 49, 29, 995633)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
