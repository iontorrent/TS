...
#20120831 change log:
# 1) added sample prep kits
# 2) added ERCC control seq
# 3) remove templateKit Ion PGM Template OT 300 Kit


class Migration(SchemaMigration):

    def forwards(self, orm):                    
        # 1.1) add sample prep kit
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Exome Kit (4 rxn)", 
            description = "Ion TargetSeq(tm) Exome Kit (4 rxn)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Exome Kit (4 rxn) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "4477742")

        print "*** AFTER get_or_create samplePrepKit Part 4477742 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()

        # 1.2)
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Exome Kit (12 rxn)", 
            description = "Ion TargetSeq(tm) Exome Kit (12 rxn)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Exome Kit (12 rxn) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "4477743")

        print "*** AFTER get_or_create samplePrepKit Part 4477743 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()

        # 1.3)
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Exome Kit (48 rxn)", 
            description = "Ion TargetSeq(tm) Exome Kit (48 rxn)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Exome Kit (48 rxn) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "4477744")

        print "*** AFTER get_or_create samplePrepKit Part 4477744 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()

        # 1.4)
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Custom Enrichment Kit (100kb-500kb)", 
            description = "Ion TargetSeq(tm) Custom Enrichment Kit (100kb-500kb)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Custom Enrichment Kit (100kb-500kb) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "A14230")

        print "*** AFTER get_or_create samplePrepKit Part A14230 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()

        # 1.5)
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Custom Enrichment Kit (500kb-2Mb)", 
            description = "Ion TargetSeq(tm) Custom Enrichment Kit (500kb-2Mb)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Custom Enrichment Kit (500kb-2Mb) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "A14229")

        print "*** AFTER get_or_create samplePrepKit Part A14229 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()

        # 1.6)
        samplePrepKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SamplePrepKit", name = "Ion TargetSeq(tm) Custom Enrichment Kit (2Mb-10Mb)", 
            description = "Ion TargetSeq(tm) Custom Enrichment Kit (2Mb-10Mb)", flowCount = 0)

        print "*** AFTER get_or_create Ion TargetSeq(tm) Custom Enrichment Kit (2Mb-10Mb) isCreated=%s " % (str(isCreated)) 

        samplePrepKit.save()
        
        samplePrepKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "A14228")

        print "*** AFTER get_or_create samplePrepKit Part A14228 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            samplePrepKit.kitpart_set.add(samplePrepKitPart);
            samplePrepKit.save()
                   
        # 2) add ERCC control seq kit
        controlSeqKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "ControlSequenceKit", name = "ERCC RNA Spike-In Mix", 
            description = "ERCC RNA Spike-In Mix", flowCount = 0)

        print "*** AFTER get_or_create ERCC RNA Spike-In Mix isCreated=%s " % (str(isCreated)) 

        controlSeqKit.save()
        
        controlSeqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = samplePrepKit, barcode = "4456740")

        print "*** AFTER get_or_create controlSeqKit Part 4456740 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            controlSeqKit.kitpart_set.add(controlSeqKitPart);
            controlSeqKit.save()  
                     
        # 3) remove seqKitIonPGM300Kit
        templateKits = orm.kitinfo.objects.filter(kitType = "TemplatingKit", name="Ion PGM Template OT 300 Kit")
        for templateKit in templateKits:
            templateKit.delete();

            print "*** AFTER deleting templatingKit Ion PGM Template OT 300 Kit "



    def backwards(self, orm):       
        # 1) revert addition of samplePrep kit
        try:
            samplePrepKits = orm.kitinfo.objects.filter(kitType = "SamplePrepKit", name__in=["Ion TargetSeq(tm) Exome Kit (4 rxn)", "Ion TargetSeq(tm) Exome Kit (12 rxn)", "Ion TargetSeq(tm) Exome Kit (48 rxn)"])
            for samplePrepKit in samplePrepKits:
                samplePrepKit.delete()
        except:            
            print "*** Cannot delete TargetSeq Exome samplePrep kit " 

        try:
            samplePrepKits = orm.kitinfo.objects.filter(kitType = "SamplePrepKit", name__in=["Ion TargetSeq(tm) Custom Enrichment Kit (100kb-500kb)", "Ion TargetSeq(tm) Custom Enrichment Kit (500kb-2Mb)", "Ion TargetSeq(tm) Custom Enrichment Kit (2Mb-10Mb)"])
            for samplePrepKit in samplePrepKits:
                samplePrepKit.delete()
        except:            
            print "*** Cannot delete TargetSeq custom enrichment samplePrep kit " 
              
        
        # 2) revert addition of controleSeq kit
        try:
            controlSeqKits = orm.kitinfo.objects.filter(kitType = "ControlSequenceKit", name="ERCC RNA Spike-In Mix")
            for controlSeqKit in controlSeqKits:
                controlSeqKit.delete()
        except:            
            print "*** Cannot delete ERCC RNA Spike-In Mix controlSeq kit " 
                     
        # 3) revert templating kit deletion
        templateKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "TemplatingKit", name = "Ion PGM Template OT 300 Kit", 
            description = "Ion PGM Template OT 300 Kit", flowCount = 0)

        print "*** AFTER get_or_create Ion PGM Template OT 300 Kit isCreated=%s " % (str(isCreated)) 

        templateKit.instrumentType = 'pgm'
        templateKit.save()
        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4480974")

        print "*** AFTER get_or_create templateKit Part 4480974 isCreated=%s " % (str(isCreated)) 
        if isCreated:
