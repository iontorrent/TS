...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # this is to address issue regarding 3' adapters entered by users.
        
        # Adding field 'ThreePrimeadapter.uid'
        db.add_column(u'rundb_threeprimeadapter', 'uid',
                      self.gf('django.db.models.fields.CharField')(default= "DFLT_0001", max_length=32),
                      keep_default=False)

        # Commit above column addition and restart transaction
        db.commit_transaction()
        db.start_transaction()
        
        # update field 'ThreePrimeadapter.uid' to make value unique
        for adapter in orm.threeprimeadapter.objects.all():
            random_uid = "".join([random.choice(string.letters) for i in range(8)])
            adapter.uid = random_uid
                            
            print "*** 0180 >>> update threePrimeAdapter name=%s; uid=%s" %(adapter.name, random_uid)
            adapter.save()

        # Commit above updates and restart transaction
        db.commit_transaction()
        db.start_transaction()
        
        db.alter_column('rundb_threeprimeadapter', 'uid', self.gf('django.db.models.fields.CharField')(unique=True, max_length=32))
        

    def backwards(self, orm):
        # Deleting field 'ThreePrimeadapter.uid'
        db.delete_column(u'rundb_threeprimeadapter', 'uid')
