...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Update latestEAS to point to the latest ExperimentAnalysisSettings.
        # if more than one experiment EAS objects - use editable EAS, if exists, otherwise reusable EAS
        for plan in orm.plannedexperiment.objects.all():
            try:
                exp = plan.experiment
                eas_set = exp.eas_set.all().order_by("-date", "-id")
                if eas_set.count() == 1:
                    plan.latestEAS = eas_set[0]
                    plan.save()
                else:
                    ...
                    if queryset.exists():
                        plan.latestEAS = queryset[0]
                        plan.save()
            except:
                print 'Warning: Unable to update latestEAS for PlannedExperiment %s(%s)' % (plan.planName, plan.pk)
