...
#20120731 change log:
# 1) changed 300 seqKit flow count value in kitInfo per product management
# 2) changed application product values in applProduct per product management
# 3) changed system template values in plannedExperiment per product management
# 4) changed template names from xxx Plan to xxx Template as part of terminology change
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        #1) Sequencing Kit:         Ion PGM Sequencing 300 Kit     - 4480445
        seqKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SequencingKit", name = "Ion PGM Sequencing 300 Kit", 
            description = "Ion PGM Sequencing 300 Kit", nucleotideType="")
        
        print "*** AFTER get_or_create Ion PGM Sequencing 300 Kit isCreated=%s " % (str(isCreated)) 
        
        #update 300 kit flow count is 640 instead of 780
        seqKit.flowCount = 640
        seqKit.save();
                
        #2) applProduct updates
        defaultPELibAdapterKit = orm.kitinfo.objects.get(kitType = "AdapterKit" , name = "Paired-end library adapter kit")
        defaultSeqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "Ion PGM Sequencing 300 Kit")
        defaultAmpsLibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name = "Ion AmpliSeq 2.0 Library Kit")
                
        defaultAmpsBetaLibKit = orm.kitinfo.objects.get(kitType = "LibraryKit", name = "Ion AmpliSeq 2.0 Beta Kit")
        
        # update applProduct data
        for appl in orm.runtype.objects.all():               
            for applProduct in orm.applproduct.objects.filter(applType = appl):
                if (appl.runType == 'AMPS'):
                    applProduct.defaultLibraryKit = defaultAmpsLibKit
                    applProduct.defaultPairedEndLibraryKit = defaultAmpsLibKit
                    applProduct.defaultFlowCount = 520
                                                            
                    print "*** updating applProduct name=%s " % (applProduct.productName)
                    
                    applProduct.save()

                if (appl.runType == 'GENS'):
                    applProduct.defaultFlowCount = 640
                ...
                if (appl.runType == 'RNA'):
                    applProduct.defaultPairedEndSequencingKit = defaultSeqKit
                    applProduct.defaultPairedEndAdapterKit = defaultPELibAdapterKit
                    applProduct.defaultFlowCount = 520
        ...
        # 3) changed system template values in plannedExperiment per product management
        # 4) changed template names from xxx Plan to xxx Template as part of terminology change                    
        for sysTemplate in orm.plannedexperiment.objects.filter(isReusable=True, isSystem=True):
            if (sysTemplate.isSystemDefault):
                sysTemplate.flows = 640
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                
                sysTemplate.save()
                
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Paired End Cancel Panel Plan"):
                sysTemplate.planDisplayedName = "Ion AmpliSeq Paired End Cancer Panel Template"
                sysTemplate.planName = "Ion_AmpliSeq_Paired_End Cancer_Panel_Template"
                
                sysTemplate.librarykitname = defaultAmpsLibKit.name
                
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                                            
                sysTemplate.save()
                
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Cancel Panel Plan"):
                sysTemplate.planDisplayedName = "Ion AmpliSeq Cancer Panel Template"
                sysTemplate.planName = "Ion_AmpliSeq_Cancer_Panel_Template"
            ...
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Cancel Panel Plan Beta Lib Chem"):                                
                sysTemplate.planDisplayedName = "Ion AmpliSeq Cancer Panel Template Beta Lib Chem"
                sysTemplate.planName = "Ion_AmpliSeq_Cancer_Panel_Template_Beta_Lib_Chem"
                                
                sysTemplate.librarykitname = defaultAmpsBetaLibKit.name
                
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                                            
                sysTemplate.save()                
            
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Cancel Panel Plan 1_0 Lib Chem"):
                sysTemplate.planDisplayedName = "Ion AmpliSeq Cancer Panel Template 1_0 Lib Chem"
                sysTemplate.planName = "Ion_AmpliSeq_Cancer_Panel_Template_1_0_Lib_Chem"
                
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                                            
                sysTemplate.save()                
            
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Paired End Inherited Disease Panel"):                                
                sysTemplate.librarykitname = defaultAmpsLibKit.name
            ...
            if (sysTemplate.planDisplayedName == "System Generic Seq Plan"):       
                sysTemplate.planDisplayedName = "System Generic Seq Template"
                sysTemplate.planName = "System_Generic_Seq_Template"
                                
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                                            
                sysTemplate.save()                                
            
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Inherited Disease Panel"):                                
                sysTemplate.librarykitname = defaultAmpsLibKit.name
            ...
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Paired End Comprehensive Cancer Panel"): 
                sysTemplate.planName = "Ion_AmpliSeq_Paired_End_Comprehensive_Cancer_Panel"                               
                sysTemplate.librarykitname = defaultAmpsLibKit.name
            ...
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Comprehensive Cancer Panel"):                                
                sysTemplate.librarykitname = defaultAmpsLibKit.name
            ...
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Paired End Cancer Hotspot Panel 2_0 Plan"):
                sysTemplate.planDisplayedName = "Ion AmpliSeq Paired End Cancer Hotspot Panel 2_0 Template"
                sysTemplate.planName = "Ion_AmpliSeq_Paired_End_Cancer_Hotspot_Panel_2_0_Template"
            ...
            if (sysTemplate.planDisplayedName == "Ion AmpliSeq Cancer Hotspot Panel 2_0 Plan"):     
                sysTemplate.planDisplayedName = "Ion AmpliSeq Cancer Hotspot Panel 2_0 Template"
                sysTemplate.planName = "Ion_AmpliSeq_Cancer_Hotspot_Panel_2_0_Template"
            ...
            if (sysTemplate.planDisplayedName == "System Generic Seq PairedEnd Plan"):     
                sysTemplate.planDisplayedName = "System Generic Seq PairedEnd Template"
                sysTemplate.planName = "System_Generic_Seq_PairedEnd_Template"
                
                print "*** updating plannedExperiment system template name=%s " % (sysTemplate.planDisplayedName)
                                            
                sysTemplate.save()                                
                
            if (sysTemplate.planDisplayedName == "System Default Plan"):     
                sysTemplate.planDisplayedName = "System Default Template"
                sysTemplate.planName = "System_Default_Template"
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 26, 12, 54, 9, 223775)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 26, 12, 54, 9, 223626)'}),
        ...
        'rundb.plugin': {
            ...
            'date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 26, 12, 54, 8, 795267)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
