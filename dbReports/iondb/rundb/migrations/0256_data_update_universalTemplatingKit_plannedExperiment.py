...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        convert all existing plans with Ion PGM IC Universal as the templating kit to Ion PGM IC Hi-Q Kit
        """
        
        rec_count = 0
        for plan in orm.plannedExperiment.objects.filter(templatingKitName = "Ion PGM IC Universal"): 
            rec_count += 1
            
            plan.templatingKitName = "Ion PGM IC Hi-Q Kit"
            print ">> 0256 plan.id=%d; name=%s updated with new templatingKitName value" %(plan.id, plan.planDisplayedName)            
            plan.save()
            
        print ">> 0256 Total %d plannedExperiment records updated." %(rec_count)


    def backwards(self, orm):
        
        rec_count = 0
        for plan in orm.plannedExperiment.objects.filter(templatingKitName = "Ion PGM IC Hi-Q Kit"): 
            rec_count += 1
            
            plan.templatingKitName = "Ion PGM IC Universal"
            print ">> 0256 plan.id=%d; name=%s REVERTED to original templatingKitName value" %(plan.id, plan.planDisplayedName)            
            plan.save()
            
        print ">> 0256 Total %d plannedExperiment records reverted." %(rec_count)
