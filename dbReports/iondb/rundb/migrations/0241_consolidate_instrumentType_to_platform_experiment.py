...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Intent usage of Experiment's platform attribute is same as PlannedExperiment's instrumentType. 
        # Update instrumentType with platform before we remove the platform attribute
        
        
        hasErrors = False

        plans = orm.plannedexperiment.objects.filter(experiment__id__isnull=True)
        if plans:
            hasErrors = True
            print ">> 0247 *** WARNING: ", plans.count(), " plan(s) have no experiment associated with them and need to be manually cleaned up upon review: "
            for plan in plans:
                print ">> 0247 WARNING: Plan name=%s: database_primaryKey=%d; planStatus=%s; planExecuted=%s; isTemplate=%s" %(plan.planName, plan.id, plan.planStatus, str(plan.planExecuted), str(plan.isReusable))
        
        
        rec_count = 0
        update_count = 0
        
        for plan in orm.plannedexperiment.objects.exclude(experiment__id__isnull = True).exclude(instrumentType__isnull = True):     
            if plan.experiment:
                rec_count += 1
                
                chipType = plan.experiment.chipType
                platform = plan.experiment.platform
                
                if chipType and plan.instrumentType.lower() != platform.lower():
                    if platform.lower() == "pgm":
                        if not chipType.startswith("3"):
                            print ">> 0241 CHIP-PLATFORM DISCREPANCY #1 PROTON chip - experiment.id=%d; plan.id=%d; chipType=%s; instrumentType=%s; platform=%s" %(plan.experiment.id, plan.id, chipType, plan.instrumentType, plan.experiment.platform)
                            update_count += 1             
                            plan.experiment.platform = plan.instrumentType.upper() if plan.instrumentType else "PROTON"
                            plan.experiment.save()
                    else:
                        if chipType.startswith("3"):
                            print ">> 0241 CHIP-PLATFORM DISCREPANCY #2 PGM chip - experiment.id=%d; plan.id=%d; chipType=%s; instrumentType=%s; platform=%s" %(plan.experiment.id, plan.id, chipType, plan.instrumentType, plan.experiment.platform)
                            update_count += 1             
                            plan.experiment.platform = plan.instrumentType.upper() if plan.instrumentType else "PGM"
                            plan.experiment.save()

        print ">> 0241 Total %d experiment records processed and %d plan records updated." %(rec_count, update_count)
