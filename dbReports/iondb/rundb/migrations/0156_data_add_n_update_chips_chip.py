...
#20130402 change log:
# 1) updated existing chip entries for their instrument types
# 2) added new PGM chips
# 3) added new Proton chips
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        #1) set instrument type of existing chips to be "pgm" or "proton"
        for chip in orm.chip.objects.filter(name__in=["314", "316", "318"]):
            chip.instrumentType = 'pgm'
            chip.save()
            
            print "*** AFTER updating chip %s as for pgm " % (chip.name) 
              
        for chip in orm.chip.objects.filter(name ="900"):
            chip.instrumentType = 'proton'
            chip.save()
            
            print "*** AFTER updating chip %s as for proton " % (chip.name)   
 
                
        #2) added PGM chips      
        chip, isCreated = orm.chip.objects.get_or_create(name = "316v2", slots = "1",
            defaults={"description" : "316v2", "instrumentType" : "pgm"})

        print "*** AFTER get_or_create chip 316v2 isCreated=%s " % (str(isCreated)) 
        
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "318v2", slots = "1",
            defaults={"description" : "318", "instrumentType" : "pgm"})

        print "*** AFTER get_or_create chip 318v2 isCreated=%s " % (str(isCreated)) 
 
                        
        #3a) added proton P0 chips          
        chip, isCreated = orm.chip.objects.get_or_create(name = "P1.0.19", slots = "1",
            defaults={"description" : "P0", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P1.0.19 isCreated=%s " % (str(isCreated))

                       
        #3b) added proton P1 chips                   
        chip, isCreated = orm.chip.objects.get_or_create(name = "P1.1.16", slots = "1",
            defaults={"description" : "P1", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P1.1.16 isCreated=%s " % (str(isCreated)) 
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "P1.1.17", slots = "1",
            defaults={"description" : "P1", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P1.1.17 isCreated=%s " % (str(isCreated)) 
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "P1.2.18", slots = "1",
            defaults={"description" : "P1", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P1.2.18 isCreated=%s " % (str(isCreated)) 
                        
        #3c) added proton P2 chips          
        chip, isCreated = orm.chip.objects.get_or_create(name = "P2.0.16", slots = "1",
            defaults={"description" : "P2", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P2.0.16 isCreated=%s " % (str(isCreated)) 
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "P2.1.16", slots = "1",
            defaults={"description" : "P2", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P2.1.16 isCreated=%s " % (str(isCreated)) 
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "P2.2.16", slots = "1",
            defaults={"description" : "P2", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip P2.2.16 isCreated=%s " % (str(isCreated)) 


    def backwards(self, orm):
        #1) revert instrument type
        for chip in orm.chip.objects.all():
            chip.instrumentType = ''
            chip.save()
            
            print "*** revert instrument type update for chip %s " % (chip.name) 

        #2)revert PGM chips added
        try:
            for chip in orm.chip.objects.filter(name__in=["316v2", "318v2"]):
                chip.delete()
                
            print "*** Successfully deleting the new PGM chips"                 
        except:            
            print "*** Cannot delete PGM chip" 
            
            
        #3a) revert Proton P0 chips added
        try:
            for chip in orm.chip.objects.filter(name__in=["P1.0.19"]):
                chip.delete()
                
            print "*** Successfully deleting the new Proton P0 chips" 
        except:            
            print "*** Cannot delete Proton P0 chip" 

            
        #3b) revert Proton P1 chips added
        try:
            for chip in orm.chip.objects.filter(name__in=["P1.1.16", "P1.1.17", "P1.2.18"]):
                chip.delete()
                
            print "*** Successfully deleting the new Proton P1 chips"                 
        except:            
            print "*** Cannot delete Proton P1 chip" 
           
        #3c) revert Proton P2 chips added
        try:
            for chip in orm.chip.objects.filter(name__in=["P2.0.16", "P2.1.16", "P2.2.16"]):
                chip.delete()
                
            print "*** Successfully deleting the new Proton P2 chips"                 
        except:            
            print "*** Cannot delete Proton P2 chip" 
