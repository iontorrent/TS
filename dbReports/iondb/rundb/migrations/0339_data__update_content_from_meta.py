...
def get_type(meta, publisher, isUpload):
    upload_type = publisher
    if meta:
        if isUpload and meta.get('is_ampliseq', False):
            upload_type = AMPLISEQ
        elif 'sse' in meta and meta['sse']:
            upload_type = SSE
        elif 'hotspot' in meta:
            upload_type = HOTSPOT if meta['hotspot'] else TARGET
        elif 'upload_type' in meta:
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        For all Content and ContentUpload objects, update fields created in previous migration from meta JSON blob
        '''
        for upload in orm.contentupload.objects.all():
            if not upload.meta:
                continue

            meta = json.loads(upload.meta)
            upload.source = meta.get('source', '')
            upload.upload_type = get_type(meta, upload.publisher.name, isUpload=True)

            if meta.get('upload_date'):
                upload.upload_date = meta['upload_date']
            elif os.path.exists(upload.file_path):
                ctime = time.ctime(os.stat(upload.file_path).st_mtime)
                upload.upload_date = datetime.datetime.strptime(ctime, "%a %b %d %H:%M:%S %Y")
        ...
        for content in orm.content.objects.all():
            ...
            content.type = get_type(meta, content.publisher.name, isUpload=False)
            content.enabled = meta.get('enabled', True)
            content.description = meta.get('description', '')
            content.notes = meta.get('notes', '')
            if 'reference' in meta:
                content.extra = meta['reference']
