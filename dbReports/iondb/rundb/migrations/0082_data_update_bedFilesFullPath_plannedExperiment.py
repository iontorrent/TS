...
#20120930 change log:
#1) fixed bed files and hotSpot bed file to use full path for all plans and all non-system templates

class Migration(SchemaMigration):           

    def forwards(self, orm):
        #1.a) update bed file to use full path
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = False).exclude(bedfile__exact="none").exclude(bedfile__isnull=True).exclude(bedfile__exact=''):
            bedFile = plannedExp.bedfile

            #if found, bedFile is an incomplete bed file path
            try:
                #use the latest bedFile path
                bedFileInContent = orm.content.objects.filter(path = bedFile).order_by('-id')[0]
                plannedExp.bedfile = bedFileInContent.file
                
                print "*** going to update plan=%s from bedFile=%s to %s" % (plannedExp.planName, bedFile, bedFileInContent.file) 
                plannedExp.save() 
                
            except IndexError:                
                print "*** No bedFile content found. Skipped updating plan=%s for bedFile=%s" % (plannedExp.planName, bedFile) 

        
        #1.b) update hotSpot bed file to use full path
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = False).exclude(regionfile__exact="none").exclude(regionfile__isnull=True).exclude(regionfile__exact=''):
            bedFile = plannedExp.regionfile
            
            #if found, bedFile is an incomplete bed file path
            try:
                bedFileInContent = orm.content.objects.filter(path = bedFile).order_by('-id')[0]
                plannedExp.regionfile = bedFileInContent.file
                
                print "*** going to update plan=%s from HOTSPOT regionFile=%s to %s" % (plannedExp.planName, bedFile, bedFileInContent.file) 
                plannedExp.save() 
                
            except IndexError:                
                print "*** No bedFile content found. Skipped updating plan=%s for HOTSPOT bedFile=%s" % (plannedExp.planName, bedFile) 

    
    
    def backwards(self, orm):
        #1.a) revert bed file from using full path
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = False).exclude(bedfile__exact="none").exclude(bedfile__isnull=True).exclude(bedfile__exact=''):
            bedFile = plannedExp.bedfile
            
            #if found, bedFile is complete bed file path
            try:
                bedFileInContent = orm.content.objects.filter(file = bedFile).order_by('-id')[0]
                plannedExp.bedfile = bedFileInContent.path
                
                print "*** going to revert plan=%s from bedFile=%s to %s" % (plannedExp.planName, bedFile, bedFileInContent.path) 
                plannedExp.save() 
                
            except IndexError:
                print "*** No bedFile content found. Skipped reverting plan=%s for bedFile=%s" % (plannedExp.planName, bedFile) 

        
        #1.b) revert hotSpot bed file from using full path
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = False).exclude(regionfile__exact="none").exclude(regionfile__isnull=True).exclude(regionfile__exact=''):
            bedFile = plannedExp.regionfile
            ...
            try:
                bedFileInContent = orm.content.objects.filter(file = bedFile).order_by('-id')[0]
                plannedExp.regionfile = bedFileInContent.path
                
                print "*** going to revert plan=%s from HOTSPOT regionFile=%s to %s" % (plannedExp.planName, bedFile, bedFileInContent.file) 
                plannedExp.save() 
                
            except IndexError:
                print "*** Skipped reverting plan=%s for HOTSPOT regionFile=%s" % (plannedExp.planName, bedFile) 
