...
class Migration(DataMigration):
    ...
    # in this method we will be asserting that the home and disabled locations exist and make sure that there is a default
    def forwards(self, orm):
        # here we are assert that there is a home director and not doing anything if there are multiple due to it being an rare-edge case
        HOME = 'Home'
        home, created = orm.location.objects.get_or_create(name = HOME)
           
        # we are going to assert the existance of the "disabled" location
        DISABLED = 'Disabled'
        DISABLED_COMMENT = 'A location which will not be assigned to any File Servers so that Rigs assigned to this location will not be treated as valid Rigs when ionCrawler attempts to find new raw data directories. This is so that we do not have to delete a Rig from the Rigs table but still want to prevent new Experiments from appearing associated with the Rig.'
        if not orm.location.objects.filter(name = DISABLED):
           orm.location.objects.get_or_create(name = DISABLED, comments = DISABLED_COMMENT, defaultlocation = False)
        
        # in this section we are going to assert for there to be one and only one default location
        defaults = list(orm.location.objects.filter(defaultlocation = True))
        defaultsCount = len(defaults)
        if defaultsCount == 0:
            home.defaultlocation = True
            home.save()
        elif defaultsCount > 1:
            # the override in the save method will assert that all other records are not-default
            defaults[0].save()

    # nothing to do for rolling backwards
