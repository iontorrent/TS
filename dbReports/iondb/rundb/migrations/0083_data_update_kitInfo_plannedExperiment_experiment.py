...
#20121005 change log:
#1) changed unique kit name to prepare for seq kit name reuse
#2) updated plannedExperiment seq kit name from IonPGM200Kit-v2 to IonPGM200Kit-v2-obsolete
#3) updated experiment seq kit name from IonPGM200Kit-v2 to IonPGM200Kit-v2-obsolete


class Migration(SchemaMigration):

    def forwards(self, orm):
        #1) change kitInfo's seqKit name. If we have the old seq kit with the SAP part numbers of interest in db, rename its name
        count = 0
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name="IonPGM200Kit-v2"):
            kitParts = orm.kitpart.objects.filter(kit = seqKit, barcode__in =["4478321","4478322","4478323","4478324"])
            
            count =  kitParts.count()
            if (count > 0):
                print "*** going to change kitInfo old seqKit from IonPGM200Kit-v2 to IonPGM200Kit-v2-obsolete"
                                
                seqKit.name = "IonPGM200Kit-v2-obsolete"
                seqKit.save()                
            else:
                print "*** No old seqKit IonPGM200Kit-v2 found. No need to update kitInfo. "
        
        if (count > 0):
            #2) change plan's seqKit names
            for plannedExp in orm.plannedexperiment.objects.filter(sequencekitname = "IonPGM200Kit-v2"):
                print "*** going to change seqKitName in plannedExperiment=%s " % (plannedExp.planName) 

                plannedExp.sequencekitname = "IonPGM200Kit-v2-obsolete"
                plannedExp.save()
        
            #3) change experiment's seqKit names
            for exp in orm.experiment.objects.filter(sequencekitname = "IonPGM200Kit-v2"):
                print "*** going to change seqKitName in experiment=%s " % (exp.expName) 

                exp.sequencekitname = "IonPGM200Kit-v2-obsolete"
                exp.save()              
        else:
            print "*** No old seqKit IonPGM200Kit-v2 found. Nothing to update plannedExperiment or experiment. "
