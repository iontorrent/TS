...
#20120816 change log:
# 1) for historical reverse plans, set the runMode to pe
# 2) for historical reverse runs, set the runMode to pe

class Migration(SchemaMigration):

    def forwards(self, orm):               
        #1) mark runMode of reverse plan as pe     
        for plan in orm.plannedexperiment.objects.filter(isReverseRun = True):
            if (plan.runMode != "pe"):
                ##print "*** going to change runMode in reverse plannedExperiment=%s " % (plan.planName) 
                plan.runMode = "pe"
                plan.save()

        #2) mark runMode of reverse run as pe 
        for exp in orm.experiment.objects.filter(isReverseRun = True):
            if (exp.runMode != "pe"):
                ##print "*** going to change runMode in reverse experiment=%s " % (exp.expName) 
                exp.runMode = "pe"
                exp.save()
