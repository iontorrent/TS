...
#20120831 change log:
# 1) changed application product defaults from using 300bp seq kit to 200bp seq kit with flow count 500
# 2) changed system templates from using 300bp seq kit to 200bp seq kit with flow count 500


class Migration(SchemaMigration):

    def forwards(self, orm):
        defaultSeqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "IonPGM200Kit")                  

        # update applProduct data
        for applProduct in orm.applproduct.objects.all():
            applProduct.defaultSequencingKit = defaultSeqKit
            applProduct.defaultFlowCount = 500
            
            print "*** updating applProduct name=%s " % (applProduct.productName)
            applProduct.save()

        # update system templates data
        for plannedExp in orm.plannedexperiment.objects.filter(isSystem = True, isReusable = True, sequencekitname = "IonPGM300Kit"):

            if plannedExp.planDisplayedName.find("Ion PGM E_coli DH10B Control 300") == -1:
                plannedExp.sequencekitname = "IonPGM200Kit"
                plannedExp.flows = 500
