...
#20130910 change log:
# 1) changed status from loaded to created for sample and sampleSet


class Migration(DataMigration):

    def forwards(self, orm):
        loadedSamples = orm.Sample.objects.filter(status = "loaded")
        ...
        if loadedSamples:
            for sample in loadedSamples:
                ...
                sample.save()
                print "*** 0195.. after updating status of sample.id=%d; name=%s " % (sample.id, sample.displayedName)  
        else:
                print "*** 0195.. NO sample status update needed."
                
        loadedSampleSets = orm.SampleSet.objects.filter(status = "loaded")        
        if loadedSampleSets:
            for sampleSet in loadedSampleSets:
                sampleSet.status = "created"
                            
                sampleSet.save()
                print "*** 0195.. after updating status of sampleSet.id=%d; name=%s " % (sampleSet.id, sampleSet.displayedName)  
        else:
                print "*** 0195.. NO sample set status update needed."



    def backwards(self, orm):
        loadedSamples = orm.Sample.objects.filter(status = "created")
        ...
        if loadedSamples:
            for sample in loadedSamples:
                ...
                sample.save()
                print "*** 0195.. after reverting the status of sample.id=%d; name=%s " % (sample.id, sample.displayedName)  
        else:
                print "*** 0195.. NO sample status reversion needed."


        loadedSampleSets = orm.SampleSet.objects.filter(status = "created")       
        if loadedSampleSets:
            for sampleSet in loadedSampleSets:
                sampleSet.status = "loaded"
                            
                sampleSet.save()
                print "*** 0195.. after reverting status of sampleSet.id=%d; name=%s " % (sampleSet.id, sampleSet.displayedName)  
        else:
                print "*** 0195.. NO sample set status reversion needed."
