...
class Migration(DataMigration):
    def forwards(self, orm):
        """
        set base_recalibration_mode value according the existing base_recalbrate value
        """
        
        standard_recal_update_count = 0        
        globalConfigs = orm.globalconfig.objects.filter(base_recalibrate = True).exclude(base_recalibration_mode = "standard_recal")        

        for globalConfig in globalConfigs: 
            globalConfig.base_recalibration_mode = "standard_recal"
            standard_recal_update_count += 1
            
            globalConfig.save()
          
        no_recal_update_count = 0  
        globalConfigs = orm.globalconfig.objects.filter(base_recalibrate = False).exclude(base_recalibration_mode = "no_recal")        

        for globalConfig in globalConfigs: 
            globalConfig.base_recalibration_mode = "no_recal"
            no_recal_update_count += 1
                
            globalConfig.save()
        
        print ">> 0252 updated recal mode in %d globalConfig records to use Standard Recalibration" %(standard_recal_update_count)        
        print ">> 0252 updated recal mode in %d globalConfig records to use No Recalibration" %(no_recal_update_count)

            
        standard_recal_update_count = 0        
        eases = orm.experimentanalysissettings.objects.filter(base_recalibrate = True).exclude(base_recalibration_mode = "standard_recal")        
        
        for eas in eases:
            eas.base_recalibration_mode = "standard_recal"
            ##print "... 0252 going to update experimentAnalysisSettings pk=%d to standard_recal" %(eas.pk)
            standard_recal_update_count += 1
            eas.save()                       
        
        no_recal_update_count = 0       
        eases = orm.experimentanalysissettings.objects.filter(base_recalibrate = False).exclude(base_recalibration_mode = "no_recal")        
        
        for eas in eases:
            eas.base_recalibration_mode = "no_recal"
            ##print "... 0252 going to update experimentAnalysisSettings pk=%d to no_recal" %(eas.pk)            
            no_recal_update_count += 1
                
            eas.save()
        
        print ">> 0252 updated recal mode in %d experimentAnalysisSettings records to use Standard Recalibration" %(standard_recal_update_count)        
        print ">> 0252 updated recal mode in %d experimentAnalysisSettings records to use No Recalibration" %(no_recal_update_count)


    def backwards(self, orm):   
        """
        revert base_recalibration_mode value to db default value
        """  
              
        revert_update_count = 0        
        globalConfigs = orm.globalconfig.objects.exclude(base_recalibration_mode = "standard_recal")        

        for globalConfig in globalConfigs: 
            globalConfig.base_recalibration_mode = "standard_recal"
            revert_update_count += 1
            
            globalConfig.save()
        
        print ">> 0252 reverted recal mode in %d globalConfig records to use Standard Recalibration" %(revert_update_count)        
            
        
        revert_update_count = 0        
        eases = orm.experimentanalysissettings.objects.exclude(base_recalibration_mode = "standard_recal")        
        ...
        print ">> 0252 reverted recal mode in %d experimentAnalysisSettings records to use Standard Recalibration" %(revert_update_count)        
