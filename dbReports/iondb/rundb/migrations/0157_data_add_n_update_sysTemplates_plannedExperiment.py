...
#20130419 change log:
# 1) updated system plan templates for their pre-selected sequencing and templating kits 
# 2) added system plan template "Ion PGM E_coli DH10B Control 400"
# 3) removed system plan template "Ion AmpliSeq Cancer Panel Beta Lib Chem"
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        for sysTemplate in orm.PlannedExperiment.objects.filter(isSystem = True):

            if sysTemplate.planDisplayedName == "Ion AmpliSeq Cancer Panel Beta Lib Chem":
                id = sysTemplate.id
                name = sysTemplate.planDisplayedName
                
                sysTemplate.delete()
                        
                print "*** 0157.. after deleting sysTemplate.id=%d; name=%s " % (id, name)  
                
            elif sysTemplate.planDisplayedName.find("Proton") == -1:
                sysTemplate.templatingKitName = "Ion PGM Template OT2 200 Kit"
                
                if sysTemplate.planDisplayedName.find("DH10B Control 300") == -1:                    
                    sysTemplate.experiment.sequencekitname = "IonPGM200Kit-v2"
                
                    sysTemplate.save()
                    sysTemplate.experiment.save()
                    
                    print "*** 0157.. after updating sysTemplate.id=%d; name=%s; experiement.id=%d " % (sysTemplate.id, sysTemplate.planDisplayedName, sysTemplate.experiment.id)  
                    
                else:
                    #if it is control 300, no change to the sequence kit
                    sysTemplate.save()
                    print "*** 0157.. after updating sysTemplate.id=%d; name=%s " % (sysTemplate.id, sysTemplate.planDisplayedName)  
                    ...
                    newSysTemplate.templatingKitName = "Ion PGM Template OT2 400 Kit"
                    newSysTemplate.chipType = "314"
                    ...
                    print "*** 0157.. after saving new sysTemplate.id=%d; name=%s " % (newSysTemplate.id, newSysTemplate.planDisplayedName)  
                    ...
                    print "*** 0157.. after saving new sysTemplate.experiment.id=%d; name=%s " % (expObj.id, expObj.expName)  

                    # copy EAS
                    for easObj in sysTemplate.experiment.eas_set.all():
                        easObj.pk = None
                    ...
                    for qcObj in sysTemplate.plannedexperimentqc_set.all():
                        ...
                        print "*** 0157.. after saving new sysTemplate.qc.id=%d " % (qcObj.id)  


            else:
                sysTemplate.templatingKitName = "Ion PI Template OT2 200 Kit"
                sysTemplate.experiment.sequencekitname = "ProtonI200Kit"
                
                sysTemplate.save()
                sysTemplate.experiment.save()
                
                print "*** 0157.. after updating Proton sysTemplate.id=%d; name=%s; experiment.id=%d " % (sysTemplate.id, sysTemplate.planDisplayedName, sysTemplate.experiment.id)  
    ...
    def backwards(self, orm):
        for sysTemplate in orm.PlannedExperiment.objects.filter(isSystem = True):
            ...
            if sysTemplate.planDisplayedName == "Ion AmpliSeq Cancer Panel 1_0 Lib Chem":
                sysTemplate.templatingKitName = "Ion OneTouch 200 Template Kit v2 DL"
                sysTemplate.experiment.sequencekitname = "IonPGM200Kit"

                sysTemplate.save()
                sysTemplate.experiment.save()                

                print "*** 0157.. after reverting the updates in sysTemplate.id=%d; name=%s; experiment.id=%d " % (sysTemplate.id, sysTemplate.planDisplayedName, sysTemplate.experiment.id)  
                ...
                newSysTemplate.planName = "Ion_AmpliSeq_Cancer_Panel_Beta_Lib_Chem"
                newSysTemplate.templatingKitName = "Ion OneTouch 200 Template Kit v2 DL"
                ...
                expObj.expName = newSysTemplate.planGUID
                expObj.unique = newSysTemplate.planGUID
                expObj.sequencekitname = "IonPGM200Kit"
                expObj.flows = 500
                expObj.plan = newSysTemplate
                expObj.date = currentTime                  
                expObj.save()
                    
                print "*** 0157.. after reverting the deletion of sysTemplate.experiment.id=%d; name=%s " % (expObj.id, expObj.expName)  

                # copy EAS
                for easObj in sysTemplate.experiment.eas_set.all():
                    easObj.pk = None
                    easObj.experiment = expObj
                    easObj.libraryKitName = "Ion AmpliSeq 2.0 Beta Kit"
                    easObj.date = currentTime      
                    easObj.save()
                ...
                for qcObj in sysTemplate.plannedexperimentqc_set.all():
                    ...
                    print "*** 0157.. after reverting the deletion of sysTemplate.qc.id=%d " % (qcObj.id)  

                
            elif sysTemplate.planDisplayedName.find("Proton") == -1:
                sysTemplate.templatingKitName = "Ion OneTouch 200 Template Kit v2 DL"
                
                if sysTemplate.planDisplayedName.find("DH10B Control 400") == -1:
                    if sysTemplate.planDisplayedName.find("DH10B Control 300") == -1: 
                        sysTemplate.experiment.sequencekitname = "IonPGM200Kit"
                    else:                    
                        sysTemplate.experiment.sequencekitname = "IonPGM300Kit"
                    ...
                    print "*** 0157.. after reverting the creation of sysTemplate.id=%d; name=%s " % (id, name)  

            else:
                sysTemplate.templatingKitName = "Ion Proton I Template OT2 Kit"
                sysTemplate.experiment.sequencekitname = "ProtonIKit"
                
                sysTemplate.save()
                sysTemplate.experiment.save()
                print "*** 0157.. after reverting the updates in Proton sysTemplate.id=%d; name=%s experiment.id=%d" % (sysTemplate.id, sysTemplate.planDisplayedName, sysTemplate.experiment.id)  
    ...
    models = {
        ...
        'rundb.libmetrics': {
            ...
            'duplicate_read_percentage': ('django.db.models.fields.FloatField', [], {'null': 'True', 'blank': 'True'}),
