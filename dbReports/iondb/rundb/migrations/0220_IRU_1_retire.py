...
"""Find the old IRU 1.X plans and mark them as retired"""
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            iru_plugin = orm.Plugin.objects.get(name="IonReporterUploader", active=True)
            iru_config = json.loads(iru_plugin.config)
            iru_configs = iru_config["userconfigs"]

            #make a flat dict of all the IRU config info where the id of the account is the dict key
            flat = {}
            for username, per_user_configs_dict in iru_configs.iteritems():
                for config in per_user_configs_dict:
                    flat[config["id"]] = config
        except:
            print "Unabe to find IRU config, can't update the IRU messages"
            return

        changed_eas_list = []
        eas_set = orm.ExperimentAnalysisSettings.objects.all()
        for eas in eas_set:
            selected_plugins = eas.selectedPlugins
            if selected_plugins:
                try:
                    selected_plugins = json.loads(str(selected_plugins))
                except:
                    print "Unable to parse JSON blob of selected plugins"
                    return
            else:
                continue
            if 'IonReporterUploader' in selected_plugins:
                ...
                if 'userInput' in iru:
                    if 'accountId' in iru['userInput']:
                        try:
                            id = iru['userInput']['accountId']

                            #try to find the version
                            try:
                                version = flat[id]["version"][-2]
                            except:
                                version = False
                            ...
                            if version == "1":
                                try:
                                    eas_meta = json.loads(eas.experiment.plan.metaData)
                                except:
                                    print "\t Failed to get a dict, making a new one"
                                    eas_meta = {}

                                if "legacyIR" not in eas_meta:
                                    eas_meta["legacyIR"] = {str(eas.pk): "" }

                                #stash the old selected IR account
                                eas_meta["legacyIR"][str(eas.pk)] = selected_plugins["IonReporterUploader"]
                                eas.experiment.plan.metaData = json.dumps(eas_meta)
                                eas.save()
                                eas.experiment.plan.save()

                                #Remove the old one
                                del selected_plugins["IonReporterUploader"]
                                _selected_plugins = json.dumps(selected_plugins)
                                eas.selectedPlugins = _selected_plugins

                                eas.save()
                                changed_eas_list.append(eas)

                        except:
                            traceback.print_exc()
                            print "failed to get the config info needed to change the name"

        #if any old IR 1 accounts got unselected, add a warning message
        if changed_eas_list:
            message = orm.Message(body="Removed IR 1 account from " + str(len(changed_eas_list))  + " plans", level=30)
            message.save()
