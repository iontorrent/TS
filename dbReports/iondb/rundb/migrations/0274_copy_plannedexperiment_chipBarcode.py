...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for plan in orm.plannedExperiment.objects.filter(planExecuted=False, chipBarcode__isnull=False).exclude(chipBarcode=''):
            if plan.experiment:
                plan.experiment.chipBarcode = plan.chipBarcode
                plan.experiment.save()

    def backwards(self, orm):
        for exp in orm.experiment.objects.filter(plan__planExecuted=False, chipBarcode__isnull=False).exclude(chipBarcode=''):
            if exp.plan:
                exp.plan.chipBarcode = exp.chipBarcode
                exp.plan.save()
