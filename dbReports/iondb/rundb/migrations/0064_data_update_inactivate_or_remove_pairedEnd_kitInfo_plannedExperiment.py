...
#20120815 change log:
# 1) inactived all paired-end related kits from kitInfo
# 2) removed all paired-end system templates from plannedExperiment

class Migration(SchemaMigration):

    def forwards(self, orm):               
        #1) mark pe kits as inactive
        for kit in orm.kitinfo.objects.filter(runMode = "pe"):
            kit.isActive = False
            kit.save();
            
            print "*** AFTER marking kit %s as inactive " % (kit.name)   
            
        #2) remove pe system templates     
        for sysTemplate in orm.plannedexperiment.objects.filter(runMode = "pe", isReusable=True, isSystem=True):
            print "*** removing pairedEnd system template %s in plannedExperiment " % (sysTemplate.planDisplayedName)
    ...
    def backwards(self, orm):      
        #revert changes
        #1) revert old kits to being active
        for kit in orm.kitinfo.objects.filter(runMode = "pe"):
            kit.isActive = True
            kit.save();
            
            print "*** AFTER marking kit %s as active " % (kit.name)   
  
        #2) no-op for pairedEnd system template removal   
