...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        sql = "SELECT column_name FROM information_schema.columns " + \
              "WHERE table_name = 'rundb_plugin'"
        gc_names = db.execute(sql)

        if oldField in sum(gc_names, ()):
            df_selected = {}
            query = "SELECT name, autorun FROM rundb_plugin " + \
                    "WHERE selected=True " + \
                    "AND   active=True " + \
                    "AND   autorun=True"
            autorunInfo = db.execute(query)

            if sum(autorunInfo, ()):
                for name, autorun in autorunInfo:
                    df_selected[name] = autorun
                with open('/tmp/autorun_data.json', 'w') as autorun:
                    json.dump(df_selected, autorun)
        else:
            print ("0291.. migration script already applied or autorun column does not exists")

        # Deleting field 'Plugin.autorunMutable'
        db.delete_column(u'rundb_plugin', 'autorunMutable')

        # Deleting field 'Plugin.autorun'
        db.delete_column(u'rundb_plugin', 'autorun')


    def backwards(self, orm):
        # Adding field 'Plugin.autorunMutable'
        db.add_column(u'rundb_plugin', 'autorunMutable',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)

        # Adding field 'Plugin.autorun'
        db.add_column(u'rundb_plugin', 'autorun',
