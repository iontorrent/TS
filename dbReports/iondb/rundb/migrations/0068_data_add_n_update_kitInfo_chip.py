...
#20120821 change log:
# 1) set instrument type of all existing sequencing kits to be "pgm"
# 2) added proton sequencing kit
# 3) changed flowCount for sequencing kit IonPGM200Kit
# 4) added proton templating kit
# 5) changed library kit name for Ion AmpliSeq Kit
# 6) changed experiment library kit name for Ion AmpliSeq Kit
# 7) changed plannedExperiment library kit name for Ion AmpliSeq Kit
# 8) added proton chip
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        #1) set instrument type of existing sequencing kits to be "pgm"
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name__in=["IonSeqKit", "IonSeq200Kit"]):
            seqKit.instrumentType= 'pgm'
            seqKit.save()
            
            print "*** AFTER updating seqKit %s as pgm " % (seqKit.name)  
             
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name__icontains="PGM"):
            seqKit.instrumentType= 'pgm'
            seqKit.save()
        ...
        # 2) add proton sequencing kit
        seqKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "SequencingKit", name = "ProtonIKit", 
            description = "Ion Proton I Sequencing Kit", flowCount = 260, nucleotideType="")
        seqKit.instrumentType = 'proton'
        seqKit.save()
        
        print "*** AFTER get_or_create Ion Proton I Sequencing Kit isCreated=%s " % (str(isCreated)) 

        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4478850")
        print "*** AFTER get_or_create seqKit Part 4478850 isCreated=%s " % (str(isCreated)) 

        if isCreated:
            seqKit.kitpart_set.add(seqKitPart);
            seqKit.save();
            
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4478307")
        print "*** AFTER get_or_create seqKit Part 4478307 isCreated=%s " % (str(isCreated)) 

        seqKit.kitpart_set.add(seqKitPart);
        
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4478308")
        print "*** AFTER get_or_create seqKit Part 4478308 isCreated=%s " % (str(isCreated)) 
        ...
        seqKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = seqKit, barcode = "4478309")
        print "*** AFTER get_or_create seqKit Part 4478309 isCreated=%s " % (str(isCreated)) 
        ...
        # 3) change flowCount for sequencing kit IonPGM200Kit
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "IonPGM200Kit")
            seqKit.flowCount = 500
            seqKit.save() 

            print "*** AFTER updating flow count for SequencingKit IonPGM200Kit to %s " % (seqKit.flowCount)
        except:
            print "Error: Sequencing kit IonPGM200Kit not found. Flow count not updated"         
 
        
        # 4) add proton templating kit
        templateKit, isCreated = orm.kitinfo.objects.get_or_create(kitType = "TemplatingKit", name = "Ion Proton I Template OT2 Kit", 
            description = "Ion Proton I Template OT2 Kit", flowCount = 0)

        print "*** AFTER get_or_create Ion Proton I Template OT2 Kit isCreated=%s " % (str(isCreated)) 

        templateKit.instrumentType = 'proton'
        templateKit.save()
        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4478851")

        print "*** AFTER get_or_create templateKit Part 4478851 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            templateKit.kitpart_set.add(templateKitPart);
            templateKit.save()
        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4478864")

        print "*** AFTER get_or_create templateKit Part 4478864 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            templateKit.kitpart_set.add(templateKitPart);
            templateKit.save()
        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4478857")

        print "*** AFTER get_or_create templateKit Part 4478857 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            templateKit.kitpart_set.add(templateKitPart);
            templateKit.save()

        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4478853")

        print "*** AFTER get_or_create templateKit Part 4478853 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            templateKit.kitpart_set.add(templateKitPart);
            templateKit.save()
            
        
        templateKitPart, isCreated = orm.kitpart.objects.get_or_create(kit = templateKit, barcode = "4478856")

        print "*** AFTER get_or_create templateKit Part 4478856 isCreated=%s " % (str(isCreated)) 
        if isCreated:
            templateKit.kitpart_set.add(templateKitPart);
            templateKit.save()
        ...
        try:
            ...
            print "*** Ion AmpliSeq Kit is not found. Going to create libKit with name Ion AmpliSeq Cancer Primer Pool. "
            ...
            print "*** AFTER get_or_create seqKit Part 4471262 isCreated=%s " % (str(isCreated)) 
        ...
        #6) change experiment's library kit names
        ...
        #7) change plan's library kit names
        ...
        # 8) added proton chip          
        chip, isCreated = orm.chip.objects.get_or_create(name = "900", slots = "1") 
        print "*** AFTER get_or_create chip 900 isCreated=%s " % (str(isCreated)) 
    ...
    def backwards(self, orm):
        ...
        #1) revert instrument type of existing sequencing kits to ""
        for seqKit in orm.kitinfo.objects.filter(kitType = "SequencingKit",  name__in=["IonSeqKit", "IonSeq200Kit"]):
            seqKit.instrumentType= ''
            seqKit.save()
            
            print "*** AFTER updating seqKit %s as blank " % (seqKit.name)  
        ...
        # 2) revert addition of proton sequencing kit
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "ProtonIKit")
            seqKit.delete()
        except:            
            print "*** Cannot delete sequencing kit ProtonIKit " 


        # 3) revert flowCount for sequencing kit IonPGM200Kit
        try:
            seqKit = orm.kitinfo.objects.get(kitType = "SequencingKit", name = "IonPGM200Kit")
            seqKit.flowCount = 520
            seqKit.save() 

            print "*** AFTER reverting flow count for SequencingKit IonPGM200Kit "
        except:
            print "Error: Sequencing kit IonPGM200Kit not found. Flow count not reverted"         
 
        
        # 4) revert addition of proton templating kit
        try:
            templateKit = orm.kitinfo.objects.get(kitType = "TemplatingKit", name = "Ion Proton I Template OT2 Kit") 
            templateKit.delete()
        except:            
            print "*** Cannot delete templating kit Ion Proton I Template OT2 Kit " 
         
            
        # 5) revert library kit name from Ion AmpliSeq Cancer Primer Pool back to Ion AmpliSeq Kit                        
        try:
        ...
        #6) revert experiment's library kit names
        ...
        #7) revert plan's library kit names
        ...
        # 8) revert addition of proton chip
        try:
            chip = orm.chip.objects.get(name = "900")
            chip.delete()
        except:            
            print "*** Cannot delete chip 900 " 
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 21, 14, 46, 33, 823216)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 21, 14, 46, 33, 822751)'}),
        ...
        'rundb.plugin': {
            ...
            'date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 8, 21, 14, 46, 33, 286046)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
