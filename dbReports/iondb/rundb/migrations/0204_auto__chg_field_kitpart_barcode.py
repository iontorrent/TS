...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        ionChefKits = orm.kitinfo.objects.filter(uid = "ICPREP0001")
        if ionChefKits:
            kitParts = orm.kitpart.objects.filter(kit = ionChefKits[0])
            
            for kitPart in kitParts:
                print ">>> 0204 >>> going to delete kitPart barcode=%s for kit=%s" %(kitPart.barcode, ionChefKits[0].name)
                kitPart.delete()
        else:
            print ">>> 0204 >>> NO kits found for ICPREP0001"              
        
        # Changing field 'KitPart.barcode'
        db.alter_column(u'rundb_kitpart', 'barcode', self.gf('django.db.models.fields.CharField')(unique=True, max_length=64))



    def backwards(self, orm):
        #initial_data.json will re-insert the kitParts. This is just to satisfy the field length restriction
        kitParts = orm.kitpart.objects.all()
        for kitPart in kitParts:
            length = len(kitPart.barcode)
            if length > 7:
                truncatedBarcode = kitPart.barcode[length -7:]
                
                anyKitParts = orm.kitpart.objects.filter(barcode = truncatedBarcode)
                if (anyKitParts):
                    print ">>> 0204 >>> ROLLBACK WARNING - cannot truncate barcode due to duplicity. going to delete kitPart.id=%s; barcode=%s " %(str(kitPart.id), kitPart.barcode)
                    kitPart.delete()
                else:
                    print ">>> 0204 >>> ROLLBACK WARNING - truncate barcode by using the last 7 chars for kitPart.id=%s; barcode=%s to 7 chars " %(str(kitPart.id), kitPart.barcode)
                    
                    kitPart.barcode = truncatedBarcode
                    kitPart.save()


                
        # Changing field 'KitPart.barcode'
        db.alter_column(u'rundb_kitpart', 'barcode', self.gf('django.db.models.fields.CharField')(max_length=7, unique=True))
