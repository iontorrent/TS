...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Experiment.sample'
        db.delete_column('rundb_experiment', 'sample')


    def backwards(self, orm):
        # Adding field 'Experiment.sample'
        db.add_column('rundb_experiment', 'sample',
        ...
        for exp in exps:                                        
            ...
            if exp.eas_set:
                barcodeKitNames = exp.eas_set.values_list('barcodeKitName', flat=True).order_by('-date')
                barcodeKitName = str(barcodeKitNames[0])
                isBarcoded = True if barcodeKitName and barcodeKitName.lower() != "none" else False
            else:
                print "!!! Warning: NO EAS found to restore data for the resurrected fields for exp.id=%s; name=%s " %(str(exp.pk), exp.expName) 

            if not isBarcoded:                
                samples = exp.samples.values_list('name', flat=True)

                if samples:
                    sampleName = str(samples[0])
                    
                    ##print "*** going to restore sample info sampleName=%s for exp.id=%s; name=%s" %(sampleName, str(exp.pk), exp.expName)                   
                    exp.sample = sampleName                    
                    exp.save()
            ##else:
              ##  print "*** No sample info restoration needed for barcoded experiment - barcodeKit=%s; exp.id=%s; name=%s" %(barcodeKitName, str(exp.pk), exp.expName)                   
