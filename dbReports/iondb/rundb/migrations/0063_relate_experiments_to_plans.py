...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Search for the plan"
        print("   Normalizing Plan and Experiment relationships.  This may take a few minutes.")
        count = 0
        Experiment = orm['rundb.Experiment']
        PlannedExperiment = orm['rundb.PlannedExperiment']
        experiments = Experiment.objects.all().order_by('date')
        plans = list(PlannedExperiment.objects.all().order_by('date'))
        # Iterate over experiments from oldest to newest
        for exp in experiments:
            plan = None
            filtered_plans = []
            # We're not using the Django ORM, so no auto deserialization
            try:
                log = json.loads(exp.log)
            except Exception as err:
                log = {}
            guid = exp.selectedPlanGUID or log.get("planned_run_guid", None)
            if guid:
                filtered_plans = filter(lambda p: p.planGUID == guid, plans)

            if not filtered_plans:
                shortid = exp.selectedPlanShortID or \
                log.get("planned_run_short_id", None) or \
                log.get("pending_run_short_id", None)
                if shortid:
                    filtered_plans = filter(lambda p: p.planShortID == shortid and p.planExecuted == True, plans)

            if filtered_plans:
                # Select the oldest plan
                plan = filtered_plans[0]
                # Strictly do not re-use plans
                plans.remove(plan)
                exp.plan = plan
                exp.save()
                count += 1
        print("   Found Plans for %d Experiments" % count)



    def backwards(self, orm):
        "Be kind, please rewind."
        orm['rundb.Experiment'].objects.all().update(plan=None)
