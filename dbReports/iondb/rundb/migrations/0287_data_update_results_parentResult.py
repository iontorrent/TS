...
def get_locations_dict(orm):
    locations_dict = dict(orm.Rig.objects.all().values_list('name', 'location__name'))
    # default location
    try:
        locations_dict['_defaultlocation'] = orm.Location.objects.get(defaultlocation=True).name
    except:
        location = orm.Location.objects.all().order_by('pk')
        locations_dict['_defaultlocation'] = location[0].name if location else 'Home'
    
    return locations_dict


def get_report_dir(result, locations_dict):
    # duplicates Result.get_report_dir function
    if result.reportLink.endswith('.php') or result.reportLink.endswith('.html'):
        report_directory = os.path.basename(os.path.dirname(result.reportLink))
    else:
        report_directory = os.path.basename(os.path.abspath(result.reportLink))
    
    location_name = locations_dict.get(result.experiment.pgmName) or locations_dict['_defaultlocation']
    return os.path.join(result.reportstorage.dirPath, location_name, report_directory)
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        '''
        Find any reports that were reAnalyzed from-Basecalling by looking at linked sigproc_results folder
        '''
        
        locations_dict = get_locations_dict(orm)
        
        experiments = orm.Experiment.objects.exclude(expDir__in=['',' ']).prefetch_related('results_set')
        print 'Processing %s runs ...' % experiments.count()
        
        stats = {'Total':0, 'Found parent Result':0, 'Found parent Result for OIA':0, 'Original Result':0,
                    'Original OIA Result':0, 'Cannot parse link for pk':0, 'Parent Result Not found':0}
    
        for exp in experiments.iterator():
            
            onboard_results_parent = False
            results = exp.results_set.order_by('pk')
            
            for result in results:
                stats['Total'] += 1
                report_dir = get_report_dir(result, locations_dict)
                sigproc_path = os.path.join(report_dir, 'sigproc_results')
                parentResult = None

                if os.path.islink(sigproc_path):
                    link_path = os.readlink(sigproc_path)
    
                    if 'onboard_results' in link_path:
                        # all on-instrument reanalysis use the same data, designate oldest report as "parent"
                        if onboard_results_parent:
                            parentResult = onboard_results_parent
                            stats['Found parent Result for OIA'] +=1
                        else:
                            onboard_results_parent = result
                            stats['Original OIA Result'] += 1
                    else:
                        try:
                            parent_pk = int(link_path.split('/')[-2].split('_')[-1])
                        except:
                            stats['Cannot parse link for pk'] += 1
                            #print 'Result %s cannot parse parent pk from %s' % (result.resultsName, link_path)
                        else:
                            try:
                                parentResult = results.get(pk=parent_pk)
                                stats['Found parent Result'] +=1
                            except:
                                stats['Parent Result Not found'] +=1
                                #print 'Result %s no parent result in database for %s' % (result.resultsName, link_path)

                    if parentResult:
                        result.parentResult = parentResult
                        result.save()
                else:
                    stats['Original Result'] += 1
                
        #for k,v in stats.items():
        #    if v > 0: print '%s:%s' % (k,v)
