...
#20130215 change log:
# updated RNA templates and not-yet-executed plans to be barcoded with barcode kit "RNA_Barcode_None" so the IA  can be trimmed downstream

class Migration(SchemaMigration):

    def forwards(self, orm):
        default_rna_barcodeKit = "RNA_Barcode_None"
        
        rnaPlans = orm.plannedexperiment.objects.filter(planExecuted = False, runType__in = ["RNA", "AMPS_RNA"])
        
        for rnaPlan in rnaPlans:
            exp = rnaPlan.experiment
            if exp:
                eas_set = exp.eas_set.all()
                if eas_set:
                    for eas in eas_set:
                        if eas.isEditable and not eas.barcodeKitName:                        
                            eas.barcodeKitName = default_rna_barcodeKit
                            eas.save()

                            print "*** 0135_ AFTER updating barcodeKitName for plan.id=%s; name=%s; isSystem=%s; isTemplate=%s; eas.id=%s; " %(str(rnaPlan.id), rnaPlan.planDisplayedName, str(rnaPlan.isSystem), str(rnaPlan.isReusable), str(eas.id))
                        else:
                            print "*** 0135_ SKIP!! bypass updating plan.id=%s; name=%s; isSystem=%s; isTemplate=%s; eas.id=%s; with barcodeKit=%s" %(str(rnaPlan.id), rnaPlan.planDisplayedName, str(rnaPlan.isSystem), str(rnaPlan.isReusable), str(eas.id), eas.barcodeKitName)
    ...
    def backwards(self, orm):
        ...
        for rnaPlan in rnaPlans:
            ...
            if exp:
               ...
               if eas_set:
                    for eas in eas_set:
                        if eas.isEditable and eas.barcodeKitName == default_rna_barcodeKit:
                            eas.barcodeKitName = ""
                            eas.save()

                            print "*** 0135_ AFTER reverting barcodeKitName for template.id=%s; name=%s; eas.id=%s; " %(str(rnaPlan.id), rnaPlan.planDisplayedName, str(eas.id))
                        else:
                            print "*** 0135_ SKIP!! bypass reverting plan.id=%s; name=%s; isSystem=%s; isTemplate=%s; eas.id=%s; with barcodeKit=%s" %(str(rnaPlan.id), rnaPlan.planDisplayedName, str(rnaPlan.isSystem), str(rnaPlan.isReusable), str(eas.id), eas.barcodeKitName)
