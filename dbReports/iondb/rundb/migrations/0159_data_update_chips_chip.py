...
#20130503 change log:
# 1) deactivated P0, P1 and P2 chips
# 2) added 314v2 and PIv2 chips
# 3) renamed 318v2's customer facing chip name as 318v2


class Migration(SchemaMigration):

    def forwards(self, orm):
        returnCode =  orm.chip.objects.filter(description__in=["P0", "P1", "P2"]).update(isActive =  False)
        print "*** 0159_ %s chips updated. " % (str(returnCode))         

        chip, isCreated = orm.chip.objects.get_or_create(name = "314v2", slots = "1",
            defaults={"description" : "314v2", "instrumentType" : "pgm"})

        print "*** 0159_ AFTER get_or_create chip 314v2 isCreated=%s " % (str(isCreated)) 
        
        chip, isCreated = orm.chip.objects.get_or_create(name = "900v2", slots = "1",
            defaults={"description" : "PIv2", "instrumentType" : "proton"})

        print "*** AFTER get_or_create chip PIv2 isCreated=%s " % (str(isCreated))

        
        returnCode =  orm.chip.objects.filter(name = "318v2").update(description = "318v2")
        print "*** 0159_ %s 318v2 chips updated. " % (str(returnCode))         


    def backwards(self, orm):
        returnCode =  orm.chip.objects.filter(description__in=["P0", "P1", "P2"]).update(isActive =  True)
        print "*** 0159_ %s chip updates reverted. " % (str(returnCode))   
              
        #2)revert PGM chips added
        try:
            for chip in orm.chip.objects.filter(name__in=["314v2", "900v2"]):
                chip.delete()
                
            print "*** 0159_ Successfully deleting the new PGM chips"                 
        except:            
            print "*** Cannot delete PGM chip" 
        
        returnCode =  orm.chip.objects.filter(name = "318v2").update(description = "318")
        print "*** 0159_ reverted %s 318v2 chip updates. " % (str(returnCode))         
