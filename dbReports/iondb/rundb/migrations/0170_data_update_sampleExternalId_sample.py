...
#20130718 change log:
# 1) converted sample's externalId from null to blank if one doesn't already exist
    ...
    def forwards(self, orm):
        ...
        for sample in samples:
            ...
            if (dupSamples.count() > 0):
                skippedCount +=1
                print "*** 0170 SKIP converting externalId from null to blank for sample.id=%d, displayedName=%s" %(sample.id, sample.displayedName)
            else:
                sample.externalId = ""
                sample.save()
                count += 1

        print "*** 0170 finished converting externalId from null to blank for %d sample(s). %d sample(s) skipped" %(count, skippedCount)
