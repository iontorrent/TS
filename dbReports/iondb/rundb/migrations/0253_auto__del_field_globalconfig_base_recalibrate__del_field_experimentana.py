...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'GlobalConfig.base_recalibrate'
        db.delete_column(u'rundb_globalconfig', 'base_recalibrate')

        # Deleting field 'ExperimentAnalysisSettings.base_recalibrate'
        db.delete_column(u'rundb_experimentanalysissettings', 'base_recalibrate')


    def backwards(self, orm):
        # Adding field 'GlobalConfig.base_recalibrate'
        db.add_column(u'rundb_globalconfig', 'base_recalibrate',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)

        # Adding field 'ExperimentAnalysisSettings.base_recalibrate'
        db.add_column(u'rundb_experimentanalysissettings', 'base_recalibrate',
                      self.gf('django.db.models.fields.BooleanField')(default=True),
                      keep_default=False)


        #when we revert the column deletion, we'll need to synch the values up to base_recalibration_mode
        
        #globalConfig table:
        globalConfig_no_recal_count = len(orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "no_recal"))
        
        if globalConfig_no_recal_count > 0:
            print ">> 0253 REVERT... going to update base_recalibrate in %d globalConfig records to False" %(globalConfig_no_recal_count)                    
            orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "no_recal").update(base_recalibrate = False)
                
        globalConfig_standard_recal_count = len(orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "standard_recal"))

        if globalConfig_standard_recal_count > 0:
            print ">> 0253 REVERT... going to update base_recalibrate in %d globalConfig records to True" %(globalConfig_standard_recal_count)        
            orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "standard_recal").update(base_recalibrate = True)
                
        globalConfig_panel_recal_count = len(orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "panel_recal"))

        if globalConfig_panel_recal_count > 0:
            print ">> 0253 REVERT... WARNING - going to update base_recalibrate in %d globalConfig records to True for panel recalibration" %(globalConfig_panel_recal_count)        
            
            globalConfigs = orm['rundb.globalconfig'].objects.filter(base_recalibration_mode = "panel_recal")
            for globalConfig in globalConfigs:
                print ">> 0253 REVERT... WARNING - globalConfig.pk=%d with panel_recal will have base_recalibrate reverted to True!!" %(globalConfig.pk)
                
                globalConfig.base_recalibrate = True
                globalConfig.save()

        
        #experimentAnalysisSettings table:
        eas_no_recal_count = len(orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "no_recal"))
        
        if eas_no_recal_count > 0:
            print ">> 0253 REVERT... going to update base_recalibrate in %d experimentAnalysisSettings records to False" %(eas_no_recal_count)                    
            orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "no_recal").update(base_recalibrate = False)
                
        eas_standard_recal_count = len(orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "standard_recal"))

        if eas_standard_recal_count > 0:
            print ">> 0253 REVERT... going to update base_recalibrate in %d experimentAnalysisSettings records to True" %(eas_standard_recal_count)                    
            orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "standard_recal").update(base_recalibrate = True)
                
        eas_panel_recal_count = len(orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "panel_recal"))

        if eas_panel_recal_count > 0:
            print ">> 0253 REVERT... WARNING - going to update base_recalibrate in %d experimentAnalysisSettings records to True for panel recalibration" %(eas_panel_recal_count)        
            
            orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "panel_recal").update(base_recalibrate = True)
            eases = orm['rundb.experimentanalysissettings'].objects.filter(base_recalibration_mode = "panel_recal")
            for eas in eases:
                print ">> 0253 REVERT... WARNING - experimentAnalysisSettings.pk=%d with panel_recal will have base_recalibrate reverted to True!!" %(eas.pk)
                
                eas.base_recalibrate = True
                eas.save()
