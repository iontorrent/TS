...
#20121202 change log:
# 1) removed Ion's pre-loaded kits and applProducts to prepare for initial_data load
...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        for kit in orm.kitinfo.objects.all().order_by("id"):
            ...
            uid = kit.uid
            if (uid.find("ADAPTR") >= 0) or (uid.find("CONSEQ") >= 0) or (uid.find("LIB") >= 0) or (uid.find("SPREP") >= 0) or (uid.find("SEQ") >= 0) or (uid.find("TEMPLT") >= 0):
                isToDelete = True
                 
            if isToDelete:    
                print "*** GOING TO delete kit=%s with uid=%s " % (kit.name, kit.uid)        
                kit.delete()

        for applProduct in orm.applproduct.objects.all():
            print "*** GOING to delete applProduct.code=%s with id=%s " %(applProduct.productCode, str(applProduct.id))
            applProduct.delete()
