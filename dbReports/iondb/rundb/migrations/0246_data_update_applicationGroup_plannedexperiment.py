...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        convert all existing AMPS_DNA and AMPS_RNA plans and templates with applicationGroup = "DNA + RNA" to "DNA" and "RNA" respectively
        """
        applicationGroups_dna = orm.ApplicationGroup.objects.filter(name = "DNA")
        applicationGroups_rna = orm.ApplicationGroup.objects.filter(name = "RNA")
        sampleGroups_self = orm.SampleGroupType_CV.objects.filter(displayedName = "Self")
        
        total_rec_count = 0
        updated_rec_count = 0
        
        if applicationGroups_dna and applicationGroups_rna:
            applicationGroup_dna = applicationGroups_dna[0]
            applicationGroup_rna = applicationGroups_rna[0]
            sampleGroup_self = sampleGroups_self[0]
            
            ##for plan_or_template in orm.PlannedExperiment.objects.filter(applicationGroup__name = "DNA + RNA", runType__in = ["AMPS", "AMPS_RNA"]):
            for plan_or_template in orm.PlannedExperiment.objects.filter(applicationGroup__name = "DNA + RNA", runType__in = ["AMPS"]):
                
                total_rec_count += 1
                #print "*** 0246.. PROCESSING... plan_or_template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
                
                if (plan_or_template.runType == "AMPS"):
                     plan_or_template.applicationGroup = applicationGroup_dna
                     print "*** 0246.. UPDATING AMPS applicationGroup... plan_or_template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
 
#                elif (plan_or_template.runType == "AMPS_RNA"):
#                     plan_or_template.applicationGroup = applicationGroup_rna
#                     print "*** 0246.. UPDATING AMPS_RNA applicationGroup... plan_or_template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType)  
                    
                if plan_or_template.sampleGrouping:
                    if plan_or_template.sampleGrouping.displayedName == "DNA_RNA":
                        plan_or_template.sampleGrouping = sampleGroup_self
                        print "*** 0246.. UPDATING sampleGrouping... plan_or_template.id=%d; name=%s; runType=%s;" % (plan_or_template.id, plan_or_template.planDisplayedName, plan_or_template.runType) 
                     
                plan_or_template.save()
                updated_rec_count += 1                     

        print ">> 0246 Total %d plan or template records processed and %d records updated." %(total_rec_count, updated_rec_count)
