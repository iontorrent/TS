...
def update_Result(saved_objs):
    ...
    result.reportLink = os.path.join(
        storage.webServerPath,
        location.name,
        "%s_%03d" % (result.resultsName, result.pk),
        "",
    )
...
def update_Plan(saved_objs):
    def find_bedfile(filename, reference):
        content_objs = (
            Content.objects.filter(
                publisher__name="BED", path__contains="/unmerged/detail/"
            )
            .filter(path__contains=reference)
            .filter(path__contains=filename)
        )
        ...
        logger.info(
            "[Data Import] Update Plan: unable to find bedfile: %s for reference: %s"
            % (filename, reference)
        )
        return ""
    ...
    if getattr(eas, "targetRegionBedFile", "") and not os.path.exists(
        eas.targetRegionBedFile
    ):
        eas.targetRegionBedFile = find_bedfile(
            os.path.basename(eas.targetRegionBedFile), eas.reference
        )
        eas.save()
    if getattr(eas, "hotSpotRegionBedFile", "") and not os.path.exists(
        eas.hotSpotRegionBedFile
    ):
        eas.hotSpotRegionBedFile = find_bedfile(
            os.path.basename(eas.hotSpotRegionBedFile), eas.reference
        )
        eas.save()
        ...
        if sample_dict["fields"].get("name"):
            try:
                sample = Sample.objects.get(
                    name=sample_dict["fields"]["name"],
                    externalId=sample_dict["fields"].get("externalId", ""),
                )
        ...
        if "dmfileset" in d["model"]:
            ...
            if version != RELVERSION:
                try:
                    ...
                    dmfilestats.filter(dmfileset__type=dmtype).update(
                        dmfileset=dmfileset
                    )
        ...
        if d["model"] == app_model_string:
            ...
            for field in obj._meta.fields:
                ...
                if field_type in ["ForeignKey", "OneToOneField", "ManyToManyField"]:
                    ...
                    if relation_name in list(saved_objs.keys()):
                        setattr(obj, field.name, saved_objs[relation_name])
                    elif field.null:
                        setattr(obj, field.name, None)
                elif field.name in data_fields:
                    setattr(obj, field.name, data_fields[field.name])
                elif field_type == "DateTimeField" and not field.null:
...
def load_serialized_json(json_path, create_result, log, add_warning):
    ...
    unique_imported = os.path.join(
        _get_imported_path(), os.path.basename(exp_fields["expDir"])
    )
    ...
    if create_result:
        ...
        result = (
            saved_objs["experiment"].results_set.filter(runid=result_fields["runid"])
            if "experiment" in saved_objs
            else None
        )
        if result:
            log(
                "Found existing result %s (%s)." % (result[0].resultsName, result[0].pk)
            )
    ...
    try:
        ...
        logger.error(
            "[Data Import] Failed creating database objects from %s." % json_path
        )
...
def copy_files_to_destination(
    source_dir, destination, dmfileset, cached_file_list, log, add_warning
):
    ...
    logger.info(
        "[Data Import] Importing %d files from %s to %s"
        % (len(to_process), source_dir, destination)
    )
    log(
        "Copy files to destination: %d files, source=%s destination=%s"
        % (len(to_process), source_dir, destination)
    )
    ...
    for plugin, count in list(plugin_warnings.items()):
...
def generate_report_pdf(source_dir, result, dmfilestat, log, add_warning):
    ...
    if os.path.exists(
        os.path.join(source_dir, os.path.basename(source_dir) + "-full.pdf")
    ):
        _copy_to_dir(
            os.path.join(source_dir, os.path.basename(source_dir) + "-full.pdf"),
            source_dir,
        ...
        os.rename(
            os.path.join(report_dir, os.path.basename(source_dir) + "-full.pdf"),
            pdf_filepath,
        )
...
class ImportData:
    def __init__(self, name, selected, username, copy_data, copy_report):
        ...
        self.dmtypes = list(selected.keys())
        self.selected_str = ", ".join(list(selected.keys()))
        self.createResult = (dmactions_types.OUT in selected) or (
            dmactions_types.BASE in selected
        )
        self.json_path = (
            selected.get(dmactions_types.OUT)
            or selected.get(dmactions_types.BASE)
            or list(selected.values())[0]
        )
        self.warnings = []
        ...
        self.categories = []
        for dmtype, json_path in list(selected.items()):
        ...
        for category in self.categories:
            if category["copy_files"]:
                ...
                if dmtype == dmactions_types.OUT:
                    ...
                    if os.path.exists(
                        os.path.join(category["src_path"], "onboard_results")
                    ):
    ...
    def update_diskspace(self, file_list):
        for category in self.categories:
            if category["copy_files"]:
                dmtype = category["dmtype"]
                dmfileset = (
                    self.result.get_filestat(dmtype).dmfileset
                    if self.result
                    else DMFileSet.objects.get(version=RELVERSION, type=dmtype)
                )
    ...
    def fail(self, err, trace):
        ...
        if self.result:
            EventLog.objects.add_entry(
                self.result, "Importing Error: %s %s." % (self.name, err), self.user
            )
            self.result.dmfilestat_set.filter(action_state="IG").update(
                action_state="E"
            )
...
def data_import(name, selected, username, copy_data=False, copy_report=True):
    ...
    try:
        ...
        importing.start()
        importing.log(
            "Selected: %s, copy data: %s, copy report: %s."
...
def process_import(importing, copy_data, copy_report):
    ...
    if copy_data or copy_report:
        ...
        file_list = get_walk_filelist(
            list(source_paths), list_dir=False, save_list=False
        )
    ...
    for category in importing.categories:
        ...
        if category["copy_files"]:
            ...
            try:
                generate_report_pdf(
                    source_dir, result, dmfilestat, importing.log, importing.add_warning
                )
            except Exception:
    ...
    if result:
        ...
        if copy_data:
            ...
            DMFileStat.objects.filter(
                dmfileset__type=dmactions_types.SIG, result__experiment=exp
            ).update(action_state="L")
        else:
...
def find_data_to_import(start_dir, maxdepth=7):
    ...
    musthave = [
        ...
        },
        {
            ...
            "search_paths": [
                "",
                "sigproc_results",
                "sigproc_results/block_X*_Y*",
                "onboard_results/sigproc_results/block_X*_Y*",
            ],
        },
        {
    ...
    for match in p1.stdout.readlines():
        ...
        for test in musthave:
            if test["category"] != dmactions_types.SIG and not has_result(
                resultsName, json_path
            ):
                continue
            for path in test["search_paths"]:
                if glob(os.path.join(root, path, test["filename"])):
                    found.setdefault(resultsName, {}).update(
                        {test["category"]: json_path}
                    )
                    break
    found_results = [{"name": k, "categories": v} for k, v in list(found.items())]
        ...
        if category["copy_files"]:
            ...
            if destination:
                ...
                if category["diskspace"]:
                    dest[destination] = (
                        dest.setdefault(destination, 0) + category["diskspace"]
                    )
            else:
                raise Exception(
                    "Missing %s copy files destination" % category["dmtype"]
                )
