...
def get_search_parameters():
    def get_chip_choices():
        ...
        used_chip_values = [
            str(value).strip()
            for value in list(
                Experiment.objects.values_list("chipType", flat=True)
                .distinct("chipType")
                .order_by("chipType")
            )
        ]
        ...
        for chip_value in used_chip_values:
            if len(chip_value) > 0:
                try:
                    display_name = Chip.objects.get(
                        name=chip_value
                    ).getChipDisplayedName()
    ...
    for expkey, key in eas_keys:
        experiment_params[expkey] = list(
            ExperimentAnalysisSettings.objects.values_list(key, flat=True)
            .distinct(key)
            .order_by(key)
        )
        experiment_params[expkey] = [
            value for value in experiment_params[expkey] if len(str(value).strip()) > 0
        ]
    for key in list(report_params.keys()):
        report_params[key] = list(
            Results.objects.values_list(key, flat=True).distinct(key).order_by(key)
        )
    combined_params = {
        "flows": sorted(
            set(experiment_params["flows"] + report_params["processedflows"])
        ),
        "projects": Project.objects.values_list("name", flat=True)
        .distinct("name")
        .order_by("name"),
        "plugins": Plugin.objects.values_list("name", flat=True)
        .distinct("name")
        .order_by("name"),
    }
    mesh_params = {"nodes": IonMeshNode.objects.all()}
...
def get_serialized_exps(request, pageSize):
    ...
    objects = (
        resource.get_object_list(request)
...
class ExperimentListView(ListView):
    ...
    queryset = (
        ...
        .exclude(repResult=None)
        .order_by("-repResult__timeStamp")
    )
    ...
    def get_object(self):
        ...
        if result.experiment.has_status():
            ...
            if result.experiment.in_progress:
                result.experiment.progress_percent = (
                    100
                    * float(result.experiment.ftpStatus)
                    / float(result.experiment.flows)
                )
        return result
...
def data_table(request):
    data = {"search": get_search_parameters()}
    return render_to_response(
        "rundb/data/completed_table.html",
        data,
...
def project_view(request, pk=None):  # TODO: DEAD CODE
    pr = ProjectResource()
    ...
    return render_to_response(
        ...
        context_instance=RequestContext(
            ...
            {
                # Other things here.
                "project_json": pr.serialize(
                    None, pr.full_dehydrate(pr_bundle), "application/json"
                ),
                "project": project,
                "method": "GET",
                "readonly": True,
...
def project_add(request, pk=None):
    ...
    ctx = template.RequestContext(
        request,
        {
            ...
            "method": "POST",
            "title": _("projects.add.title"),  # 'Add'
            "projectNameLabel": _(
                "projects.add.fields.name.label"
            ),  # 'Add Project Name'
            "readonly": False,
            "action": reverse(
                "api_dispatch_list",
                kwargs={"resource_name": "project", "api_name": "v1"},
            ),
        },
    )
    return render_to_response(
        "rundb/data/modal_project_details.html", context_instance=ctx
    )
...
def project_delete(request, pk=None):
    ...
    ctx = template.RequestContext(
        request,
        {
            ...
            "name": project.name,
            "method": "DELETE",
            "title": _("projects.delete.title"),  # 'Confirm Delete Project'
            "confirmationText": _("projects.delete.confirmation.text")
            % {
    ...
    return render_to_response(
        "rundb/data/modal_confirm_delete.html", context_instance=ctx
    )
    ...
    return render_to_response(
        ...
        context_instance=RequestContext(
            request,
            {
                ...
                "otherList": json.dumps(otherList),
                "method": "PATCH",
                "title": _("projects.edit.title"),  # 'Edit'
                "projectNameLabel": _(
                    "projects.edit.fields.name.label"
                ),  # 'Project Name'
                "readonly": True,
                "action": reverse(
                    "api_dispatch_detail",
...
def project_log(request, pk=None):
    if request.method == "GET":
        ...
        title = _("projects.project.vieweventlog.title") % {
            "projectName": selected.name,
            "id": pk,
        }  # "Project History for %s (%s):" % (selected.name, pk)
        ...
        return render_to_response(
            "rundb/common/modal_event_log.html", context_instance=ctx
        )
...
def project_results(request, pk):
    ...
    contextValues = {
        "project": selected,
        "filter_thumbnails": thumbs_exist,
        "title": _("projectresults.title") % {"projectName": selected.name},
    }
    ctx = template.RequestContext(request, contextValues)
...
def project_compare_pdf(request, pk):
    ...
    if not os.path.exists(pdf_path):
        return HttpResponse(
            open(os.path.join(directory, "comparison_report.log")).read(), "text/plain"
        )
    response = HttpResponse(FileWrapper(open(pdf_path)), content_type="application/pdf")
    response["Content-Length"] = os.path.getsize(pdf_path)
    response["Content-Disposition"] = (
        "attachment; filename=project_compare_%s.pdf" % project.name
    )
    return response
...
def results_from_project(request, results_pks, project_pk):
    try:
        _results_to_project_helper(request, results_pks, project_pk, "Remove")
        return HttpResponse()
    except Exception:
    ...
    if request.method == "GET":
        ctx = template.RequestContext(
            request,
            {
                ...
                "action": urlresolvers.reverse(
                    "results_to_project", args=[results_pks]
                ),
                "method": "POST",
            },
        )
        return render_to_response(
            "rundb/data/modal_projects_select.html", context_instance=ctx
        )
    if request.method == "POST":
        ...
        try:
            for project_pk in project_pks:
                _results_to_project_helper(request, int(project_pk), results_pks, "Add")
            return HttpResponse()
        except Exception as e:
            logger.exception(e)
            raise
...
def validate_results_to_combine(selected_results, override_samples=False):
    ...
    for i, r in enumerate(selected_results):
        version = {}
        for name, shortname in ver_map.items():
            version[name] = next(
                (
                    v.split(":")[1].strip()
                    for v in r.analysisVersion.split(",")
                    if v.split(":")[0].strip() == shortname
                ),
                "",
            )
        ...
        if not version["tmap"]:
            r.tmap_version = version["analysis"]
        if not version["alignment"]:
            r.alignment_version = version["analysis"]

        if not common:
            if r.resultsType != "CombinedAlignments" or i == (
                len(selected_results) - 1
            ):
                common = {
                    "floworder": r.experiment.flowsInOrder,
                    "barcodeId": r.eas.barcodeKitName,
                    "barcodedSamples": r.eas.barcodedSamples
                    if r.eas.barcodeKitName
                    else {},
                    "sample": r.experiment.get_sample()
                    if not r.eas.barcodeKitName
                    else "",
                }
    ...
    if (
        len(
            set(
                [
                    r.experiment.flowsInOrder
                    for r in selected_results
                    if r.resultsType != "CombinedAlignments"
                ]
            )
        )
        > 1
    ...
    if len(barcodeSet) > 1:
        ...
        common["barcodeId"] = (
            minstr if all(s.startswith(minstr) for s in barcodeSet) else ""
        )
        ...
        if common["barcodeId"]:
            if (
                len(
                    set(
                        [
                            json.dumps(r.eas.barcodedSamples)
                            for r in selected_results
                            if r.resultsType != "CombinedAlignments"
                        ]
                    )
...
def results_to_combine(request, results_pks, project_pk):
    if request.method == "GET":
        selected_results = Results.objects.filter(
            id__in=results_pks.split(",")
        ).order_by("-timeStamp")
        warnings, common = validate_results_to_combine(selected_results)
        barcodes = ""
        if common["barcodeId"]:
            barcodes = dnaBarcode.objects.filter(name=common["barcodeId"]).order_by(
                "name", "index"
            )
        ...
        ctx = template.RequestContext(
            request,
            {
                ...
                "barcodes": barcodes,
                "action": urlresolvers.reverse(
                    "results_to_combine", args=(results_pks, project_pk)
                ),
                "method": "POST",
            },
        )
        return render_to_response(
            "rundb/data/modal_combine_results.html", context_instance=ctx
        )
    if request.method == "POST":
        try:
            json_data = json.loads(request.body)
            result = _combine_results_sendto_project(
                project_pk, json_data, request.user.username
            )
...
def _combine_results_sendto_project(project_pk, json_data, username=""):
    ...
    for parent in parents:
        if (
            parent.dmfilestat_set.get(dmfileset__type=dmactions_types.OUT).action_state
            == "DD"
        ):
    ...
    if override_samples:
        ...
        if barcodedSamples and common["barcodedSamples"]:
            ...
            for sample_name, value in list(barcodedSamples.items()):
                for barcode in value["barcodes"]:
                    barcodeSampleInfo = [
                        v.get("barcodeSampleInfo", {}).get(barcode)
                        for v in list(common["barcodedSamples"].values())
                        if v.get("barcodeSampleInfo", {}).get(barcode)
                    ]
                    if barcodeSampleInfo:
                        barcodedSamples[sample_name].setdefault(
                            "barcodeSampleInfo", {}
                        )[barcode] = barcodeSampleInfo[0]
        else:
    ...
    filePrefix = (
        "CombineAlignments"
    )  # this would normally be Experiment name that's prefixed to all filenames
    ...
    result.sffLink = os.path.join(
        result.reportLink, "%s_%s.sff" % (filePrefix, result.resultsName)
    )
    ...
    for parent in parents:
        ...
        reportDir = (
            dmfilestat.archivepath
            if dmfilestat.action_state == "AD"
            else parent.get_report_dir()
        )
    ...
    if not latest_w_plan:
        grandparents = sum(
            [v.split(delim) for v in parents.values_list("parentIDs", flat=True)], []
        )
        grandparents = [v for v in set(grandparents) if v]
        latest_w_plan = Results.objects.filter(
            id__in=grandparents, experiment__plan__isnull=False
        ).order_by("-timeStamp")
    ...
    from ion import pipelineScriptPath

    scriptpath = os.path.join(pipelineScriptPath, "combineReports.py")
    ...
    # log project history
    message = (
        "Combine results %s into name= %s (%s), auto-assign to project name= %s (%s)."
...
def create_combined_result(resultsName):
    ...
    otherResults = Results.objects.filter(resultsName__contains=resultsName).order_by(
        "pk"
    )
    if otherResults:
...
def experiment_edit(request, pk):  # TODO: SUSPECTED UNUSED DEAD CODE
    exp = get_object_or_404(Experiment, pk=pk)
    eas, eas_created = exp.get_or_create_EAS(editable=True)
    plan = exp.plan

    barcodes = {}
    for bc in dnaBarcode.objects.order_by("name", "index").values(
    ...
    if request.method == "GET":
        ...
        if eas.libraryKitName:
            ...
            libkitset = KitInfo.objects.filter(
                kitType="LibraryKit", kitpart__barcode=eas.libraryKitBarcode
            )
            if len(libkitset) == 1:
        ...
        for plugin in plugins_list:
            pluginsUserInput[str(plugin.id)] = eas.selectedPlugins.get(
                plugin.name, {}
            ).get("userInput", "")
    ...
    if request.method == "POST":
        ...
        if exp_form.is_valid() and eas_form.is_valid():
            ...
            if not eas.barcodeKitName:
                ...
                exp.samples.clear()
                for value in list(eas.barcodedSamples.values()):
                    sampleId = value["id"]
                    sample = Sample.objects.get(pk=sampleId)
                    exp.samples.add(sample)
    ...
    return render_to_response(
        "rundb/data/modal_experiment_edit.html",
        context_instance=template.RequestContext(request, ctxd),
    )
...
def datamanagement(request):
    ...
    if os.path.exists("/opt/ion/.ion-internal-server"):
        ...
        dm_tables = []
        for path in (
            FileServer.objects.all()
            .order_by("pk")
            .values_list("filesPrefix", flat=True)
        ):
            dm_tables.append(
                {
                    "filesPrefix": path,
                    "url": "/rundb/api/v1/compositedatamanagement/?format=json&expDir__startswith=%s"
                    % path,
                }
            )
    else:
    ...
    for dmfileset in dm_filesets:
        if dmfileset.backup_directory in ["None", None, ""]:
            ...
            dmfileset.mounted = bool(
                is_mounted(dmfileset.backup_directory)
            ) and os.path.exists(dmfileset.backup_directory)

    # Disk Usage section
    fs_stats = {}
    fs_stats_mounted_paths = []
    for path in (
        FileServer.objects.all().order_by("pk").values_list("filesPrefix", flat=True)
    ):
        try:
            if os.path.exists(path):
                ...
                fs_stats[path]["percentkeep"] = (
                    100 * (keeper_used / total_gb) if total_gb > 0 else 0
                )
    ...
    for bdir, name in backup_dirs:
        try:
            ...
            if (
                mounted
                and bdir not in archive_stats
                and bdir not in fs_stats
                and bdir not in fs_stats_mounted_paths
            ):
...
def dm_actions(request, results_pks):
    ...
    to_update = DMFileStat.objects.filter(
        diskspace=None, result__in=results_pks.split(",")
    )
        ...
        for result in results:
            ...
            if not info:
                ...
                if info["keep"] != dmfilestat.getpreserved():
                    info["keep"] = "*"
                if (info["diskspace"] is not None) and (
                    dmfilestat.diskspace is not None
                ):
    ...
    if len(results) == 1:
        name = "%s: %s" % (
            ugettext("dm.actions.field.result.resultsName.label"),
            result.resultsName,
        )
        subtitle = "%s: %s" % (
            ugettext("dm.actions.field.result.experiment.expName.label"),
            result.experiment.expName,
        )
    else:
        # multiple results (available from Project page)
        name = validation.format(
            ugettext("dm.actions.title.multiple"), {"count": len(results)}
        )  # "Selected %s results."
...
def dm_action_selected(request, results_pks, action):
    ...
    logger.info(
        "dm_action_selected: request '%s' on report(s): %s" % (action, results_pks)
    )
    ...
    try:
        ...
        backup_directory = (
            data["backup_dir"] if data["backup_dir"] != "default" else None
        )

        for resultPK in results_pks.split(","):
            logger.debug("Matching dmfilestats contain %s reportpk" % resultPK)
            dmfilestat_dict[resultPK] = DMFileStat.objects.select_related().filter(
                dmfileset__type__in=data["categories"], result__id=int(resultPK)
            )

            for dmfilestat in dmfilestat_dict[resultPK]:
                # validate export/archive destination folders
                if action in ["export", "archive"]:
                    dmactions.destination_validation(
                        dmfilestat, backup_directory, manual_action=True
                    )
                ...
                try:
                        ...
                        if not data["confirmed"]:
                            return HttpResponse(
                                json.dumps(
                                    {
                                        "warning": str(e)
                                        + "<br>Exporting now may produce incomplete data set."
                                    }
                                ),
                    ...
                    if not data["confirmed"]:
                        return HttpResponse(
                            json.dumps({"warning": str(e)}), mimetype="application/json"
                        )
                ...
                if action == "archive" and dmfilestat.getpreserved():
                    if not data["confirmed"]:
                        return HttpResponse(
                            json.dumps(
                                {
                                    "warning": "%s currently marked Keep."
                ...
                if dmfilestat.isarchived() and not os.path.exists(
                    dmfilestat.archivepath
                ):
                    return HttpResponseServerError(
                        "%s archive location %s is not available."
                        % (dmfilestat.dmfileset.type, dmfilestat.archivepath)
                    )

        async_task_result = dmtasks.action_group.delay(
        ...
        msg = (
            "Source directory %s no longer exists. Setting action_state to Deleted"
            % e.message
        )
        logger.info(msg)
        EventLog.objects.add_entry(
            dmfilestat.result, msg, username=request.user.username
        )
    except Exception as e:
...
def dm_configuration(request):
    ...
    if request.method == "GET":
        dm_filesets = DMFileSet.objects.filter(version=settings.RELVERSION).order_by(
            "pk"
        )
        ...
        return render_to_response(
            "rundb/data/dm_configuration.html", context_instance=ctx
        )
        ...
        try:
            for key, value in list(data.items()):
                if key == "filesets":
                    for category, params in list(value.items()):
                        # log += '<b>%s:</b> %s<br>' % (category,
                        # json.dumps(params).translate(None, "{}\"\'") )
                        dmfileset = dm_filesets.filter(type=category)
                        current_params = dmfileset.values(*list(params.keys()))[0]
                        changed_params = [
                            key
                            for key, value in list(params.items())
                            if isdiff(value, current_params.get(key))
                        ]
            ...
            logger.exception("dm_configuration: error: %s" % unicode(e))
            return HttpResponseServerError(
                "%s %s" % (ugettext("validation.messages.error_prefix"), unicode(e))
            )  # TODO: i18n
...
def _add_dm_configuration_log(request, log):
    ...
    ev = EventLog(
        object_pk=0, content_type=ct, username=request.user.username, text=log
    )
    ev.save()
...
def delete_ack(request):
    ...
    if not runPK:
        return HttpResponse(
            json.dumps({"status": "error, no runPK POSTed"}),
            mimetype="application/json",
        )
    if not runState:
        return HttpResponse(
            json.dumps({"status": "error, no runState POSTed"}),
            mimetype="application/json",
        )
    ...
    ret = DMFileStat.objects.filter(
        result__pk__in=results_pks, dmfileset__type=dmactions_types.SIG
    ).update(action_state=runState)
    ...
    return HttpResponse(
        json.dumps({"runState": runState, "count": ret, "runPK": runPK}),
        mimetype="application/json",
    )
...
def preserve_data(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            msg = (
                "%s marked exempt from auto-action"
                if keep
                else "%s no longer exempt from auto-action"
            )
            msg = msg % typeStr
            for result in results:
                ...
                if dmtype == "reanalysis":
                    ...
                    if (
                        result.experiment.log.get("oninstranalysis", "") == "yes"
                        and not result.isThumbnail
                    ):
        ...
        return HttpResponse(
            json.dumps(
                {"reportPK": reportPK, "type": typeStr, "keep": filestat.getpreserved()}
            ),
            mimetype="application/json",
        )
...
def get_dir_choices():
    ...
    for choice in set(
        DMFileSet.objects.exclude(backup_directory__in=["", "None"]).values_list(
            "backup_directory", flat=True
        )
    ):
        if choice and not (choice, choice) in basicChoice:
            choice_str = (
                choice if bool(is_mounted(choice)) else "%s (not mounted)" % choice
            )
...
def dm_log(request, pk=None):
    if request.method == "GET":
        ...
        ct = ContentType.objects.get_for_model(selected)
        title = _("dm.log.title") % {
            "resultsName": selected.resultsName,
            "id": pk,
        }  # "Data Management Actions for %s (%s)" % (selected.resultsName, pk)
        eventlog_messages_empty = _("eventlog.messages.empty") % {"title": title}
        ctx = RequestContext(
            request,
            {
...
def dm_configuration_log(request):
    if request.method == "GET":
        ct = ContentType.objects.get_for_model(DMFileSet)
        title = _(
            "dm.configuration_log.title"
        )  # "Data Management Configuration History"
        eventlog_messages_empty = _("eventlog.messages.empty") % {"title": title}
        ctx = RequestContext(
            request,
            {
...
def dm_list_files(request, resultPK, action):
    ...
    data = json.loads(request.body)
    dmfilestat = DMFileStat.objects.select_related().filter(
        dmfileset__type__in=data["categories"], result__id=int(resultPK)
    )
    dmfilestat = dmfilestat[0]

    # Hack - generate serialized json file for the DataXfer plugin
    dmactions.write_serialized_json(
        dmfilestat.result, dmfilestat.result.get_report_dir()
    )
...
def browse_backup_dirs(request, path):
        ...
        for name, full_dir_path, stat in dirs:
            ...
            if exclude_archived == "true":
                if (
                    ...
                    or name == "exportedReports"
                    or glob.glob(os.path.join(full_dir_path, "serialized_*.json"))
                ):
                    dir_path = ""
    ...
    return render_to_response(
        "rundb/data/modal_browse_dirs.html",
...
def import_data(request):
    if request.method == "GET":
        backup_dirs = get_dir_choices()
        return render_to_response(
            "rundb/data/modal_import_data.html",
            context_instance=RequestContext(request, {"backup_dirs": backup_dirs}),
        )
    elif request.method == "POST":
        ...
        for result in postData:
            ...
            async_result = data_import.delay(
                name, result, request.user.username, copy_data, copy_report
            )
        return HttpResponse()
...
def import_data_find(request, path):
    ...
    if path and os.path.exists(path):
        try:
            found_results = find_data_to_import.delay(path).get(timeout=180)
        except celeryTimeoutError:
            return HttpResponseServerError(
                'Timeout while searching directory. If your archive drive is large please use "Browse" button to select data to import.'
            )

        if len(found_results) == 0:
            return HttpResponseNotFound(
                "Did not find any data exported or archived with TS version 4.0 or later in %s."
                % path
            )
        else:
            results_list = []
...
def dmactions_jobs(request):
    ...
    ).select_related("result", "dmfileset")
    ...
    ctx = json.dumps(
        {"objects": dmactions_jobs, "total": len(dmactions_jobs)}, cls=DjangoJSONEncoder
    )
