...
from iondb.rundb.tasks import setResultDiskspace

def prune_directory(dirname, pruneRules, logger):
    '''Apply pruning rules to given directory'''
    ...
    for dirpath, dirnames, filenames in os.walk(dirname, followlinks=False):
    ...
    return toDel, errFiles, totalSize, numFilesDel
...
def pruneReport(pkR, comment, _logger):
    ...
    try:
        ...
        toDel, errFiles, totalSize, numFilesDel = prune_directory(reportDir, pruneRules, logger)
        ...
        if len(toDel) > 0 and len(errFiles) == 0:
            ...
            raise Exception("All %d files failed to be removed.  See /var/log/ion/data_management.log." % len(errFiles))

        elif len(errFiles) > 0 and len(errFiles) < len(toDel):
            # Some files had an error being removed
            raise Exception("%d of %d files failed to be removed.  See /var/log/ion/data_management.log." % (len(errFiles), len(toDel)))
        ...
        # update diskusage for this result's database object
        setResultDiskspace(pkR)
