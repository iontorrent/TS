...
def delete(user, user_comment, dmfilestat, lockfile, msg_banner, confirmed=False):
    ...
    try:
        ...
        filelistdict = _get_file_list_dict(
            dmfilestat, DELETE, user, user_comment, msg_banner
        )
        locallockid = "%s_%s" % (
            dmfilestat.result.resultsName,
            dm_utils.slugify(dmfilestat.dmfileset.type),
        )
...
def export(user, user_comment, dmfilestat, lockfile, msg_banner, backup_directory=None):
    ...
    try:
        ...
        _create_destination(
            dmfilestat, EXPORT, dmfilestat.dmfileset.type, backup_directory
        )
    except Exception:
        raise
    try:
        if dmfilestat.dmfileset.type == dmactions_types.OUT:
            _create_archival_files(dmfilestat)
        filelistdict = _get_file_list_dict(
            dmfilestat, EXPORT, user, user_comment, msg_banner
        )
        locallockid = "%s_%s" % (
            dmfilestat.result.resultsName,
            dm_utils.slugify(dmfilestat.dmfileset.type),
...
def archive(
    ...
    try:
        ...
        destination_validation(dmfilestat, backup_directory)
        _create_destination(
            dmfilestat, ARCHIVE, dmfilestat.dmfileset.type, backup_directory
        )
    except Exception:
        raise
    ...
    try:
        if dmfilestat.dmfileset.type == dmactions_types.OUT:
            _create_archival_files(dmfilestat)
        filelistdict = _get_file_list_dict(
            dmfilestat, ARCHIVE, user, user_comment, msg_banner
        )
        locallockid = "%s_%s" % (
            dmfilestat.result.resultsName,
            dm_utils.slugify(dmfilestat.dmfileset.type),
...
def test(user, user_comment, dmfilestat, lockfile, msg_banner, backup_directory=None):
    ...
    try:
        filelistdict = _get_file_list_dict(
            dmfilestat, TEST, user, user_comment, msg_banner
        )
        locallockid = "%s_%s" % (
            dmfilestat.result.resultsName,
            dm_utils.slugify(dmfilestat.dmfileset.type),
...
def destination_validation(dmfilestat, backup_directory=None, manual_action=False):
    ...
    try:
        ...
        if not os.path.isdir(backup_directory):
            raise DMExceptions.MediaNotAvailable(
                "Backup media for %s is not a directory: %s"
        ...
        if not is_mounted(backup_directory):
            raise DMExceptions.MediaNotAvailable(
                "Backup media for %s is not mounted: %s"
                % (dmfilestat.dmfileset.type, backup_directory)
            )
    ...
    if _skipdiskspacecheck(backup_directory):
        logger.warn(
            "%s - skipping destination disk space check"
            % (sys._getframe().f_code.co_name),
            extra=logid,
        )
        pass
        ...
        try:
            ...
            if manual_action:
                ...
                for obj in DMFileStat.objects.filter(
                    ...
                    try:
                        if obj.archivepath and not os.path.normpath(
                            obj.archivepath
                        ).startswith(os.path.normpath(backup_directory)):
                            continue
                        elif not os.path.normpath(
                            obj.dmfileset.backup_directory
                        ).startswith(os.path.normpath(backup_directory)):
                            continue
                ...
                logger.debug(
                    "Required %dMB, pending %dMB, free %dMB"
...
def action_validation(dmfilestat, action, confirmed=False):
    ...
    if dmfilestat.files_in_use:
        errmsg = "%s files are currently being used by %s." % (
            dmfilestat.dmfileset.type,
            dmfilestat.files_in_use,
        )
        logger.info(
            errmsg, extra=logid
        )  # For developers only, more informative error string
    ...
    if action == DELETE and dmfilestat.getpreserved():
        errmsg = (
            "Unable to delete files: %s currently marked as Keep."
            % dmfilestat.dmfileset.type
        )
    ...
    if (
        action == DELETE
        and dmfilestat.dmfileset.type == dmactions_types.BASE
        and not confirmed
    ):
        related = get_related_dmfilestats(dmfilestat)
        # make sure to exclude current dmfilestat in case itself is linked
        if related is not None:
            related = related.exclude(pk=dmfilestat.pk)
        if related is not None and related.count() > 0:
            errmsg = (
                "Basecalling Input files are used by reanalysis started from BaseCalling: %s"
...
def get_related_dmfilestats(dmfilestat):
    ...
    if dmfilestat.dmfileset.type == dmactions_types.SIG:
        ...
        related_results = Results.objects.filter(
            experiment=dmfilestat.result.experiment
        )
        sigproc_path = os.path.join(
        ...
        if related_results and os.path.exists(sigproc_path):
            ...
            for result in related_results:
                ...
                try:
                    if os.path.islink(testpath) and os.path.samefile(
                        sigproc_path, testpath
                    ):
            ...
            if len(linked) > 0:
                related = DMFileStat.objects.filter(
                    dmfileset__type=dmactions_types.BASE, result__pk__in=linked
                )
    return related
    ...
    try:
        if filesettype == dmactions_types.SIG:
            ...
            if action == ARCHIVE and dest_dir not in backup_directory:
                dmfilestat.archivepath = os.path.join(
                    backup_directory, "archivedReports", dest_dir
                )
            elif action == EXPORT and dest_dir not in backup_directory:
                dmfilestat.archivepath = os.path.join(
                    backup_directory, "exportedReports", dest_dir
                )
...
def _get_file_list_dict(dmfilestat, action, user, user_comment, msg_banner):
    ...
    if dmfilestat.isdeleted():
        errmsg = "The %s for %s are deleted" % (
            dmfilestat.dmfileset.type,
            dmfilestat.result.resultsName,
        )
        logger.warn(errmsg, extra=logid)
    ...
    add_linked_sigproc = (
        False
        if (action == DELETE or dmfilestat.dmfileset.type == dmactions_types.INTR)
        else True
    )
    ...
    for start_dir in search_dirs:
        ...
        logger.info(
            "%d files to process at %s"
            % (len(list(set(to_process) - set(to_keep))), start_dir),
            extra=logid,
        )
...
def _process_fileset_task(list_of_file_dict, lockfile, lock_id):
    ...
    locallock = TaskLock(
        lock_id, timeout=60
    )  # short timeout in case lock release code doesn't get called

    if not (locallock.lock()):
        logger.warn(
            "lock file exists: %s(%s)" % (lock_id, locallock.get()), extra=logid
        )
...
def _process_task(pfilename):
    ...
    try:
        try:
            ...
            logger.error(
                "Error accessing file: %s.  Cannot continue the DM action!"
                % (pfilename),
                extra=logid,
            )
        ...
        for d_cnt, mydict in enumerate(list_of_file_dict):
            ...
            logger.debug(
                "%d, start_dir: %s" % (d_cnt, mydict["start_dir"]), extra=logid
            )
            logger.info(
                "%6d %s %s"
                % (
                    len(mydict["to_process"]),
                    dmfilestat.dmfileset.type,
                    dmfilestat.result.resultsName,
                ),
                extra=logid,
                ...
                if len(mydict["to_process"]) > 0:
                    ...
                    try:
                        ...
                        if e.errno == errno.ENOSPC:
                            ...
                            logger.warn(
                                "%04d No longer exists %s" % (j, path), extra=logid
                            )
                            continue
                        else:
                            raise
                    ...
                    if (
                        not mydict["action"] in [EXPORT, TEST]
                            ...
                            if len(os.listdir(thisdir)) == 0:
                                if not "plugin_out" in thisdir:
                                    try:
                                        os.rmdir(thisdir)
                                        logger.debug(
                                            "Removed empty directory: %s" % thisdir,
                                            extra=logid,
                                        )
                                    except Exception as e:
                                        logger.warn(
                                            "rmdir [%d] %s: %s"
                                            % (e.errno, e.strerror, thisdir),
                                            extra=logid,
                                        )
                        except OSError as e:
                            if e.errno == errno.ENOENT:
                                logger.warn(
                                    "del_empty_dir Does not exist %s" % (path),
                                    extra=logid,
                                )
                                continue
                            else:
        ...
        logger.error(
            "This %s action will need to be manually completed." % (mydict["action"]),
            extra=logid,
        )
        logger.error(
    ...
    if not terminate:
        ...
        # Do the user notification
        try:
            _action_complete_update(
                mydict["user"], mydict["user_comment"], dmfilestat, mydict["action"]
            )
...
def _get_keeper_list(dmfilestat, action):
    ...
    if action == EXPORT:
        ...
        for settype, patterns in dmfilestat.dmfileset.keepwith.items():
            # Are the types specified in dmfilestat.dmfileset.keepwith still local?
            if not dmfilestat.result.dmfilestat_set.get(
                dmfileset__type=settype
            ).isdisposed():
                    ...
                    try:
                        ...
                        if e.errno == errno.ENOENT:
                            ...
                            try:
                                os.unlink(filepath)
                                logger.info(
                                    "Broken symlink removed: %s" % filepath, extra=logid
                                )
                            except OSError as e:
                                logger.warn(e, extra=logid)
...
def _update_related_objects(
    user, user_comment, dmfilestat, action, msg, action_state=None
):
    logger.debug("Function: %s()" % sys._getframe().f_code.co_name, extra=logid)
    ...
    if not dmfilestat.in_process():
        ...
        if dmfilestat.dmfileset.type == dmactions_types.SIG:
            ...
            DMFileStat.objects.filter(
                ...
                action_state=dmfilestat.action_state,
                archivepath=dmfilestat.archivepath,
                user_comment=dmfilestat.user_comment,
            )
...
def _create_symlink(dmfilestat):
    ...
    if os.path.isdir(dmfilestat.result.experiment.expDir):
        if len(os.listdir(dmfilestat.result.experiment.expDir)) == 0:
            logger.debug(
                "expDir exists and is empty: %s" % dmfilestat.result.experiment.expDir,
                extra=logid,
            )
            os.rmdir(dmfilestat.result.experiment.expDir)
            os.symlink(dmfilestat.archivepath, dmfilestat.result.experiment.expDir)
            logger.debug(
                "symlink created: %s" % dmfilestat.result.experiment.expDir, extra=logid
            )
        else:
            logger.debug(
                "no symlink, expDir is not empty: %s"
                % dmfilestat.result.experiment.expDir,
                extra=logid,
            )
    else:
        logger.debug(
            "expDir no longer exists: %s" % dmfilestat.result.experiment.expDir,
            extra=logid,
        )
        os.symlink(dmfilestat.archivepath, dmfilestat.result.experiment.expDir)
        logger.debug(
...
def set_action_param_var(list_of_dict_files):
    ...
    with tempfile.NamedTemporaryFile(
        dir=store_dir, delete=False, mode="w+b", prefix=prefix
    ) as fileh:
...
def write_serialized_json(result, destination):
    ...
    try:
        ...
        logger.error(
            "Unable to save serialized.json for %s(%d)"
...
def get_file_list(dmfilestat):
    ...
    try:
        ...
        for start_dir in search_dirs:
            if os.path.isdir(start_dir):
                tmp_process, tmp_keep = dm_utils._file_selector(
                    start_dir,
                    dmfilestat.dmfileset.include,
                    dmfilestat.dmfileset.exclude,
                    kpatterns,
