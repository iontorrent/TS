...
def connect_nodetest(node):
    ...
    status_dict = {"name": node, "status": "good", "connect_tests": [], "error": stderr}
...
def config_nodetest(node, head_versions):
    ...
    status_dict = {"status": "good", "config_tests": [], "error": stderr}
    ...
    for line in stdout.splitlines():
        try:
            ...
            if test_name == "version_test":
                ...
                version_test_errors = []
                for key, version in list(versions.items()):
                ...
                status_dict["version_test_errors"] = "\n".join(
                    [
                        "%s %s (%s)" % (v[0], v[1], v[2])
                        for v in sorted(version_test_errors)
                    ]
                )
