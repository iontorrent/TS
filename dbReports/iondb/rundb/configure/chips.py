...
def getChipPdf(request, path):
    ...
    if m:
        ...
        rawtraceBuffer = (
            "<tr><td>Raw Traces</td><td></td><td>%s</td></tr>" % rawtracestartdate
        ) + rawtraceBuffer
    ...
    f.write(
        "<tr><td>Initial pH:</td><td>%s</td><td>%s</td></tr>\n" % (initialph, startdate)
    )
        ...
        try:
            ...
            for rigname in filelist:
                if rigname in rignames:
                    rig_path = os.path.join(fs.filesPrefix, rigname)
                    rigfiles = os.listdir(rig_path)
                    for rigfile in rigfiles:
                        if rigfile.startswith("support_" + rigname):
                            logger.info("Found a Proton diag file: " + rigfile)
                            protonDiags.append(os.path.join(rig_path, rigfile))
