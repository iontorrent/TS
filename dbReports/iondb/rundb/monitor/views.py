...
def monitor(request):
    ...
    return render_to_response(
        "rundb/monitor/monitor.html", context, context_instance=RequestContext(request)
    )
...
def instruments(request):
    ...
    if len(queryset) == 0:
        ...
        m1.testDat = json.loads(
            '{"test1":{"test2":"inner"},"test2.5":{"test3":"alsoinner"},"test4":"outer"}'
        )
        m1.name = "Debug"
        m1.save()
        queryset = models.MonitorData.objects.all()
    ...
    return render_to_response(
        "rundb/monitor/instruments.html",
        context,
...
def chef(request):
    ...
    for plan in plans:
        data = {
            ...
            "template_prep_remaining_time": utils.convert_seconds_to_hhmmss_string(
                plan.experiment.chefRemainingSeconds
            ),
            "template_prep_estimated_end_time": utils.convert_seconds_to_datetime_string(
                plan.experiment.chefLastUpdate, plan.experiment.chefRemainingSeconds
            ),
        }
        ...
        if samplesets_w_libprep:
            ...
            data.update(
                {
                    "lib_prep_progress": samplesets_w_libprep[
                        0
                    ].libraryPrepInstrumentData.progress,
                    "lib_prep_status": samplesets_w_libprep[
                        0
                    ].libraryPrepInstrumentData.instrumentStatus,
                }
            )
            ...
            chef_table.append(
                {
                    ...
                    "sample_prep_type": "Library Prep",  # TODO: i18n - not used by UI
                    ...
                    "remaining_time": utils.convert_seconds_to_hhmmss_string(
