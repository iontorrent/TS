...
class Run:
    ...
    def update_application_and_samples(self, plan):
        ...
        if barcodedSamples:
            ...
            for key, value in barcodedSamples.items():
                samples_list.append({
                    "sampleName": key,
                    "barcodes": ' '.join(sorted(value["barcodes"])) if "barcodes" in value else ""
                })
    ...
    def create_plugins(self, result):
        ...
        self.state = DONE
        self.status_string = '<a href="' +self.url+ '">View run report</a>'
        ...
        if n_active > 0:
            ...
            self.progress_string = "%d/%d" % (n_total - n_active, n_total)
...
class Runs:
    ...
    def __init__(self):
        self.uids = []
        self.runs = []
        # also keep a list of active results from jobServer
...
def results_in_range(runs, timelimit):
    ...
    4) if ftpStatus is NOT "Complete or "Missing File(s)": stage = Sequencing
    5) if ftpStatus is "Complete or "Missing File(s)":
    ...
    for result in results:
        if result.experiment.ftpStatus == "Complete" or result.experiment.ftpStatus == "Missing File(s)":
...
def add_active_analysis(runs):
    ...
    2) limit to auto-analysis only
    3) if ftpStatus is NOT "Complete: stage = Sequencing
    3) if ftpStatus is "Complete and Result is non-thumbnail: stage = Analysis
    '''
    ...
    for result in results:
        ...
        if not runs.contains(uid):
            if result.experiment.ftpStatus == "Complete" and not result.isThumbnail:
                r = Run(ANALYSIS, result)
                runs.add_run(r)
            else:
                r = Run(SEQUENCING, result.experiment)
