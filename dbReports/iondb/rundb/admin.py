...
def outbound_net_port():
    ...
    stdout, stderr = script("/sbin/route|awk '/default/{print $8;exit}'")
    port = stdout.strip()
    return port
...
def parse_http_proxy_string(proxy_string):
    ...
    try:
        proxy_string = proxy_string.partition("#")[
            0
        ]  # Remove comments (whole-line or suffixed)
...
def ssh_status(dest_host, dest_port):
    try:
        ...
        if have_proxy:
            ...
            have_proxy, user, password, px_host, px_port = parse_http_proxy_string(
                proxy_string
            )

            if (
                len(user) > 0 and len(password) > 0
            ):  # TODO syntax with user and pass is untested
                ...
                px_cmd = (
                    "ProxyCommand connect -H " + px_host + ":" + str(px_port) + " %h %p"
                )
...
def how_are_you(request, host, port):
    ...
    return http.HttpResponse(
        '{"feeling":"%s"}' % status, mimetype="application/javascript"
    )
...
def network(request):
    ...
    if request.method == "POST" and "network-mode" in request.POST:
        ...
        if form.is_valid():
            ...
            logger.warning(
                "User entered invalid network settings:\n%s" % str(form.errors)
            )
        ...
        return render_to_response(
            ...
            RequestContext(
                request,
                {"network": {"mac": mac_address(), "form": form}, "is_TsVm": is_TsVm()},
            ),
        )
...
def manage(request):
    ...
    if request.method == "GET":
        ...
        return render_to_response("admin/manage.html", {}, RequestContext(request, {}))
...
def update(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            allow_update = (
                files.getSpaceKB("/") > 1048576 and files.getSpaceKB("/var") > 1048576
            )
            if not allow_update:
        ...
        return render_to_response(
            "admin/update.html",
            {
                "about": about,
                "meta": meta_version,
                "show_available": config.ts_update_status
...
def version_lock(request, enable):
    ...
    return render_to_response(
        "admin/update.html", context_instance=RequestContext(request)
    )
...
def tsvm_control(request, action=""):
    if request.method == "GET":
        ...
        return render_to_response(
            "admin/tsvm_control.html", RequestContext(request, ctxd)
        )
        ...
        if "setup" in action:
            ...
            stdout = subprocess.check_output(
                ["sudo", "/opt/ion/iondb/bin/ion_check_for_new_tsvm.py"]
            )
            response_object = json.loads(stdout)
        elif "update" in action:
            response_string = subprocess.check_output(
                ["sudo", "/opt/ion/iondb/bin/install_new_tsvm.py"]
            )
        ...
        return http.HttpResponse(
            json.dumps(response_object), content_type="application/json"
        )
...
def ot_log(request):
    ...
    try:
        ...
        if log[-1] == "DONE":
            log = (
                "OneTouch update finished! <a href='/admin'>You can leave this page</a>"
            )
    except IOError:
...
def updateOneTouch(request):
    ...
    if request.method == "POST":
        ...
        if not os.path.exists("/tmp/OTlock"):
            ...
            try:
                ...
                pass
            async_task = tasks.updateOneTouch.delay()
    ...
    if request.method == "GET":
        ...
        return render_to_response(
            "admin/updateOneTouch.html", {}, RequestContext(request, {})
        )
...
class ExperimentAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status",)
    search_fields = ["expName", "plan__planName"]
    ...
    def redo_from_scratch(self, request, queryset):
        ...
        return HttpResponseRedirect(
            "/admin/experiment/exp_redo_from_scratch/?ids=%s" % (",".join(selected))
        )
...
def exp_redo_from_scratch(request):
    ...
    if request.method == "GET":
        ...
        for exp in exps:
            results_names = [
                "Results: %s" % result
                for result in exp.results_set.values_list("resultsName", flat=True)
            ]
        ...
        return render_to_response(
            "admin/experiment_redo_from_scratch.html", RequestContext(request, context)
        )
    ...
    if request.method == "POST":
        ...
        admin.site._registry[Experiment].message_user(
            request, "%s experiments were deleted." % len(pks)
        )
...
class GlobalConfigAdmin(admin.ModelAdmin):
    ...
    formfield_overrides = {
        models.CharField: {
            "widget": Textarea(attrs={"size": "512", "rows": 4, "cols": 80})
        }
    }
...
class RunTypeAdmin(admin.ModelAdmin):
    ...
    def application_groups(self, obj):
        return ", ".join(
            [applicationGroup.name for applicationGroup in obj.applicationGroups.all()]
        )
...
class PlannedExperimentAdmin(admin.ModelAdmin):
    ...
    search_fields = ["planShortID", "planName"]
    filter_horizontal = ("projects",)
...
class ExperimentAnalysisSettingsAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status",)
    search_fields = ["experiment__expName", "experiment__plan__planName"]
    ordering = ("-id",)
...
class SampleSetItemInline(admin.StackedInline):
    ...
    fields = [
        ("sample", "dnabarcode", "description", "creator"),
        ("nucleotideType", "pcrPlateColumn", "pcrPlateRow"),
        ("controlType", "gender", "relationshipRole", "relationshipGroup"),
        (
            "cancerType",
            "cellularityPct",
            "biopsyDays",
...
class ApplProductAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "isDefault",
        "isVisible",
        "isDefaultForInstrumentType",
        "instrumentType",
        "defaultChipType",
        "categories",
        "defaultLibraryKit",
        "defaultTemplateKit",
        "defaultIonChefPrepKit",
        "defaultSequencingKit",
        "defaultIonChefSequencingKit",
        "defaultFlowCount",
        "isActive",
        "productCode",
    )
