...
def get_csa(request, pk):
    ...
    try:
        ...
        csa_path = makeCSA.makeCSA(
            report_dir, raw_data_dir, None, result.experiment.chefLogPath
        )

        # thumbnails will also include fullchip report pdf
        if result.isThumbnail:
            fullchip = result.experiment.results_set.exclude(
                metaData__contains="thumb"
            ).order_by("-timeStamp")
            if fullchip:
                ...
                try:
                    ...
                    with zipfile.ZipFile(
                        ...
                        f.write(
                            fullchip_pdf, "full_%s" % os.path.basename(fullchip_pdf)
                        )
                except Exception:
                    logger.error(traceback.format_exc())

        response = http.HttpResponse(
            FileWrapper(open(csa_path)), mimetype="application/zip"
        )
...
def get_summary_pdf(request, pk):
    ...
    if pdf_file_contents:
        ...
        response["Content-Disposition"] = (
            "attachment; filename=" + ret.resultsName + "-full.pdf"
        )
    else:
        return HttpResponseRedirect(
            url_with_querystring(reverse("report", args=[pk]), nosummarypdf="True")
        )
...
def get_report_pdf(request, pk):
    ...
    if pdf_file_contents:
        response = http.HttpResponse(pdf_file_contents, mimetype="application/pdf")
        response["Content-Disposition"] = (
            "attachment; filename=" + ret.resultsName + ".pdf"
        )
        return response
    else:
        return HttpResponseRedirect(
...
def get_plugin_pdf(request, pk):
    ...
    if pdf_file_contents:
        response = http.HttpResponse(pdf_file_contents, mimetype="application/pdf")
        response["Content-Disposition"] = (
            "attachment; filename=" + ret.resultsName + "-plugins.pdf"
        )
        return response
    else:
        return HttpResponseRedirect(
            url_with_querystring(reverse("report", args=[pk]), noplugins="True")
        )
...
def getBlockStatus(report, blockdir, namespace="global"):
    ...
    try:
        ...
        analysis_return_code = os.path.join(
        ...
        old_progress_file = os.path.join(
            report.get_report_dir(), blockdir, "progress.txt"
        )
...
def _is_to_show_all_stats(report):
    ...
    isToShowAllStats = (
        False
        if report.experiment and report.experiment.isPQ and not isInternalServer
        else True
    )
...
def testfragments_read(report):
    ...
    try:
        ...
        for tf_name, tf_data in testfragments.items():
            ...
            if "100Q17" in list(tf_data.keys()):
            ...
            if isToShowExtraStats:
                ...
                testfragments[tf_name]["histogram_filename"] = (
                    "new_Q17_%s.png" % tf_name
                )
                testfragments[tf_name]["num_reads"] = num_reads
            else:
                testfragments[tf_name]["conversion_50AQ17"] = (
                    "further investigation needed" if conversion_50AQ17 < 22 else "pass"
                )
                testfragments[tf_name]["conversion_100AQ17"] = NO_VALUE
                testfragments[tf_name]["histogram_filename"] = NO_VALUE
                testfragments[tf_name]["num_reads"] = (
                    "no call" if num_reads < 1000 else num_reads
                )
...
def InitLog_read(report):
        ...
        with open(initLogfile) as f:
            ...
            for line in lines:
                try:
                    ...
                    logger.debug(
                        "UnicodeDecodeError in rundb.report.view.InitLog_read while reading the InitLog.txt"
                    )
                    return {
                        "err": "The InitLog.txt file contains invlaid characters.Please check"
                    }
                ...
                if isValidS5InitLog:
                    ...
                    if len(initLogData) != 2:
                        ...
                        logger.debug(
                            "Input present before RAWTRACE rundb.report.view.InitLog_read: %s"
                            % line
                        )
                        continue
                    else:
    ...
    expected_consumables = {
        "sequenc": "Ion S5 Sequencing Reagents",
    ...
    existing_consumables_generic = [
        expected_consumables[key]
        for key in list(expected_consumables.keys())
        if any(key in xs.lower() for xs in existing_consumables)
    ]

    InitLog_read["missing_consumables"] = list(
        set(expected_consumables.values()) - set(existing_consumables_generic)
    )
    InitLog_read["existing_consumables"] = [
        initLogDict[key] for key in existing_consumables
    ]
...
def basecaller_read(report):
    ...
    try:
        bcd["total_reads"] = filtered_library_isps = basecaller["Filtering"][
            "LibraryReport"
        ]["final_library_reads"]
        bcd["polyclonal"] = basecaller["Filtering"]["LibraryReport"][
            "filtered_polyclonal"
        ]
        bcd["low_quality"] = basecaller["Filtering"]["LibraryReport"][
            "filtered_low_quality"
        ]
        bcd["primer_dimer"] = basecaller["Filtering"]["LibraryReport"][
            "filtered_primer_dimer"
        ]
...
def _basecaller_basic_png(report):
    ...
    basic_beadogram_dir = os.path.join(
        report.get_report_dir(), "basecaller_results/wells_beadogram_basic.png"
    )
    if os.path.exists(basic_beadogram_dir):
        return

    logger.debug(
        "going to create wells_beadogram_basic,png basic_beadogram_dir=%s"
        % (basic_beadogram_dir)
    )
    ...
    try:
        wells_beadogram.generate_wells_beadogram_basic(
            basecaller_results, sigproc_results
        )
    except Exception:
...
def getFormatForVal(value):
    if numpy.isnan(value):
        value = (
            0
        )  # Value here doesn't really matter b/c printing nan will always result in -nan
    if value == 0:
        value = 0.01
...
def report_chef_display(report):
    ...
    chef_info = _get_value_from_experiment(
        report.experiment, chef_infoList_experiment, []
    )

    plan_samplePrepProtocol = ("samplePrepProtocol", "Templating Protocol Planned")
    chef_info.insert(
        17,
        _get_chef_protocol_value_from_plan(
            report.experiment.plan, plan_samplePrepProtocol
        ),
    )
    return chef_info
...
def report_S5_consumable_display(report):
    ...
    try:
        ...
        if "Chip Type" not in chip_efuseOrderedDict:
            logger.debug(
                "Warning for Chip Type in report_S5_consumable_display: %s" % warning
            )
        ...
        if "Chip Barcode" not in chip_efuseOrderedDict:
            logger.debug(
                "Warning for Chip Barcode in report_S5_consumable_display: %s" % warning
            )
            if expChipBarcode:
                chip_efuseOrderedDict["Chip Barcode"] = expChipBarcode
    except Exception as Err:
        logger.debug(
            "ERROR in rundb.report.view.report_S5_consumable_display: %s" % Err
        )
        return {"err": get_msgDesc()["002"]}

    if not chip_efuseOrderedDict:
        chip_efuseOrderedDict["Chip Type"] = get_msgDesc()["001"].format("Chip Type")
        chip_efuseOrderedDict["Chip Barcode"] = get_msgDesc()["001"].format(
            "Chip Barcode"
        )
...
def report_chef_libPrep_display(report):
    ...
    libPrepSampleSets = (
        plan.sampleSets.filter(libraryPrepInstrumentData__isnull=False) if plan else []
    )
...
def find_output_file_groups(report, datasets, barcodes):
    ...
    current_group["vcf"] = "%s/plugin_out/variantCaller_out/TSVC_variants.vcf.gz" % (
        web_link,
    )
    output_file_groups.append(current_group)
    logger.info("current_group: %s", current_group)
    ...
    if datasets and "barcode_config" in datasets:
        ...
        for barcode in barcodes:
            ...
            if report.eas.reference or report.eas.barcoded_samples_reference_names:
                barcode["bam_link"] = "%s%s/%s_%s_%s.bam" % (
                    web_link,
                    download_dir,
                    re.sub("_rawlib$", "", barcode["file_prefix"]),
                    report.experiment.expName,
                    report.resultsName,
                )
                barcode["bai_link"] = "%s%s/%s_%s_%s.bam.bai" % (
                    web_link,
                    download_dir,
                    re.sub("_rawlib$", "", barcode["file_prefix"]),
                    report.experiment.expName,
                ...
                barcode["bai_link"] = "%s/basecaller_results/%s.basecaller.bam.bai" % (
                    web_link,
                    barcode["file_prefix"],
                )
            ...
            for key in ["basecaller_bam_link", "bam_link", "bai_link"]:
                if not (
                    barcode[key]
                    and os.path.exists(barcode[key].replace(web_link, report_path))
                ):
                    barcode[key] = None
...
def get_recalibration_panel(datasets):
    ...
    if datasets and "IonControl" in datasets:
        sorted_groups = sorted(
            list(datasets["IonControl"]["read_groups"].values()),
            key=lambda d: d.get("barcode_name", None),
        )
    ...
    for filename in files:
        for folder in subfolders:
            if os.path.isfile(os.path.join(report_dir, folder, filename)):
                source_files[filename] = os.path.normpath(
                    os.path.join(reportWebLink, folder, filename)
                )
                break
    return source_files
...
def _report_context(request, report_pk):
    ...
    otherReports = report.experiment.results_set.exclude(pk=report_pk).order_by(
        "-timeStamp"
    )
    ...
    pluginList = report.pluginresult_set.all().select_related(
        "result", "result__reportstorage", "plugin"
    )
    for major_plugin in pluginList:
        if major_plugin.plugin.majorBlock:
            ...
            try:
                majorPluginFiles = glob.glob(
                    os.path.join(major_plugin.path(), "*_block.html")
                )[0]
                majorPluginImages = glob.glob(
                    os.path.join(
                        report.get_report_dir(),
                        "pdf",
            ...
            if majorPluginImages:
                major_plugins_images[major_plugin.plugin.name] = sorted(
                    majorPluginImages
                )
            else:
    ...
    read_stats = ionstats_read_stats(
        report
    ...
    if report.resultsType and report.resultsType == "CombinedAlignments":
        ...
        CA_barcodes = []
        try:
            CA_barcodes_json_path = os.path.join(
                report.get_report_dir(), "CA_barcode_summary.json"
            )
                    ...
                    CA_barcodes.append(
                        {
                            ...
                            "AQ7_num_bases": CA_barcode[
                                "Filtered_Mapped_Bases_in_Q7_Alignments"
                            ],
                            "full_num_reads": CA_barcode["Total_number_of_Reads"],
                            "AQ7_mean_read_length": CA_barcode[
                                "Filtered_Q7_Mean_Alignment_Length"
                            ],
                        }
                    )
        except Exception:
        ...
        try:
            ...
            parents = [
                (pk, name)
                for pk, name in zip(paramsJson["parentIDs"], paramsJson["parentNames"])
            ]
    ...
    try:
        qcTypes = dict(
            qc
            for qc in report.experiment.plan.plannedexperimentqc_set.all().values_list(
                "qcType__qcName", "threshold"
            )
        )
    except Exception:
        qcTypes = {}
    ...
    key_signal_details = _(
        "report.section.unaligned.beadfind.fields.keysignal.details"
    ) % {
        "key_signal_threshold": key_signal_threshold
    }  # Your quality threshold is <strong>{{key_signal_threshold}}</strong><br/>Average 1-mer signal in library key<br/>The colored bars indicate the quality threshold set in your plan, blue is above and red is under.
    ...
    try:
        if "Adjusted Addressable Wells" in beadfind:
            ...
            addressable_wells = int(beadfind["Total Wells"]) - int(
                beadfind["Excluded Wells"]
            )
        ...
        bead_loading_details = _(
            "report.section.unaligned.beadfind.fields.bead_loading.details"
        ) % {
            "bead_loading_threshold": bead_loading_threshold
        }  # Your quality threshold is <strong>{{bead_loading_threshold}}%</strong><br/>The colored bars indicate the quality threshold set in your plan, blue is above and red is under.

        beadsummary = dict()
        beadsummary["total_addressable_wells"] = addressable_wells
        beadsummary["bead_wells"] = beadfind["Bead Wells"]
        beadsummary["p_bead_wells"] = percent(
            beadfind["Bead Wells"], beadsummary["total_addressable_wells"]
        )
        beadsummary["live_beads"] = beadfind["Live Beads"]
        beadsummary["p_live_beads"] = percent(
            beadfind["Live Beads"], beadfind["Bead Wells"]
        )
        beadsummary["test_fragment_beads"] = beadfind["Test Fragment Beads"]
        beadsummary["p_test_fragment_beads"] = percent(
            beadfind["Test Fragment Beads"], beadfind["Live Beads"]
        )
        beadsummary["library_beads"] = beadfind["Library Beads"]
        beadsummary["p_library_beads"] = percent(
            beadfind["Library Beads"], beadfind["Live Beads"]
        )
    except Exception:
        logger.warn(
            "Failed to build Beadfind report content for %s." % report.resultsName
        )

    # Basecaller
    try:
        usable_sequence = basecaller and int(
            round(
                100.0
                * float(basecaller["total_reads"])
                / float(beadfind["Library Beads"])
            )
        )
        usable_sequence_threshold = qcTypes.get("Usable Sequence (%)", 0)
        usable_sequence_details = _(
            "report.section.unaligned.basecaller.fields.usable_sequence.details"
        ) % {
            "usable_sequence_threshold": usable_sequence_threshold
        }  # Your quality threshold is <strong>{{usable_sequence_threshold}}%</strong><br/>The colored bars indicate the quality threshold set in your plan, blue is above and red is under.'}
        # quality = load_ini(report,"basecaller_results","quality.summary")

        basecaller["p_primer_dimer"] = percent(
            basecaller["primer_dimer"], beadfind["Library Beads"]
        )
        basecaller["p_total_reads"] = percent(
            basecaller["total_reads"], beadfind["Library Beads"]
        )
        ...
        if basecaller["isToShowClonality"]:
            basecaller["p_polyclonal"] = percent(
                basecaller["polyclonal"], beadfind["Library Beads"]
            )
            basecaller["p_low_quality"] = percent(
                basecaller["low_quality"], beadfind["Library Beads"]
            )
        else:
            basecaller["p_low_quality"] = percent(
                basecaller["polyclonal"] + basecaller["low_quality"],
                beadfind["Library Beads"],
            )
            basecaller["p_polyclonal"] = 0
            basecaller["isToShowClonality"] = False

    except Exception:
        logger.warn(
            "Failed to build Basecaller report content for %s." % report.resultsName
        )
    ...
    if not ionstats_alignment:
        ...
        if alignStats and alignment_ini:
            ionstats_alignment["aligned"] = {
                "num_bases": alignStats["total_mapped_target_bases"]
            }
            ionstats_alignment["error_by_position"] = [
                alignStats["accuracy_total_errors"]
            ]
            ionstats_alignment["AQ17"] = {
                "num_bases": alignment_ini["Filtered Mapped Bases in Q17 Alignments"],
            ...
            ionstats_alignment["AQ20"] = {
                "num_bases": alignment_ini["Filtered Mapped Bases in Q20 Alignments"],
            ...
            ionstats_alignment["AQ47"] = {
                "num_bases": alignment_ini["Filtered Mapped Bases in Q47 Alignments"],
    ...
    # Alignment
    try:
        reference = models.ReferenceGenome.objects.filter(
            short_name=eas_reference
        ).order_by("-index_version")[0]
    ...
    if reference:
        try:
            if genome_length:
                avg_coverage_depth_of_target = round(
                    float(ionstats_alignment["aligned"]["num_bases"]) / genome_length, 1
                )
                avg_coverage_depth_of_target = str(avg_coverage_depth_of_target) + "X"
                for c in ["AQ17", "AQ20", "AQ47"]:
                    try:
                        ionstats_alignment[c]["mean_coverage"] = round(
                            float(ionstats_alignment[c]["num_bases"]) / genome_length, 1
                        )
                    except Exception:
                        pass
            ...
            if float(ionstats_alignment["aligned"]["num_bases"]) > 0:
                raw_accuracy = round(
                    (
                        1
                        - float(sum(ionstats_alignment["error_by_position"]))
                        / float(ionstats_alignment["aligned"]["num_bases"])
                    )
                    * 100.0,
                    1,
                )
            ...
            try:
                for c in ["aligned"]:
                    ionstats_alignment[c]["p_num_reads"] = (
                        100.0
                        * ionstats_alignment[c]["num_reads"]
                        / float(ionstats_alignment["full"]["num_reads"])
                    )
                    ionstats_alignment[c]["p_num_bases"] = (
                        100.0
                        * ionstats_alignment[c]["num_bases"]
                        / float(ionstats_alignment["full"]["num_reads"])
                    )

                ionstats_alignment["unaligned"] = {}
                ionstats_alignment["unaligned"]["num_reads"] = int(
                    ionstats_alignment["full"]["num_reads"]
                ) - int(ionstats_alignment["aligned"]["num_reads"])
                # close enough, and ensures they sum to 100 despite rounding
                ionstats_alignment["unaligned"]["p_num_reads"] = (
                    100.0 - ionstats_alignment["aligned"]["p_num_reads"]
                )
            ...
            logger.warn(
                "Failed to build Alignment report content for %s." % report.resultsName
            )
    ...
    if report.libmetrics and report.libmetrics.duplicate_reads is not None:
        ...
        if report.libmetrics.totalNumReads:
            duplicate_metrics["duplicate_read_percentage"] = (
                100.0
                * report.libmetrics.duplicate_reads
                / float(report.libmetrics.totalNumReads)
            )
        else:
    ...
    isToShowAlignmentStats = (
        False if experiment.plan and experiment.plan.runType == "RNA" else True
    )
    alternateAlignmentStatsMessage = _(
        "report.section.aligned.messages.noalignmentstatsshown"
    )  # "For RNA Sequencing, please refer to the RNASeq Analysis plugin output for alignment statistics "
        ...
        if _should_build_report_for_blocks(isInternalServer, report, experiment):
            ...
            for b in proton_log_blocks:
                if not "thumbnail" in b:
                    ...
                    proton_block_tuples.append(
                        (int(xoffset[1:]), int(yoffset[1:]), directory)
                    )
            # sort by 2 keys
            sorted_block_tuples = sorted(
                proton_block_tuples, key=lambda block: (-block[1], block[0])
            )
            ...
            for block in pb:
                ...
                proton_blocks.append(
                    ProtonResultBlock(blockdir, getBlockStatus(report, blockdir))
                )
    except Exception as err:
        logger.exception(
            "Failed to create proton block content for report [%s]."
            % report.get_report_dir()
        )
    ...
    logger.info("output_file_groups: %s", output_file_groups)
    ...
    context["no_live_beads_status_message"] = _(
        "report.section.summary.messages.no_live_beads_status"
    ...
    context["do_show_progress"] = (
        ...
        # See: TS-9782
        report.status
        not in ("Completed", "No Live Beads")
    )

    context["progress_message"] = report.i18nstatus()
    ...
    if report.status == "No Live Beads":
        context["disable_actions"].extend(
            ["upload_to_ir", "reanalyze", "run_plugins", "report_pdf"]
        )
...
def _should_build_report_for_blocks(isInternalServer, report, experiment):
    ...
    return (
        isInternalServer
        and len(report.experiment.log.get("blocks", "")) > 0
        and not report.isThumbnail
    )
...
def report_display(request, report_pk):
    try:
        ...
        html_path = (
            "rundb/reports/printreport.tex"
            if request.GET.get("latex", False)
            else "rundb/reports/report.html"
        )
        return render_to_response(html_path, context_instance=ctx)
    except Exception as exc:
        return HttpResponseRedirect(
            url_with_querystring(
                reverse("report_log", kwargs={"pk": report_pk}), error=str(exc)
            )
        )
...
def report_log(request, pk):
    ...
    if not dmfilestat.isdisposed():
        ...
        file_links.append(
            (
                "Classic Report",
                _("report.classic.name"),
                os.path.join(report_link, name),
                os.path.exists(os.path.join(root_path, name)),
            )
        )
    ...
    for name in file_names:
        file_links.append(
            (
                name,
                name,
                reverse("report_metal", args=[report.pk, name]),
                os.path.exists(os.path.join(root_path, name)),
            )
        )
    ...
    if dmfilestat.action_state == "AD":
        serialized_json_path = os.path.join(
            dmfilestat.archivepath, "serialized_%s.json" % report.resultsName
        )
    ...
    return render_to_response(
        "rundb/reports/report_log.html", context, RequestContext(request)
    )
...
def update_experiment_analysis_settings(eas, **kwargs):
    ...
    for key, new_value in list(kwargs.items()):
...
def analyze(request, exp_pk, report_pk):
    ...
    selected_names = [pl["name"] for pl in list(eas.selectedPlugins.values())]
    ...
    if post_dict:
        ...
        try:
            ...
            return render_to_response(
                "rundb/reports/analysis_started.html", context_instance=ctx
            )
        ...
        return render_to_response(
            "rundb/reports/analyze.html", context_instance=RequestContext(request, ctxd)
        )
...
def reanalyze(request, exp, eas, plugins_list, start_from_report=None):
    ...
    params = False
    javascript = ""
    has_thumbnail = exp.getPlatform in ["s5", "proton"] or exp.rawdatastyle == "tiled"
    configuration_errors = list()

    # get the list of report addresses
    resultList = models.Results.objects.filter(
        experiment=exp, status__contains="Completed"
    ).order_by("timeStamp")
    previousReports = []
    ...
    for plugin in plugins_list:
        pluginsUserInput[str(plugin.id)] = eas.selectedPlugins.get(plugin.name, {}).get(
            "userInput", ""
        )
    ...
    if resultList:
        ...
        if dmfilestat.isdisposed():
            warnings["sigproc"] = (
                "Warning: Signal Processing Input data is %s"
        ...
        for r in resultList:
            ...
            if dmfilestat.isdisposed():
                warnings[r.pk] = (
                    "Warning: Basecalling Input data is %s"
                    % dmfilestat.get_action_state_display()
                )
            else:
                report_dir = r.get_report_dir()
                if not os.path.exists(report_dir) or not os.path.exists(
                    os.path.join(report_dir, "sigproc_results")
                ):
    ...
    if eas.barcodedSamples:
        for sample, value in list(eas.barcodedSamples.items()):
            try:
                    ...
                    barcodesWithSamples.append(
                        {
                            "sample": sample,
                            "barcodeId": bcId,
                            "reference": info.get(bcId, {}).get("reference")
                            if "reference" in info.get(bcId, {})
                            else eas.reference,
                            "hotSpotRegionBedFile": info.get(bcId, {}).get(
                                "hotSpotRegionBedFile"
                            )
                            if "hotSpotRegionBedFile" in info.get(bcId, {})
                            else eas.hotSpotRegionBedFile,
                            "targetRegionBedFile": info.get(bcId, {}).get(
                                "targetRegionBedFile"
                            )
                            if "targetRegionBedFile" in info.get(bcId, {})
                            else eas.targetRegionBedFile,
    ...
    if not best_match_entry:
        warnings["all"] = (
            "Warning: Analysis Parameters not found for chip type: %s <br>"
            % exp.chipType
        )
    ...
    if request.method == "POST":
        ...
        if rpf.is_valid() and eas_form.is_valid():
            ...
            thumbnailBeadfindArgs = flattenString(
                rpf.cleaned_data["thumbnailBeadfindArgs"]
            )
            thumbnailAnalysisArgs = flattenString(
                rpf.cleaned_data["thumbnailAnalysisArgs"]
            )
            prethumbnailBasecallerArgs = flattenString(
                rpf.cleaned_data["prethumbnailBasecallerArgs"]
            )
            thumbnailCalibrateArgs = flattenString(
                rpf.cleaned_data["thumbnailCalibrateArgs"]
            )
            thumbnailBasecallerArgs = flattenString(
                rpf.cleaned_data["thumbnailBasecallerArgs"]
            )
            thumbnailAlignmentArgs = flattenString(
                rpf.cleaned_data["thumbnailAlignmentArgs"]
            )
            thumbnailIonstatsArgs = flattenString(
                rpf.cleaned_data["thumbnailIonstatsArgs"]
            )
            ...
            form_pluginsUserInput = (
                json.loads(eas_form.cleaned_data["pluginsUserInput"])
                if eas_form.cleaned_data["pluginsUserInput"]
                else {}
            )
            if (
                set(plugins_list) != set(form_plugins_list)
                or pluginsUserInput != form_pluginsUserInput
            ):
            ...
            if rpf.cleaned_data["blockArgs"] == "fromWells":
                try:
                    previousReport = (
                        rpf.cleaned_data["previousThumbReport"]
                        if rpf.cleaned_data.get("do_thumbnail")
                        else rpf.cleaned_data["previousReport"]
                    )
                    ...
                    if (
                        previousEAS.thumbnailbeadfindargs
                        and previousEAS.thumbnailanalysisargs
                    ):
            ...
            if barcodedReferences:
                ...
                if barcodedReferences.get("unsupported", False):
                    ...
                    for sample in list(barcodedSamples.values()):
                        for barcode, info in list(
                            sample.get("barcodeSampleInfo", {}).items()
                        ):
                            info["reference"] = (
                                barcodedReferences[barcode]["reference"]
                                if barcodedReferences[barcode]["reference"] != "none"
                                else ""
                            )
                            # blank the SSE file if target region BED file changed
                            if (
                                info["targetRegionBedFile"]
                                != barcodedReferences[barcode]["targetRegionBedFile"]
                            ):
                                info["sseBedFile"] = ""
                            info["targetRegionBedFile"] = barcodedReferences[barcode][
                                "targetRegionBedFile"
                            ]
                            info["hotSpotRegionBedFile"] = barcodedReferences[barcode][
                                "hotSpotRegionBedFile"
                            ]
            ...
            planNucleotideType = (
                exp.plan.get_default_nucleotideType() if exp.plan else ""
            )
            ...
            eas_kwargs = {
                ...
                or globalConfig.default_library_key,
                "tfKey": rpf.cleaned_data["tfKey"]
                or globalConfig.default_test_fragment_key,
                "reference": eas_form.cleaned_data["reference"]
                if eas_form.cleaned_data["reference"] != "none"
                else "",
                ...
                "sseBedFile": eas.sseBedFile
                if eas_form.cleaned_data["targetRegionBedFile"]
    ...
    if request.method == "GET":
        ...
        rpf.fields["thumbnailBeadfindArgs"].initial = eas_args_dict[
            "thumbnailbeadfindargs"
        ]
        rpf.fields["thumbnailAnalysisArgs"].initial = eas_args_dict[
            "thumbnailanalysisargs"
        ]
        rpf.fields["thumbnailBasecallerArgs"].initial = eas_args_dict[
            "thumbnailbasecallerargs"
        ]
        rpf.fields["prethumbnailBasecallerArgs"].initial = eas_args_dict[
            "prethumbnailbasecallerargs"
        ]
        rpf.fields["thumbnailCalibrateArgs"].initial = eas_args_dict[
            "thumbnailcalibrateargs"
        ]
        rpf.fields["thumbnailAlignmentArgs"].initial = eas_args_dict[
            "thumbnailalignmentargs"
        ]
        rpf.fields["thumbnailIonstatsArgs"].initial = eas_args_dict[
            "thumbnailionstatsargs"
        ]
        ...
        rpf.fields[
            "do_base_recal"
        ].widget.choices = eas.get_base_recalibration_mode_choices()
        ...
        rpf.fields["tfKey"].initial = (
            eas.tfKey or globalConfig.default_test_fragment_key
        )
        ...
        eas_form.fields["pluginsUserInput"].initial = json.dumps(
            pluginsUserInput, cls=DjangoJSONEncoder
        )

        barcodeKits = models.dnaBarcode.objects.filter(
            Q(active=True) | Q(name=eas.barcodeKitName)
        )
        eas_form.fields["barcodeKitName"].choices = [("", "")] + list(
            barcodeKits.order_by("name").distinct("name").values_list("name", "name")
        )
...
def show_png_image(request, full_path):
    return HttpResponse(open(full_path, "rb"), mimetype="image/png")


def show_csv(request, full_path):
    reader = csv.reader(open(full_path))
    return render_to_response(
        "rundb/reports/metal/show_csv.html",
        context_instance=RequestContext(request, {"table_rows": reader}),
    )


def show_whitespace_csv(request, full_path):
...
def show_config(request, full_path):
    ...
    return render_to_response(
        "rundb/reports/metal/show_config.html",
        context_instance=RequestContext(request, {"config": reader}),
    )


def indent_json(request, full_path):
...
BINARY_FILTER = "".join([chr(x) if 32 <= x <= 126 else "." for x in range(256)])


def show_binary(request, full_path, max_read=100000):
...
def show_file(request, report, pk, path, root, full_path):
    for pattern, handle in FILE_HANDLERS:
        if pattern.search(path):
            try:
                return handle(request, full_path)
            except Exception as err:
                raise err
    raise OSError(
        "File path matches no pattern, path = '%' full_path='%s'" % (path, full_path)
    )


def show_directory(request, report, pk, path, root, full_path):
                    ...
                    if os.path.exists(rawDataPath):
                        try:
                            if not os.path.lexists(os.path.join(full_path, "rawdata")):
                                ...
                                os.symlink(
                                    rawDataPath, os.path.join(full_path, "rawdata")
                                )
                            else:
                                logger.debug(
                                    "Raw Data folder exists already. Add the rawdata to weblink"
                                )
                        except Exception as err:
                            logger.debug("SymLink creation failed due to %s" % err)
                    else:
                        logger.debug(
                            "Raw Data folder does not exists. Link to the rawdata would not be displayed"
                        )
...
def metal(request, pk, path):
    ...
    if os.path.isdir(full_path):
        return show_directory(request, report, pk, path, root, full_path)
    else:
        return show_file(request, report, pk, path, root, full_path)
...
def getZip(request, pk):
    try:
        ...
        fullPath = os.path.join(
            fullPath, "plugin_out", "FileExporter_out", "downloads.zip"
        )
...
def getVCF(request, pk):
    ...
    for V in VCNames:
        ...
        for VC in VCPaths:
            if V in VC:
                mkCmd = subprocess.Popen(
                    ["mkdir", os.path.join(fullPath, "FileExporter_out", V)]
                )
                    ...
                    if "/" in useablePath:
                        ...
                        mkCmd = subprocess.Popen(
                            [
                                "mkdir",
                                os.path.join(
                                    fullPath, "FileExporter_out", V, useablePath[0]
                                ),
                            ]
                        )
                        mkOut, mkErr = mkCmd.communicate()
    ...
    if plugin.path and plugin.script and plugin.script.endswith(".py"):
        try:
            ...
            if columns:
                if not data:
                    return HttpResponse(
                        '<div class="alert alert-error">Error: unable to retrieve samples table data</div>'
                    )
                else:
                    ctxd = {
                    ...
                    return render_to_response(
                        "rundb/reports/blocks/plugin_barcodes_table.html",
                        context_instance=RequestContext(request, ctxd),
                    )
        except Exception:
            ...
            logger.error(err)
            return HttpResponse(
                '<div class="alert alert-error">Error: unable to retrieve samples table<br>%s</div>'
                % err
            )
