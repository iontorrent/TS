...
class Run:
    ...
    def update_application_and_samples(self, plan):
        ...
        self.applicationCategoryDisplayedName = (
            plan.get_composite_applCatDisplayedName()
        )
        ...
        if barcodedSamples:
            samples_list = []
            for key, value in list(barcodedSamples.items()):
                if value.get("dualBarcodes"):
                    barcode_str = " ".join(sorted(value["dualBarcodes"]))
                else:
                    barcode_str = (
                        " ".join(sorted(value["barcodes"]))
                        if "barcodes" in value
                        else ""
                    )

                samples_list.append({"sampleName": key, "barcodes": barcode_str})
            self.barcodedSamples = sorted(samples_list, key=lambda x: x["barcodes"])

    def update_thumbnail(self, exp):
        results = exp.results_set.filter(
            resultsName__startswith="Auto_", metaData__contains="thumb"
        )
        if results:
            thumb = results[0]
    ...
    def create_plugins(self, result):
        ...
        active_plugins = pr_set.filter(
            plugin_result_jobs__state__in=RUNNING_STATES
        ).distinct()
        ...
        if n_error > 0:
            ...
            self.error_string = "%d plugin%s in Error" % (
                n_error,
                "s" if n_error > 1 else "",
            )
    ...
    def create_library_prep(self, sampleSet):
        ...
        # samples
        sampleSetItems = sampleSet.samples.order_by("dnabarcode__id_str").values_list(
            "sample__displayedName", "dnabarcode__id_str"
        )
        samples = {}
...
def results_in_range(runs, timelimit):
    ...
    """
    results = (
        Results.objects.filter(timeStamp__gt=timelimit)
        .exclude(pk__in=runs.active_results_ids)
        .exclude(metaData__contains="thumb")
    )
...
def add_active_analysis(runs):
    ...
    # need better way to tell reanalysis vs auto run
    results = results.filter(resultsName__startswith="Auto_").exclude(
...
def sequencing_in_range(runs, timelimit):
    ...
    """
    experiments = Experiment.objects.filter(status="run", date__gt=timelimit).exclude(
        ftpStatus="Complete"
    )
...
def add_active_plugins(runs):
    ...
    results = Results.objects.filter(
        pluginresult_set__plugin_result_jobs__state__in=RUNNING_STATES
    ).distinct()
    # limit to recent results
    active_plugins_timelimit = datetime.now(pytz.UTC) - timedelta(
        hours=ACTIVE_PLUGINS_LIMIT_HRS
    )
    results = results.filter(timeStamp__gt=active_plugins_timelimit).exclude(
        pk__in=runs.active_results_ids
    )
    # need better way to tell reanalysis vs auto run
    results = results.filter(resultsName__startswith="Auto_")
...
def add_template_prep(runs, timelimit):
    ...
    experiments = Experiment.objects.filter(
        plan__planExecuted=False, chefLastUpdate__gte=timelimit
    )
    for exp in experiments:
...
def add_library_prep(runs, timelimit):
    ...
    sampleSets = SampleSet.objects.filter(
        libraryPrepInstrumentData__lastUpdate__gte=timelimit
    )
