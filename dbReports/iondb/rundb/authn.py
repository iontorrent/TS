...
class IonAuthentication(MultiAuthentication):
    ...
    def __init__(
        self, allow_get=None, ion_mesh_access=False, secure_only=False, **kwargs
    ):
        backends = [
            # Basic must be first, so it sets WWW-Authenticate header in 401.
            BasicAuthentication(realm="Torrent Suite"),  ## Apache Basic Auth
            SessionAuthentication(),
    ...
    def is_authenticated(self, request, **kwargs):
        ...
        unauthorized = super(IonAuthentication, self).is_authenticated(
            request, **kwargs
        )
