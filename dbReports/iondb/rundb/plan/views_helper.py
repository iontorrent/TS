...
def get_projects(username, json_data):
    ...
    if projectIdAndNameList:
        ...
        for missing in missings:
            logger.debug(
                "views.editplannedexperiment project= %s is no longer in db" % missing
            )
    # new projects added
    newProjectNames = json_data.get("newProjects", "")
    if newProjectNames:
        newProjectNames = newProjectNames.split(",")
        projectObjList.extend(
...
def dict_bed_hotspot():  # TODO: Move to models.Content or models.Publisher or models as BEDHotspotDetails Class method.
    data = {}
    allFiles = Content.objects.filter(
        publisher__name="BED", enabled=True, path__contains="/unmerged/detail/"
    ).order_by("path")
...
def get_available_plan_count():
    ...
    return (
        PlannedExperiment.objects.filter(isReusable=False, planExecuted=False)
        .exclude(planStatus="voided")
        .count()
    )
...
def get_IR_accounts_by_userName(userName):
    ...
    if userName:
        ...
        iru_plugins = Plugin.objects.filter(
            name__iexact="IonReporterUploader", selected=True, active=True
        )
        if iru_plugins:
...
def get_template_categories():
    ...
    categories = [
        # Favorites
        {
            "tag": "favorites",
            "displayedName": _(
                "plan.templates.categories.favorites.displayedName"
            ),  # Favorites
        ...
        # Recently Created
        {
            ...
            "displayedName": _(
                "plan.templates.categories.recently_created.displayedName"
            ),  # All
            "api_filter": "",
    ...
    if settings.FEATURE_FLAGS.PLAN_TEMPLATE_RESEARCHAPPLICATIONS:
        ...
        applicationGroup_tagSeq = ApplicationGroup.objects.filter(
            name="onco_liquidBiopsy"
        )
        applicationGroup_immuneRepertoire = ApplicationGroup.objects.filter(
            name="immune_repertoire"
        )
        applicationGroup_mutationLoad = ApplicationGroup.objects.filter(
            name="mutation_load"
        )

        category_solidTumor = common_CV.objects.filter(
            cv_type="applicationCategory", value="onco_solidTumor"
        )
        category_onco_immune = common_CV.objects.filter(
            cv_type="applicationCategory", value="onco_immune"
        )
        category_repro = common_CV.objects.filter(
            cv_type="applicationCategory", value="repro"
        )
        category_16s = common_CV.objects.filter(
            cv_type="applicationCategory", value="16s"
        )
        category_inheritedDisease = common_CV.objects.filter(
            cv_type="applicationCategory", value="inheritedDisease"
        )
        category_oncoHeme = common_CV.objects.filter(
            cv_type="applicationCategory", value="onco_heme"
        )
        category_immune = common_CV.objects.filter(
            cv_type="applicationCategory", value="immunology"
        )
        category_carrierSeq = common_CV.objects.filter(
            cv_type="applicationCategory", value="carrierSeq"
        )
        ...
        categories.extend(
            [
                ...
                # HID
                {
                    ...
                    "isActive": applicationGroup_HID[0].isActive
                    if applicationGroup_HID
                    else False,
                    "ampliSeq_upload": True,
                    "code": 11,
                },
                # Immune Repertoire
                {
                    ...
                    "isActive": applicationGroup_immuneRepertoire[0].isActive
                    if applicationGroup_immuneRepertoire
                    else False,
                    "code": 12,
                },
                # immunology
                ...
                # Inherited disease
                {
                    ...
                    "displayedName": category_inheritedDisease[0].displayedValue
                    if category_inheritedDisease
                    else "--",
                    "api_filter": "&categories__icontains=inheritedDisease",
                    "img": "resources/img/appl_category_inherited_disease.png",
                    "isActive": category_inheritedDisease[0].isActive
                    if category_inheritedDisease
                    else False,
                    "ampliSeq_upload": True,
                    "code": 1,
                ...
                # Mutation load
                {
                    ...
                    "isActive": applicationGroup_mutationLoad[0].isActive
                    if applicationGroup_mutationLoad
                    else False,
                    "code": 13,
                },
                # Oncology - hemeOnc
                {
                    ...
                    "displayedName": category_oncoHeme[0].displayedValue
                    if category_oncoHeme
                    else "--",
                    "api_filter": "&categories__icontains=onco_heme",
                    "img": "resources/img/appl_category_onco_heme.png",
                    "isActive": category_oncoHeme[0].isActive
                    if category_oncoHeme
                    else False,
                    "ampliSeq_upload": True,
                    "code": 1,
                ...
                # Oncology - immunology
                {
                    ...
                    "displayedName": category_onco_immune[0].displayedValue
                    if category_onco_immune
                    else "--",
                    "api_filter": "&categories__icontains=onco_immune",
                    "img": "resources/img/appl_category_onco_immune.png",
                    "isActive": category_onco_immune[0].isActive
                    if category_onco_immune
                    else False,
                    "ampliSeq_upload": True,
                    "code": 1,
                ...
                # Oncology - Liquid Biopsy
                {
                    ...
                    "isActive": applicationGroup_tagSeq[0].isActive
                    if applicationGroup_tagSeq
                    else False,
                    "code": 10,
                },
                # Oncology - solid tumor
                {
                    ...
                    "displayedName": category_solidTumor[0].displayedValue
                    if category_solidTumor
                    else "--",
                    "api_filter": "&categories__icontains=onco_solidTumor",
                    "img": "resources/img/appl_category_onco_solid_tumor.png",
                    "isActive": category_solidTumor[0].isActive
                    if category_solidTumor
                    else False,
                    "ampliSeq_upload": True,
                    "code": 8,
                ...
                # Pharmacogenomics
                {
                    ...
                    "isActive": applicationGroup_PGx[0].isActive
                    if applicationGroup_PGx
                    else False,
                    "ampliSeq_upload": True,
                    "code": 9,
                },
                ...
                {
                    "tag": "category_repro",
                    "displayedName": category_repro[0].displayedValue
                    if category_repro
                    else "--",
                    "api_filter": "&categories__icontains=repro",
                    "img": "resources/img/repro2.svg",
                ...
                {
                    "tag": "category_16s",
                    "displayedName": category_16s[0].displayedValue
                    if category_16s
                    else "--",
                ...
                # Carrier Sequencing
                {
                    "tag": "category_carrierSeq",
                    "displayedName": category_carrierSeq[0].displayedValue
                    if category_carrierSeq
                    else "--",
                    "api_filter": "&categories__icontains=carrierSeq",
                    "img": "resources/img/carrier-screen.svg",
                    "isActive": category_carrierSeq[0].isActive
                    if category_carrierSeq
                    else False,
                    "ampliSeq_upload": True,
                    "code": 3,
