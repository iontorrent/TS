...
class ApplicationFieldNames:
    ...
    APPL_PRODUCTS = (
        "applProducts"
    )  # this is for all applProduct definitions for the selected application and target technique
    APPL_PRODUCTS_CATEGORIZED = (
        "applProducts_categorized"
    )  # this is for all categorized applProduct definitions for the selected application and target technique
    RUN_TYPE = "runType"
...
class ApplicationStepData(AbstractStepData):
    def __init__(self, sh_type, prev_step_url=reverse_lazy("page_plan_ionreporter")):
        ...
        self.prepopulatedFields[ApplicationFieldNames.RUN_TYPES] = list(
            RunType.objects.filter(isActive=True).order_by("description")
        )
        ...
        self.prepopulatedFields[
            ApplicationFieldNames.APPLICATION_GROUPS
        ] = ApplicationGroup.objects.filter(isActive=True).order_by("description")
    ...
    def updateSavedObjectsFromSavedFields(self):
        ...
        if self.savedFields[ApplicationFieldNames.RUN_TYPE]:
            self.savedObjects[ApplicationFieldNames.RUN_TYPE] = RunType.objects.get(
                pk=self.savedFields[ApplicationFieldNames.RUN_TYPE]
            )
            self.savedObjects[
                ApplicationFieldNames.APPL_PRODUCT
            ] = ApplProduct.get_default_for_runType(
                self.savedObjects[ApplicationFieldNames.RUN_TYPE].runType,
                applicationGroupName=self.savedFields[
                    ApplicationFieldNames.APPLICATION_GROUP_NAME
                ],
                instrumentType=self.prepopulatedFields[
            ...
            self.prepopulatedFields[
                ...
                applType__runType=self.savedObjects[
            ...
            if self.savedFields[ApplicationFieldNames.APPLICATION_GROUP_NAME]:
                moreApplProducts = ApplProduct.objects.filter(
                    isActive=True,
                    isDefaultForInstrumentType=True,
                    applType__runType=self.savedObjects[
                        ApplicationFieldNames.RUN_TYPE
                    ].runType,
                    applicationGroup__name=self.savedFields[
                        ApplicationFieldNames.APPLICATION_GROUP_NAME
                    ],
                )
        ...
        self.savedObjects[ApplicationFieldNames.UPDATE_KITS_DEFAULTS] = (
            previous_run_type != self.savedObjects[ApplicationFieldNames.RUN_TYPE]
        ) or (
            previous_appl_product
            != self.savedObjects[ApplicationFieldNames.APPL_PRODUCT]
        )
    ...
    #     ir_qs = Plugin.objects.filter(name__icontains=StepNames.IONREPORTER)
