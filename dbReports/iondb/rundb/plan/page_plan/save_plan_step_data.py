...
class SavePlanStepData(AbstractStepData):
    ...
    def updateSavedFieldsForSamples_may_no_longer_needed(self):
        ...
        planHotSptRegionBedFile = str(
            self.prepopulatedFields[SavePlanFieldNames.PLAN_HOTSPOT_REGION_BED_FILE]
        )
        planTargetRegionBedFile = str(
            self.prepopulatedFields[SavePlanFieldNames.PLAN_TARGET_REGION_BED_FILE]
        )
        ...
        for index, row in enumerate(myTable):
            ...
            if sample_name:
                ...
                if runType == "AMPS_DNA_RNA" and sample_nucleotideType == "RNA":
                    ...
                    if (
                        not sampleReference
                        and self.savedObjects[SavePlanFieldNames.APPL_PRODUCT]
                        and not self.savedObjects[
                            SavePlanFieldNames.APPL_PRODUCT
                        ].isReferenceBySampleSupported
                        and isCreate
                    ):
                        logger.debug(
                            "save_plan_step_data.updateSavedFieldsForSamples() GOING to set sampleReference to planReference... planReference=%s"
                            % (planReference)
                        )
                ...
                if newSampleHotspotRegionBedFile != sampleHotSpotRegionBedFile:
                    row[
                        SavePlanFieldNames.BARCODE_SAMPLE_HOTSPOT_REGION_BED_FILE
                    ] = newSampleHotspotRegionBedFile
                    hasChanged = True
                if newSampleTargetRegionBedFile != sampleTargetRegionBedFile:
                    row[
                        SavePlanFieldNames.BARCODE_SAMPLE_TARGET_REGION_BED_FILE
                    ] = newSampleTargetRegionBedFile
                    hasChanged = True
        ...
        if hasAnyChanges:
            logger.debug(
                "save_plan_step_data.updateSavedFieldsForSamples() hasAnyChanges AFTER CHANGES... type=%s; myTable=%s"
                % (type(myTable), myTable)
            )

            # convert list with single quotes to str with double quotes. Then convert it to be unicode
            self.savedFields[SavePlanFieldNames.SAMPLES_TABLE] = unicode(
                json.dumps(myTable)
            )
            ...
            logger.debug(
                "save_plan_step_data.updateSavedFieldsForSamples() hasAnyChanges AFTER json.loads... type=%s; self.savedObjects[samplesTableList]=%s"
                % (
                    type(self.savedObjects[SavePlanFieldNames.SAMPLES_TABLE_LIST]),
                    self.savedObjects[SavePlanFieldNames.SAMPLES_TABLE_LIST],
                )
            )
