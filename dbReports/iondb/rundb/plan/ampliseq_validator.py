...
def validate_reference(meta, args, reference):
    ...
    print(
        "The requested Reference from ampliSeq Bundle is already installed in the System. So Exiting...."
    )
    return False
...
def validate_bed_files(meta, args):
    ...
    if target_regions_bed and target_regions_bed not in files:
        api.patch(
            "contentupload", args.upload_id, status="Error: malformed AmpliSeq archive"
        )
        print(
            "ERROR: Target region file %s not present in AmpliSeq archive"
            % target_regions_bed
        )
        sys.exit(1)
    ...
    if sse_bed and not target_regions_bed:
        ...
        print(
            "ERROR: Missing associated Target region file with SSE file %s in AmpliSeq archive"
            % sse_bed
        )
        sys.exit(1)
