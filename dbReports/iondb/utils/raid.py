...
def load_raid_status_json(filename="/var/spool/ion/raidstatus.json"):
    ...
    try:
        with open(filename) as fhandle:
            ...
            contents["date"] = datetime.strptime(
                contents["date"], "%Y-%m-%d %H:%M:%S.%f"
            )
    except Exception:
        pass
    return contents
...
def get_raid_status_json(raidinfojson):
    ...
    def status_rules(key, value, system_name="", enclosure_name=""):
        ...
        if alert_map.get(key):
            ...
            match = [
                v
                for v in drive_temperature_rules
                if v[0] == system_name and v[1] == enclosure_name
            ]
            if match:
    ...
    try:
        blob = {"date": "%s" % datetime.now(), "raid_status": raid_status}
        with open(
            os.path.join("/", "var", "spool", "ion", "raidstatus.json"), "w"
        ) as fhandle:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Parse raid info from ion_raidinfo_json script"""
    )
    parser.add_argument(dest="filename", help="specify input file")
