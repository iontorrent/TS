...
def apply_prepopulated_values_to_step_helper(request, step_helper):
    ...
    if "Save_plan" in step_helper.steps:
        samples_table = json.loads(
            step_helper.steps["Save_plan"].savedFields["samplesTable"]
        )
        for i, sample in enumerate(samples_table):
            if "sampleName" not in sample or sample["sampleName"] == "":
                sample["sampleName"] = "Sample %i" % (i + 1)
        step_helper.steps["Save_plan"].savedFields["samplesTable"] = json.dumps(
            samples_table
        )
    ...
    if "prepopulated-planning-session" in request.GET:
        key_name = (
            "prepopulated-planning-session-"
            + request.GET["prepopulated-planning-session"]
        )
        ...
        if cached_values:
            ...
            for step_name, step_fields in list(cached_values["step_helper"].items()):
                for field_name, value in list(step_fields.items()):
                    step_helper.steps[step_name].savedFields[field_name] = value

            # Call some util methods on some steps to parse savedFields data
            for step in list(step_helper.steps.values()):
                try:
            ...
            request.session["post_planning_redirect_url"] = cached_values[
                "redirect_url"
            ]
