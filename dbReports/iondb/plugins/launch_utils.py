...
def find_IRU_account(pluginconfig, accountId):
    if accountId:
        for username, configs in list(pluginconfig.get("userconfigs", {}).items()):
...
def get_plugins_dict(pg, selectedPlugins=""):
    ...
    for p in pg:
        ...
        for key in list(p.pluginsettings.keys()):
...
def toposort(graph):
    ...
    for value in set(sum(list(graph.values()), [])):
...
def depsolve(plugins, pk):
    ...
    for name in plugin_names:
        ...
        for dependency in active_plugins_deps[name]:
            if dependency not in active_plugins_deps:
                logger.error(
                    "Plugin %s requested dependency on %s, which isn't installed",
                    name,
                    dependency,
                )
                # del plugins[name]
    ...
    for name in sorted_names:
        if name not in list(plugins.keys()):
...
def add_hold_jid(plugin, plugins_dict, runlevel, satisfied_dependencies):
    ...
    if "hold_jid" not in list(plugin.keys()):
        plugin["hold_jid"] = []
    ...
    for name in plugin.get("depends", []):
        ...
        if (
            name in satisfied_dependencies
            and satisfied_dependencies[name].get("jid")
            and satisfied_dependencies[name]["jid"] not in plugin["hold_jid"]
        ):
    ...
    if runlevel == RunLevel.LAST or (Feature.EXPORT in plugin.get("features", [])):
        for name, p in list(plugins_dict.items()):
