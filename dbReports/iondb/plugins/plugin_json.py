...
def get_runinfo(
    ...
    sigproc_dir = os.path.normpath(
        os.path.join(report_dir, ion_params["SIGPROC_RESULTS"])
    )
    basecaller_dir = os.path.normpath(
        os.path.join(report_dir, ion_params["BASECALLER_RESULTS"])
    )
    alignment_dir = os.path.normpath(
        os.path.join(report_dir, ion_params["ALIGNMENT_RESULTS"])
    )
    ...
    if runlevel == RunLevel.BLOCK and blockId:
        ...
        plugin_out_dir = os.path.join(
            report_dir, block, plugin_out_dir.replace(report_dir, "").lstrip("/")
        )
    ...
    try:
        chipDescription = Chip.objects.filter(
            name=ion_params.get("chipType", "")
        ).values_list("description", flat=True)[0]
    except Exception:
        chipDescription = ""
...
def get_expmeta(ion_params, report_dir, report_pk):
    ...
    return {
        ...
        "output_file_name_stem": reanalysis_indicator
        + "_".join([plan.get("planName")[:80], server_name[:30], str(report_pk)]),
        "analysis_date": str(
            datetime.date(datetime.fromtimestamp(os.path.getmtime(ion_params_path)))
        ),
    }
...
def get_pluginconfig(plugin, instance_pluginconfig):
    ...
    pluginconfig = (
        instance_pluginconfig
        or plugin.get("pluginconfig", {})
        or plugin.get("userInput", {})
        or {}
    )
...
def make_plugin_json(
    ...
    try:
        ...
        ion_params = getparameter_minimal(
            os.path.join(report_dir, "ion_params_00.json")
        )
