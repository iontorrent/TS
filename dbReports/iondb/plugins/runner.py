...
class PluginRunner:
    ...
    def lookupPlugin(self, plugin_name, plugin_version=None):
        ...
        if plugin_version:
            ...
            plugin = models.Plugin.objects.get(
                name=plugin_name, version=plugin_version, active=True
            )
        else:
            qs = models.Plugin.objects.filter(name=plugin_name, active=True).exclude(
                path=""
            )
            if not qs:
    ...
    def writePluginLauncher(self, pluginout, pluginname, content):
        ...
        pluginWrapperFile = os.path.join(
            pluginout, "ion_plugin_%s_launch.sh" % pluginname
        )
        with open(pluginWrapperFile, "w") as f:
            f.write(content)
        os.chmod(pluginWrapperFile, 0o0775)
    ...
    def writePluginJson(self, start_json):
        ...
        start_json_fname = os.path.join(
            start_json["runinfo"]["results_dir"], "startplugin.json"
        )
        ...
        try:
            ...
            barcode_json_fname = os.path.join(
                start_json["runinfo"]["results_dir"], "barcodes.json"
            )

            with open(barcode_json_fname, "w") as fp:
                json.dump(
                    barcodeSampleInfo.data(start_json), fp, indent=2, cls=JSONEncoder
                )
